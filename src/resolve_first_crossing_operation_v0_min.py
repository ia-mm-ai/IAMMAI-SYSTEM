"""Resolve one bounded first-crossing operation result from declared summaries.
...
authority, standing, repair, discovery, and downstream authorization.
"""
...
class FirstCrossingOperationV0MinError(Exception):
...
DETERMINISTIC_FILENAME = (
    "first_crossing_operation_001__first_crossing_operation_v0_min_result.json"
)
...
def build_first_crossing_operation_v0_min_summary(result: Mapping[str, Any]) -> dict[str, Any]:
...
def build_first_crossing_operation_v0_min_request(**overrides: Any) -> dict[str, Any]:
...
def resolve_first_crossing_operation_v0_min(
    ...
    if declared_first_crossing_operation is None:
        request = build_first_crossing_operation_v0_min_request()
...
def resolve_first_crossing_operation_v0_min_from_path(
    ...
    return resolve_first_crossing_operation_v0_min(payload)
...
def _next_output_path(path: Path) -> Path:
    ...
    for index in range(1, 10000):
        ...
        if not candidate.exists():
            return candidate
    raise FirstCrossingOperationV0MinError("no deterministic output suffix available")


def write_first_crossing_operation_v0_min_result(
    ...
    if not isinstance(result, Mapping):
        raise FirstCrossingOperationV0MinError("WRITE_REFUSED: result must be a mapping")
    requested = (
        Path(output_path)
    ...
    try:
        ...
        raise FirstCrossingOperationV0MinError(f"WRITE_REFUSED: {error}") from error
    return destination
