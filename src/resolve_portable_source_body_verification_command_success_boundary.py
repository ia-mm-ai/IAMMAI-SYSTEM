"""Resolve one bounded portable source-body verification command success boundary.

This resolver is downstream of command result v2. It may record one future
command-success-step boundary only. It does not create command success, success
body, source, authority, currentness, final completion, public readiness,
deployment readiness, continuation, reusable permission, derivative reception,
vessel relation, another reception request, or follow-on work.

Command result v2 remains command-result posture only. V1 command-result and
v1 output-capture predecessor failures remain visible, unrepaired, unhidden,
...
class PortableSourceBodyVerificationCommandSuccessBoundaryError(Exception):
    """Raised for explicit unreadable or contradictory success-boundary input."""


RESOLVER_MODULE = "resolve_portable_source_body_verification_command_success_boundary"
...
def _has_raw_body(value: Any) -> bool:
    if isinstance(value, Mapping):
        for key, nested in value.items():
            if str(key) in RAW_BODY_KEYS and nested not in (None, False, "", [], {}):
                return True
        ...
        for key, nested in value.items():
            ...
            if key_text in RAW_BODY_KEYS:
                result[key_text] = {
                    ...
                    "reason": "selected basis remains reference-shaped",
                }
            else:
...
def _build_sections(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    v2.setdefault(
        ...
        _first(request.get("selected_command_result_v2_successor_metadata_preserved"), _deep_find(v2, ("successor_metadata_preserved",))),
    )
    ...
    v2.setdefault(
        ...
        _first(request.get("selected_command_result_v2_predecessor_failure_preserved"), _deep_find(v2, ("v1_predecessor_failure_preserved",))),
    )
    v2.setdefault("v1_repaired", _first(request.get("selected_command_result_v2_v1_repaired"), _deep_find(v2, ("v1_repaired", "v1_command_result_repaired"), False)))
    v2.setdefault("v1_hidden", _first(request.get("selected_command_result_v2_v1_hidden"), _deep_find(v2, ("v1_hidden", "v1_command_result_hidden"), False)))
    v2.setdefault("v1_claimed_passed", _first(request.get("selected_command_result_v2_v1_claimed_passed"), _deep_find(v2, ("v1_claimed_passed", "v1_command_result_claimed_passed"), False)))
    erased = request.get("selected_command_result_v2_erased_v1")
    v2.setdefault("v2_successor_does_not_erase_v1", False if _truthy(erased) else _deep_find(v2, ("v2_successor_does_not_erase_v1",), None))
    v2.setdefault("bounded_command_result_recorded", _first(request.get("selected_command_result_v2_bounded_command_result_recorded"), _deep_find(v2, ("bounded_command_result_recorded",))))
    v2.setdefault("command_success_created", _first(request.get("selected_command_result_v2_already_created_success"), _deep_find(v2, ("command_success_created",), False)))
    ...
    result_boundary.setdefault("one_future_command_result_step_declared", _deep_find(result_boundary, ("one_future_command_result_step_declared",)))
    ...
    artifact.setdefault("bounded_command_output_report_artifact_recorded", _deep_find(artifact, ("bounded_command_output_report_artifact_recorded",)))
    artifact.setdefault("command_success_created", _deep_find(artifact, ("command_success_created",), False))

    output_capture = sections["selected_output_capture_v2_basis"]
    output_capture.setdefault("outcome", _outcome(output_capture))
    output_capture.setdefault("result_version", request.get("selected_output_capture_v2_result_version") or _version(output_capture))
    output_capture.setdefault("failed_check_count", _first(request.get("selected_output_capture_v2_failed_check_count"), _failed_count(output_capture)))
    output_capture.setdefault("json_safe_result", _first(request.get("selected_output_capture_v2_json_safe_result"), _deep_find(output_capture, ("json_safe_result",))))
    output_capture.setdefault("v1_predecessor_failure_preserved", _deep_find(output_capture, ("v1_predecessor_failure_preserved",)))
    output_capture.setdefault("v1_repaired", _deep_find(output_capture, ("v1_repaired",), False))
    output_capture.setdefault("v1_hidden", _deep_find(output_capture, ("v1_hidden",), False))
    output_capture.setdefault("v1_claimed_passed", _deep_find(output_capture, ("v1_claimed_passed",), False))
    output_capture.setdefault("v2_successor_does_not_erase_v1", _deep_find(output_capture, ("v2_successor_does_not_erase_v1",)))
    for invented in (
        ...
        output_capture.setdefault(invented, _deep_find(output_capture, (invented,), False))
...
def _metadata(request: Mapping[str, Any], checks: Sequence[Mapping[str, Any]], outcome: str) -> dict[str, Any]:
    ...
    return {
        "portable_source_body_verification_command_success_boundary_result_id": (
            f"{request_id}__portable_source_body_verification_command_success_boundary_result"
        ),
    ...
    if failed:
        return (
            OUTCOME_BLOCKED,
            {
                ...
                "block_scope": "portable_source_body_verification_command_success_boundary",
            },
        )
...
def _build_result(request: Mapping[str, Any], checks: Sequence[Mapping[str, Any]], outcome: str | None = None, block: Mapping[str, Any] | None = None) -> dict[str, Any]:
    clean_request = _json_safe(_sanitize(request))
    sections = _build_sections(clean_request if isinstance(clean_request, Mapping) else {})
    ...
    result: dict[str, Any] = {
        ...
        **sections,
        "command_success_boundary_scope": {
            "declared_scope": _scope_values(request.get("command_success_boundary_scope")),
    ...
    result["portable_source_body_verification_command_success_boundary_summary"] = (
        build_portable_source_body_verification_command_success_boundary_summary(result)
    )
    return result
...
def resolve_portable_source_body_verification_command_success_boundary(
...
def resolve_portable_source_body_verification_command_success_boundary_from_path(
    declared_command_success_boundary_request_path: Path | str,
) -> dict[str, Any]:
    """Read a JSON object request and resolve command success boundary posture."""
    ...
    return resolve_portable_source_body_verification_command_success_boundary(parsed)
...
def _dedupe_path(path: Path) -> Path:
    ...
    for index in range(1, 10000):
        ...
        if not candidate.exists():
            return candidate
    raise PortableSourceBodyVerificationCommandSuccessBoundaryError(
        "could not create deterministic non-overwriting output path"
    )


def write_portable_source_body_verification_command_success_boundary_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write a bounded command success boundary result without overwriting."""

    if not isinstance(result, Mapping):
        raise PortableSourceBodyVerificationCommandSuccessBoundaryError("result must be a mapping")
    metadata = _mapping(
    ...
    path = (
        ...
        / f"{_safe_filename(request_id)}__portable_source_body_verification_command_success_boundary_result.json"
    )
...
def build_portable_source_body_verification_command_success_boundary_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build a compact summary preserving command-success-boundary posture."""
...
def build_declared_portable_source_body_verification_command_success_boundary_request(
    command_success_boundary_request_id: str = "command_success_boundary_reference_review_001",
    command_success_boundary_question: str = CORE_QUESTION,
    command_success_boundary_intent: str = INTENT_RECORD,
    **overrides: Any,
) -> dict[str, Any]:
    """Build a valid reference-shaped command success boundary request."""
