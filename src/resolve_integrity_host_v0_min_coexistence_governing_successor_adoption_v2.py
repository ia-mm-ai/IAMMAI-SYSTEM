...
This v2 successor line preserves the first adoption resolver's bounded
structure while accepting the actual governing re-resolution artifact contract:
the selected transition-result section is emitted as ``selected_transition_result``.
The older ``selected_reresolution_result`` name remains tolerated as a bounded
compatibility alias, but it is no longer required.
...
def _validate_reresolution_result(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    _selected_transition_result_section(payload)
...
def _selected_transition_result_section(payload: Mapping[str, Any]) -> Mapping[str, Any] | None:
    if "selected_transition_result" in payload:
        value = payload.get("selected_transition_result")
        if value is None:
            return None
        if isinstance(value, Mapping):
            return value
        raise GoverningSuccessorAdoptionError(
            "governing re-resolution result.selected_transition_result must be an object or null"
        )
    if "selected_reresolution_result" in payload:
        value = payload.get("selected_reresolution_result")
        if value is None:
            return None
        if isinstance(value, Mapping):
            return value
        raise GoverningSuccessorAdoptionError(
            "governing re-resolution result.selected_reresolution_result must be an object or null"
        )
    raise GoverningSuccessorAdoptionError(
        "governing re-resolution result must include selected_transition_result"
    )
