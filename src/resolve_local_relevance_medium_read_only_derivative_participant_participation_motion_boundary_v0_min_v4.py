"""v4 resolver for the local read-only derivative participant boundary.

This successor preserves the v3 canonicalized blocked-result posture and adds
bounded role-admission v1 predecessor-evidence derivation from the declared
terminal summary plus the recorded live role-admission artifact.
"""
...
OUTPUT_ROOT = (
    REPO_ROOT
    / "artifacts"
    / "integrity_host_v0_min_coexistence_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4"
)
...
class LocalRelevanceMediumReadOnlyDerivativeParticipantParticipationMotionBoundaryV0MinV4Error(
    Exception
):
    """Bounded resolver error for invalid direct write or path operations."""


def build_declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_request(
    ...
    """Build the bounded request mapping consumed by the v4 resolver."""
...
def resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_from_path(
    ...
    return resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4(
        loaded,
        _pre_checks=checks,
    )


def resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4(
    ...
    *,
    ...
    role_terminal_status, role_terminal_text = _load_terminal_summary(
        ...
        checks,
        exact_evidence_available=role_v1_exact,
    )
    continuation_terminal_status, _ = _load_terminal_summary_non_blocking(
...
def write_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_result(
    ...
    if not isinstance(result, Mapping):
        raise LocalRelevanceMediumReadOnlyDerivativeParticipantParticipationMotionBoundaryV0MinV4Error(
    ...
    filename = (
        f"{boundary_id}__"
        "local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_result.json"
    )
...
def build_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build the compact public summary for a v4 result mapping."""
...
def _result_from_parts(
    ...
    *,
    ...
    summary = build_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_summary(
        {
    ...
    additional_basis_required = [
        ...
        or check.get("block_code") == "ROLE_ADMISSION_V1_FAILURE_EVIDENCE_NOT_PRESERVED"
    ]
    not_recorded_basis = [
        check["check_name"]
        for check in failed
...
def _validate_evidence_basis(
    ...
    *,
    ...
    continuation_ok = _any_exact_true(
        [*role_sections, *continuation_sections],
        "continuation_v1_failure_evidence_preserved",
    )
    predecessor_ok = _any_exact_true(
        [*role_sections, *continuation_sections], "predecessor_failure_evidence_preserved"
    )
    role_v1_ok, source = _derive_role_admission_v1_failure_evidence(
        role_sections,
        role_terminal_status,
        role_terminal_text,
        role_header_ok=role_header_ok,
    ...
    _add_check(
        ...
        continuation_ok,
        None if continuation_ok else "EVIDENCE_NOT_PRESERVED",
        expected=True,
    ...
    _add_check(
        ...
        predecessor_ok,
        None if predecessor_ok else "EVIDENCE_NOT_PRESERVED",
        expected=True,
...
def _derive_role_admission_v1_failure_evidence(
    ...
    *,
    ...
    if _any_exact_true(role_sections, "role_admission_v1_failure_evidence_preserved"):
        return True, "role_artifact_exact"

    predecessor_preserved = _any_exact_true(role_sections, "predecessor_failure_evidence_preserved")
    ...
    terminal_status["v1_predecessor_evidence_text_present"] = terminal_evidence_text
    terminal_status[
...
def _load_terminal_summary(
    ...
    *,
    ...
    if path is None:
        _add_check(
            checks,
            "role_admission_terminal_summary_path_declared_when_needed",
            exact_evidence_available,
            None if exact_evidence_available else "ROLE_ADMISSION_TERMINAL_SUMMARY_PATH_MISSING",
            expected="path_or_exact_role_admission_v1_failure_evidence",
            actual=_MISSING,
        )
        return status, None
    ...
    try:
        ...
        _add_check(
            checks,
            "role_admission_terminal_summary_readable_when_needed",
            exact_evidence_available,
            None if exact_evidence_available else "ROLE_ADMISSION_TERMINAL_SUMMARY_UNREADABLE",
            expected="readable_text_or_exact_role_admission_v1_failure_evidence",
            actual="unreadable",
        )
        return status, None
...
def _load_terminal_summary_non_blocking(value: Any) -> tuple[dict[str, Any], str | None]:
    path = _path_from_value(value)
    status = _empty_terminal_summary_status(path)
    if path is None:
        return status, None
...
def _result_non_claims_canonical_false(sections: Sequence[Mapping[str, Any]]) -> bool:
    for section in sections:
        if section.get("result_level_non_claims_canonical_false") is True:
            return True
    for section in sections:
        if all(section.get(key) is False for key in REQUIRED_FALSE_NON_CLAIMS):
            return True
    return False
...
def _terminal_text_has_v1_predecessor_evidence(text: str | None) -> bool:
    ...
    predecessor_evidence = (
        ...
        or ("predecessor" in normalized and "evidence" in normalized and "preserved" in normalized)
    )
