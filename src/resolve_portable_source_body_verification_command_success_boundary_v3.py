"""Resolve command-success-boundary v3 with contained basis and full summary.

This resolver is an additive successor to
``resolve_portable_source_body_verification_command_success_boundary_v2``.
It preserves v1 as predecessor raw full-body return containment failure
evidence and preserves v2 as predecessor summary-surface conformance failure
evidence. V3 keeps the v2 raw-body containment correction and adds the missing
summary surface for ``bounded_command_output_report_artifact_preserved``.
...
class PortableSourceBodyVerificationCommandSuccessBoundaryV3Error(Exception):
    """Raised for explicit unreadable or contradictory v3 boundary input."""


RESOLVER_MODULE = "resolve_portable_source_body_verification_command_success_boundary_v3"
SUCCESSOR_OF = "resolve_portable_source_body_verification_command_success_boundary_v2"
PREDECESSOR_CHAIN = [
    "resolve_portable_source_body_verification_command_success_boundary",
    "resolve_portable_source_body_verification_command_success_boundary_v2",
]
SUCCESSOR_REASON = (
    "v3 preserves v1 raw full-body return containment failure evidence, "
    "preserves v2 summary-surface conformance failure evidence, and corrects "
    "the v2 summary omission of bounded_command_output_report_artifact_preserved."
)
...
SUMMARY_STATEMENT_FIELDS = ALLOWED_TRUE_RECORDED_FIELDS + (
    "raw_full_prior_artifact_body_returned",
)
...
EXPLICIT_BLOCK_CODES = (
    ...
    "SUMMARY_SURFACE_CONFORMANCE_FAILED",
    "BOUNDED_COMMAND_OUTPUT_REPORT_ARTIFACT_PRESERVED_SUMMARY_MISSING",
    "ARTIFACTS_MUTATED",
    "DEPLOYMENT_CREATED",
    "RUNTIME_HOSTING_CREATED",
    "PUBLIC_RELEASE_CREATED",
...
def _build_sections(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    command_result = sections["selected_command_result_v2_basis"]
    command_result.setdefault(
        "outcome",
        request.get("selected_command_result_v2_result_outcome") or _outcome(command_result),
    )
    command_result.setdefault(
        "result_version",
        request.get("selected_command_result_v2_result_version") or _version(command_result),
    )
    command_result.setdefault(
        "failed_check_count",
        _first(
            request.get("selected_command_result_v2_failed_check_count"),
            _failed_count(command_result),
        ),
    )
    command_result.setdefault(
        ...
        _first(
            ...
            _deep_find(command_result, ("successor_metadata_preserved",)),
        ),
    )
    command_result.setdefault(
        "successor_of",
        _deep_find(command_result, ("successor_of",), COMMAND_RESULT_V2_SUCCESSOR_OF),
    )
    command_result.setdefault(
        "resolver_module",
        _deep_find(command_result, ("resolver_module",), COMMAND_RESULT_V2_RESOLVER_MODULE),
    )
    command_result.setdefault(
        "v1_predecessor_failure_preserved",
        _first(
            request.get("selected_command_result_v2_predecessor_failure_preserved"),
            _deep_find(command_result, ("v1_predecessor_failure_preserved",)),
        ),
    )
    command_result.setdefault(
        "v1_repaired",
        _first(
            ...
            _deep_find(command_result, ("v1_repaired", "v1_command_result_repaired"), False),
        ),
    )
    command_result.setdefault(
        "v1_hidden",
        _first(
            ...
            _deep_find(command_result, ("v1_hidden", "v1_command_result_hidden"), False),
        ),
    )
    command_result.setdefault(
        "v1_claimed_passed",
        _first(
            ...
            _deep_find(
                command_result,
                ("v1_claimed_passed", "v1_command_result_claimed_passed"),
                False,
            ),
        ),
    )
    ...
    command_result.setdefault(
        ...
        False
        if _truthy(erased)
        else _deep_find(command_result, ("v2_successor_does_not_erase_v1",), None),
    )
    command_result.setdefault(
        "bounded_command_result_recorded",
        _first(
            request.get("selected_command_result_v2_bounded_command_result_recorded"),
            _deep_find(command_result, ("bounded_command_result_recorded",)),
        ),
    )
    command_result.setdefault(
        "command_success_created",
        _first(
            ...
            _deep_find(command_result, ("command_success_created",), False),
        ),
    )
    if request.get("selected_command_result_v2_result_path") is not None:
        command_result.setdefault(
            "selected_command_result_v2_result_path",
            str(request["selected_command_result_v2_result_path"]),
        )
    ...
    result_boundary.setdefault(
        "command_success_created",
        _deep_find(result_boundary, ("command_success_created",), False),
    )
    ...
    artifact.setdefault(
        "command_success_created",
        _deep_find(artifact, ("command_success_created",), False),
    )
    ...
    capture.setdefault(
        "result_version",
        request.get("selected_output_capture_v2_result_version") or _version(capture),
    )
    capture.setdefault(
        "failed_check_count",
    ...
    capture.setdefault(
        ...
        _first(
            ...
            _deep_find(capture, ("json_safe_result",)),
        ),
    )
    capture.setdefault(
        "v1_predecessor_failure_preserved",
        _deep_find(capture, ("v1_predecessor_failure_preserved",)),
    )
    ...
    capture.setdefault(
        "v2_successor_does_not_erase_v1",
        _deep_find(capture, ("v2_successor_does_not_erase_v1",)),
    )
    for invented in (
    ...
    execution.setdefault("outcome", _outcome(execution))
    execution.setdefault("failed_check_count", _failed_count(execution))
    execution.setdefault(
        "execution_trace_audit_only",
        _deep_find(execution, ("execution_trace_audit_only",)),
    )
...
def _evaluate_checks(request: Mapping[str, Any], sections: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for key, code in BASIS_BLOCK_CODES.items():
        ...
        _check(checks, f"{key} reference-shaped", not _has_raw_body(request.get(key)), "reference-shaped basis without raw full body", request.get(key), "SELECTED_BASIS_NOT_REFERENCE_SHAPED")
    ...
    command_result = sections["selected_command_result_v2_basis"]
    _check(checks, "command result v2 outcome recorded", command_result.get("outcome") == COMMAND_RESULT_V2_OUTCOME, COMMAND_RESULT_V2_OUTCOME, command_result.get("outcome"), "COMMAND_RESULT_V2_NOT_RECORDED")
    _check(checks, "command result v2 version 0.2.0", command_result.get("result_version") == COMMAND_RESULT_V2_VERSION, COMMAND_RESULT_V2_VERSION, command_result.get("result_version"), "COMMAND_RESULT_V2_VERSION_NOT_0_2_0")
    _check(checks, "command result v2 failed check count zero", command_result.get("failed_check_count") == 0, 0, command_result.get("failed_check_count"), "COMMAND_RESULT_V2_FAILED_CHECKS_PRESENT")
    _check(checks, "command result v2 successor metadata present", command_result.get("successor_metadata_preserved") is True and command_result.get("successor_of") == COMMAND_RESULT_V2_SUCCESSOR_OF, COMMAND_RESULT_V2_SUCCESSOR_OF, {"successor_metadata_preserved": command_result.get("successor_metadata_preserved"), "successor_of": command_result.get("successor_of")}, "COMMAND_RESULT_V2_SUCCESSOR_METADATA_MISSING")
    _check(checks, "command result v2 predecessor failure evidence preserved", command_result.get("v1_predecessor_failure_preserved") is True, True, command_result.get("v1_predecessor_failure_preserved"), "COMMAND_RESULT_V2_PREDECESSOR_FAILURE_EVIDENCE_MISSING")
    _check(checks, "command result v2 v1 repaired false", not _truthy(command_result.get("v1_repaired")), False, command_result.get("v1_repaired"), "COMMAND_RESULT_V2_REPAIRED_V1")
    _check(checks, "command result v2 v1 hidden false", not _truthy(command_result.get("v1_hidden")), False, command_result.get("v1_hidden"), "COMMAND_RESULT_V2_HID_V1")
    _check(checks, "command result v2 v1 claimed passed false", not _truthy(command_result.get("v1_claimed_passed")), False, command_result.get("v1_claimed_passed"), "COMMAND_RESULT_V2_CLAIMED_V1_PASSED")
    _check(checks, "command result v2 did not erase v1", command_result.get("v2_successor_does_not_erase_v1") is True and not _deep_true(request, ("selected_command_result_v2_erased_v1", "v2_command_result_erased_v1")), True, command_result.get("v2_successor_does_not_erase_v1"), "COMMAND_RESULT_V2_ERASED_V1")
    _check(checks, "command result v2 bounded command result recorded", command_result.get("bounded_command_result_recorded") is True, True, command_result.get("bounded_command_result_recorded"), "COMMAND_RESULT_V2_NOT_BOUNDED")
    _check(checks, "command result v2 command success not created", not _truthy(command_result.get("command_success_created")), False, command_result.get("command_success_created"), "COMMAND_RESULT_V2_ALREADY_CREATED_SUCCESS")
    ...
    raw_present = _has_raw_body(request)
    _check(checks, "raw full prior artifact body not emitted", not raw_present, "reference-shaped basis only", "raw body present" if raw_present else None, "FULL_PRIOR_ARTIFACT_BODY_EMITTED")
    _check(checks, "raw full prior artifact body not returned", not raw_present, "returned result excludes raw full prior artifact body", "raw body present" if raw_present else None, "RAW_FULL_PRIOR_ARTIFACT_BODY_RETURNED")
    ...
    result.update(
        {
            ...
            "v3_successor_does_not_erase_v1": True,
            "v3_successor_does_not_erase_v2": True,
...
def _metadata(request: Mapping[str, Any], checks: Sequence[Mapping[str, Any]], outcome: str) -> dict[str, Any]:
    ...
    return {
        "portable_source_body_verification_command_success_boundary_result_id": (
            f"{request_id}__portable_source_body_verification_command_success_boundary_v3_result"
        ),
        ...
        "predecessor_chain": list(PREDECESSOR_CHAIN),
    ...
    if failed:
        return (
            OUTCOME_BLOCKED,
            {
                ...
                "block_scope": "portable_source_body_verification_command_success_boundary_v3",
            },
        )
...
def _build_result(
    ...
    result["portable_source_body_verification_command_success_boundary_summary"] = (
        build_portable_source_body_verification_command_success_boundary_v3_summary(result)
    )
...
def resolve_portable_source_body_verification_command_success_boundary_v3(
...
def resolve_portable_source_body_verification_command_success_boundary_v3_from_path(
    declared_command_success_boundary_request_path: Path | str,
) -> dict[str, Any]:
    """Read a JSON object request and resolve command success boundary v3 posture."""
    ...
    return resolve_portable_source_body_verification_command_success_boundary_v3(parsed)
...
def _dedupe_path(path: Path) -> Path:
    ...
    for index in range(1, 10000):
        ...
        if not candidate.exists():
            return candidate
    raise PortableSourceBodyVerificationCommandSuccessBoundaryV3Error(
        "could not create deterministic non-overwriting output path"
    )


def write_portable_source_body_verification_command_success_boundary_v3_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write a bounded v3 command success boundary result without overwriting."""

    if not isinstance(result, Mapping):
        raise PortableSourceBodyVerificationCommandSuccessBoundaryV3Error("result must be a mapping")
    metadata = _mapping(
    ...
    path = (
        ...
        / f"{_safe_filename(request_id)}__portable_source_body_verification_command_success_boundary_v3_result.json"
    )
...
def build_portable_source_body_verification_command_success_boundary_v3_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build a summary preserving v3 successor and all recorded statement fields."""
    ...
    command_result = _mapping(result.get("selected_command_result_v2_basis"))
    ...
    summary: dict[str, Any] = {
        ...
        "v3_result_version": metadata.get("result_version"),
        "resolver_module": metadata.get("resolver_module"),
        "successor_of": metadata.get("successor_of"),
        "predecessor_chain": metadata.get("predecessor_chain"),
        ...
        "v2_predecessor_failure_preserved": metadata.get("v2_predecessor_failure_preserved"),
        "v2_repaired": metadata.get("v2_repaired"),
        "v2_hidden": metadata.get("v2_hidden"),
        "v2_claimed_passed": metadata.get("v2_claimed_passed"),
        "v3_successor_does_not_erase_v1": metadata.get("v3_successor_does_not_erase_v1"),
        "v3_successor_does_not_erase_v2": metadata.get("v3_successor_does_not_erase_v2"),
        ...
        "summary_surface_conformance_preserved": metadata.get("summary_surface_conformance_preserved"),
        "selected_command_result_v2_outcome": command_result.get("outcome"),
        "selected_command_result_v2_version": command_result.get("result_version"),
        "selected_command_result_v2_failed_check_count": command_result.get("failed_check_count"),
        "selected_command_result_v2_successor_metadata_preserved": command_result.get("successor_metadata_preserved"),
    ...
    for key in SUMMARY_STATEMENT_FIELDS:
        if key == "raw_full_prior_artifact_body_returned":
            summary[key] = statement.get(key, metadata.get(key, False))
        else:
...
def build_declared_portable_source_body_verification_command_success_boundary_v3_request(
    command_success_boundary_request_id: str = "command_success_boundary_reference_review_001_v3",
    command_success_boundary_question: str = CORE_QUESTION,
    command_success_boundary_intent: str = INTENT_RECORD,
    **overrides: Any,
) -> dict[str, Any]:
    """Build a valid reference-shaped command success boundary v3 request."""
...
resolve_portable_source_body_verification_command_success_boundary = (
    resolve_portable_source_body_verification_command_success_boundary_v3
)
resolve_portable_source_body_verification_command_success_boundary_from_path = (
    resolve_portable_source_body_verification_command_success_boundary_v3_from_path
)
write_portable_source_body_verification_command_success_boundary_result = (
    write_portable_source_body_verification_command_success_boundary_v3_result
)
build_portable_source_body_verification_command_success_boundary_summary = (
    build_portable_source_body_verification_command_success_boundary_v3_summary
)
build_declared_portable_source_body_verification_command_success_boundary_request = (
    build_declared_portable_source_body_verification_command_success_boundary_v3_request
)
