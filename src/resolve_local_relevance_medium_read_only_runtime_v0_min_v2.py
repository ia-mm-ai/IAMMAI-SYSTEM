"""Resolve one local read-only selected-state runtime, v2.

This additive successor preserves the v0 runtime resolver as failed-lineage
evidence. It does not patch, mutate, replace, hide, or claim passed v0.

The v2 correction is narrow: runtime-permission posture is extracted only from
the relevant runtime-permission basis layers. A present false or non-boolean
posture in those layers blocks with the existing public runtime-permission
block code; a true posture in one relevant layer is not invalidated by missing
fields in another relevant layer.
"""
...
class LocalRelevanceMediumReadOnlyRuntimeV0MinV2Error(RuntimeError):
    """Bounded resolver error for v2 runtime request/path handling."""


RESULT_VERSION = "0.1.0"
RESOLVER_MODULE = "resolve_local_relevance_medium_read_only_runtime_v0_min_v2"
SUCCESSOR_OF_RESOLVER_MODULE = "resolve_local_relevance_medium_read_only_runtime_v0_min"
...
def build_declared_local_relevance_medium_read_only_runtime_v0_min_v2_request(
    ...
    **extra_fields: Any,
    ...
    if selected_command != SELECTED_COMMAND:
        raise LocalRelevanceMediumReadOnlyRuntimeV0MinV2Error(
...
def build_local_relevance_medium_read_only_runtime_v0_min_v2_summary(
...
def _compose_result(
    ...
    block = (
        {
            ...
            "reason": "runtime v2 blocked by bounded public check failure",
        }
    ...
    result["local_relevance_medium_read_only_runtime_summary"] = (
        build_local_relevance_medium_read_only_runtime_v0_min_v2_summary(result)
    )
    return result
...
def _blocked_result_from_code(
    ...
    bounded_request = dict(
        request
        or build_declared_local_relevance_medium_read_only_runtime_v0_min_v2_request()
    )
    checks = [
        _new_check(
...
def resolve_local_relevance_medium_read_only_runtime_v0_min_v2(
    declared_local_relevance_medium_read_only_runtime: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Resolve one bounded local read-only selected-state runtime, v2."""

    if declared_local_relevance_medium_read_only_runtime is None:
        request = build_declared_local_relevance_medium_read_only_runtime_v0_min_v2_request()
...
def resolve_local_relevance_medium_read_only_runtime_v0_min_v2_from_path(
    ...
    try:
        ...
        return resolve_local_relevance_medium_read_only_runtime_v0_min_v2(request)
    if not isinstance(loaded, MappingABC):
        return _blocked_result_from_code(
            "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_REQUEST_MALFORMED",
            type(loaded).__name__,
        )
    return resolve_local_relevance_medium_read_only_runtime_v0_min_v2(loaded)
...
def _result_output_path(result: Mapping[str, Any], output_path: Path | str | None) -> Path:
    ...
    if output_path is not None:
        ...
        return (
            candidate
            / f"{runtime_id}__local_relevance_medium_read_only_runtime_v0_min_v2_result.json"
        )
    return (
        OUTPUT_ROOT
        / f"{runtime_id}__local_relevance_medium_read_only_runtime_v0_min_v2_result.json"
...
def write_local_relevance_medium_read_only_runtime_v0_min_v2_result(
    result: Mapping[str, Any],
    ...
    with candidate.open("w", encoding="utf-8") as handle:
        json.dump(dict(result), ensure_ascii=False, indent=2, sort_keys=True)
        handle.write("\n")
    return candidate
