"""Bounded carrier registry and persistence boundary resolver.

This resolver records what a carrier registry or persistence surface may
preserve without becoming law. It preserves indexability and reconstructable
references only. It does not implement a registry, database, storage backend,
synchronization layer, currentness, authority, distributed standing, full body
transfer, continuation, distributed operation, or evidence erasure.
...
class CarrierRegistryPersistenceBoundaryError(Exception):
    """Hard failure for malformed or unreadable explicit registry inputs."""
...
RESOLVER_MODULE = "resolve_carrier_registry_persistence_boundary"
RESULT_VERSION = "0.1.0"
RESULT_TYPE = "carrier_registry_persistence_boundary_result"
...
def resolve_carrier_registry_persistence_boundary(
    ...
    """Resolve one bounded carrier registry/persistence request."""
...
def resolve_carrier_registry_persistence_boundary_from_path(
    ...
    except CarrierRegistryPersistenceBoundaryError as exc:
...
def write_carrier_registry_persistence_result(
    ...
    """Write an additive registry/persistence result without overwriting."""

    if not isinstance(result, Mapping):
        raise CarrierRegistryPersistenceBoundaryError(
            "DECLARED_REGISTRY_REQUEST_MALFORMED",
    ...
    if output_path is None:
        ...
        output_path = CARRIER_REGISTRY_PERSISTENCE_BOUNDARY_ROOT / (
            f"{_safe_filename_part(basis_id)}__carrier_registry_persistence_result.json"
        )
...
def build_carrier_registry_persistence_summary(
    ...
    """Build a compact summary for a registry/persistence result."""

    checks = _mapping_list(result.get("registry_persistence_checks"))
    question = _as_mapping(result.get("declared_registry_question"))
    purpose = _as_mapping(result.get("persistence_purpose"))
    category = _as_mapping(result.get("selected_record_category"))
    ...
    return {
        ...
        "referenced_artifact_identity_preserved": bool(
            statement.get("referenced_artifact_identity_preserved")
        ),
        "referenced_artifact_outcome_preserved": bool(
            statement.get("referenced_artifact_outcome_preserved")
        ),
        "lifecycle_reference_preserved": bool(
            statement.get("lifecycle_reference_preserved")
        ),
        "relation_reference_preserved": bool(statement.get("relation_reference_preserved")),
        "divergence_reference_preserved": bool(
            statement.get("divergence_reference_preserved")
        ),
        "currentness_participation_reference_preserved": bool(
            statement.get("currentness_participation_reference_preserved")
        ),
        "evidence_reference_preserved": bool(statement.get("evidence_reference_preserved")),
        "non_claims_preserved": bool(statement.get("non_claims_preserved")),
...
def build_declared_carrier_registry_persistence_request(
    ...
    *,
    ...
    """Build one bounded registry/persistence request without inferring force."""
...
def _declared_registry_question(
    ...
    return {
        ...
        "registry_intent": _normalize_token(request.get("registry_intent")),
...
def _persistence_purpose(request: Mapping[str, Any]) -> dict[str, Any]:
    purpose = request.get("persistence_purpose")
    return {
        "persistence_purpose": purpose,
...
def _selected_record_category(request: Mapping[str, Any]) -> dict[str, Any]:
    category = _normalize_token(request.get("selected_record_category"))
    return {
        "selected_record_category": category,
        ...
        "raw_selected_record_category": copy.deepcopy(
            request.get("selected_record_category")
        ),
    }
...
def _selected_referenced_artifacts(
    ...
    for item in raw_items:
        if not isinstance(item, Mapping):
            parseable = False
            continue
        artifact = copy.deepcopy(dict(item))
        artifact_id = _artifact_identity(artifact)
        outcome = _artifact_outcome(artifact)
        entries.append(
            {
                "referenced_artifact_id": artifact_id,
                "referenced_artifact_outcome": outcome,
                "referenced_artifact_path": artifact.get("path")
                or artifact.get("artifact_path"),
                "carrier_id": artifact.get("carrier_id")
                or artifact.get("selected_carrier_id"),
    ...
    return {
        ...
        "referenced_artifact_entries": entries,
        "referenced_artifact_ids": artifact_ids,
        "referenced_artifact_outcomes": outcomes,
        "referenced_artifacts_preserved": bool(entries),
        "referenced_artifact_identity_preserved": bool(entries)
        and all(_nonempty(artifact_id) for artifact_id in artifact_ids),
        "referenced_artifact_outcome_preserved": bool(entries)
        and all(_nonempty(outcome) for outcome in outcomes),
...
def _registry_persistence_basis(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    lifecycle = _reference_posture(request.get("lifecycle_status_reference"))
    relation = _reference_posture(request.get("relation_reference"))
    divergence = _reference_posture(request.get("divergence_reference"))
    currentness = _reference_posture(
        request.get("currentness_participation_reference")
    )
    admission = _reference_posture(request.get("admission_reference"))
    receipt = _reference_posture(request.get("receipt_reference"))
    experiment = _reference_posture(request.get("experiment_reference"))
    integrity = _reference_posture(request.get("integrity_reference"))
    return {
        ...
        "raw_registry_persistence_basis": copy.deepcopy(raw_basis),
...
def _build_checks(
    ...
    checks = [
        ...
        _check(
            ...
            or bool(artifacts.get("referenced_artifact_identity_preserved")),
        ...
        _check(
            ...
            not artifacts.get("referenced_artifacts_supplied")
            or bool(artifacts.get("referenced_artifact_outcome_preserved")),
        ...
        _reference_check(
            ...
            basis.get("lifecycle_status_reference"),
        ),
        _reference_check(
        ...
        _check(
            ...
            not artifacts.get("referenced_artifacts_supplied")
            or bool(artifacts.get("referenced_artifacts_preserved")),
...
def _registry_persistence_statement(
    ...
    return {
        ...
        "referenced_artifact_identity_preserved": bool(
            artifacts.get("referenced_artifact_identity_preserved")
        ),
        "referenced_artifact_outcome_preserved": bool(
            artifacts.get("referenced_artifact_outcome_preserved")
        ),
        "lifecycle_reference_preserved": bool(
            basis.get("lifecycle_status_reference", {}).get("preserved")
        ),
        "relation_reference_preserved": bool(
            basis.get("relation_reference", {}).get("preserved")
        ),
        "divergence_reference_preserved": bool(
            basis.get("divergence_reference", {}).get("preserved")
        ),
        "currentness_participation_reference_preserved": bool(
            basis.get("currentness_participation_reference", {}).get("preserved")
        ),
        ...
        "evidence_reference_preserved": bool(artifacts.get("referenced_artifacts_preserved")),
        "non_claims_preserved": True,
        "passed_check_count": sum(1 for check in checks if check.get("passed") is True),
        "failed_check_count": sum(1 for check in checks if check.get("passed") is False),
...
def _category_requires_carrier(category: str | None) -> bool:
    return category in CATEGORIES_REQUIRING_SELECTED_CARRIER
...
def _non_overwriting_path(path: Path) -> Path:
    ...
    raise CarrierRegistryPersistenceBoundaryError(
        "NON_CLAIM_MISSING_OR_FLIPPED",
        "Could not allocate a non-overwriting registry/persistence result path.",
    )
