"""Resolve one local relevance medium second relevance orientation view.

This resolver records one small instrument-shaped local orientation view from
one clean LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT artifact. It
orients a local reader to what is inspectably present in the second receipt,
which second received identifiers stand as received identifiers only, what
cannot be inferred, and what remains unavailable.

It does not expand the second receipt or second reception and does not create a
second local index entry, local medium multiplicity result, relation,
comparison, repeated reception permission, arbitrary reception, feed, index
system, registry, search, ranking, source, authority, currentness, action,
...
class LocalRelevanceMediumSecondRelevanceOrientationViewV0MinError(Exception):
    """Bounded error for malformed second-orientation request path reads."""
...
OUTCOME_RECORDED = "LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_RECORDED"
OUTCOME_NOT_RECORDED = "LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_NOT_RECORDED"
OUTCOME_REQUIRES_ADDITIONAL_BASIS = (
    "LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_REQUIRES_ADDITIONAL_BASIS"
)
OUTCOME_BLOCKED = "LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_BLOCKED"
OUTCOME_FAMILY = (
    OUTCOME_RECORDED,
...
SUPPORTED_SECOND_ORIENTATION_SCOPE_VALUES = (SECOND_ORIENTATION_SCOPE,)
SUPPORTED_SECOND_ORIENTATION_TYPE_VALUES = (SECOND_ORIENTATION_VIEW_TYPE,)

INTENT_RECORD = "RECORD_LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW"
INTENT_DO_NOT_RECORD = "DO_NOT_RECORD_LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW"
INTENT_BLOCK = "BLOCK_LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW"
SUPPORTED_INTENTS = (INTENT_RECORD, INTENT_DO_NOT_RECORD, INTENT_BLOCK)

INSPECTABLY_PRESENT_ITEMS = (
...
ORIENTATION_STATEMENT = (
    "One clean second bounded relevance receipt and its referenced second bounded "
    "relevance reception artifact are inspectably present. The view orients to "
    "second received identifiers only and does not infer authority, currentness, "
    "truth, action, synchronization, participation, runtime permission, public "
    "interface, distributed behavior, relation, comparison, multiplicity result, "
    "or follow-on work."
)
...
REQUIRED_FALSE_NON_CLAIMS = (
    ...
    "follow_on_work_authorized",
    "artifact_existence_treated_as_second_orientation_authority",
    "latest_file_posture_treated_as_second_orientation_authority",
    "repo_local_availability_treated_as_second_orientation_authority",
    "hidden_repo_state_used_as_second_orientation_content",
    "hidden_repo_state_used_as_second_orientation_authority",
...
ALLOWED_TRUE_RECORDED_FIELDS = (
    ...
    "inspectably_present_preserved",
    "standing_identifiers_preserved",
    "non_inference_preserved",
    "unavailable_preserved",
    "result_level_non_claims_canonical_false",
)

BLOCK_CODES = (
    "LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_QUESTION_UNDECLARED",
    "LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_INTENT_UNSUPPORTED",
    "LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_BLOCK_REQUESTED",
    "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_PATH_MISSING",
    "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_UNREADABLE",
    "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_NOT_JSON_OBJECT",
    "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_NOT_RECORDED",
    "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_FAILED_CHECKS_PRESENT",
    "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_VERSION_NOT_0_1_0",
    "SECOND_BOUNDED_RELEVANCE_RECEIPT_OBJECT_MISSING",
    ...
    "SECOND_ORIENTATION_SCOPE_MISSING",
    "SECOND_ORIENTATION_SCOPE_NOT_LOCAL_ONLY",
    "SECOND_ORIENTATION_VIEW_TYPE_MISSING",
    "SECOND_ORIENTATION_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW",
    "SECOND_ORIENTATION_VIEW_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW",
    "INSPECTABLY_PRESENT_MISSING_OR_INCOMPLETE",
    "STANDING_IDENTIFIERS_MISSING_OR_NOT_PRESERVED",
    "NON_INFERENCE_MISSING_OR_FLIPPED",
    "UNAVAILABLE_MISSING_REQUIRED_CLAIMS",
    "SECOND_RECEIPT_EXPANDED",
    "SECOND_RECEPTION_EXPANDED",
    "SECOND_INDEX_ENTRY_CREATED",
    "LOCAL_MEDIUM_MULTIPLICITY_RESULT_CREATED",
    ...
    "DECLARED_LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_REQUEST_MALFORMED",
    "DECLARED_LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_REQUEST_UNREADABLE",
)
...
FALSE_REQUEST_BLOCKS = (
    ("second_receipt_expanded", "SECOND_RECEIPT_EXPANDED"),
    ("second_reception_expanded", "SECOND_RECEPTION_EXPANDED"),
...
def _read_json_object(path_value: Any) -> tuple[dict[str, Any] | None, str | None]:
    if not path_value:
        return None, "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_PATH_MISSING"
    try:
        path = Path(str(path_value))
        with path.open("r", encoding="utf-8") as handle:
            loaded = json.load(handle)
    except (OSError, json.JSONDecodeError, TypeError, ValueError):
        return None, "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_UNREADABLE"
    if not isinstance(loaded, Mapping):
        return None, "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_NOT_JSON_OBJECT"
...
def _failed_check_count_from_artifact(artifact: Mapping[str, Any], summary: Mapping[str, Any]) -> Any:
    metadata = _as_mapping(
        artifact.get("local_relevance_medium_second_bounded_relevance_receipt_metadata")
    )
    checks = artifact.get("local_relevance_medium_second_bounded_relevance_receipt_checks")
...
def _extract_second_receipt_facts(
    artifact: Mapping[str, Any] | None,
    artifact_path: str | None,
) -> dict[str, Any]:
    if artifact is None:
        return {"basis_second_bounded_relevance_receipt_artifact": artifact_path}

    metadata = _as_mapping(
        artifact.get("local_relevance_medium_second_bounded_relevance_receipt_metadata")
    )
    summary = _as_mapping(
        artifact.get("local_relevance_medium_second_bounded_relevance_receipt_summary")
    )
    second_object = _as_mapping(artifact.get("second_bounded_relevance_receipt_object"))

    return {
        "basis_second_bounded_relevance_receipt_artifact": artifact_path,
        "basis_second_bounded_relevance_receipt_outcome": artifact.get("outcome"),
        "basis_second_bounded_relevance_receipt_result_version": _first_present(
            metadata.get("local_relevance_medium_second_bounded_relevance_receipt_version"),
            metadata.get("result_version"),
            summary.get("result_version"),
            second_object.get("second_receipt_version"),
        ...
        "second_bounded_relevance_receipt_object": second_object,
        "second_receipt_type": second_object.get("second_receipt_type"),
        "second_receipt_scope": second_object.get("second_receipt_scope"),
        "basis_second_bounded_relevance_reception_artifact": second_object.get(
...
def _standing_identifiers(facts: Mapping[str, Any]) -> dict[str, Any]:
    return {
...
def _expected_standing_identifiers() -> dict[str, str]:
    return {
...
def _non_inference() -> dict[str, bool]:
    return {key: False for key in NON_INFERENCE_FALSE_FIELDS}
...
def _build_checks(
    ...
    question = request.get("local_relevance_medium_second_relevance_orientation_view_question")
    intent = request.get("local_relevance_medium_second_relevance_orientation_view_intent")
    selected_path = request.get("selected_second_bounded_relevance_receipt_artifact")
    orientation_scope = request.get("second_orientation_scope")
    orientation_type = request.get("second_orientation_view_type")
    receipt_object = facts.get("second_bounded_relevance_receipt_object")
    checks: list[dict[str, Any]] = []

    checks.append(
        _check(
            "second_orientation_question_declared",
            isinstance(question, str) and bool(question.strip()),
            "declared second orientation question",
            question,
            "LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_QUESTION_UNDECLARED",
        )
    )
    ...
    path_declared = bool(selected_path) and not _request_flag(
        request, "selected_second_bounded_relevance_receipt_artifact_missing"
    )
    checks.append(
        _check(
            "selected_second_bounded_relevance_receipt_artifact_path_declared",
            path_declared,
            "selected second bounded relevance receipt artifact path declared",
            selected_path,
            "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_PATH_MISSING",
        )
    )
    checks.append(
        _check(
            "selected_second_bounded_relevance_receipt_artifact_readable_json",
            read_error
            not in {
                "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_UNREADABLE",
                "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_PATH_MISSING",
            ...
            (
                "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_PATH_MISSING"
                if read_error == "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_PATH_MISSING"
                else "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_UNREADABLE"
            ),
        )
    )
    checks.append(
        _check(
            "selected_second_bounded_relevance_receipt_artifact_json_object",
            read_error != "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_NOT_JSON_OBJECT",
            "JSON object",
    ...
    artifact_not_recorded = _request_flag(
        request, "selected_second_bounded_relevance_receipt_artifact_not_recorded"
    )
    checks.append(
        _check(
            "second_bounded_relevance_receipt_artifact_outcome_recorded",
            (
                facts.get("basis_second_bounded_relevance_receipt_outcome")
                == SECOND_RECEIPT_RECORDED_OUTCOME
    ...
    version_not_clean = _request_flag(
        request, "selected_second_bounded_relevance_receipt_artifact_version_not_0_1_0"
    )
    checks.append(
        _check(
            "second_bounded_relevance_receipt_artifact_result_version_0_1_0",
            (
                facts.get("basis_second_bounded_relevance_receipt_result_version")
                == RESULT_VERSION
    ...
    failed_checks_present = _request_flag(
        request, "selected_second_bounded_relevance_receipt_artifact_failed_checks_present"
    )
    failed_count = facts.get("basis_second_bounded_relevance_receipt_failed_check_count")
    checks.append(
        _check(
            "second_bounded_relevance_receipt_artifact_failed_check_count_zero",
    ...
    checks.append(
        _check(
            "second_bounded_relevance_receipt_object_present",
            bool(receipt_object)
            and not _request_flag(request, "second_bounded_relevance_receipt_object_missing"),
            "one second bounded relevance receipt object",
            "present" if receipt_object else "missing",
            "SECOND_BOUNDED_RELEVANCE_RECEIPT_OBJECT_MISSING",
        )
    )
    checks.append(
        _check(
            ...
            facts.get("second_receipt_type") == SECOND_RECEIPT_TYPE
            and not _request_flag(request, "second_receipt_type_not_second_bounded_relevance_receipt"),
            SECOND_RECEIPT_TYPE,
            facts.get("second_receipt_type"),
            "SECOND_RECEIPT_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT",
        )
    )
    checks.append(
        _check(
            ...
            facts.get("second_receipt_scope") == SECOND_RECEIPT_SCOPE
            and not _request_flag(request, "second_receipt_scope_not_inspectable_only"),
            SECOND_RECEIPT_SCOPE,
            facts.get("second_receipt_scope"),
            "SECOND_RECEIPT_SCOPE_NOT_INSPECTABLE_ONLY",
        )
    )
    checks.append(
    ...
    checks.append(
        _check(
            "second_orientation_view_type_declared",
            bool(orientation_type),
            SECOND_ORIENTATION_VIEW_TYPE,
            orientation_type,
            "SECOND_ORIENTATION_VIEW_TYPE_MISSING",
        )
    ...
    checks.append(
        _check(
            ...
            orientation_type == SECOND_ORIENTATION_VIEW_TYPE
    ...
    checks.append(
        _check(
            "second_orientation_scope_declared",
    ...
    checks.append(
        _check(
            "second_orientation_scope_local_only",
            orientation_scope == SECOND_ORIENTATION_SCOPE
    ...
    checks.append(
        _check(
            "inspectably_present_preserved",
            not _request_flag(request, "inspectably_present_missing_or_incomplete"),
            list(INSPECTABLY_PRESENT_ITEMS),
            list(INSPECTABLY_PRESENT_ITEMS),
            "INSPECTABLY_PRESENT_MISSING_OR_INCOMPLETE",
        )
    )
    checks.append(
        _check(
            "standing_identifiers_preserved",
            _standing_identifiers(facts) == _expected_standing_identifiers()
            and not _request_flag(request, "standing_identifiers_missing_or_not_preserved"),
            _expected_standing_identifiers(),
            _standing_identifiers(facts),
            "STANDING_IDENTIFIERS_MISSING_OR_NOT_PRESERVED",
        )
    )
    checks.append(
        _check(
            "non_inference_preserved",
            all(value is False for value in _non_inference().values())
            and not _request_flag(request, "non_inference_missing_or_flipped"),
            _non_inference(),
            _non_inference(),
            "NON_INFERENCE_MISSING_OR_FLIPPED",
        )
    ...
    checks.append(
        _check(
            "unavailable_preserved",
            not _request_flag(request, "unavailable_missing_required_claims"),
            list(UNAVAILABLE_ITEMS),
            list(UNAVAILABLE_ITEMS),
            "UNAVAILABLE_MISSING_REQUIRED_CLAIMS",
        )
    )
...
def _build_second_relevance_orientation_view(
    orientation_view_id: str,
    facts: Mapping[str, Any],
) -> dict[str, Any]:
    return {
        "second_orientation_view_id": orientation_view_id,
        "second_orientation_view_type": SECOND_ORIENTATION_VIEW_TYPE,
        "second_orientation_view_version": RESULT_VERSION,
        "second_orientation_scope": SECOND_ORIENTATION_SCOPE,
        "basis_second_bounded_relevance_receipt_artifact": facts.get(
            "basis_second_bounded_relevance_receipt_artifact"
        ),
        "basis_second_bounded_relevance_receipt_outcome": facts.get(
            "basis_second_bounded_relevance_receipt_outcome"
        ),
        "basis_second_bounded_relevance_receipt_result_version": facts.get(
            "basis_second_bounded_relevance_receipt_result_version"
        ),
        "basis_second_bounded_relevance_receipt_failed_check_count": facts.get(
            "basis_second_bounded_relevance_receipt_failed_check_count"
        ),
        ...
        "second_carrier_context_id": facts.get("second_carrier_context_id"),
        "second_reception_envelope_id": facts.get("second_reception_envelope_id"),
        "inspectably_present": list(INSPECTABLY_PRESENT_ITEMS),
        "standing_identifiers": _standing_identifiers(facts),
        "non_inference": _non_inference(),
        "unavailable": list(UNAVAILABLE_ITEMS),
        "orientation_statement": ORIENTATION_STATEMENT,
        "second_receipt_expanded": False,
        "second_reception_expanded": False,
...
def _build_statement(
    ...
    return {
        "local_relevance_medium_second_relevance_orientation_view_recorded": False,
        ...
        "second_received_signal_id_preserved": bool(
            orientation_view.get("second_received_signal_id")
        ),
        "second_relevance_basis_id_preserved": bool(
            orientation_view.get("second_relevance_basis_id")
        ),
        "second_relevance_scope_id_preserved": bool(
            orientation_view.get("second_relevance_scope_id")
        ),
        "second_carrier_context_id_preserved": bool(
            orientation_view.get("second_carrier_context_id")
        ),
        "second_reception_envelope_id_preserved": bool(
            orientation_view.get("second_reception_envelope_id")
        ),
        "second_orientation_scope_local_only": orientation_view.get("second_orientation_scope")
        == SECOND_ORIENTATION_SCOPE,
        "inspectably_present_preserved": bool(orientation_view.get("inspectably_present")),
        "standing_identifiers_preserved": bool(orientation_view.get("standing_identifiers")),
        "non_inference_preserved": bool(orientation_view.get("non_inference")),
        "unavailable_preserved": bool(orientation_view.get("unavailable")),
...
def build_local_relevance_medium_second_relevance_orientation_view_v0_min_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    checks = result.get("local_relevance_medium_second_relevance_orientation_view_checks", [])
    if not isinstance(checks, list):
        checks = []
    passed_count, failed_count = _checks_counts(checks)
    metadata = _as_mapping(
        result.get("local_relevance_medium_second_relevance_orientation_view_metadata")
    )
    block = _as_mapping(result.get("block"))
    statement = _as_mapping(
        result.get("local_relevance_medium_second_relevance_orientation_view_statement")
    )
    orientation_view = _as_mapping(result.get("second_relevance_orientation_view"))
    question = result.get("declared_local_relevance_medium_second_relevance_orientation_view_question")

    return {
        "outcome": result.get("outcome"),
        "block_code": block.get("block_code") or block.get("code"),
        "block_reason": block.get("reason"),
        "second_orientation_id": orientation_view.get("second_orientation_view_id")
        or metadata.get("local_relevance_medium_second_relevance_orientation_view_id"),
        "question": question,
        "intent": metadata.get("intent"),
        "passed_check_count": passed_count,
        "failed_check_count": failed_count,
        "result_version": metadata.get(
            "local_relevance_medium_second_relevance_orientation_view_version"
        )
        or RESULT_VERSION,
        "resolver_module": metadata.get("resolver_module") or RESOLVER_MODULE,
        "second_orientation_view_recorded": statement.get(
            "local_relevance_medium_second_relevance_orientation_view_recorded", False
        ),
        ...
        "second_orientation_scope_local_only": statement.get(
            "second_orientation_scope_local_only", False
        ),
        "inspectably_present_preserved": statement.get("inspectably_present_preserved", False),
        "standing_identifiers_preserved": statement.get("standing_identifiers_preserved", False),
        "non_inference_preserved": statement.get("non_inference_preserved", False),
        "unavailable_preserved": statement.get("unavailable_preserved", False),
        "second_orientation_view_object_summary": {
            "second_orientation_view_type": orientation_view.get("second_orientation_view_type"),
            "second_orientation_scope": orientation_view.get("second_orientation_scope"),
            "second_received_signal_id": orientation_view.get("second_received_signal_id"),
            "second_relevance_basis_id": orientation_view.get("second_relevance_basis_id"),
            "second_relevance_scope_id": orientation_view.get("second_relevance_scope_id"),
            "second_carrier_context_id": orientation_view.get("second_carrier_context_id"),
            "second_reception_envelope_id": orientation_view.get("second_reception_envelope_id"),
            "inspectably_present": orientation_view.get("inspectably_present", []),
            "standing_identifiers": orientation_view.get("standing_identifiers", {}),
        },
        "second_receipt_and_second_reception_not_expanded": True,
        "second_index_entry_not_created": True,
...
def _assemble_result(
    ...
    requested_outcome = request.get(
        "requested_local_relevance_medium_second_relevance_orientation_view_outcome"
    )
    intent = request.get("local_relevance_medium_second_relevance_orientation_view_intent")
    ...
    orientation_view_id = str(
        request.get("local_relevance_medium_second_relevance_orientation_view_id")
        or DEFAULT_SECOND_ORIENTATION_VIEW_ID
    )
    orientation_view = (
        _build_second_relevance_orientation_view(orientation_view_id, facts) if recorded else {}
    )
    statement = _build_statement(recorded, orientation_view, facts)
    ...
    block = {
        ...
        if outcome == OUTCOME_BLOCKED and request.get("block_reason")
            ...
            "local relevance medium second relevance orientation view blocked"
    ...
    result: dict[str, Any] = {
        "local_relevance_medium_second_relevance_orientation_view_metadata": {
            ...
            "local_relevance_medium_second_relevance_orientation_view_type": (
                SECOND_ORIENTATION_VIEW_TYPE
            ),
            "local_relevance_medium_second_relevance_orientation_view_version": (
                RESULT_VERSION
        ...
        "declared_local_relevance_medium_second_relevance_orientation_view_question": (
            _sanitize(request.get("local_relevance_medium_second_relevance_orientation_view_question"))
        ),
        "selected_second_bounded_relevance_receipt_artifact_basis": {
            "selected_second_bounded_relevance_receipt_artifact": _sanitize(
                request.get("selected_second_bounded_relevance_receipt_artifact")
            ),
            "basis_second_bounded_relevance_receipt_artifact": _sanitize(
                facts.get("basis_second_bounded_relevance_receipt_artifact")
            ),
            "basis_second_bounded_relevance_receipt_outcome": facts.get(
                "basis_second_bounded_relevance_receipt_outcome"
        ...
        "second_relevance_orientation_view": _sanitize(orientation_view),
        "local_relevance_medium_second_relevance_orientation_view_checks": _sanitize(checks),
        "local_relevance_medium_second_relevance_orientation_view_statement": statement,
    ...
    result["local_relevance_medium_second_relevance_orientation_view_summary"] = (
        build_local_relevance_medium_second_relevance_orientation_view_v0_min_summary(result)
    )
    return result


def resolve_local_relevance_medium_second_relevance_orientation_view_v0_min(
    declared_local_relevance_medium_second_relevance_orientation_view: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    if declared_local_relevance_medium_second_relevance_orientation_view is None:
        request = build_declared_local_relevance_medium_second_relevance_orientation_view_v0_min_request()
    elif isinstance(declared_local_relevance_medium_second_relevance_orientation_view, Mapping):
        request = copy.deepcopy(
            dict(declared_local_relevance_medium_second_relevance_orientation_view)
        )
    else:
        request = {
            "local_relevance_medium_second_relevance_orientation_view_id": (
            ...
            "local_relevance_medium_second_relevance_orientation_view_question": None,
            "local_relevance_medium_second_relevance_orientation_view_intent": None,
            "selected_second_bounded_relevance_receipt_artifact": None,
            "second_orientation_scope": None,
            "second_orientation_view_type": None,
            "declared_non_claims": {},
            "block_reason": "declared request was not a mapping",
        }
        facts = _extract_second_receipt_facts(None, None)
        checks = [
            _check(
                "declared_request_mapping",
                False,
                "mapping request",
                type(declared_local_relevance_medium_second_relevance_orientation_view).__name__,
                "DECLARED_LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW_REQUEST_MALFORMED",
            )
        ]
    ...
    selected_path = request.get("selected_second_bounded_relevance_receipt_artifact")
    if _request_flag(request, "selected_second_bounded_relevance_receipt_artifact_missing"):
        artifact, read_error = None, "SECOND_BOUNDED_RELEVANCE_RECEIPT_ARTIFACT_PATH_MISSING"
    else:
        artifact, read_error = _read_json_object(selected_path)
    facts = _extract_second_receipt_facts(artifact, str(selected_path) if selected_path else None)
    checks = _build_checks(request, artifact, read_error, facts)
    return _assemble_result(request, facts, checks)


def resolve_local_relevance_medium_second_relevance_orientation_view_v0_min_from_path(
    declared_local_relevance_medium_second_relevance_orientation_view_path: Path | str,
) -> dict[str, Any]:
    path = Path(declared_local_relevance_medium_second_relevance_orientation_view_path)
    try:
        with path.open("r", encoding="utf-8") as handle:
            loaded = json.load(handle)
    except (OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
        raise LocalRelevanceMediumSecondRelevanceOrientationViewV0MinError(
            "declared local relevance medium second relevance orientation view request unreadable"
        ) from exc
    if not isinstance(loaded, Mapping):
        request = {
            "local_relevance_medium_second_relevance_orientation_view_id": (
        ...
        checks.extend(_build_checks(request, None, None, facts))
        return _assemble_result(request, facts, checks)
    return resolve_local_relevance_medium_second_relevance_orientation_view_v0_min(loaded)
...
def write_local_relevance_medium_second_relevance_orientation_view_v0_min_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    result_copy = copy.deepcopy(dict(result))
    metadata = _as_mapping(
        result_copy.get("local_relevance_medium_second_relevance_orientation_view_metadata")
    )
    orientation_view_id = str(
        metadata.get("local_relevance_medium_second_relevance_orientation_view_id")
        or DEFAULT_SECOND_ORIENTATION_VIEW_ID
    )
    if output_path is None:
        target = OUTPUT_ROOT / (
            f"{orientation_view_id}__local_relevance_medium_second_relevance_orientation_view_v0_min_result.json"
        )
    else:
...
def build_declared_local_relevance_medium_second_relevance_orientation_view_v0_min_request(
    local_relevance_medium_second_relevance_orientation_view_id: str = (
        DEFAULT_SECOND_ORIENTATION_VIEW_ID
    ),
    selected_second_bounded_relevance_receipt_artifact: Path | str = DEFAULT_SECOND_RECEIPT_ARTIFACT,
    second_orientation_view_type: str = SECOND_ORIENTATION_VIEW_TYPE,
    second_orientation_scope: str = SECOND_ORIENTATION_SCOPE,
    ...
    **overrides: Any,
    ...
    request: dict[str, Any] = {
        ...
        "local_relevance_medium_second_relevance_orientation_view_question": (
            "Given one clean LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT, "
            "may one LOCAL_RELEVANCE_MEDIUM_SECOND_RELEVANCE_ORIENTATION_VIEW be "
            "recorded that orients a local reader to what is inspectably present "
            "in the second receipt, what second received identifiers stand as "
            "received identifiers, what cannot be inferred, and what remains "
            "unavailable, without creating second local index entry, local medium "
            "multiplicity result, relation, comparison, repeated reception "
            "permission, arbitrary reception, feed, index system, registry, search, "
            "ranking, source transfer, source receipt, authority, currentness, "
            "truth, action, synchronization, participation authorization, "
            "participant role, runtime permission, public API, participant-facing "
            "interface, distributed network behavior, operation permission, or "
            "follow-on work?"
        ),
        "local_relevance_medium_second_relevance_orientation_view_intent": intent,
        "selected_second_bounded_relevance_receipt_artifact": str(
            selected_second_bounded_relevance_receipt_artifact
        ),
        "second_orientation_scope": second_orientation_scope,
        "second_orientation_view_type": second_orientation_view_type,
