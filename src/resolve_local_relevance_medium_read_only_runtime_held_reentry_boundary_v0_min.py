"""Resolve one local read-only runtime-held-re-entry boundary.

This resolver records one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY object only. It
reads one clean runtime-held-state artifact, one clean runtime-held-state
...
execution artifact as basis references.

The boundary is local, read-only, selected-state-runtime-held-reentry-
consideration-only, raw-state-body-excluding, state-mutation-refusing,
state-update-refusing, closure-token-aware, non-reentry-shaped,
...
class LocalRelevanceMediumReadOnlyRuntimeHeldReentryBoundaryV0MinError(RuntimeError):
    """Bounded resolver error for runtime-held-re-entry boundary handling."""
...
DEFAULT_BOUNDARY_ID = "local_relevance_medium_read_only_runtime_held_reentry_boundary_001"
...
INTENT_RECORD = "RECORD_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY"
INTENT_DO_NOT_RECORD = (
    "DO_NOT_RECORD_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY"
)
INTENT_BLOCK = "BLOCK_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY"
...
RUNTIME_HELD_STATE_BOUNDARY_LAYERS = (
...
BASE_BLOCK_CODES = (
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_QUESTION_UNDECLARED",
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_INTENT_UNSUPPORTED",
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_BLOCK_REQUESTED",
    ...
    "BOUNDARY_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY",
    "BOUNDARY_SCOPE_MISSING",
    "BOUNDARY_SCOPE_NOT_SELECTED_RUNTIME_HELD_REENTRY_CONSIDERATION_ONLY",
    "RUNTIME_HELD_REENTRY_CREATED",
    "PRIOR_RESULT_REENTRY_CYCLE_CREATED",
    "SECOND_OPERATION_CREATED",
    "CONTINUATION_CREATED",
    ...
    "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_REQUEST_MALFORMED",
    "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_REQUEST_UNREADABLE",
)
...
TOP_LEVEL_BLOCK_FIELD_CODES = {
    key: _auto_block_code(key) for key in REQUIRED_FALSE_NON_CLAIMS
}
...
ARTIFACT_SPECS = (
    {
        ...
        "layers": RUNTIME_HELD_STATE_LAYERS,
    },
    {
        ...
        "layers": RUNTIME_HELD_STATE_BOUNDARY_LAYERS,
    },
    {
    ...
    },
    {
        ...
        "layers": RUNTIME_BOUNDARY_LAYERS,
    },
    {
        ...
        "layers": RUNTIME_PERMISSION_LAYERS,
    },
    {
        ...
        "layers": OPERATION_EXECUTION_LAYERS,
    },
)

CORE_QUESTION = (
    "Given one clean LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE for "
    ...
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY be recorded "
    "that permits a future local read-only runtime-held re-entry to be "
    "considered as a separately bounded step, without creating runtime-held "
    "re-entry, creating prior-result re-entry cycle, creating second operation, "
    "creating continuation, creating runtime hosting, creating runtime loop, "
...
WHAT_REMAINS_OPEN = (
    ...
    "local relevance medium read-only runtime-held re-entry boundary terminal summary",
...
def _new_check(
    ...
    if not passed:
        public_code = code if code in BLOCK_CODES else "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_REQUEST_MALFORMED"
        ...
        for key, nested_value in value.items():
            ...
            if str(key).endswith("_checks") or key == "checks":
                if isinstance(nested_value, list):
                    collected.extend(
                        [dict(item) for item in nested_value if _is_mapping(item)]
                    )
            else:
...
def _build_boundary_object(
    ...
    boundary: dict[str, Any] = {
        "boundary_id": str(
            request.get("local_relevance_medium_read_only_runtime_held_reentry_boundary_id")
        ...
        "future_runtime_held_reentry_may_be_considered": bool(recorded),
...
def _build_statement(
    ...
    return {
        "local_relevance_medium_read_only_runtime_held_reentry_boundary_recorded": recorded,
        ...
        is True,
        "runtime_held_state_created": boundary.get("runtime_held_state_created") is True,
        ...
        is True,
        "held_state_basis_reference_only": boundary.get("held_state_basis_reference_only")
        is True,
        "selected_runtime_held_state_boundary_recorded": boundary.get(
            "selected_runtime_held_state_boundary_recorded"
        )
        is True,
        ...
        is True,
        "future_runtime_may_be_considered": boundary.get("future_runtime_may_be_considered")
        is True,
        ...
        is True,
        "runtime_permission_read_only": boundary.get("runtime_permission_read_only") is True,
        ...
        is True,
        "future_runtime_held_reentry_may_be_considered": boundary.get(
            "future_runtime_held_reentry_may_be_considered"
        )
        is True,
        "raw_state_body_embedded": False,
...
def _metadata(request: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "local_relevance_medium_read_only_runtime_held_reentry_boundary_id": str(
            request.get("local_relevance_medium_read_only_runtime_held_reentry_boundary_id")
            or DEFAULT_BOUNDARY_ID
        ),
        "local_relevance_medium_read_only_runtime_held_reentry_boundary_type": BOUNDARY_TYPE,
        "local_relevance_medium_read_only_runtime_held_reentry_boundary_version": RESULT_VERSION,
        "result_version": RESULT_VERSION,
...
def _compose_result(
    ...
    boundary = _build_boundary_object(request, bases, upstream, recorded)
    ...
    result: dict[str, Any] = {
        ...
        "declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_question": {
            "question": _sanitize(
                request.get(
                    "local_relevance_medium_read_only_runtime_held_reentry_boundary_question"
                )
            ),
            "intent": _sanitize(
                request.get(
        ...
        "local_relevance_medium_read_only_runtime_held_reentry_boundary": _sanitize(
            boundary
        ),
        "local_relevance_medium_read_only_runtime_held_reentry_boundary_checks": checks,
        "local_relevance_medium_read_only_runtime_held_reentry_boundary_statement": (
            _build_statement(boundary, bases, recorded)
        ),
    ...
    result["local_relevance_medium_read_only_runtime_held_reentry_boundary_summary"] = (
...
def _blocked_result_from_code(
    ...
    checks = [
        _new_check(
            "declared local relevance medium read-only runtime-held-re-entry boundary request readable",
            False,
...
def build_declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_request(
    *,
    local_relevance_medium_read_only_runtime_held_reentry_boundary_id: str = DEFAULT_BOUNDARY_ID,
    ...
    **extra_fields: Any,
    ...
    """Build a declared request for this one boundary resolver."""
    ...
    request: dict[str, Any] = {
        ...
        "local_relevance_medium_read_only_runtime_held_reentry_boundary_intent": intent,
...
def _validate_request_shape(
    ...
    checks.append(
        _new_check(
            "runtime-held-re-entry boundary question declared",
            isinstance(question, str) and bool(question.strip()),
            "declared runtime-held-re-entry boundary question",
            question,
            "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_QUESTION_UNDECLARED",
        )
    )
    ...
    checks.append(
        _new_check(
            "runtime-held-re-entry boundary intent supported",
    ...
    checks.append(
        _new_check(
            "runtime-held-re-entry boundary block intent not requested",
            intent != INTENT_BLOCK,
            "not block intent",
            intent,
            "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_BLOCK_REQUESTED",
...
def _validate_upstream_postures(
    ...
    runtime_held_state_artifact = artifacts.get("selected_runtime_held_state_artifact")
    runtime_held_state_boundary_artifact = artifacts.get(
        "selected_runtime_held_state_boundary_artifact"
    )
    runtime_artifact = artifacts.get("selected_runtime_artifact")
    runtime_boundary_artifact = artifacts.get("selected_runtime_boundary_artifact")
    runtime_permission_artifact = artifacts.get("selected_runtime_permission_artifact")
    operation_execution_artifact = artifacts.get("selected_operation_execution_artifact")
    ...
    upstream["selected_runtime_held_state_boundary_recorded"] = (
        _add_bool_posture_check(
            checks,
    ...
    upstream["runtime_created"] = _add_bool_posture_check(
        checks, "runtime created", runtime_artifact, ("runtime_created",), "RUNTIME_NOT_CREATED"
    )
...
def _add_generated_boundary_checks(
    checks: list[dict[str, Any]],
    boundary: Mapping[str, Any],
    recorded: bool,
) -> None:
    true_checks = {
        "selected runtime-held state recorded": "selected_runtime_held_state_recorded",
    ...
    for name, field in true_checks.items():
    ...
    for field in BOUNDARY_OBJECT_FALSE_FIELDS:
        false_checks[f"{field} false"] = field
    for name, field in false_checks.items():
        checks.append(
            _new_check(
                name,
...
def resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
    declared_local_relevance_medium_read_only_runtime_held_reentry_boundary: Mapping[
        str, Any
    ]
    | None = None,
) -> dict[str, Any]:
    """Resolve one selected-state runtime-held-re-entry boundary object."""

    if not isinstance(
        declared_local_relevance_medium_read_only_runtime_held_reentry_boundary,
        MappingABC,
    ):
        return _blocked_result_from_code(
            "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_REQUEST_MALFORMED",
            type(declared_local_relevance_medium_read_only_runtime_held_reentry_boundary).__name__,
        )

    request = copy.deepcopy(
        dict(declared_local_relevance_medium_read_only_runtime_held_reentry_boundary)
    )
    checks: list[dict[str, Any]] = [
        _new_check(
            "declared runtime-held-re-entry boundary request mapping",
            True,
            "mapping",
            "mapping",
            "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY_REQUEST_MALFORMED",
    ...
    boundary = _build_boundary_object(request, bases, upstream, recorded)
    _add_generated_boundary_checks(checks, boundary, recorded)

    outcome = OUTCOME_RECORDED if _first_failed_code(checks) is None else OUTCOME_BLOCKED
    if outcome == OUTCOME_BLOCKED:
        boundary = _build_boundary_object(request, bases, upstream, False)
    return _compose_result(request, checks, outcome, bases, upstream if outcome == OUTCOME_RECORDED else _empty_upstream())


def resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_from_path(
    declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_path: Path | str,
) -> dict[str, Any]:
    """Read a declared runtime-held-re-entry boundary request from JSON."""

    try:
        loaded = json.loads(
            Path(declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_path).read_text(
                encoding="utf-8"
            )
        )
    ...
    return resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
        loaded
    )


def build_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_summary(
    ...
    summary = {
        ...
        or metadata.get("local_relevance_medium_read_only_runtime_held_reentry_boundary_id"),
        "question": result.get(
            "declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_question",
            {},
        ).get("question")
        ...
        "held_state_basis_reference_only": boundary.get("held_state_basis_reference_only")
        is True,
        "raw_state_body_embedded_false_posture": boundary.get("raw_state_body_embedded")
        is False,
        "state_mutation_performed_false_posture": boundary.get("state_mutation_performed")
        is False,
        "state_update_performed_false_posture": boundary.get("state_update_performed")
        is False,
        "selected_runtime_held_state_boundary_recorded": boundary.get(
            "selected_runtime_held_state_boundary_recorded"
        )
        ...
        "filesystem_discovery_not_performed": boundary.get("filesystem_discovery_performed")
        is False,
...
def write_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write a resolver result under this boundary output root without overwrite."""

    if not isinstance(result, MappingABC):
        raise LocalRelevanceMediumReadOnlyRuntimeHeldReentryBoundaryV0MinError(
    ...
    try:
        ...
        raise LocalRelevanceMediumReadOnlyRuntimeHeldReentryBoundaryV0MinError(
            "output path must remain under the runtime-held-re-entry boundary output root"
        ) from exc
