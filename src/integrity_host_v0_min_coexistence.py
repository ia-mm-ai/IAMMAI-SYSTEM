"""Successor in-memory host for bounded same-host coexistence.

This module implements only the relation slice described by
``spec/INTEGRITY_RELATION_V0_MIN_SPEC.md``. It is a successor code line to
``src/integrity_host_v0_min.py``; it does not rewrite the original
one-open-object proof host and does not promote same-host coexistence into
global IAMMAI system law.

The host preserves the original proof seam:

    CANDIDATE -> PRESENT -> STANDING

STAND remains distinct from FINALIZE. FINALIZE, INVALIDATE, and EVOLVE remain
standing-only resolutions. HOLD remains target-specific orthogonal
permissibility. Unresolved same-host coexistence is accepted only for explicit
distinct-matter cohosting, and every attempted action appends one transition
record, including refusals.
"""
...
class RefusalCode(str, Enum):
    """Closed local refusal family for v0-min coexistence results."""
...
class IntegrityHostV0MinCoexistence:
    """Bounded same-host coexistence successor to the v0-min proof host.

    Distinct-matter unresolved coexistence is a host-local proof extension. It
    is not global IAMMAI relation law.
    """
    ...
    def _check_unresolved_coexistence(
        ...
        *,
        ...
        overlap_open_ids = tuple(open_id for open_id in open_ids if open_id in overlap_ids)
        if overlap_open_ids:
            return (
                self._refuse(
                    action_type,
