...
This successor keeps the original bounded self-orientation model intact while
aligning it to the live standing artifact line. It remains one internal,
derived recognition surface over already-standing artifacts.

V2 exists for two narrow reasons:

- align receipt reading with the live continuity-transfer-receipt payload shape
- preserve already-selected basis surfaces on later blocked outcomes

It does not replay the host, merge preserved runs, widen source scope, turn
derivative/operator artifacts into source authority, or introduce a new
governance or continuity doctrine.
"""
...
class CurrentSelfOrientationV2Error(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        self.selected_inputs = copy.deepcopy(dict(selected_inputs)) if isinstance(selected_inputs, Mapping) else None
...
CURRENT_SELF_ORIENTATION_RESULT_VERSION = "0.2.0"
DEFAULT_RESULT_STEM = "current_self_orientation_v2_result"
...
def _string_required(value: Any, context: str, block_code: str) -> str:
    ...
    if normalized is None:
        raise CurrentSelfOrientationV2Error(
...
def _require_mapping(value: Any, context: str, block_code: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise CurrentSelfOrientationV2Error(f"{context} must be an object", block_code)
    return value
...
def _selected_inputs_for_exception(
    selected_inputs: Mapping[str, Any] | None,
) -> dict[str, Any] | None:
    if not isinstance(selected_inputs, Mapping):
        return None
    return _clone_mapping(selected_inputs)


def _attach_selected_inputs(
    exc: CurrentSelfOrientationV2Error,
    selected_inputs: Mapping[str, Any] | None,
) -> CurrentSelfOrientationV2Error:
    if exc.selected_inputs is None and isinstance(selected_inputs, Mapping):
        exc.selected_inputs = _selected_inputs_for_exception(selected_inputs)
    return exc
...
def _body_pass_selected_inputs(
    ...
    selected_inputs["selected_current_state_what_stands_now_result"] = _clone_mapping(
        selected_inputs["selected_source_surface"]
    )
...
def _discover_latest_successful_body_pass_result() -> tuple[Path, dict[str, Any]]:
    ...
    for path in reversed(candidates):
        ...
        except CurrentSelfOrientationV2Error:
            continue
...
def _read_answer_read_result(
    ...
    return (
        _repo_path(answer_read_path),
        answer_read_result,
        answer_resolver.build_current_state_answer_read_summary(answer_read_result),
    )
    ...
    for path in _discover_artifacts(OPENAI_API_DERIVATIVE_VESSEL_V3_ROOT, "vessel v3"):
        ...
        matches.append(
            (
                path,
                artifact,
                vessel_resolver.build_bounded_current_state_vessel_summary(artifact),
            )
        )
    if not matches:
        raise CurrentSelfOrientationV2Error(
...
def _discover_matching_operator_brief_results(
    ...
    for path in _discover_artifacts(OPERATOR_TERMINAL_BRIEF_ROOT, "operator terminal brief"):
        ...
        matches.append(
            (path, artifact, brief_resolver.build_operator_terminal_brief_summary(artifact))
        )
    if not matches:
        raise CurrentSelfOrientationV2Error(
...
def _receipt_derivative_payload(
    receipt_result: Mapping[str, Any],
) -> Mapping[str, Any]:
    live_payload = receipt_result.get("received_payload")
    if isinstance(live_payload, Mapping):
        return live_payload
    compatibility_payload = receipt_result.get("receipt_payload")
    if isinstance(compatibility_payload, Mapping):
        return compatibility_payload
    raise CurrentSelfOrientationV2Error(
        "continuity-transfer receipt payload must be present as received_payload or receipt_payload",
        "SOURCE_ARTIFACT_MALFORMED",
    )
...
def _validate_derivative_posture(
    ...
    receipt_payload = _receipt_derivative_payload(receipt_result)
...
def _result_id(body_pass_result_id: str | None, outcome: str) -> str:
    ...
    suffix = (
        "current_self_orientation_v2_self_oriented"
        if outcome == OUTCOME_SELF_ORIENTED
        else "current_self_orientation_v2_blocked"
    )
...
def _summary_from_result(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "correspondence_checks_passed": bool(checks) and all(
...
def _selected_basis_values(selected_inputs: Mapping[str, Any]) -> dict[str, Any]:
    body_pass = selected_inputs.get("selected_body_pass_result")
    body_pass = body_pass if isinstance(body_pass, Mapping) else {}
    source = selected_inputs.get("selected_source_surface")
    source = source if isinstance(source, Mapping) else {}
    answer = selected_inputs.get("selected_current_state_answer_read_result")
    answer = answer if isinstance(answer, Mapping) else {}
    effective_references = selected_inputs.get("selected_effective_references")
    effective_references = (
        dict(effective_references)
        if isinstance(effective_references, Mapping)
        and any(value is not None for value in effective_references.values())
        else {}
    )
    basis = {}
    if body_pass.get("result_id") is not None:
        basis["orientation_anchor"] = "v0_body_pass_result"
        basis["selected_body_pass_result_id"] = body_pass.get("result_id")
    if source.get("result_id") is not None:
        basis["selected_source_surface_id"] = source.get("result_id")
    if answer.get("result_id") is not None:
        basis["selected_current_state_answer_read_id"] = answer.get("result_id")
    if effective_references:
        basis["effective_references"] = effective_references
    return basis
...
def _blocked_result(
    *,
    ...
    body_pass = body_pass if isinstance(body_pass, Mapping) else {}
    result = {
        "current_self_orientation_v2_metadata": _metadata(
            _string_or_none(body_pass.get("result_id")),
            OUTCOME_BLOCKED,
        ),
        "selected_orientation_inputs": _clone_mapping(selected_inputs),
        "recognized_current_executable_core_line": {},
        "recognized_governing_effective_basis": {},
        "recognized_current_state_surfaces": {},
        "recognized_continuity_surfaces": {},
        ...
        "self_orientation_basis": _selected_basis_values(selected_inputs),
...
def _resolve_current_self_orientation_internal(
    ...
    selected_inputs = _empty_selected_orientation_inputs(
        "provided_mapping" if body_pass_result is not None else body_pass_result_path
    )
    try:
        ...
        query_results = _discover_matching_query_results(
            selected_answer_id,
            effective_references,
        )
        ...
        stand_summary = stand_resolver.build_current_state_what_stands_now_summary(
            stand_result
        )
        ...
        recognized_open_surfaces = _recognized_open_surfaces(
            open_results,
            merged_non_claims,
        )
        recognized_touch_admissibility_surfaces = (
            _recognized_touch_admissibility_surfaces(
                touch_result,
                touch_path,
            )
        )
        ...
        if failed_check is not None:
            raise CurrentSelfOrientationV2Error(
                ...
                _string_or_none(failed_check.get("block_code"))
    ...
    except CurrentSelfOrientationV2Error as exc:
        raise _attach_selected_inputs(exc, selected_inputs)
...
def resolve_current_self_orientation(
    ...
    try:
        ...
        preserved_selected_inputs = (
            _selected_inputs_for_exception(exc.selected_inputs)
            if isinstance(exc.selected_inputs, Mapping)
            else None
        )
        if preserved_selected_inputs is None and body_pass_result is not None and isinstance(body_pass_result, Mapping):
            try:
                preserved_selected_inputs = _body_pass_selected_inputs(
                    body_pass_result,
                    "provided_mapping",
                )
            except CurrentSelfOrientationV2Error:
                preserved_selected_inputs = _empty_selected_orientation_inputs(
                    "provided_mapping"
                )
        selected_inputs = preserved_selected_inputs or selected_inputs
...
def resolve_current_self_orientation_from_path(
    ...
    try:
        ...
        if preserved_selected_inputs is None:
            try:
                artifact = _read_json_file(
                    body_pass_result_path,
                    context="selected v0 body pass result",
...
def build_current_self_orientation_summary(
    ...
    if not isinstance(result, Mapping):
        raise CurrentSelfOrientationV2Error("self-orientation result must be a mapping")
    return _summary_from_result(result)


def _safe_default_output_path(
    result: Mapping[str, Any],
    root: Path | str = CURRENT_SELF_ORIENTATION_V2_ROOT,
) -> Path:
...
def write_current_self_orientation_result(
    ...
    if target.exists():
        raise FileExistsError(
            f"current self-orientation v2 result already exists: {target}"
        )
