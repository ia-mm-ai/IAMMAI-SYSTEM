"""Resolve one bounded body-signal recognition candidate, v2.

This successor preserves the v1 signal-recognition model and corrects one live
source-identity boundary: a selected artifact's own path/top-level identity
must not be replaced by incidental nested surfaces inside that artifact.

The resolver evaluates exactly one standing source artifact/result and one
candidate signal declaration. It decides only ``SIGNAL_RECOGNIZED`` or
``BLOCKED``. Recognition is not authority, permission, currentness, routing,
priority, scheduling, aggregation, workflow, roadmap, RADIO 22, or
continuation.
"""
...
class BodySignalRecognitionV2Error(RuntimeError):
...
DEFAULT_RESULT_STEM = "body_signal_recognition_v2_result"
...
FAMILY_METADATA_PREFIXES = {
    "v0_body_pass_result": ("v0_body_pass", "body_pass"),
    "current_self_orientation_v3_result": ("self_orientation", "current_self_orientation"),
    "current_self_orientation_reentry_admissibility_result": ("reentry_admissibility",),
    "current_self_orientation_reentry_receipt_result": ("reentry_receipt",),
    "current_state_what_stands_now_result": ("what_stands_now", "current_state"),
    "current_state_what_remains_open_result": ("what_remains_open", "current_state"),
    "current_state_query_result": ("current_state_query", "current_state"),
    "current_state_touch_permission_result": ("touch_permission", "current_state_touch"),
    "continuity_transfer_unit_result": ("continuity_transfer", "transfer"),
    "continuity_transfer_receipt_result": ("continuity_transfer_receipt", "transfer_receipt"),
    "received_derivative_participation_result": ("received_derivative_participation", "participation"),
    "received_derivative_action_permission_result": ("received_derivative_action_permission", "action_permission"),
    "continuity_memory_seam_result": ("continuity_memory_seam", "seam"),
    "openai_api_derivative_vessel_bounded_current_state_read_v3_result": (
        "openai_api_derivative_vessel",
        "derivative_vessel",
        "vessel",
    ),
    "operator_terminal_brief_result": ("operator_terminal_brief", "operator_facing", "brief"),
}
...
BLOCK_REASONS = {
    ...
    "CANDIDATE_SIGNAL_DECLARATION_MISSING": "A candidate signal declaration is required.",
    "CANDIDATE_SIGNAL_DECLARATION_MALFORMED": "The candidate signal declaration is malformed.",
    ...
    "SIGNAL_ATTEMPTS_CURRENTNESS": "The candidate signal attempts to create currentness.",
    ...
    "SOURCE_FAMILY_EXTRACTION_AMBIGUOUS": (
        "The selected source family could not be extracted without ambiguity."
    ),
}
...
def _read_json_file(path: Path | str) -> dict[str, Any]:
    ...
    try:
        ...
        raise BodySignalRecognitionV2Error(
    ...
    *,
    ...
    for mapping in mappings:
        for key, value in mapping.items():
            key_text = str(key)
            if not allow_selected and key_text.startswith("selected_"):
                continue
...
def _first_key_with_prefixes(
    mappings: Sequence[Mapping[str, Any]],
    prefixes: Sequence[str],
    suffixes: Sequence[str],
) -> Any:
    normalized_prefixes = tuple(_normal_text(prefix) for prefix in prefixes)
    for mapping in mappings:
        for key, value in mapping.items():
            key_text = _normal_text(key)
            if key_text.startswith("selected"):
                continue
            if not any(key_text.startswith(prefix) for prefix in normalized_prefixes):
                continue
            if any(key_text.endswith(_normal_text(suffix)) for suffix in suffixes):
                if _string_or_none(value) is not None:
                    return value
    return None


def _source_basis(candidate_signal: Mapping[str, Any] | None) -> dict[str, Any]:
...
def _path_family(path: Path | str | None) -> str | None:
    display = _display_path(path)
    if display is None:
        return None
    normalized = display.replace("\\", "/").strip("/")
    for root, family in PATH_FAMILY_RULES:
...
def _path_family_source_path(
    source: Mapping[str, Any],
    source_path_hint: Path | str | None,
    candidate_basis: Mapping[str, Any],
) -> tuple[str | None, str | None, bool]:
    if source_path_hint is not None:
        display = _display_path(source_path_hint)
        return display, _path_family(display), True
    ...
    display = _string_or_none(source_path)
    return display, _path_family(display), source_path_exposed


def _family_prefixes(family: str | None) -> tuple[str, ...]:
    if family is None:
        return ()
    normalized = _normal_text(family)
    prefixes = list(FAMILY_METADATA_PREFIXES.get(normalized, ()))
    if normalized.endswith("_result"):
        prefixes.append(normalized[: -len("_result")])
    prefixes.append(normalized)
    return tuple(dict.fromkeys(prefixes))


def _type_implies_family(source_type: Any) -> str | None:
    text = _normal_text(source_type)
    type_rules = (
        ("v0_body_pass_result", ("v0_body_pass", "body_pass")),
        ("current_self_orientation_v3_result", ("current_self_orientation_v3",)),
        ("current_self_orientation_reentry_admissibility_result", ("reentry_admissibility",)),
        ("current_self_orientation_reentry_receipt_result", ("reentry_receipt",)),
        ("operator_terminal_brief_result", ("operator_terminal_brief", "operator_facing")),
        (
            "openai_api_derivative_vessel_bounded_current_state_read_v3_result",
            ("openai_api_derivative_vessel", "derivative_vessel"),
        ),
        ("continuity_memory_seam_result", ("continuity_memory_seam", "memory_seam")),
        (
            "received_derivative_action_permission_result",
            ("received_derivative_action_permission", "action_permission"),
        ),
        ("received_derivative_participation_result", ("received_derivative_participation",)),
        ("continuity_transfer_receipt_result", ("continuity_transfer_receipt",)),
        ("continuity_transfer_unit_result", ("continuity_transfer_unit",)),
        ("current_state_touch_permission_result", ("touch_permission", "admissibility_and_touch")),
        ("current_state_what_remains_open_result", ("what_remains_open",)),
        ("current_state_what_stands_now_result", ("what_stands_now",)),
        ("current_state_query_result", ("current_state_query",)),
        ("current_state_result", ("current_state", "answer_read", "readout")),
    )
    for family, markers in type_rules:
        if any(marker in text for marker in markers):
            return family
    return None


def _shallow_infer_family(source: Mapping[str, Any], source_type: Any) -> str | None:
    haystack = _text_blob(list(source.keys()), source_type, deep=False)
    family_rules = (
        ("v0_body_pass_result", ("v0_body_pass", "body_pass")),
        ("current_self_orientation_v3_result", ("current_self_orientation_v3",)),
        ("current_self_orientation_reentry_admissibility_result", ("reentry_admissibility",)),
        ("current_self_orientation_reentry_receipt_result", ("reentry_receipt",)),
        ("operator_terminal_brief_result", ("operator_terminal_brief", "operator_facing")),
        ("openai_api_derivative_vessel_bounded_current_state_read_v3_result", ("derivative_vessel", "openai_api_derivative")),
        ("continuity_memory_seam_result", ("continuity_memory_seam", "memory_seam")),
        ("received_derivative_action_permission_result", ("received_derivative_action_permission", "action_permission")),
        ("received_derivative_participation_result", ("received_derivative_participation",)),
        ("continuity_transfer_receipt_result", ("continuity_transfer_receipt", "transfer_receipt")),
        ("continuity_transfer_unit_result", ("continuity_transfer_unit", "continuity_transfer")),
        ("current_state_touch_permission_result", ("touch_permission", "admissibility_and_touch")),
        ("current_state_what_remains_open_result", ("what_remains_open",)),
        ("current_state_what_stands_now_result", ("what_stands_now",)),
        ("current_state_query_result", ("current_state_query",)),
        ("current_state_result", ("current_state", "answer_read", "readout")),
        ("current_governing_basis_result", ("current_governing", "governing", "effective", "execution_authority")),
    )
...
def _family_specific_value(
    ...
    if not prefixes:
        return None
    value = _first_key_with_prefixes(metadata, prefixes, suffixes)
    if value is not None:
        return value
    return _first_key_with_prefixes(summaries, prefixes, suffixes)
...
def _source_identity(
    ...
    candidate_basis = _source_basis(candidate_signal)
    metadata = _metadata_mappings(source)
    summaries = _summary_mappings(source)
    selected_path, selected_path_family, source_path_exposed = _path_family_source_path(
        source,
        source_path_hint,
        candidate_basis,
    )

    top_level_type = _first_from_mappings(
        (source,),
        ("source_artifact_type", "result_type", "artifact_type", "type"),
    )
    family_type = _family_specific_value(
        metadata,
        summaries,
        selected_path_family,
        ("_result_type", "_artifact_type", "result_type", "artifact_type"),
    )
    source_type = top_level_type or family_type
    if source_type is None:
        source_type = _first_key_ending(
            metadata,
            ("_result_type", "_artifact_type"),
    ...
    top_level_family = _first_from_mappings(
        (source,),
    ...
    source_family = (
        selected_path_family
        or top_level_family
        or _type_implies_family(source_type)
        or _shallow_infer_family(source, source_type)
    )
    source_family_exposed = source_family is not None
    if source_family is None:
        source_family = candidate_basis.get("source_artifact_family")

    source_id = _first_from_mappings(
        (source,),
        ("source_artifact_id", "result_id", "artifact_id"),
    )
    if source_id is None:
        source_id = _family_specific_value(
            metadata,
            summaries,
            source_family,
            ("_result_id", "_artifact_id", "result_id", "artifact_id"),
        )
    if source_id is None:
        source_id = _first_key_ending(
            metadata,
            ("_result_id", "_artifact_id"),
            allow_selected=False,
        )
    if source_id is None:
        source_id = _first_key_ending(
    ...
    source_outcome = _first_from_mappings(
        (source,),
        ("outcome", "source_artifact_outcome", "result_outcome"),
    )
    if source_outcome is None:
        source_outcome = _family_specific_value(
            metadata,
            summaries,
            source_family,
            ("_outcome", "outcome", "_result_outcome"),
        )
    source_outcome_exposed = source_outcome is not None
    ...
    nested_text = _text_blob(source, deep=True)
    nested_incidental_family_present = any(
        marker in nested_text
        for marker in (
            "received_derivative_action_permission",
            "received_derivative_participation",
            "continuity_transfer_receipt",
            "continuity_transfer_unit",
            "current_state_touch_permission",
        )
    )

    return {
        "source_artifact_id": _string_or_none(source_id),
        "source_artifact_path": _string_or_none(selected_path),
        ...
        "_source_outcome_exposed": source_outcome_exposed,
        "_path_derived_family": selected_path_family,
        "_nested_incidental_family_present": nested_incidental_family_present,
    }
...
def _source_haystack(source: Mapping[str, Any], identity: Mapping[str, Any]) -> str:
    # V2 intentionally uses selected identity plus shallow source keys. It does
    # not recurse through nested selected-organ surfaces to determine family.
    return _text_blob(
...
def _category_matches_source(
    ...
    family = _normal_text(identity.get("source_artifact_family"))
    ...
    if category == "BODY_PASS_SIGNAL":
        return (
            ...
            and _normal_text(identity.get("source_artifact_outcome"))
            == "v0_body_pass_confirmed"
        ) or "body_pass" in haystack
    ...
    if category == "TOUCH_ADMISSIBILITY_SIGNAL":
        return (
            ...
            and ("touch" in haystack or "admissibility" in haystack)
        )
...
def _source_identity_checks(source_identity: Mapping[str, Any]) -> list[dict[str, Any]]:
    path_family = source_identity.get("_path_derived_family")
    selected_family = source_identity.get("source_artifact_family")
    selected_outcome = source_identity.get("source_artifact_outcome")
    nested_present = bool(source_identity.get("_nested_incidental_family_present"))
    is_body_pass_path = path_family == "v0_body_pass_result"
    is_confirmed_body_pass = _normal_text(selected_outcome) == "v0_body_pass_confirmed"

    return [
        _check(
            "path_derived_selected_source_family_not_overridden_by_nested_incidental_fields",
            path_family is None or _loose_family_equal(selected_family, path_family),
            "path-derived selected source family remains selected family",
            {"path_family": path_family, "selected_family": selected_family},
            "SOURCE_FAMILY_EXTRACTION_AMBIGUOUS",
        ),
        _check(
            "body_pass_path_with_confirmed_outcome_recognized_as_v0_body_pass_result",
            not is_body_pass_path
            or _string_or_none(selected_outcome) is None
            or (
                _loose_family_equal(selected_family, "v0_body_pass_result")
                and is_confirmed_body_pass
            ),
            "body-pass path with V0_BODY_PASS_CONFIRMED is v0_body_pass_result",
            {
        ...
        _check(
            ...
            not (is_body_pass_path and nested_present)
            or _loose_family_equal(selected_family, "v0_body_pass_result"),
            "nested selected organ surfaces do not replace the selected body-pass source",
            {
                "nested_incidental_family_present": nested_present,
                "selected_family": selected_family,
            },
            "SOURCE_FAMILY_EXTRACTION_AMBIGUOUS",
        ),
    ...
    *,
    ...
    checks.append(
        _check(
            ...
            "readable"
            if source_exists and source_readable and not source_malformed
            else "unreadable or malformed",
            "SOURCE_ARTIFACT_UNREADABLE" if not source_malformed else "SOURCE_ARTIFACT_MALFORMED",
        )
    )
    checks.extend(_source_identity_checks(source_identity))
    checks.append(
        _check(
    ...
    checks.append(
        _check(
            "signal_category_corresponds_to_selected_source_artifact_family_outcome",
            category_matches,
            "signal category corresponds to selected source family/outcome",
            {
    ...
    checks.append(
        _check(
            "candidate_preserves_selected_source_family",
            family_preserved,
    ...
    outcome_preserved = (
        ...
        and _loose_equal(candidate_outcome, source_outcome)
    )
    checks.append(
        _check(
    ...
    checks.append(
        _check(
            ...
            "all true"
            if bad_non_claim is None
            else {bad_non_claim: declared_non_claims.get(bad_non_claim)},
...
def _result_id(
    ...
    return (
        ...
        f"body_signal_recognition_v2_{outcome.lower()}"
    )
...
def _recognition_basis(
    ...
    return {
        ...
        "path_derived_source_family_preserved": source_identity.get("_path_derived_family")
        is None
        or _loose_family_equal(
            source_identity.get("source_artifact_family"),
            source_identity.get("_path_derived_family"),
        ),
    }
...
def _build_result(
    *,
    ...
    result: dict[str, Any] = {
        "body_signal_recognition_v2_metadata": _result_metadata(
            source_identity,
            candidate_copy,
            outcome,
        ),
...
def resolve_body_signal_recognition(
    ...
    except BodySignalRecognitionV2Error as exc:
        return _blocked_result(
            exc.block_code,
...
def build_body_signal_recognition_summary(result: Mapping[str, Any]) -> dict[str, Any]:
    """Build a bounded summary for one body-signal recognition v2 result."""
    ...
    return {
        ...
        "path_source_family_preserved": _check_passed(
            checks,
            "path_derived_selected_source_family_not_overridden_by_nested_incidental_fields",
        ),
        "key_non_claims": {
...
def _safe_default_output_path(
    ...
    root: Path | str = BODY_SIGNAL_RECOGNITION_V2_ROOT,
) -> Path:
    candidate = result.get("selected_candidate_signal")
    candidate = candidate if isinstance(candidate, Mapping) else {}
...
def write_body_signal_recognition_result(
    ...
    """Write one additive body-signal recognition v2 JSON artifact."""
