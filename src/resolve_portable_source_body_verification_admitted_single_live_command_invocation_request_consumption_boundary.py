"""Resolve the admitted single live command invocation request consumption boundary.

This resolver answers one question only:

    Can the admitted single live command invocation request be bounded for one
    future consumption review without consuming the request or creating command
    invocation, command execution, command output, command result, command
    success, standing invocation lane, repeat invocation permission, source,
...
The resolver records consumption-boundary conditions only. It does not consume
the admitted request, invoke a command, execute a command, create command
output/result/success, repair or hide the predecessor line, mutate artifacts,
deploy, host, publish, create permission, or authorize follow-on work.
"""
...
class PortableSourceBodyVerificationAdmittedSingleLiveCommandInvocationRequestConsumptionBoundaryError(
    Exception
):
    """Raised for hard consumption-boundary input/output failures."""


RESOLVER_MODULE = (
    "resolve_portable_source_body_verification_admitted_single_live_command_"
    "invocation_request_consumption_boundary"
)
RESULT_VERSION = "0.1.0"
PORTABLE_SOURCE_BODY_VERIFICATION_ADMITTED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_CONSUMPTION_BOUNDARY_ROOT = Path(
    "artifacts/"
    "integrity_host_v0_min_coexistence_portable_source_body_verification_"
    "admitted_single_live_command_invocation_request_consumption_boundary"
)
...
OUTCOME_NOT_RECORDED = (
    "ADMITTED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_CONSUMPTION_BOUNDARY_NOT_RECORDED"
)
OUTCOME_REQUIRES_ADDITIONAL_BASIS = (
    "ADMITTED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_CONSUMPTION_BOUNDARY_"
...
INTENT_DO_NOT_RECORD = (
    "DO_NOT_RECORD_ADMITTED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_CONSUMPTION_BOUNDARY"
)
INTENT_BLOCK = (
    "BLOCK_ADMITTED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_CONSUMPTION_BOUNDARY_REVIEW"
)
SUPPORTED_CONSUMPTION_BOUNDARY_INTENTS = {
    INTENT_RECORD,
    INTENT_DO_NOT_RECORD,
    INTENT_BLOCK,
}
...
SUPPORTED_CONSUMPTION_BOUNDARY_SCOPE = {
    "CONSUMPTION_BOUNDARY_ONLY",
    "ADMITTED_REQUEST_NOT_CONSUMED_HERE",
    "ONE_SHOT_CONSUMPTION_REVIEW_ONLY",
    ...
    "REQUEST_ADMISSION_IS_NOT_EXECUTION_PERMISSION",
    "REQUEST_ADMISSION_IS_NOT_COMMAND_SUCCESS",
...
FULL_BODY_OMISSION_MARKER = (
    ...
    "invocation request consumption boundary]"
)
...
COLLAPSE_FIELD_CODES = (
    ...
    ("request_consumed", "REQUEST_CONSUMED_HERE"),
    ("request_consumption_created", "REQUEST_CONSUMED_HERE"),
    (
        "single_invocation_request_admission_recorded_as_execution",
        "REQUEST_ADMISSION_TREATED_AS_EXECUTION_PERMISSION",
    ),
...
BLOCK_REASONS = {
    "DECLARED_CONSUMPTION_BOUNDARY_REQUEST_MALFORMED": (
        "Declared consumption-boundary request is malformed."
    ),
    "DECLARED_CONSUMPTION_BOUNDARY_REQUEST_UNREADABLE": (
        "Declared consumption-boundary request path could not be read."
    ),
    "CONSUMPTION_BOUNDARY_QUESTION_UNDECLARED": (
        "Consumption-boundary question is undeclared."
    ),
    "CONSUMPTION_BOUNDARY_INTENT_UNSUPPORTED": (
        "Consumption-boundary intent is unsupported."
    ),
    "CONSUMPTION_BOUNDARY_REVIEW_EXPLICITLY_BLOCKED": (
        "Consumption-boundary review was explicitly blocked by declared intent."
    ),
    "V2_ADMITTED_REQUEST_BASIS_MISSING": (
        "Selected admitted request basis is missing."
    ),
    ...
    "V2_RAW_FULL_PRIOR_ARTIFACT_BODY_RETURNED": (
        "Selected admitted request basis contains raw full prior artifact body posture."
    ),
    "V2_TERMINAL_SUMMARY_BASIS_MISSING": "Selected terminal summary basis is missing.",
    "V1_PREDECESSOR_FAILURE_BASIS_MISSING": (
    ...
    "UNSUPPORTED_CONSUMPTION_BOUNDARY_SCOPE": (
        "Consumption-boundary scope contains unsupported values."
    ),
    "EXECUTION_SEPARATION_POSTURE_MISSING": (
        "Execution separation posture is missing."
    ),
    "REQUEST_CONSUMED_HERE": "The request is treated as consumed here.",
...
def _scope_values(value: Any) -> list[str]:
    if isinstance(value, Mapping):
        for key in ("selected_scope_values", "scope_values", "consumption_boundary_scope"):
...
def _basis_section(value: Any, label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "selected_basis_does_not_consume_request": True,
        "selected_basis_does_not_authorize_command_invocation": True,
        "selected_basis_does_not_authorize_command_execution": True,
...
def _selected_terminal_summary_section(request: Mapping[str, Any]) -> dict[str, Any]:
    section = _basis_section(
        request.get("selected_v2_terminal_summary_basis"),
        "selected_v2_terminal_summary_basis",
    )
    section.update(
        {
            "selected_v2_terminal_summary_path": _sanitize(
                request.get("selected_v2_terminal_summary_path")
                or _find_first(
                    request.get("selected_v2_terminal_summary_basis"),
    ...
    section.update(
        {
            ...
            "successor_does_not_erase_v1": True,
            "predecessor_failure_evidence_is_lineage_evidence_only": True,
            "predecessor_failure_evidence_does_not_block_clean_successor_basis": True,
        }
    )
    return section
...
def _posture_section(value: Any, label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "returned_result_containment_preserved": label
...
def _scope_section(value: Any) -> dict[str, Any]:
    ...
    unsupported = [item for item in values if item not in SUPPORTED_CONSUMPTION_BOUNDARY_SCOPE]
    ...
    return {
        ...
        "consumption_boundary_only": "CONSUMPTION_BOUNDARY_ONLY" in value_set,
        "admitted_request_not_consumed_here": (
            "ADMITTED_REQUEST_NOT_CONSUMED_HERE" in value_set
        ),
        "one_shot_consumption_review_only": (
            "ONE_SHOT_CONSUMPTION_REVIEW_ONLY" in value_set
        ),
        ...
        "execution_requires_separate_review": (
            "EXECUTION_REQUIRES_SEPARATE_REVIEW" in value_set
        ),
        "request_admission_is_not_execution_permission": (
            "REQUEST_ADMISSION_IS_NOT_EXECUTION_PERMISSION" in value_set
        ),
        "request_admission_is_not_command_success": (
            "REQUEST_ADMISSION_IS_NOT_COMMAND_SUCCESS" in value_set
        ),
        "v1_predecessor_failure_remains_visible": (
...
def _build_checks(request: Mapping[str, Any], malformed_code: str | None) -> list[dict[str, Any]]:
    if malformed_code:
        return [
            _check(
                "declared consumption-boundary request is a mapping",
                False,
                "mapping",
                malformed_code,
                malformed_code,
    ...
    checks.append(
        _check(
            ...
            _present(request.get("consumption_boundary_question")),
            "declared question",
            request.get("consumption_boundary_question"),
            "CONSUMPTION_BOUNDARY_QUESTION_UNDECLARED",
        )
    )
    intent = request.get("consumption_boundary_intent")
    checks.append(
        _check(
            "consumption-boundary intent supported",
            intent in SUPPORTED_CONSUMPTION_BOUNDARY_INTENTS,
            sorted(SUPPORTED_CONSUMPTION_BOUNDARY_INTENTS),
            intent,
            "CONSUMPTION_BOUNDARY_INTENT_UNSUPPORTED",
        )
    )
    ...
    checks.extend(
        [
            ...
            _check(
                "v2 raw full prior artifact body not returned",
                not _contains_forbidden_full_body_key(_v2_basis(request))
                and not _contains_truthy_key(
                    _v2_basis(request), "raw_full_prior_artifact_body_returned"
                ),
                "no raw full prior artifact body returned by selected admitted request basis",
                {
                    "forbidden_full_body_keys_detected": _forbidden_full_body_keys(
                        _v2_basis(request)
                    ),
    ...
    scope = _scope_section(request.get("consumption_boundary_scope"))
    checks.append(
        _check(
            "consumption-boundary scope supported",
            scope["all_selected_scope_values_supported"],
            sorted(SUPPORTED_CONSUMPTION_BOUNDARY_SCOPE),
    ...
    collapse_key = _first_truthy_key(request, [key for key, _code in COLLAPSE_FIELD_CODES])
    collapse_code = None
    if collapse_key is not None:
        for key, code in COLLAPSE_FIELD_CODES:
            if key == collapse_key.lower():
                collapse_code = code
                break
    checks.append(
        _check(
            "request not consumed here",
            not _contains_truthy_key(request, "request_consumed_here")
            and not _contains_truthy_key(request, "request_consumed"),
            False,
            _first_truthy_key(request, ("request_consumed_here", "request_consumed")),
            "REQUEST_CONSUMED_HERE",
        )
    )
    checks.append(
    ...
    for check_name, key, code in (
        ...
        checks.append(
            _check(check_name, not _contains_truthy_key(request, key), False, _first_truthy_key(request, (key,)), code)
        )
    ...
    checks.append(
        _check(
            ...
            collapse_code or "DEPLOYMENT_CREATED",
        )
    )
    checks.append(
        _check(
            ...
            collapse_code or "OPERATION_PERMISSION_CREATED",
        )
    )
    checks.append(
        _check(
            ...
            collapse_code or "CONTINUATION_AUTHORIZED",
        )
    )
    checks.append(
        _check(
            ...
            collapse_code or "DERIVATIVE_RECEPTION_AUTHORIZED",
        )
    )
    checks.append(
        _check(
...
def _determine_block_code(
    ...
    if request.get("consumption_boundary_intent") == INTENT_BLOCK:
        return "CONSUMPTION_BOUNDARY_REVIEW_EXPLICITLY_BLOCKED"
    if request.get("requested_consumption_boundary_outcome") == OUTCOME_BLOCKED:
        return "CONSUMPTION_BOUNDARY_REVIEW_EXPLICITLY_BLOCKED"
    requested = request.get("requested_consumption_boundary_outcome")
    if _present(requested) and requested not in OUTCOME_FAMILY:
        return "DECLARED_CONSUMPTION_BOUNDARY_REQUEST_MALFORMED"
    if _contains_forbidden_full_body_key(_v2_basis(request)) or _contains_truthy_key(
        _v2_basis(request), "raw_full_prior_artifact_body_returned"
    ):
        return "V2_RAW_FULL_PRIOR_ARTIFACT_BODY_RETURNED"
...
def _determine_outcome(request: Mapping[str, Any], block_code: str | None) -> str:
    ...
    if request.get("consumption_boundary_intent") == INTENT_DO_NOT_RECORD:
    ...
    statement.update(
        {
            ...
            "single_invocation_request_admission_recorded_as_execution": False,
            ...
            "request_admission_treated_as_execution_permission": False,
            "request_admission_treated_as_command_success": False,
            ...
            "selected_v2_basis_preserved_as_reference": bool(
                sections["selected_v2_admitted_request_basis"].get(
                    "selected_basis_is_reference_shaped"
                )
            ),
            "selected_v2_terminal_summary_basis_preserved_as_reference": bool(
                sections["selected_v2_terminal_summary_basis"].get(
                    "selected_basis_is_reference_shaped"
                )
            ),
            "selected_v1_predecessor_failure_basis_preserved_as_reference": bool(
                sections["selected_v1_predecessor_failure_basis"].get(
                    "selected_basis_is_reference_shaped"
                )
            ),
        }
...
def _non_meaning() -> dict[str, bool]:
    labels = (
        ...
        "v1_was_hidden",
        "v1_passed",
        "v2_admission_became_execution_permission",
        "deployment_created",
        "runtime_hosting_created",
        "public_release_created",
        "public_readiness_created",
    ...
    return {f"consumption_boundary_does_not_mean_{label}": True for label in labels}
...
def _not_recorded(
    ...
    return {
        ...
        "not_recorded_does_not_consume_request": True,
        ...
        "not_recorded_does_not_create_standing_lane": True,
        "not_recorded_does_not_create_repeat_permission": True,
        "not_recorded_does_not_deploy_publish_host": True,
        "not_recorded_does_not_currentize_complete_continue": True,
...
def _metadata(request: Mapping[str, Any]) -> dict[str, Any]:
    request_id = _safe_component(request.get("consumption_boundary_request_id"), "unidentified")
    return {
        ...
        "admitted_single_live_command_invocation_request_consumption_result_type": (
            ...
            "request_consumption_boundary_result"
        ),
...
def _declared_question(request: Mapping[str, Any], request_path: Path | str | None) -> dict[str, Any]:
    return {
        "consumption_boundary_request_id": _sanitize(request.get("consumption_boundary_request_id")),
        "consumption_boundary_question": _sanitize(request.get("consumption_boundary_question")),
        "consumption_boundary_intent": _sanitize(request.get("consumption_boundary_intent")),
        "requested_consumption_boundary_outcome": _sanitize(
            request.get("requested_consumption_boundary_outcome")
        ),
        "declared_consumption_boundary_request_path": str(request_path) if request_path is not None else None,
        "question_answered": (
            "Can the admitted single live command invocation request be bounded "
            "for one future consumption review without consuming the request or "
            "creating command invocation, command execution, command output, "
            "command result, command success, standing invocation lane, repeat "
            "invocation permission, source, authority, currentness, final "
        ...
        "consumption_boundary_is_not_request_consumption": True,
        "consumption_boundary_is_not_command_invocation": True,
        "consumption_boundary_is_not_command_execution": True,
        "consumption_boundary_is_not_output_result_success": True,
        "consumption_boundary_does_not_authorize_next_work": True,
    }
...
def _read_json_object(path_value: Path | str) -> tuple[dict[str, Any] | None, str | None, str | None]:
    ...
    try:
        ...
        return None, "DECLARED_CONSUMPTION_BOUNDARY_REQUEST_UNREADABLE", str(exc)
    except json.JSONDecodeError as exc:
        return None, "DECLARED_CONSUMPTION_BOUNDARY_REQUEST_MALFORMED", str(exc)
    if not isinstance(loaded, dict):
        return None, "DECLARED_CONSUMPTION_BOUNDARY_REQUEST_MALFORMED", "JSON document must be an object."
...
def _build_result(
    ...
    *,
    ...
    if not isinstance(request_value, Mapping) and malformed_code is None:
        malformed_code = "DECLARED_CONSUMPTION_BOUNDARY_REQUEST_MALFORMED"

    sections: dict[str, dict[str, Any]] = {
        "selected_v2_admitted_request_basis": _selected_v2_basis_section(request),
        "selected_v2_terminal_summary_basis": _selected_terminal_summary_section(request),
    ...
    result: dict[str, Any] = {
        ...
        "declared_consumption_boundary_question": _declared_question(request, request_path),
        "selected_v2_admitted_request_basis": sections["selected_v2_admitted_request_basis"],
        "selected_v2_terminal_summary_basis": sections["selected_v2_terminal_summary_basis"],
        "selected_v1_predecessor_failure_basis": sections["selected_v1_predecessor_failure_basis"],
        ...
        "selected_command_implementation_boundary_basis": sections[
            "selected_command_implementation_boundary_basis"
        ],
        "selected_command_boundary_basis": sections["selected_command_boundary_basis"],
        "selected_artifact_emission_containment_basis": sections[
            "selected_artifact_emission_containment_basis"
        ],
        ...
        "consumption_boundary_scope": _scope_section(request.get("consumption_boundary_scope")),
        "consumption_boundary_checks": _checks_summary(checks),
        "consumption_boundary_statement": statement,
        "consumption_boundary_non_meaning": _non_meaning(),
        "additional_basis_required": _additional_basis(outcome, request),
        "not_recorded_basis": _not_recorded(outcome, request, checks),
...
def resolve_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_boundary(
    declared_consumption_boundary_request: Mapping[str, Any] | None = None,
) -> dict:
    """Resolve one declared consumption-boundary request mapping."""

    if declared_consumption_boundary_request is None:
        return _build_result({})
    if not isinstance(declared_consumption_boundary_request, Mapping):
        return _build_result(
            {},
            malformed_code="DECLARED_CONSUMPTION_BOUNDARY_REQUEST_MALFORMED",
        )
    return _build_result(declared_consumption_boundary_request)


def resolve_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_boundary_from_path(
    declared_consumption_boundary_request_path: Path | str,
) -> dict:
    """Resolve one declared consumption-boundary JSON object from a path."""

    request, error_code, error_reason = _read_json_object(declared_consumption_boundary_request_path)
    if error_code:
        return _build_result(
            {},
            request_path=declared_consumption_boundary_request_path,
            malformed_code=error_code,
            path_reason=error_reason,
        )
    return _build_result(request, request_path=declared_consumption_boundary_request_path)


def build_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_boundary_summary(
    result: Mapping[str, Any]
) -> dict:
    """Build a compact non-authoritative consumption-boundary summary."""

    safe_result = _sanitize(result)
    statement = _as_mapping(safe_result.get("consumption_boundary_statement"))
    checks = _as_mapping(safe_result.get("consumption_boundary_checks"))
    block = _as_mapping(safe_result.get("block"))
    question = _as_mapping(safe_result.get("declared_consumption_boundary_question"))
    ...
    terminal_summary = _as_mapping(safe_result.get("selected_v2_terminal_summary_basis"))
    predecessor = _as_mapping(safe_result.get("selected_v1_predecessor_failure_basis"))
    return {
        "outcome": safe_result.get("outcome"),
        "block_code": block.get("block_code"),
        "block_reason": block.get("block_reason"),
        "consumption_boundary_request_id": question.get("consumption_boundary_request_id"),
        "consumption_boundary_question": question.get("consumption_boundary_question"),
        "consumption_boundary_intent": question.get("consumption_boundary_intent"),
        "passed_check_count": checks.get("passed_check_count", 0),
        "failed_check_count": checks.get("failed_check_count", 0),
        "consumption_boundary_recorded": statement.get(
            "admitted_single_live_command_invocation_request_consumption_boundary_recorded",
            False,
        ),
        "selected_v2_admitted_request_basis_preserved": statement.get(
            "selected_v2_admitted_request_basis_preserved", False
        ),
        "v2_successor_lineage_preserved": statement.get(
            "v2_successor_lineage_preserved", False
        ),
        "v1_predecessor_failure_preserved": statement.get(
            "v1_predecessor_failure_preserved", False
        ),
        "single_consumption_review_conditions_declared": statement.get(
            "single_consumption_review_conditions_declared", False
        ),
        "one_shot_consumption_posture_declared": statement.get(
            "one_shot_consumption_posture_declared", False
        ),
        ...
        "selected_v2_terminal_summary_reference": terminal_summary.get(
            "selected_v2_terminal_summary_path"
        ),
        "selected_v1_predecessor_failure_reference": predecessor.get(
            "selected_v1_predecessor_artifact_path"
        ),
        "request_not_consumed_here": statement.get("request_not_consumed_here", True),
        ...
        "request_admission_not_execution_permission": statement.get(
            "request_admission_not_execution_permission", True
        ),
        "request_admission_not_command_success": statement.get(
            "request_admission_not_command_success", True
        ),
...
def build_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_summary(
    ...
    """Alias for the boundary summary helper."""

    return build_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_boundary_summary(
        result
    )
...
def _unique_path(path: Path) -> Path:
    ...
    for index in range(1, 10000):
        ...
        if not candidate.exists():
            return candidate
    raise PortableSourceBodyVerificationAdmittedSingleLiveCommandInvocationRequestConsumptionBoundaryError(
        f"Could not allocate non-overwriting output path for {path}"
    )


def write_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_boundary_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write one additive bounded consumption-boundary result JSON file."""

    safe_result = _sanitize(result)
    question = _as_mapping(safe_result.get("declared_consumption_boundary_question"))
    request_id = _safe_component(question.get("consumption_boundary_request_id"), "unidentified")
    filename = (
...
def write_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_result(
    ...
    """Alias for the boundary result writer."""

    return write_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_boundary_result(
        result, output_path
    )


def build_declared_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_request(
    consumption_boundary_request_id: str,
    consumption_boundary_question: str,
    selected_v2_admitted_request_basis: Mapping[str, Any] | str,
    selected_v2_terminal_summary_basis: Mapping[str, Any] | str,
    ...
    consumption_boundary_scope: Sequence[str] | Mapping[str, Any],
    ...
    *,
    ...
    requested_consumption_boundary_outcome: str = OUTCOME_RECORDED,
    additional_basis_context: Mapping[str, Any] | None = None,
    not_recorded_basis: Mapping[str, Any] | str | None = None,
) -> dict:
    """Build a declared consumption-boundary request with false non-claims."""

    request = {
        "consumption_boundary_request_id": consumption_boundary_request_id,
        "consumption_boundary_question": consumption_boundary_question,
        "consumption_boundary_intent": consumption_boundary_intent,
        "selected_v2_admitted_request_basis": _copy(selected_v2_admitted_request_basis),
        "selected_v2_terminal_summary_basis": _copy(selected_v2_terminal_summary_basis),
        ...
        "consumption_boundary_scope": _copy(consumption_boundary_scope),
        "reference_shaped_input_posture": {
            "reference_shaped_input_posture_declared": True,
            "full_prior_artifact_body_not_emitted": True,
        },
        "non_authority_posture": {
            "non_authority_posture_declared": True,
            "request_admission_not_authority": True,
        },
        "non_currentness_posture": {
            "non_currentness_posture_declared": True,
            "request_admission_not_currentness": True,
        },
        "non_final_completion_posture": {
            "non_final_completion_posture_declared": True,
            "request_admission_not_final_completion": True,
        },
        "requested_consumption_boundary_outcome": requested_consumption_boundary_outcome,
        "declared_non_claims": _false_non_claims(),
    }
    if selected_v2_admitted_request_artifact_path is not None:
        request["selected_v2_admitted_request_artifact_path"] = selected_v2_admitted_request_artifact_path
    if selected_v2_admitted_request_artifact_id is not None:
        request["selected_v2_admitted_request_artifact_id"] = selected_v2_admitted_request_artifact_id
    if selected_v2_admitted_request_outcome is not None:
        request["selected_v2_admitted_request_outcome"] = selected_v2_admitted_request_outcome
    if selected_v2_admitted_request_version is not None:
        request["selected_v2_admitted_request_version"] = selected_v2_admitted_request_version
    if selected_v2_failed_check_count is not None:
        request["selected_v2_failed_check_count"] = selected_v2_failed_check_count
    ...
    if not_recorded_basis is not None:
        request["not_recorded_basis"] = _copy(not_recorded_basis)
    return request
