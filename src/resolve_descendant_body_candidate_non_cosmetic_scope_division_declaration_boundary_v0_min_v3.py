"""Resolve the v3 candidate non-cosmetic scope-division declaration boundary.

V1 and v2 remain preserved lineage. This v3 successor keeps the v2 boundary
semantics and canonical false non-claim coverage, while fixing only the v2
live-marker acceptance defect for the completed descendant-body differentiation
terminal summary. It records one boundary result only and does not define or
implement the future scope declaration operation, declare scopes, emit content,
rerun upstream operations, authorize standing/runtime/authority, scan, repair,
validate prior unsupported claims, or authorize follow-on work.
"""
...
class DescendantBodyCandidateNonCosmeticScopeDivisionDeclarationBoundaryV0MinV3Error(
    Exception
):
    """Raised when bounded v3 result writing cannot be completed."""
...
def _marker_groups(*markers: str | tuple[str, ...]) -> tuple[tuple[str, ...], ...]:
    ...
    for marker in markers:
        groups.append(marker if isinstance(marker, tuple) else (marker,))
    return tuple(groups)
...
BASIS_EMISSION_BOUNDARY_MARKERS = _marker_groups(
    ...
    (
        ...
        "future scope-division operation shape",
    ),
    (
        "Candidate-specific basis must be basis-bearing, not label-bearing",
        "candidate-specific basis-bearing",
    ...
    (
        ...
        "digest difference is not distinctness",
    ),
)
...
DISTINCTNESS_OPERATION_MARKERS = _marker_groups(
    ...
    (
        ...
        "candidate_record_count_compared 2",
        "two candidate records compared",
    ),
...
DIFFERENTIATION_POSTURE_CLASSES = {
    "completion_or_operation_result": (
        "DESCENDANT_BODY_DIFFERENTIATION_OPERATION_RECORDED",
        "descendant-body differentiation operation line",
        "completed descendant-body differentiation operation line",
        "differentiation operation result",
        "completed operation-result summary",
        "live artifact recorded cleanly",
    ...
    "nonstanding": (
        "non-standing candidate records",
    ...
    "not_descendant_body": (
        ...
        "are not descendant bodies",
        "candidate records remain not descendant bodies",
        "candidate_records_are_not_descendant_bodies = true",
        "descendant bodies were not created",
        "descendant_body_created = false",
        "descendant_body_created false",
        "descendant_bodies_not_created = true",
    ...
    "crossing_absent": (
        ...
        "first crossing was not authorized",
        "no crossing",
    ),
    "relation_absent": (
...
def _contains_marker_group(text: str, group: tuple[str, ...]) -> bool:
    ...
    for marker in group:
        needle = marker.lower()
        normalized_needle = _normalize_text(marker)
        if needle in haystack or normalized_needle in normalized:
            return True
    return False


def _contains_any(text: str, markers: tuple[str, ...]) -> bool:
    return _contains_marker_group(text, markers)
...
def _differentiation_posture_summary(reference: Any) -> dict[str, Any]:
    text, readable = _read_declared_text(reference)
    class_hits = {key: False for key in DIFFERENTIATION_POSTURE_CLASSES}
    if readable:
        for key, markers in DIFFERENTIATION_POSTURE_CLASSES.items():
            class_hits[key] = _contains_any(text, markers)

    posture_classes = {
        "completion_or_operation_result": class_hits["completion_or_operation_result"],
        "exactly_two_candidate_records": class_hits["exactly_two_candidate_records"],
        "nonstanding_and_not_descendant_body": class_hits["nonstanding"]
        and class_hits["not_descendant_body"],
        "standing_or_descendant_creation_not_authorized": class_hits[
            "standing_or_descendant_creation_not_authorized"
        ],
        "crossing_and_relation_not_authorized": class_hits["crossing_absent"]
        and class_hits["relation_absent"],
    }
    missing = [key for key, value in posture_classes.items() if value is not True]
    return {
        "reference": _safe_string(reference),
        "readable": readable,
        "required_marker_count": len(posture_classes),
        "present_marker_count": sum(1 for value in posture_classes.values() if value),
        "markers_present": readable and not missing,
        "posture_classes": posture_classes,
        "raw_class_hits": class_hits,
...
def build_declared_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v3_request(
    **overrides: Any,
) -> dict[str, Any]:
    """Build the default declared request for the v3 repo-local boundary."""
...
def _append_required_value_checks(
    ...
    exact_fields = (
        ...
        (
            ...
            "FUTURE_DECLARATION_OPERATION_TYPE_MISSING",
            "FUTURE_DECLARATION_OPERATION_TYPE_NOT_EXPECTED",
            "future declaration operation type",
        ),
        (
            ...
            "RUPTURE_CLASS_BLOCKED_MISSING",
            "RUPTURE_CLASS_BLOCKED_NOT_EXPECTED",
            "rupture class blocked",
        ),
    )
    for field, expected, missing_code, mismatch_code, label in exact_fields:
        actual = declared.get(field)
        checks.append(_check(f"{label} declared", actual is not None, expected, actual, missing_code))
        checks.append(_check(f"{label} exact", actual == expected, expected, actual, mismatch_code))
...
def _basis_marker_summaries(
    ...
    differentiation = _differentiation_posture_summary(
        declared.get("completed_differentiation_operation_terminal_summary_reference")
    )
    summaries["completed_differentiation_operation_terminal_summary"] = differentiation
    checks.append(
        _check(
            "completed differentiation operation terminal summary markers present",
            differentiation["markers_present"] is True,
            "all five differentiation posture classes present",
            {
                "reference": differentiation["reference"],
                "readable": differentiation["readable"],
                "present_marker_count": differentiation["present_marker_count"],
                "required_marker_count": differentiation["required_marker_count"],
                "posture_classes": differentiation["posture_classes"],
                "missing_posture_classes": differentiation["missing_posture_classes"],
            },
            "COMPLETED_DIFFERENTIATION_OPERATION_TERMINAL_SUMMARY_MARKER_MISSING",
        )
    )
...
def resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v3(
    declared_candidate_non_cosmetic_scope_division_declaration_boundary: Mapping[str, Any]
    | None = None,
) -> dict[str, Any]:
    """Resolve one v3 boundary result from a declared request mapping."""
...
def resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v3_from_path(
    declared_candidate_non_cosmetic_scope_division_declaration_boundary_path: Path | str,
) -> dict[str, Any]:
    """Resolve one v3 boundary result from a JSON request path."""
    ...
    return resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v3(
        loaded
    )


def build_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v3_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build a compact v3 summary without raw Markdown bodies."""
    ...
    for key in ALLOWED_TRUE_RECORDED_FIELDS:
        if key == "result_level_non_claims_canonical_false":
            summary[key] = _result_level_non_claims_canonical_false(
    ...
    summary.update(_canonical_non_claims())
    summary["affected_file_treated_as_clean_basis"] = False
    summary["contaminated_lineage_treated_as_clean_basis"] = False
    summary["scope_label_laundering_treated_as_basis"] = False
    summary["cosmetic_scope_naming_treated_as_basis"] = False
    summary["id_role_label_difference_treated_as_scope_basis"] = False
    summary["shared_evidence_treated_as_scope_basis"] = False
    summary["operation_evidence_alone_treated_as_scope_basis"] = False
    summary["contaminated_lineage_treated_as_clean_scope_basis"] = False
    return summary
...
def _assert_output_path_allowed(path: Path) -> None:
    ...
    for forbidden in FORBIDDEN_OUTPUT_ROOT_PARTS:
        if forbidden in parts:
            raise DescendantBodyCandidateNonCosmeticScopeDivisionDeclarationBoundaryV0MinV3Error(
                f"Refusing to write boundary result under forbidden prior root: {forbidden}"
            )


def write_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v3_result(
    result: Mapping[str, Any], output_path: Path | str | None = None
) -> Path:
    """Write a stable v3 JSON result without silently overwriting existing files."""
    ...
    if output_path is None:
        destination = (
            ...
            / f"{boundary_id}__candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v3_result.json"
        )
    else:
