"""Resolve v2 of one descendant-body candidate-record distinctness boundary.

This self-contained successor preserves the v1 boundary semantics and only
widens completed-operation terminal-summary marker acceptance for equivalent
non-standing candidate-record posture. It records one
...
class DescendantBodyCandidateRecordDistinctnessOperationBoundaryV0MinV2Error(
    RuntimeError
):
...
OUTPUT_ROOT = Path(
    ...
    "distinctness_operation_boundary_v0_min_v2"
)
...
NON_STANDING_CANDIDATE_RECORD_MARKERS = (
    "Candidate records are not standing descendants",
    ...
    (
        "Candidate records are not descendant bodies, not standing descendants, "
        "not crossing authorization, not relation"
    ),
...
MARKER_ALTERNATIVES = {
    "Candidate records are not standing descendants": tuple(
        marker
        for marker in NON_STANDING_CANDIDATE_RECORD_MARKERS
        if marker != "Candidate records are not standing descendants"
    ),
    "Candidate records are not crossing authorization": (
        (
            "Candidate records are not descendant bodies, not standing descendants, "
...
def resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2(
...
def resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_from_path(
    ...
    if not _is_mapping(payload):
        ...
        return _build_result({}, checks, OUTCOME_BLOCKED)
    return resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2(
        payload
    )


def build_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_summary(
...
def write_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    if not _is_mapping(result):
        raise DescendantBodyCandidateRecordDistinctnessOperationBoundaryV0MinV2Error(
...
def build_declared_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_request(
    *,
