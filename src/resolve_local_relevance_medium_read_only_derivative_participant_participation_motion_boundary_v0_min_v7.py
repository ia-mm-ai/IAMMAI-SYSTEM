"""v7 resolver for the local read-only derivative participant boundary.

This successor preserves the v6 boundary resolver surface and narrows the
change to continuation terminal-summary text recognition for compact wording
that preserves continuation v1 failure evidence without repairing, hiding, or
claiming the failure passed.
"""
...
RESOLVER_MODULE = (
    "resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7"
)
BOUNDARY_TYPE = (
...
OUTPUT_ROOT = (
    REPO_ROOT
    / "artifacts"
    / "integrity_host_v0_min_coexistence_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7"
)
...
_CONTINUATION_NEGATED_POSTURE_PHRASES = (
    "not repaired hidden or claimed passed",
    "not repaired hidden and claimed passed",
    "not repaired not hidden and not claimed passed",
    "not repaired not hidden not claimed passed",
    "does not repair hide or claim passed",
    "does not repair hide and claim passed",
    "does not repair does not hide and does not claim passed",
    "does not repair does not hide does not claim passed",
    "not repaired",
    "not repair",
    "does not repair",
    "not hidden",
    ...
    "does not claim passed",
    "does not claim it passed",
    "does not claim the failure passed",
)


class LocalRelevanceMediumReadOnlyDerivativeParticipantParticipationMotionBoundaryV0MinV7Error(
    Exception
):
    """Bounded resolver error for invalid direct write or path operations."""


def build_declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7_request(
    ...
    """Build the bounded request mapping consumed by the v7 resolver."""
...
def resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7_from_path(
    ...
    return resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7(
        loaded,
        _pre_checks=checks,
    )


def resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7(
    ...
    *,
    ...
    if declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary is None:
        request: dict[str, Any] = (
            build_declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7_request()
        )
...
def write_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7_result(
    ...
    if not isinstance(result, Mapping):
        raise LocalRelevanceMediumReadOnlyDerivativeParticipantParticipationMotionBoundaryV0MinV7Error(
    ...
    filename = (
        f"{boundary_id}__"
        "local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7_result.json"
    )
...
def build_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build the compact public summary for a v7 result mapping."""
...
def _result_from_parts(
    ...
    *,
    ...
    summary = build_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v7_summary(
        {
    ...
    return {
        ...
        "local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_metadata": {
            ...
            "predecessor_v6_preserved_as_evidence_only": True,
        },
...
def _terminal_text_has_continuation_v1_predecessor_evidence(text: str | None) -> bool:
    ...
    if not normalized:
        return False
    return _continuation_v1_reference_present(normalized) and _continuation_v1_evidence_text_present(
        normalized
    )
...
def _terminal_text_has_continuation_v1_not_repaired_hidden_or_claimed_passed(
    ...
    if not normalized:
        return False
    if not _continuation_v1_reference_present(normalized):
        return False
    if not _continuation_v1_evidence_text_present(normalized):
        return False
    ...
    claimed = (
        ...
        or "does not claim passed" in normalized
        or "does not claim it passed" in normalized
        or "does not claim the failure passed" in normalized
    )
    posture_preserved = compact_posture or (repaired and hidden and claimed)
    if not posture_preserved:
        return False
    return not _has_unnegated_continuation_v1_failure_bad_posture(normalized)


def _continuation_v1_reference_present(normalized: str) -> bool:
    return "continuation" in normalized and "v1" in normalized


def _continuation_v1_evidence_text_present(normalized: str) -> bool:
    return (
        ...
        or "failed lineage evidence" in normalized
        or ("preserves" in normalized and "evidence" in normalized)
        or ("preserved" in normalized and "evidence" in normalized)
    )


def _has_unnegated_continuation_v1_failure_bad_posture(normalized: str) -> bool:
    remainder = f" {normalized} "
    for phrase in sorted(_CONTINUATION_NEGATED_POSTURE_PHRASES, key=len, reverse=True):
        remainder = remainder.replace(f" {phrase} ", " ")
    tokens = remainder.split()
    for index, token in enumerate(tokens):
        if token in {"repair", "repairs", "repaired", "repairing"} and not _has_recent_negation(
            tokens, index
        ):
            return True
        if token in {"hide", "hides", "hidden", "hiding"} and not _has_recent_negation(
            tokens, index
        ):
            return True
        if token in {"claim", "claims", "claimed", "claiming"}:
            next_token = tokens[index + 1] if index + 1 < len(tokens) else ""
            later_token = tokens[index + 2] if index + 2 < len(tokens) else ""
            if (next_token == "passed" or later_token == "passed") and not _has_recent_negation(
                tokens, index
            ):
                return True
    return False
...
def _has_recent_negation(tokens: Sequence[str], index: int) -> bool:
    window = tokens[max(0, index - 3) : index]
    if "not" in window:
        return True
    return len(window) >= 2 and window[-2:] == ["does", "not"]
