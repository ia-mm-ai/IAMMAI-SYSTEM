"""Resolve a bounded single live command invocation request/admission boundary.

This resolver records request/admission posture only. It does not invoke the
command, execute the command, create command output, create command result,
create command success, create a standing invocation lane, or create repeat
invocation permission. Any actual command invocation remains future work unless
separately admitted by a later execution step.
"""
...
class PortableSourceBodyVerificationSingleLiveCommandInvocationRequestAdmissionBoundaryError(
    Exception
):
    """Raised for hard request/admission path or internal shape failures."""
...
PORTABLE_SOURCE_BODY_VERIFICATION_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_ADMISSION_BOUNDARY_ROOT = Path(
    "artifacts/"
    "integrity_host_v0_min_coexistence_portable_source_body_verification_"
    "single_live_command_invocation_request_admission_boundary"
)
...
REQUIRED_REQUEST_FIELDS = (
    ...
    ("proposed one-shot invocation mode declared", "proposed_one_shot_invocation_mode", "PROPOSED_INVOCATION_MODE_MISSING"),
    ("proposed invocation surface declared", "proposed_invocation_surface", "PROPOSED_INVOCATION_SURFACE_MISSING"),
    ("proposed input reference bundle declared", "proposed_input_reference_bundle", "INPUT_REFERENCE_BUNDLE_MISSING"),
    ("proposed output/report destination declared", "proposed_output_report_destination", "OUTPUT_REPORT_DESTINATION_MISSING"),
...
BLOCK_REASONS = {
    ...
    "FULL_PRIOR_ARTIFACTS_EMBEDDED": "Request/admission embeds full prior artifact bodies.",
...
def _sanitize(value: Any) -> Any:
    if isinstance(value, Mapping):
        return {
            key: "[omitted: full prior artifact body is blocked]"
            if key in FULL_ARTIFACT_BODY_KEYS
            else _sanitize(nested)
...
def _basis_id(value: Any) -> Any:
    ...
    return _first_present(mapping, ("selected_result_id", "selected_id", "result_id", "id"))
...
def _basis_path(value: Any) -> Any:
    if isinstance(value, str):
        return value
    mapping = _as_mapping(value)
    return _first_present(mapping, ("selected_result_path", "selected_path", "result_path", "path"))
...
def _reference_section(
    ...
    return {
        ...
        "selected_id": selected_id if _present(selected_id) else _basis_id(value),
        "selected_path": selected_path if _present(selected_path) else _basis_path(value),
        "selected_outcome_or_status": selected_status if _present(selected_status) else _basis_outcome(value),
        "selected_basis": _sanitize(value),
        "reference_shaped_basis_only": not _contains_key(value, FULL_ARTIFACT_BODY_KEYS),
        ...
        "full_prior_artifacts_are_not_embedded": not _contains_key(value, FULL_ARTIFACT_BODY_KEYS),
...
def _build_checks(request: Mapping[str, Any], malformed_code: str | None) -> list[dict[str, Any]]:
    ...
    has_full_body = _contains_key(request, FULL_ARTIFACT_BODY_KEYS)
    checks.append(
        _check(
            "full prior artifacts not embedded",
            not has_full_body,
            "no full prior artifact body keys",
            "full prior artifact body key present" if has_full_body else None,
            "FULL_PRIOR_ARTIFACTS_EMBEDDED",
        )
    )
    checks.append(
        _check(
            ...
            _false_non_claims(),
            _sanitize(request.get("declared_non_claims")),
...
def _block(code: str | None, reason: str | None = None) -> dict[str, Any]:
    return {
        ...
        "block_code": code,
        "block_reason": reason if reason is not None else BLOCK_REASONS.get(code or "", code),
    }
    ...
    statement.update(
        {
            ...
            "invocation_must_not_mutate_artifacts": True,
            "invocation_must_not_embed_full_prior_artifacts": True,
            "request_admission_basis_is_reference_shaped": not _contains_key(request, FULL_ARTIFACT_BODY_KEYS),
        }
    )
...
def _metadata(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "single_live_command_invocation_request_admission_result_id": f"{request_id}__single_live_command_invocation_request_admission_result",
...
def _declared_question(request: Mapping[str, Any], request_path: Path | str | None) -> dict[str, Any]:
    return {
        "invocation_request_admission_id": request.get("invocation_request_admission_id"),
        "invocation_request_admission_question": request.get("invocation_request_admission_question"),
        "invocation_request_admission_intent": request.get("invocation_request_admission_intent"),
        "requested_invocation_request_admission_outcome": request.get("requested_invocation_request_admission_outcome"),
...
def _build_result(
    ...
    *,
    ...
    result["single_live_command_invocation_request_admission_summary"] = build_portable_source_body_verification_single_live_command_invocation_request_admission_summary(result)
    return result


def resolve_portable_source_body_verification_single_live_command_invocation_request_admission_boundary(
    declared_invocation_request_admission: Mapping[str, Any] | None = None,
) -> dict:
    """Resolve one declared request/admission mapping."""
...
def resolve_portable_source_body_verification_single_live_command_invocation_request_admission_boundary_from_path(
...
def build_portable_source_body_verification_single_live_command_invocation_request_admission_summary(
    result: Mapping[str, Any]
) -> dict:
    """Build a compact non-authoritative request/admission summary."""

    statement = _as_mapping(result.get("request_admission_statement"))
    checks = _as_mapping(result.get("request_admission_checks"))
    block = _as_mapping(result.get("block"))
    question = _as_mapping(result.get("declared_invocation_request_admission_question"))
    ...
    return {
        ...
        "not_admitted": result.get("outcome") == OUTCOME_NOT_ADMITTED,
...
def write_portable_source_body_verification_single_live_command_invocation_request_admission_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write one additive request/admission result JSON artifact."""
    ...
    if output_path is None:
        ...
        path = (
            ...
            / f"{request_id}__single_live_command_invocation_request_admission_result.json"
        )
    else:
        ...
        if path.suffix.lower() != ".json":
            question = _as_mapping(result_copy.get("declared_invocation_request_admission_question"))
            request_id = _safe_component(question.get("invocation_request_admission_id"), "invocation_request_admission")
            path = path / f"{request_id}__single_live_command_invocation_request_admission_result.json"
...
def build_declared_portable_source_body_verification_single_live_command_invocation_request_admission(
    invocation_request_admission_id: str,
    invocation_request_admission_question: str,
    ...
    proposed_one_shot_invocation_mode: Mapping[str, Any] | str,
    ...
    single_invocation_scope: Mapping[str, Any] | str,
    no_repeat_posture: Mapping[str, Any] | str,
    ...
    request_admission_scope: Sequence[str] | Mapping[str, Any],
    invocation_request_admission_intent: str = INTENT_ADMIT,
    *,
    requested_invocation_request_admission_outcome: str = OUTCOME_ADMITTED,
    additional_basis_context: Mapping[str, Any] | None = None,
    not_admitted_basis: Mapping[str, Any] | str | None = None,
) -> dict:
    """Build a valid request/admission declaration with false non-claims."""

    return {
        "invocation_request_admission_id": invocation_request_admission_id,
        "invocation_request_admission_question": invocation_request_admission_question,
        "invocation_request_admission_intent": invocation_request_admission_intent,
        ...
        "selected_command_implementation_basis": _copy(selected_command_implementation_basis),
        ...
        "proposed_one_shot_invocation_mode": _copy(proposed_one_shot_invocation_mode),
        "proposed_invocation_surface": _copy(proposed_invocation_surface),
        "proposed_input_reference_bundle": _copy(proposed_input_reference_bundle),
        "proposed_output_report_destination": _copy(proposed_output_report_destination),
        "single_invocation_scope": _copy(single_invocation_scope),
        "no_repeat_posture": _copy(no_repeat_posture),
        "no_standing_invocation_lane_posture": _copy(no_standing_invocation_lane_posture),
        "refusal_conditions": _copy(refusal_conditions),
        "output_limits": _copy(output_limits),
        "result_limits": _copy(result_limits),
        "success_limits": _copy(success_limits),
        "request_admission_scope": _copy(request_admission_scope),
        ...
        "additional_basis_context": _copy(additional_basis_context or {}),
        "not_admitted_basis": _copy(not_admitted_basis or {}),
        "requested_invocation_request_admission_outcome": requested_invocation_request_admission_outcome,
