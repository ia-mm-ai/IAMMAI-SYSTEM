"""Bounded portable source-body verification second-carrier receipt boundary.

This module is downstream of recorded packet transfer. It may record one future
second-carrier receipt step boundary only. The boundary is not second-carrier
receipt, receipt artifact creation, second-carrier execution, external result
capture, cross-carrier evidence review, source transfer, source receipt,
...
class PortableSourceBodyVerificationSecondCarrierReceiptBoundaryError(Exception):
    """Raised for hard second-carrier-receipt-boundary input or output failures."""


RESOLVER_MODULE = "resolve_portable_source_body_verification_second_carrier_receipt_boundary"
RESULT_TYPE = "portable_source_body_verification_second_carrier_receipt_boundary_result"
...
CORE_QUESTION = (
    "Can the recorded packet-transfer basis be bounded for one future "
    "second-carrier receipt step without creating receipt yet, creating "
    "second-carrier execution, creating external result, creating cross-carrier "
...
OUTCOME_NOT_RECORDED = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_RECEIPT_BOUNDARY_NOT_RECORDED"
)
OUTCOME_REQUIRES_ADDITIONAL_BASIS = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_RECEIPT_BOUNDARY_REQUIRES_ADDITIONAL_BASIS"
)
OUTCOME_BLOCKED = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_RECEIPT_BOUNDARY_BLOCKED"
)
OUTCOME_FAMILY = (
    OUTCOME_RECORDED,
    OUTCOME_NOT_RECORDED,
...
INTENT_RECORD = (
    "RECORD_PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_RECEIPT_BOUNDARY"
)
INTENT_DO_NOT_RECORD = (
    "DO_NOT_RECORD_PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_RECEIPT_BOUNDARY"
)
INTENT_BLOCK = (
    "BLOCK_PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_RECEIPT_BOUNDARY"
)
...
SUPPORTED_SECOND_CARRIER_RECEIPT_BOUNDARY_SCOPE = frozenset(
    {
        "SECOND_CARRIER_RECEIPT_BOUNDARY_ONLY",
        "ONE_FUTURE_SECOND_CARRIER_RECEIPT_STEP_ONLY",
...
BLOCK_CODES = frozenset(
    {
        "DECLARED_SECOND_CARRIER_RECEIPT_BOUNDARY_REQUEST_MALFORMED",
        "DECLARED_SECOND_CARRIER_RECEIPT_BOUNDARY_REQUEST_UNREADABLE",
        "SECOND_CARRIER_RECEIPT_BOUNDARY_BLOCK_REQUESTED",
        "SECOND_CARRIER_RECEIPT_BOUNDARY_NOT_RECORD_REQUESTED",
...
NON_CLAIM_BLOCK_CODE_BY_KEY = {
    ...
    "receipt_artifact_created": "RECEIPT_ARTIFACT_CREATED",
...
POSTURE_TO_STATEMENT = {
    ...
    "packet_transfer_not_receipt_posture": "packet_transfer_not_receipt",
    "receipt_not_created_posture": "receipt_not_created",
    "receipt_artifact_not_created_posture": "receipt_artifact_not_created",
...
WHAT_REMAINS_OPEN = (
    "second-carrier receipt boundary test",
    "second-carrier receipt boundary live artifact",
    "second-carrier receipt boundary terminal summary, if needed",
    "second-carrier receipt spec/resolver/test/live artifact",
...
def _build_metadata(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "portable_source_body_verification_second_carrier_receipt_boundary_result_id": (
            request_id
        ),
        "portable_source_body_verification_second_carrier_receipt_boundary_request_id": (
            request_id
        ),
        "portable_source_body_verification_second_carrier_receipt_boundary_result_type": (
            RESULT_TYPE
        ),
...
def _build_declared_question(request: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "second_carrier_receipt_boundary_request_id": request.get(
            "second_carrier_receipt_boundary_request_id"
        ),
        "second_carrier_receipt_boundary_question": request.get(
            "second_carrier_receipt_boundary_question"
        ),
        "second_carrier_receipt_boundary_intent": request.get(
            "second_carrier_receipt_boundary_intent"
        ),
        ...
        section.update(
            {
                ...
                "selected_packet_transfer_transfer_not_receipt": _extract_value(
                    request,
                    (
                        "selected_packet_transfer_transfer_not_receipt",
                        "transfer_not_receipt",
                    ),
                    (key,),
                ),
                "selected_packet_transfer_transfer_not_execution": _extract_value(
                    request,
                    (
                        "selected_packet_transfer_transfer_not_execution",
...
def _build_non_meaning() -> dict[str, bool]:
    return {
        ...
        "receipt_boundary_became_receipt": False,
        "receipt_boundary_became_execution": False,
        "receipt_boundary_became_external_result": False,
        "receipt_boundary_became_cross_carrier_proof": False,
        "receipt_boundary_became_source_transfer": False,
        "receipt_boundary_became_source_receipt": False,
        "receipt_boundary_became_reception_authorization": False,
        "receipt_boundary_became_source": False,
        "receipt_boundary_became_authority": False,
        "receipt_boundary_became_currentness": False,
        "transfer_became_receipt": False,
    ...
    if outcome == OUTCOME_NOT_RECORDED:
        return {
            ...
            "reason": _sanitize_reference(
                request.get(
                    "not_recorded_basis",
                    "second-carrier receipt boundary not recorded by declared intent",
                )
            ),
...
def _build_checks(
    ...
    if malformed_code is not None:
        add(
            "declared second-carrier receipt boundary request is a mapping",
            False,
            "mapping request",
    ...
    question = request.get("second_carrier_receipt_boundary_question")
    intent = request.get("second_carrier_receipt_boundary_intent")
    scope = _scope_values(request.get("second_carrier_receipt_boundary_scope"))

    add(
        "second-carrier receipt boundary question declared",
    ...
    add(
        "second-carrier receipt boundary intent supported",
    ...
    if intent == INTENT_BLOCK:
        add(
            "explicit second-carrier receipt boundary block requested",
            False,
            "non-blocking second-carrier receipt boundary intent",
            intent,
            "SECOND_CARRIER_RECEIPT_BOUNDARY_BLOCK_REQUESTED",
        )

    unsupported_scope = sorted(
        set(scope) - set(SUPPORTED_SECOND_CARRIER_RECEIPT_BOUNDARY_SCOPE)
    )
    add(
        "second-carrier receipt boundary scope supported",
        bool(scope) and not unsupported_scope,
        sorted(SUPPORTED_SECOND_CARRIER_RECEIPT_BOUNDARY_SCOPE),
        scope,
        "UNSUPPORTED_SECOND_CARRIER_RECEIPT_BOUNDARY_SCOPE",
    )
    ...
    transfer_not_execution = _to_bool(
        _extract_value(
            request,
            ("selected_packet_transfer_transfer_not_execution", "transfer_not_execution"),
            ("selected_packet_transfer_basis",),
        )
    )
    add(
        "packet transfer records transfer not execution",
        transfer_not_execution is True,
        True,
        transfer_not_execution,
        "PACKET_TRANSFER_TREATED_TRANSFER_AS_EXECUTION",
    )

    packet_transfer_false_expectations = (
        (
            "packet transfer did not treat transfer as receipt",
            "selected_packet_transfer_treated_transfer_as_receipt",
            "packet_transfer_treated_transfer_as_receipt",
            "PACKET_TRANSFER_TREATED_TRANSFER_AS_RECEIPT",
        ),
        (
            "packet transfer did not treat transfer as execution",
            "selected_packet_transfer_treated_transfer_as_execution",
            "packet_transfer_treated_transfer_as_execution",
            "PACKET_TRANSFER_TREATED_TRANSFER_AS_EXECUTION",
        ),
        (
            "packet transfer did not create second-carrier receipt",
            "selected_packet_transfer_created_second_carrier_receipt",
            "second_carrier_receipt_created",
            "PACKET_TRANSFER_CREATED_SECOND_CARRIER_RECEIPT",
        ),
        (
            "packet transfer did not authorize second-carrier execution",
            "selected_packet_transfer_authorized_second_carrier_execution",
            "second_carrier_execution_authorized",
            "PACKET_TRANSFER_AUTHORIZED_SECOND_CARRIER_EXECUTION",
        ),
        (
            "packet transfer did not create external result",
            "selected_packet_transfer_created_external_result",
            "external_result_created",
            "PACKET_TRANSFER_CREATED_EXTERNAL_RESULT",
        ),
        (
            "packet transfer did not create cross-carrier evidence",
            "selected_packet_transfer_created_cross_carrier_evidence",
            "cross_carrier_evidence_created",
            "PACKET_TRANSFER_CREATED_CROSS_CARRIER_EVIDENCE",
        ),
        (
            "packet transfer did not treat repo-local availability as receipt authority",
            "selected_packet_transfer_treated_repo_local_availability_as_receipt_authority",
            "repo_local_availability_treated_as_receipt_authority",
            "PACKET_TRANSFER_TREATED_REPO_LOCAL_AVAILABILITY_AS_RECEIPT_AUTHORITY",
        ),
        (
            "packet transfer did not use hidden repo state as receipt authority",
            "selected_packet_transfer_used_hidden_repo_state_as_receipt_authority",
            "hidden_repo_state_used_as_receipt_authority",
            "PACKET_TRANSFER_USED_HIDDEN_REPO_STATE_AS_RECEIPT_AUTHORITY",
        ),
        (
            "packet transfer did not treat carrier possession as receipt authority",
            "selected_packet_transfer_treated_carrier_possession_as_receipt_authority",
            "carrier_possession_treated_as_receipt_authority",
            "PACKET_TRANSFER_TREATED_CARRIER_POSSESSION_AS_RECEIPT_AUTHORITY",
        ),
        (
            "packet transfer did not treat copy presence as receipt authority",
            "selected_packet_transfer_treated_copy_presence_as_receipt_authority",
            "copy_presence_treated_as_receipt_authority",
            "PACKET_TRANSFER_TREATED_COPY_PRESENCE_AS_RECEIPT_AUTHORITY",
        ),
        (
            "packet transfer did not return raw full prior artifact body outside bounded transfer",
            "selected_packet_transfer_raw_full_prior_artifact_body_returned_outside_bounded_transfer",
            "raw_full_prior_artifact_body_returned",
            "PACKET_TRANSFER_RETURNED_RAW_FULL_PRIOR_ARTIFACT_BODY_OUTSIDE_BOUNDED_TRANSFER",
        ),
    )
    for name, top_key, nested_key, code in packet_transfer_false_expectations:
        actual = _to_bool(
            _extract_value(request, (top_key, nested_key), ("selected_packet_transfer_basis",)),
            False,
        )
        add(name, actual is False, False, actual, code)

    packet_transfer_true_expectations = (
        (
            "packet transfer records carrier possession not receipt authority",
            "selected_packet_transfer_carrier_possession_not_receipt_authority",
            "carrier_possession_not_receipt_authority",
            "PACKET_TRANSFER_TREATED_CARRIER_POSSESSION_AS_RECEIPT_AUTHORITY",
        ),
        (
            "packet transfer records copy presence not receipt authority",
            "selected_packet_transfer_copy_presence_not_receipt_authority",
            "copy_presence_not_receipt_authority",
            "PACKET_TRANSFER_TREATED_COPY_PRESENCE_AS_RECEIPT_AUTHORITY",
        ),
        (
            "packet transfer records hidden repo state excluded",
            "selected_packet_transfer_hidden_repo_state_excluded",
            "hidden_repo_state_excluded",
            "PACKET_TRANSFER_USED_HIDDEN_REPO_STATE_AS_RECEIPT_AUTHORITY",
        ),
    )
    for name, top_key, nested_key, code in packet_transfer_true_expectations:
        actual = _to_bool(
            _extract_value(request, (top_key, nested_key), ("selected_packet_transfer_basis",)),
            None,
        )
        add(name, actual is True, True, actual, code)
    ...
    for name, key, code in lineage_false_expectations:
        actual = _to_bool(
            _extract_value(
                request,
                (key,),
                ("selected_command_report_lineage_basis", "selected_predecessor_failure_basis"),
            ),
            False,
        )
        add(name, actual is False, False, actual, code)
...
def _resolve_outcome(request: Mapping[str, Any], checks: Sequence[Mapping[str, Any]]) -> str:
    ...
    requested = request.get("requested_second_carrier_receipt_boundary_outcome")
    ...
    if request.get("second_carrier_receipt_boundary_intent") == INTENT_DO_NOT_RECORD:
        return OUTCOME_NOT_RECORDED
...
def _build_result(
    ...
    result: dict[str, Any] = {
        ...
        "declared_second_carrier_receipt_boundary_question": _build_declared_question(request),
    }
    ...
    result.update(
        {
            "second_carrier_receipt_boundary_scope": {
                "supported_scope": sorted(SUPPORTED_SECOND_CARRIER_RECEIPT_BOUNDARY_SCOPE),
                "declared_scope": list(
                    _scope_values(request.get("second_carrier_receipt_boundary_scope"))
                ),
                "unsupported_scope": sorted(
                    set(_scope_values(request.get("second_carrier_receipt_boundary_scope")))
                    - set(SUPPORTED_SECOND_CARRIER_RECEIPT_BOUNDARY_SCOPE)
                ),
            },
            "second_carrier_receipt_boundary_checks": checks,
            "second_carrier_receipt_boundary_statement": statement,
            "second_carrier_receipt_boundary_non_meaning": _build_non_meaning(),
    ...
    result["portable_source_body_verification_second_carrier_receipt_boundary_summary"] = (
        build_portable_source_body_verification_second_carrier_receipt_boundary_summary(result)
    )
    return _json_safe(result)


def resolve_portable_source_body_verification_second_carrier_receipt_boundary(
    declared_second_carrier_receipt_boundary_request: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Resolve one future receipt-boundary posture without creating receipt."""

    malformed_code = None
    if declared_second_carrier_receipt_boundary_request is None or not _is_mapping(
        declared_second_carrier_receipt_boundary_request
    ):
        request: dict[str, Any] = {}
        malformed_code = "DECLARED_SECOND_CARRIER_RECEIPT_BOUNDARY_REQUEST_MALFORMED"
    else:
        request = copy.deepcopy(dict(declared_second_carrier_receipt_boundary_request))
...
def resolve_portable_source_body_verification_second_carrier_receipt_boundary_from_path(
    declared_second_carrier_receipt_boundary_request_path: Path | str,
) -> dict[str, Any]:
    """Read a JSON object request from a path and resolve receipt-boundary posture."""

    path = Path(declared_second_carrier_receipt_boundary_request_path)
    try:
        parsed = json.loads(path.read_text(encoding="utf-8"))
    except FileNotFoundError as exc:
        raise PortableSourceBodyVerificationSecondCarrierReceiptBoundaryError(
            f"declared second-carrier receipt boundary request path is unreadable: {path}"
        ) from exc
        ...
        raise PortableSourceBodyVerificationSecondCarrierReceiptBoundaryError(
            f"declared second-carrier receipt boundary request JSON is malformed: {path}"
        ) from exc

    if not _is_mapping(parsed):
        return resolve_portable_source_body_verification_second_carrier_receipt_boundary(
            {"_forced_block_code": "DECLARED_SECOND_CARRIER_RECEIPT_BOUNDARY_REQUEST_MALFORMED"}
        )
    return resolve_portable_source_body_verification_second_carrier_receipt_boundary(parsed)


def build_portable_source_body_verification_second_carrier_receipt_boundary_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build a compact summary from a second-carrier receipt boundary result."""

    metadata = result.get(
        "portable_source_body_verification_second_carrier_receipt_boundary_metadata", {}
    )
    declared = result.get("declared_second_carrier_receipt_boundary_question", {})
    statement = result.get("second_carrier_receipt_boundary_statement", {})
    checks = result.get("second_carrier_receipt_boundary_checks", [])
    block = result.get("block") or {}
    non_claims = result.get("non_claims", {})
    packet_transfer_basis = result.get("selected_packet_transfer_basis", {})
    ...
    if not _is_mapping(non_claims):
        non_claims = {}
    if not _is_mapping(packet_transfer_basis):
        packet_transfer_basis = {}
    ...
    return {
        ...
        "question": declared.get("second_carrier_receipt_boundary_question"),
        "intent": declared.get("second_carrier_receipt_boundary_intent"),
        "passed_check_count": _check_count(checks, True),
        "failed_check_count": _check_count(checks, False),
        "second_carrier_receipt_boundary_recorded": bool(
            statement.get("second_carrier_receipt_boundary_recorded", False)
        ),
        "one_future_second_carrier_receipt_step_declared": bool(
            statement.get("one_future_second_carrier_receipt_step_declared", False)
        ),
        ...
        "packet_transfer_not_receipt": bool(
            statement.get("packet_transfer_not_receipt", False)
        ),
        "receipt_not_created": bool(statement.get("receipt_not_created", False)),
        "receipt_artifact_not_created": bool(
            statement.get("receipt_artifact_not_created", False)
        ),
        ...
        "selected_packet_transfer_outcome": packet_transfer_basis.get(
            "selected_packet_transfer_result_outcome"
        ),
        "selected_packet_transfer_version": packet_transfer_basis.get(
            "selected_packet_transfer_result_version"
        ),
        "selected_packet_transfer_failed_check_count": packet_transfer_basis.get(
            "selected_packet_transfer_failed_check_count"
        ),
        "no_receipt_execution_external_result_cross_carrier_evidence": (
            key_non_claims["second_carrier_receipt_created"] is False
            and key_non_claims["receipt_artifact_created"] is False
...
def _safe_filename_part(value: Any) -> str:
    text = str(
        value
        or "portable_source_body_verification_second_carrier_receipt_boundary_reference_review_001"
    )
    safe = []
    for char in text:
...
def write_portable_source_body_verification_second_carrier_receipt_boundary_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write a receipt-boundary result as stable UTF-8 JSON without overwriting."""

    if not _is_mapping(result):
        raise PortableSourceBodyVerificationSecondCarrierReceiptBoundaryError(
            "second-carrier receipt boundary result must be a mapping"
        )
    metadata = result.get(
        "portable_source_body_verification_second_carrier_receipt_boundary_metadata", {}
    )
    if not _is_mapping(metadata):
        raise PortableSourceBodyVerificationSecondCarrierReceiptBoundaryError(
            "second-carrier receipt boundary result metadata must be a mapping"
        )
    request_id = metadata.get(
        "portable_source_body_verification_second_carrier_receipt_boundary_request_id",
        metadata.get(
    ...
    filename = (
        ...
        "__portable_source_body_verification_second_carrier_receipt_boundary_result.json"
    )
...
def build_declared_portable_source_body_verification_second_carrier_receipt_boundary_request(
    second_carrier_receipt_boundary_request_id: str = (
        "portable_source_body_verification_second_carrier_receipt_boundary_reference_review_001"
    ),
    **overrides: Any,
) -> dict[str, Any]:
    """Build a valid receipt-boundary request with required non-claims false."""

    request: dict[str, Any] = {
        "second_carrier_receipt_boundary_request_id": (
            second_carrier_receipt_boundary_request_id
        ),
        "second_carrier_receipt_boundary_question": CORE_QUESTION,
        "second_carrier_receipt_boundary_intent": INTENT_RECORD,
        "selected_packet_transfer_result_id": (
        ...
        "selected_packet_transfer_transfer_not_execution": True,
        "selected_packet_transfer_created_second_carrier_receipt": False,
        "selected_packet_transfer_authorized_second_carrier_execution": False,
        "selected_packet_transfer_created_external_result": False,
        "selected_packet_transfer_created_cross_carrier_evidence": False,
        "selected_packet_transfer_carrier_possession_not_receipt_authority": True,
        "selected_packet_transfer_copy_presence_not_receipt_authority": True,
        "selected_packet_transfer_hidden_repo_state_excluded": True,
        "selected_packet_transfer_treated_repo_local_availability_as_receipt_authority": False,
        "selected_packet_transfer_used_hidden_repo_state_as_receipt_authority": False,
        "selected_packet_transfer_raw_full_prior_artifact_body_returned_outside_bounded_transfer": False,
        "reference_shaped_input_posture": True,
        "second_carrier_receipt_boundary_scope": sorted(
