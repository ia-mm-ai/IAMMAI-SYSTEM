"""v5 resolver for the local read-only derivative participant boundary.

This successor preserves v4 canonicalized blocked-result posture, emits the
required continuation operation sequence count, and constrains lineage evidence
derivation so explicit missing, false, repaired, hidden, or claimed-passed
evidence blocks instead of being silently replaced by terminal-summary text.
"""
...
RESOLVER_MODULE = (
    "resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5"
)
BOUNDARY_TYPE = (
...
OUTPUT_ROOT = (
    REPO_ROOT
    / "artifacts"
    / "integrity_host_v0_min_coexistence_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5"
)
...
class LocalRelevanceMediumReadOnlyDerivativeParticipantParticipationMotionBoundaryV0MinV5Error(
    Exception
):
    """Bounded resolver error for invalid direct write or path operations."""


def build_declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5_request(
    ...
    """Build the bounded request mapping consumed by the v5 resolver."""
...
def resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5_from_path(
    ...
    return resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5(
        loaded,
        _pre_checks=checks,
    )


def resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5(
    ...
    *,
    ...
    if declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary is None:
        request: dict[str, Any] = (
            build_declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5_request()
        )
...
def write_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5_result(
    ...
    if not isinstance(result, Mapping):
        raise LocalRelevanceMediumReadOnlyDerivativeParticipantParticipationMotionBoundaryV0MinV5Error(
    ...
    filename = (
        f"{boundary_id}__"
        "local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5_result.json"
    )
...
def build_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build the compact public summary for a v5 result mapping."""
...
def _result_from_parts(
    ...
    *,
    ...
    summary = build_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v5_summary(
        {
...
def _derive_continuation_v1_failure_evidence(
    ...
    role_probe = _evidence_probe(role_sections, key)
    if role_probe.present and role_probe.value is not True:
        return False, role_probe.source or f"{key}_explicit_non_true"
    continuation_probe = _evidence_probe(continuation_sections, key)
    if continuation_probe.value is True:
        return True, continuation_probe.source or key
    return False, continuation_probe.source or f"{key}_missing"
