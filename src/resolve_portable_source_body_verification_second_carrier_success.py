"""Resolve portable source-body verification second-carrier success.

This module is downstream of the clean second-carrier-success-boundary line. It
records one bounded second-carrier success posture only. Success here is not
verification, external result, cross-carrier evidence, portable verification
...
evidence, and keeps official enum, scope, outcome, block-code, posture, section,
...
class PortableSourceBodyVerificationSecondCarrierSuccessError(Exception):
    """Bounded resolver error for explicit unreadable or malformed inputs."""


RESOLVER_MODULE = "resolve_portable_source_body_verification_second_carrier_success"
RESULT_TYPE = "portable_source_body_verification_second_carrier_success_result"
...
CORE_QUESTION = (
    "Can the clean second-carrier-success-boundary basis be used to record one "
    "bounded second-carrier success posture without creating verification, "
...
OUTCOME_RECORDED = "PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_SUCCESS_RECORDED"
OUTCOME_NOT_RECORDED = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_SUCCESS_NOT_RECORDED"
)
OUTCOME_REQUIRES_ADDITIONAL_BASIS = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_SUCCESS_REQUIRES_ADDITIONAL_BASIS"
)
OUTCOME_BLOCKED = "PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_SUCCESS_BLOCKED"
OUTCOME_FAMILY = (
    OUTCOME_RECORDED,
    OUTCOME_NOT_RECORDED,
    OUTCOME_REQUIRES_ADDITIONAL_BASIS,
...
INTENT_RECORD = "RECORD_PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_SUCCESS"
INTENT_DO_NOT_RECORD = (
    "DO_NOT_RECORD_PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_SUCCESS"
)
INTENT_BLOCK = "BLOCK_PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_SUCCESS"
SUPPORTED_INTENTS = (INTENT_RECORD, INTENT_DO_NOT_RECORD, INTENT_BLOCK)

EXPECTED_SUCCESS_BOUNDARY_OUTCOME = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_SECOND_CARRIER_SUCCESS_BOUNDARY_RECORDED"
)
EXPECTED_SUCCESS_BOUNDARY_VERSION = "0.1.0"
...
SUPPORTED_SCOPE_VALUES = (
    "SECOND_CARRIER_SUCCESS_SPEC_ONLY",
    "ONE_BOUNDED_SECOND_CARRIER_SUCCESS_RECORDED",
    ...
    "SUCCESS_RECORDED_BOUNDED",
    "SUCCESS_ARTIFACT_RECORDED_OR_BOUNDED",
    "SUCCESS_NOT_VERIFICATION",
    "SUCCESS_NOT_EXTERNAL_RESULT",
    "SUCCESS_NOT_CROSS_CARRIER_EVIDENCE",
    "SUCCESS_NOT_PORTABLE_VERIFICATION_CLOSURE",
    "SUCCESS_NOT_SOURCE_TRANSFER",
    "SUCCESS_NOT_SOURCE_RECEIPT",
    "SUCCESS_NOT_RECEPTION_AUTHORIZATION",
    "ZERO_EXIT_CODE_NOT_SUCCESS_AS_STANDALONE_INFERENCE",
...
SUPPORTED_SECOND_CARRIER_SUCCESS_SCOPE = SUPPORTED_SCOPE_VALUES
...
REQUIRED_FALSE_NON_CLAIMS = (
    ...
    "zero_exit_code_treated_as_success_standalone",
...
BLOCK_CODES = (
    "DECLARED_SECOND_CARRIER_SUCCESS_REQUEST_MALFORMED",
    "DECLARED_SECOND_CARRIER_SUCCESS_REQUEST_UNREADABLE",
    "SECOND_CARRIER_SUCCESS_BLOCK_REQUESTED",
...
REDACTION_TEXT = "[bounded-redacted-raw-or-hidden-state]"
...
OFFICIAL_STRINGS = frozenset(
    ...
    + (
        ...
        CORE_QUESTION,
        EXPECTED_SUCCESS_BOUNDARY_OUTCOME,
        EXPECTED_SUCCESS_BOUNDARY_VERSION,
        EXPECTED_SECOND_CARRIER_RESULT_OUTCOME,
...
def _sanitize(value: Any, *, parent_key: str | None = None) -> Any:
    ...
    if isinstance(value, Mapping):
        return {
            str(key): _sanitize(item, parent_key=str(key))
...
def _posture_declared(request: Mapping[str, Any], key: str) -> bool:
    ...
    if isinstance(value, Mapping):
        declared = value.get("declared", value.get("posture", value.get("preserved")))
...
def _scope_values(request: Mapping[str, Any]) -> tuple[str, ...]:
    raw = request.get(
        ...
        request.get("supported_second_carrier_success_scope", ()),
    )
...
def _basis_extra(request: Mapping[str, Any], key: str) -> dict[str, Any]:
    if key == "selected_second_carrier_success_boundary_basis":
        return {
            "selected_second_carrier_success_boundary_result_path": _json_safe(
                request.get("selected_second_carrier_success_boundary_result_path")
            ),
            "selected_second_carrier_success_boundary_result_outcome": request.get(
                "selected_second_carrier_success_boundary_result_outcome"
            ),
            "selected_second_carrier_success_boundary_result_version": request.get(
                "selected_second_carrier_success_boundary_result_version"
            ),
            "selected_second_carrier_success_boundary_failed_check_count": request.get(
                "selected_second_carrier_success_boundary_failed_check_count"
            ),
            "selected_second_carrier_success_boundary_declared_future_success_step": (
                _as_bool(
                    request.get(
                        "selected_second_carrier_success_boundary_"
                        "declared_future_success_step"
                    )
                )
            ),
            "first_success_boundary_test_failure_preserved": _as_bool(
                request.get(
                    "selected_second_carrier_success_boundary_"
                    "first_success_boundary_test_failure_preserved"
                )
    ...
    if key == "selected_predecessor_failure_basis":
        return {
            ...
            "first_success_boundary_test_not_repaired": not _as_bool(
                request.get("first_success_boundary_test_repaired")
            ),
            "first_success_boundary_test_not_hidden": not _as_bool(
                request.get("first_success_boundary_test_hidden")
            ),
            "first_success_boundary_test_not_claimed_passed": not _as_bool(
                request.get("first_success_boundary_test_claimed_passed")
            ),
...
def _selected_basis(request: Mapping[str, Any], key: str) -> dict[str, Any]:
    ...
    basis = {
        ...
        "basis_role": "reference-shaped second-carrier-success basis only",
...
def _posture(request: Mapping[str, Any], key: str) -> dict[str, Any]:
    ...
    return {
        "posture_key": key,
        "declared": declared,
        "preserved_as_second_carrier_success_only": declared,
        "does_not_create_verification": True,
        "does_not_create_external_result": True,
        "does_not_create_cross_carrier_evidence": True,
        "does_not_create_portable_verification_closure": True,
...
def _basis_missing_code(key: str) -> str:
    if key.startswith("selected_second_carrier_success_boundary"):
        return "SECOND_CARRIER_SUCCESS_BOUNDARY_BASIS_MISSING"
    if key.startswith("selected_second_carrier_result_boundary_v2"):
        return "SECOND_CARRIER_RESULT_BOUNDARY_V2_BASIS_MISSING"
    if key.startswith("selected_second_carrier_result"):
    ...
    return "SELECTED_BASIS_NOT_REFERENCE_SHAPED"


def _build_checks(request: Mapping[str, Any]) -> list[dict[str, Any]]:
    checks: list[dict[str, Any]] = []
    question = request.get("second_carrier_success_question")
    intent = request.get("second_carrier_success_intent")
    checks.append(
        _check(
            "second_carrier_success_question_declared",
    ...
    checks.append(
        _check(
            "second_carrier_success_intent_supported",
    ...
    checks.append(
        _check(
            "second_carrier_success_scope_declared",
            bool(scope),
            "declared supported second-carrier-success scope",
            scope,
            "UNSUPPORTED_SECOND_CARRIER_SUCCESS_SCOPE",
        )
    )
    checks.append(
        _check(
            "second_carrier_success_scope_supported",
    ...
    boundary_failed_count = _as_int(
        request.get("selected_second_carrier_success_boundary_failed_check_count")
    )
    checks.extend(
        [
            _check(
                "second_carrier_success_boundary_outcome_recorded",
                request.get("selected_second_carrier_success_boundary_result_outcome")
                == EXPECTED_SUCCESS_BOUNDARY_OUTCOME,
                EXPECTED_SUCCESS_BOUNDARY_OUTCOME,
                request.get("selected_second_carrier_success_boundary_result_outcome"),
                "SECOND_CARRIER_SUCCESS_BOUNDARY_NOT_RECORDED",
            ...
            _check(
                "second_carrier_success_boundary_version_0_1_0",
                request.get("selected_second_carrier_success_boundary_result_version")
                == EXPECTED_SUCCESS_BOUNDARY_VERSION,
                EXPECTED_SUCCESS_BOUNDARY_VERSION,
                request.get("selected_second_carrier_success_boundary_result_version"),
                "SECOND_CARRIER_SUCCESS_BOUNDARY_VERSION_NOT_0_1_0",
            ...
            _check(
                "second_carrier_success_boundary_declared_future_success_step",
                _as_bool(
                    request.get(
                        "selected_second_carrier_success_boundary_"
                        "declared_future_success_step"
    ...
    for field, code in (
        (
            "selected_second_carrier_success_boundary_already_created_success",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_ALREADY_CREATED_SUCCESS",
        ),
        (
            "selected_second_carrier_success_boundary_already_created_verification",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_ALREADY_CREATED_VERIFICATION",
        ),
        (
            "selected_second_carrier_success_boundary_already_created_external_result",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_ALREADY_CREATED_EXTERNAL_RESULT",
        ),
        (
            "selected_second_carrier_success_boundary_already_created_cross_carrier_evidence",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_ALREADY_CREATED_CROSS_CARRIER_EVIDENCE",
        ),
        (
            "selected_second_carrier_success_boundary_already_created_portable_verification_closure",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_ALREADY_CREATED_PORTABLE_VERIFICATION_CLOSURE",
        ),
        (
            "selected_second_carrier_success_boundary_treated_success_boundary_as_success",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_TREATED_SUCCESS_BOUNDARY_AS_SUCCESS",
        ),
        (
            "selected_second_carrier_success_boundary_treated_success_boundary_as_verification",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_TREATED_SUCCESS_BOUNDARY_AS_VERIFICATION",
        ),
        (
            "selected_second_carrier_success_boundary_treated_success_boundary_as_external_result",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_TREATED_SUCCESS_BOUNDARY_AS_EXTERNAL_RESULT",
        ),
        (
            "selected_second_carrier_success_boundary_treated_success_boundary_as_cross_carrier_evidence",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_TREATED_SUCCESS_BOUNDARY_AS_CROSS_CARRIER_EVIDENCE",
        ),
        (
            "selected_second_carrier_success_boundary_treated_success_boundary_as_portable_verification_closure",
            "SECOND_CARRIER_SUCCESS_BOUNDARY_TREATED_SUCCESS_BOUNDARY_AS_PORTABLE_VERIFICATION_CLOSURE",
        ),
    ):
        checks.append(
            _check(
                f"{field}_false",
                not _as_bool(request.get(field)),
                False,
                request.get(field),
                code,
            )
    ...
    checks.extend(
        [
            _check(
                "success_boundary_kept_zero_exit_code_not_success",
                _as_bool(
                    request.get(
                        "selected_second_carrier_success_boundary_zero_exit_code_not_success"
                    )
                )
                and _negative_flag(request, "zero_exit_code_treated_as_success_standalone"),
                True,
                request.get(
                    "selected_second_carrier_success_boundary_zero_exit_code_not_success"
                ),
            ...
            _check(
                "success_boundary_kept_ok_not_verification",
                _as_bool(
                    request.get(
                        "selected_second_carrier_success_boundary_ok_not_verification"
                    )
                )
            ...
            _check(
                "success_boundary_kept_ran_7_tests_not_cross_carrier_proof",
                _as_bool(
                    request.get(
                        "selected_second_carrier_success_boundary_"
                        "ran_7_tests_not_cross_carrier_proof"
            ...
            _check(
                "success_boundary_kept_returned_capture_not_cross_carrier_proof",
                _as_bool(
                    request.get(
                        "selected_second_carrier_success_boundary_"
                        "returned_capture_not_cross_carrier_proof"
            ...
            _check(
                "success_boundary_official_enum_scope_strings_not_redacted",
                not _as_bool(
                    request.get(
                        "selected_second_carrier_success_boundary_"
                ...
                is not False,
                "official enum and scope strings unredacted",
                {
            ...
            _check(
                ...
                and _negative_flag(request, "first_success_boundary_test_repaired")
                and _negative_flag(request, "first_success_boundary_test_hidden")
                and _negative_flag(request, "first_success_boundary_test_claimed_passed"),
                True,
                request.get(
                    "selected_second_carrier_success_boundary_"
                    "first_success_boundary_test_failure_preserved"
    ...
    for field, code in (
        ("selected_second_carrier_result_already_created_success", "SECOND_CARRIER_RESULT_ALREADY_CREATED_SUCCESS"),
        ("selected_second_carrier_result_already_created_verification", "SECOND_CARRIER_RESULT_ALREADY_CREATED_VERIFICATION"),
        ("selected_second_carrier_result_already_created_external_result", "SECOND_CARRIER_RESULT_ALREADY_CREATED_EXTERNAL_RESULT"),
        ("selected_second_carrier_result_already_created_cross_carrier_evidence", "SECOND_CARRIER_RESULT_ALREADY_CREATED_CROSS_CARRIER_EVIDENCE"),
        ("selected_second_carrier_result_treated_result_as_success", "SECOND_CARRIER_RESULT_TREATED_RESULT_AS_SUCCESS"),
        ("selected_second_carrier_result_treated_result_as_verification", "SECOND_CARRIER_RESULT_TREATED_RESULT_AS_VERIFICATION"),
        ("selected_second_carrier_result_treated_zero_exit_code_as_success", "SECOND_CARRIER_RESULT_TREATED_ZERO_EXIT_CODE_AS_SUCCESS"),
        ("selected_second_carrier_result_treated_ok_as_verification", "SECOND_CARRIER_RESULT_TREATED_OK_AS_VERIFICATION"),
        ("selected_second_carrier_result_treated_ran_7_tests_as_cross_carrier_proof", "SECOND_CARRIER_RESULT_TREATED_RAN_7_TESTS_AS_CROSS_CARRIER_PROOF"),
    ):
        checks.append(_check(f"{field}_false", not _as_bool(request.get(field)), False, request.get(field), code))
    checks.extend(
        [
            _check(
    ...
    for field, code in (
        ("returned_capture_treated_as_success", "RETURNED_CAPTURE_TREATED_AS_SUCCESS"),
        ("returned_capture_treated_as_verification", "RETURNED_CAPTURE_TREATED_AS_VERIFICATION"),
        ("returned_capture_treated_as_cross_carrier_proof", "RETURNED_CAPTURE_TREATED_AS_CROSS_CARRIER_PROOF"),
    ):
        checks.append(_check(f"{field}_false", not _as_bool(request.get(field)), False, request.get(field), code))
    checks.append(
        _check(
    ...
    for key in POSTURE_KEYS:
        checks.append(
            _check(
                ...
                "required second-carrier-success posture declared",
                request.get(key),
                "NON_CLAIM_MISSING_OR_FLIPPED",
            )
        )
    ...
    for key, code in (
        ...
        ("full_prior_artifact_body_emitted_outside_bounded_success", "FULL_PRIOR_ARTIFACT_BODY_EMITTED_OUTSIDE_BOUNDED_SUCCESS"),
        ("artifacts_mutated", "ARTIFACTS_MUTATED"),
    ):
        checks.append(_check(f"{key}_false", not _as_bool(request.get(key)), False, request.get(key), code))
...
def _non_meaning() -> dict[str, bool]:
    return {
        ...
        "success_became_cross_carrier_proof": False,
        "success_became_portable_verification_closure": False,
        "success_became_source_transfer_source_receipt_or_reception_authorization": False,
        "success_became_source_authority_or_currentness": False,
        "success_artifact_became_verification_external_result_cross_carrier_evidence_or_portable_verification_closure": False,
        "returned_capture_became_proof": False,
        "zero_exit_code_became_automatic_success_without_bounded_resolver_admission": False,
...
def _what_remains_open() -> dict[str, Any]:
    return {
        "open_items": [
            "second-carrier success test",
            "second-carrier success live artifact",
            "second-carrier success terminal summary, if needed",
...
def _block(block_code: str | None, reason: Any = None) -> dict[str, Any] | None:
    if block_code is None:
        return None
    return {"blocked": True, "block_code": block_code, "block_reason": _json_safe(reason)}


def _result_id(request: Mapping[str, Any]) -> str:
    request_id = request.get("second_carrier_success_request_id")
    if isinstance(request_id, str) and request_id.strip():
        return f"{request_id.strip()}__portable_source_body_verification_second_carrier_success_result"
    return "portable_source_body_verification_second_carrier_success_result"
...
def _build_result(
    ...
    result["portable_source_body_verification_second_carrier_success_metadata"] = {
        "portable_source_body_verification_second_carrier_success_id": result_id,
        "portable_source_body_verification_second_carrier_success_type": RESULT_TYPE,
        "portable_source_body_verification_second_carrier_success_version": RESULT_VERSION,
        "generated_at": _utc_now(),
        "resolver_module": RESOLVER_MODULE,
        "source_material_mutated": False,
        "returned_capture_material_mutated": False,
        "second_carrier_success_boundary_material_mutated": False,
        "second_carrier_result_material_mutated": False,
    }
    result["declared_second_carrier_success_question"] = {
        "second_carrier_success_request_id": request.get("second_carrier_success_request_id"),
        "question": request.get("second_carrier_success_question"),
        "intent": request.get("second_carrier_success_intent"),
    ...
    result["second_carrier_success_scope"] = list(_scope_values(request))
    result["second_carrier_success_checks"] = list(checks)
    result["second_carrier_success_statement"] = statement
    result["second_carrier_success_non_meaning"] = _non_meaning()
    result["additional_basis_required"] = _additional_basis_required(outcome, checks, request)
    ...
    result["portable_source_body_verification_second_carrier_success_summary"] = (
        build_portable_source_body_verification_second_carrier_success_summary(result)
    )
...
def _blocked_result(block_code: str, reason: Any = None) -> dict[str, Any]:
    request = {
        "second_carrier_success_request_id": (
            "portable_source_body_verification_second_carrier_success_blocked"
        ),
        "second_carrier_success_question": None,
        "second_carrier_success_intent": None,
        "second_carrier_success_scope": (),
    ...
    checks = [
        _check(
            "declared_second_carrier_success_request_malformed",
            False,
            "mapping request with declared second-carrier-success question",
            reason,
            block_code,
        )
    ]
...
def resolve_portable_source_body_verification_second_carrier_success(
    declared_second_carrier_success_request: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Resolve one bounded second-carrier success request."""

    if declared_second_carrier_success_request is None:
        return _blocked_result(
            "DECLARED_SECOND_CARRIER_SUCCESS_REQUEST_MALFORMED",
            "declared request missing",
        )
    if not isinstance(declared_second_carrier_success_request, Mapping):
        return _blocked_result(
            "DECLARED_SECOND_CARRIER_SUCCESS_REQUEST_MALFORMED",
            "declared request must be a mapping",
        )

    request = _json_safe(copy.deepcopy(dict(declared_second_carrier_success_request)))
    ...
    requested_outcome = request.get("requested_second_carrier_success_outcome")
...
def resolve_portable_source_body_verification_second_carrier_success_from_path(
    declared_second_carrier_success_request_path: Path | str,
) -> dict[str, Any]:
    """Load and resolve a declared second-carrier success JSON object."""

    path = Path(declared_second_carrier_success_request_path)
    try:
        raw = path.read_text(encoding="utf-8")
    except OSError as exc:
        raise PortableSourceBodyVerificationSecondCarrierSuccessError(
            f"declared second-carrier success request unreadable: {path}"
        ) from exc
    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise PortableSourceBodyVerificationSecondCarrierSuccessError(
            f"declared second-carrier success request malformed JSON: {path}"
        ) from exc
    ...
    return resolve_portable_source_body_verification_second_carrier_success(parsed)


def build_portable_source_body_verification_second_carrier_success_summary(
    result: Mapping[str, Any]
) -> dict[str, Any]:
    """Build a bounded summary for a second-carrier success result."""

    checks = result.get("second_carrier_success_checks", [])
    if not isinstance(checks, Sequence) or isinstance(checks, (str, bytes, bytearray)):
        checks = []
    passed_count = sum(
        1 for check in checks if isinstance(check, Mapping) and check.get("passed")
    )
    failed_count = sum(
        1 for check in checks if isinstance(check, Mapping) and not check.get("passed")
    )
    metadata = result.get("portable_source_body_verification_second_carrier_success_metadata", {})
    if not isinstance(metadata, Mapping):
        metadata = {}
    question = result.get("declared_second_carrier_success_question", {})
    if not isinstance(question, Mapping):
        question = {}
    statement = result.get("second_carrier_success_statement", {})
    ...
    selected_boundary = result.get("selected_second_carrier_success_boundary_basis", {})
    ...
    if not isinstance(selected_capture, Mapping):
        selected_capture = {}
    selected_intake = result.get("selected_returned_second_carrier_live_capture_intake_basis", {})
    ...
    summary = {
        ...
        "request_id": question.get("second_carrier_success_request_id"),
        "question": question.get("question"),
        "intent": question.get("intent"),
        "passed_check_count": passed_count,
        "failed_check_count": failed_count,
        "result_version": metadata.get(
            "portable_source_body_verification_second_carrier_success_version"
        ),
        "resolver_module": metadata.get("resolver_module"),
        "selected_second_carrier_success_boundary_outcome": selected_boundary.get(
            "selected_second_carrier_success_boundary_result_outcome"
        ),
        "selected_second_carrier_success_boundary_version": selected_boundary.get(
            "selected_second_carrier_success_boundary_result_version"
        ),
        "selected_second_carrier_success_boundary_failed_check_count": selected_boundary.get(
            "selected_second_carrier_success_boundary_failed_check_count"
        ),
...
def write_portable_source_body_verification_second_carrier_success_result(
    result: Mapping[str, Any], output_path: Path | str | None = None
) -> Path:
    """Write a bounded second-carrier success result JSON artifact."""

    if not isinstance(result, Mapping):
        raise PortableSourceBodyVerificationSecondCarrierSuccessError(
            "second-carrier success result must be a mapping"
        )
    metadata = result.get("portable_source_body_verification_second_carrier_success_metadata", {})
    if not isinstance(metadata, Mapping):
        metadata = {}
    result_id = metadata.get(
        "portable_source_body_verification_second_carrier_success_id",
        "portable_source_body_verification_second_carrier_success_result",
    )
...
def build_declared_portable_source_body_verification_second_carrier_success_request(
    **overrides: Any,
) -> dict[str, Any]:
    """Build a clean declared second-carrier success request."""

    request_id = overrides.pop(
        "second_carrier_success_request_id",
        "portable_source_body_verification_second_carrier_success_reference_review_001",
    )
    request: dict[str, Any] = {
        "second_carrier_success_request_id": request_id,
        "second_carrier_success_question": CORE_QUESTION,
        "second_carrier_success_intent": INTENT_RECORD,
        "second_carrier_success_scope": list(SUPPORTED_SCOPE_VALUES),
        "declared_non_claims": _false_non_claims(),
        ...
        "selected_second_carrier_success_boundary_result_path": (
            ...
            "carrier_success_boundary_result.json"
        ),
        "selected_second_carrier_success_boundary_result_outcome": (
            EXPECTED_SUCCESS_BOUNDARY_OUTCOME
        ),
        ...
        "selected_second_carrier_success_boundary_declared_future_success_step": True,
        "selected_second_carrier_success_boundary_already_created_success": False,
        "selected_second_carrier_success_boundary_already_created_verification": False,
        "selected_second_carrier_success_boundary_already_created_external_result": False,
        "selected_second_carrier_success_boundary_already_created_cross_carrier_evidence": False,
        "selected_second_carrier_success_boundary_already_created_portable_verification_closure": False,
        "selected_second_carrier_success_boundary_treated_success_boundary_as_success": False,
        "selected_second_carrier_success_boundary_treated_success_boundary_as_verification": False,
        "selected_second_carrier_success_boundary_treated_success_boundary_as_external_result": False,
        "selected_second_carrier_success_boundary_treated_success_boundary_as_cross_carrier_evidence": False,
        "selected_second_carrier_success_boundary_treated_success_boundary_as_portable_verification_closure": False,
        "selected_second_carrier_success_boundary_zero_exit_code_not_success": True,
        "selected_second_carrier_success_boundary_ok_not_verification": True,
        "selected_second_carrier_success_boundary_ran_7_tests_not_cross_carrier_proof": True,
        "selected_second_carrier_success_boundary_returned_capture_not_cross_carrier_proof": True,
        "selected_second_carrier_success_boundary_official_enum_scope_strings_redacted": False,
        "selected_second_carrier_success_boundary_official_enum_scope_strings_not_redacted": True,
        "selected_second_carrier_success_boundary_first_success_boundary_test_failure_preserved": True,
        ...
        "selected_second_carrier_result_treated_zero_exit_code_as_success": False,
        "selected_second_carrier_result_treated_ok_as_verification": False,
        "selected_second_carrier_result_treated_ran_7_tests_as_cross_carrier_proof": False,
        ...
        "selected_returned_capture_intake_path": (
            "spec/PORTABLE_SOURCE_BODY_VERIFICATION_RETURNED_SECOND_CARRIER_LIVE_"
            "CAPTURE_INTAKE_V0.md"
        ),
        ...
        "returned_capture_treated_as_success": False,
        "returned_capture_treated_as_verification": False,
    ...
    for key in SELECTED_BASIS_KEYS:
        request[key] = {
            ...
            "basis_role": "selected bounded second-carrier-success basis only",
        }
    for key in POSTURE_KEYS:
        request[key] = {
            "posture_key": key,
