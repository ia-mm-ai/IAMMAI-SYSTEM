"""Resolve one bounded portable source-body verification command result v2.

This module is an additive successor to
``resolve_portable_source_body_verification_command_result``. The predecessor
compiled and exposed the expected public API, but it could emit six
``OUTPUT_CAPTURE_TREATED_AS_*`` block codes that were not declared in its
public ``BLOCK_CODES`` set. This v2 resolver preserves that v1 conformance
failure as visible predecessor evidence and records one bounded command result
only when the command-result basis remains non-collapse-shaped.

It does not create command success, infer success, infer currentness, infer
final completion, infer public readiness, infer deployment readiness, infer
unbounded pass/fail posture, create source, authority, continuation, reusable
permission, derivative reception, vessel relation, another reception request,
deployment, runtime hosting, public release, or follow-on work.
"""
...
class PortableSourceBodyVerificationCommandResultV2Error(Exception):
    """Raised for explicit unreadable or contradictory command-result v2 input."""


RESOLVER_MODULE = "resolve_portable_source_body_verification_command_result_v2"
SUCCESSOR_OF = "resolve_portable_source_body_verification_command_result"
SUCCESSOR_REASON = (
    "v1 command-result resolver preserved as predecessor conformance-failure "
    "evidence: v1 could emit undeclared public block codes "
    "OUTPUT_CAPTURE_TREATED_AS_RESULT, OUTPUT_CAPTURE_TREATED_AS_SUCCESS, "
    "OUTPUT_CAPTURE_TREATED_AS_SOURCE, OUTPUT_CAPTURE_TREATED_AS_AUTHORITY, "
    "OUTPUT_CAPTURE_TREATED_AS_CURRENTNESS, and "
    "OUTPUT_CAPTURE_TREATED_AS_FINAL_COMPLETION"
)
RESULT_VERSION = "0.2.0"
RESULT_TYPE = "portable_source_body_verification_command_result_v2_result"
OUTPUT_ROOT = Path(
    "artifacts/"
...
COLLAPSE_CHECKS = (
    ...
    ("command result not currentness", ("command_result_treated_as_currentness",), "COMMAND_RESULT_TREATED_AS_CURRENTNESS"),
    ("command result not final completion", ("command_result_treated_as_final_completion",), "COMMAND_RESULT_TREATED_AS_FINAL_COMPLETION"),
    ("command result not public readiness", ("command_result_treated_as_public_readiness",), "COMMAND_RESULT_TREATED_AS_PUBLIC_READINESS"),
    ("command result not deployment readiness", ("command_result_treated_as_deployment_readiness",), "COMMAND_RESULT_TREATED_AS_DEPLOYMENT_READINESS"),
    ...
    ("command output/report artifact not result authority", ("command_output_report_artifact_treated_as_result_authority", "command_output_report_artifact_treated_as_result"), "COMMAND_OUTPUT_REPORT_ARTIFACT_TREATED_AS_RESULT_AUTHORITY"),
    ...
    ("report body not result authority", ("report_body_treated_as_result_authority", "report_body_treated_as_result"), "REPORT_BODY_TREATED_AS_RESULT_AUTHORITY"),
    ...
    ("command report lineage not command result authority", ("command_report_lineage_basis_treated_as_command_result_authority", "command_report_lineage_basis_treated_as_command_result"), "COMMAND_REPORT_LINEAGE_BASIS_TREATED_AS_COMMAND_RESULT_AUTHORITY"),
    ...
    ("older command execution boundary lineage not current execution", ("older_command_execution_boundary_lineage_treated_as_current_execution",), "OLDER_COMMAND_EXECUTION_BOUNDARY_LINEAGE_TREATED_AS_CURRENT_EXECUTION"),
...
def _public_block_code(code: str) -> str:
        ...
        for key, item in value.items():
            ...
            if key_text in RAW_BODY_KEYS:
                result[key_text] = {
                    ...
                    "reason": "selected basis and command result v2 must remain bounded",
                }
            else:
...
def _check(
    ...
    code = _public_block_code(block_code)
    checks.append(
        {
...
def _build_checks(request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    primary_checks = (
        ("command result question declared", _present(request.get("command_result_question")), "question declared", request.get("command_result_question"), "COMMAND_RESULT_QUESTION_UNDECLARED"),
        ("command result intent supported", request.get("command_result_intent") in SUPPORTED_INTENTS, sorted(SUPPORTED_INTENTS), request.get("command_result_intent"), "COMMAND_RESULT_INTENT_UNSUPPORTED"),
        ("command result scope supported", bool(scope) and not unsupported_scope, sorted(SUPPORTED_COMMAND_RESULT_SCOPE), scope, "UNSUPPORTED_COMMAND_RESULT_SCOPE"),
        ("command result boundary terminal summary basis declared if supplied", "selected_command_result_boundary_terminal_summary_basis" not in request or _basis_declared(request.get("selected_command_result_boundary_terminal_summary_basis")), "terminal summary basis declared if supplied", request.get("selected_command_result_boundary_terminal_summary_basis"), "COMMAND_RESULT_BOUNDARY_BASIS_MISSING"),
        ("command result boundary live artifact basis declared", _basis_declared(request.get("selected_command_result_boundary_basis")), "command result boundary basis declared", request.get("selected_command_result_boundary_basis"), "COMMAND_RESULT_BOUNDARY_BASIS_MISSING"),
        ("command result boundary outcome recorded", result_boundary.get("outcome") == COMMAND_RESULT_BOUNDARY_OUTCOME, COMMAND_RESULT_BOUNDARY_OUTCOME, result_boundary.get("outcome"), "COMMAND_RESULT_BOUNDARY_NOT_RECORDED"),
        ("command result boundary failed check count zero", _to_int(result_boundary.get("failed_check_count")) == 0, 0, result_boundary.get("failed_check_count"), "COMMAND_RESULT_BOUNDARY_FAILED_CHECKS_PRESENT"),
        ("command result boundary one future command result step declared", result_boundary.get("one_future_command_result_step_declared") is True, True, result_boundary.get("one_future_command_result_step_declared"), "COMMAND_RESULT_BOUNDARY_STEP_NOT_DECLARED"),
        ("command result boundary command result not created", result_boundary.get("command_result_created") is False, False, result_boundary.get("command_result_created"), "COMMAND_RESULT_BOUNDARY_ALREADY_CREATED_RESULT_BODY"),
        ("command result boundary result body not invented", result_boundary.get("result_body_not_invented") is True and result_boundary.get("result_body_invented") is False, "not invented", {"result_body_not_invented": result_boundary.get("result_body_not_invented"), "result_body_invented": result_boundary.get("result_body_invented")}, "COMMAND_RESULT_BOUNDARY_ALREADY_CREATED_RESULT_BODY"),
        ("command result boundary command success not created", result_boundary.get("command_success_created") is False, False, result_boundary.get("command_success_created"), "COMMAND_RESULT_BOUNDARY_ALREADY_CREATED_SUCCESS"),
        ("command result boundary no pass inference", result_boundary.get("pass_inference_made") is False, False, result_boundary.get("pass_inference_made"), "COMMAND_RESULT_BOUNDARY_INFERRED_PASS"),
        ("command result boundary no fail inference", result_boundary.get("fail_inference_made") is False, False, result_boundary.get("fail_inference_made"), "COMMAND_RESULT_BOUNDARY_INFERRED_FAIL"),
        ("command output/report artifact basis declared", _basis_declared(request.get("selected_command_output_report_artifact_basis")), "artifact basis declared", request.get("selected_command_output_report_artifact_basis"), "COMMAND_OUTPUT_REPORT_ARTIFACT_BASIS_MISSING"),
        ("command output/report artifact terminal summary basis declared", _basis_declared(request.get("selected_command_output_report_artifact_terminal_summary_basis")), "terminal summary basis declared", request.get("selected_command_output_report_artifact_terminal_summary_basis"), "COMMAND_OUTPUT_REPORT_ARTIFACT_BASIS_MISSING"),
        ("command output/report artifact outcome recorded", artifact.get("outcome") == COMMAND_OUTPUT_REPORT_ARTIFACT_OUTCOME, COMMAND_OUTPUT_REPORT_ARTIFACT_OUTCOME, artifact.get("outcome"), "COMMAND_OUTPUT_REPORT_ARTIFACT_NOT_RECORDED"),
        ("command output/report artifact failed check count zero", _to_int(artifact.get("failed_check_count")) == 0, 0, artifact.get("failed_check_count"), "COMMAND_OUTPUT_REPORT_ARTIFACT_FAILED_CHECKS_PRESENT"),
        ("command output/report artifact bounded artifact recorded", artifact.get("bounded_command_output_report_artifact_recorded") is True, True, artifact.get("bounded_command_output_report_artifact_recorded"), "COMMAND_OUTPUT_REPORT_ARTIFACT_NOT_BOUNDED"),
        ("command output/report artifact report body absent or bounded", artifact.get("report_body_absent_or_bounded") is True, True, artifact.get("report_body_absent_or_bounded"), "REPORT_BODY_TREATED_AS_RESULT_AUTHORITY"),
        ("command output/report artifact report body not invented", artifact.get("report_body_not_invented") is True and artifact.get("report_body_invented") is False, "not invented", {"report_body_not_invented": artifact.get("report_body_not_invented"), "report_body_invented": artifact.get("report_body_invented")}, "COMMAND_OUTPUT_REPORT_ARTIFACT_INVENTED_REPORT_BODY"),
        ("command output/report artifact command result not created", artifact.get("command_result_created") is False, False, artifact.get("command_result_created"), "COMMAND_OUTPUT_REPORT_ARTIFACT_TREATED_AS_RESULT_AUTHORITY"),
        ("command output/report artifact command success not created", artifact.get("command_success_created") is False, False, artifact.get("command_success_created"), "COMMAND_OUTPUT_REPORT_ARTIFACT_ALREADY_CREATED_SUCCESS"),
        ("command output/report artifact boundary basis declared", _basis_declared(request.get("selected_command_output_report_artifact_boundary_basis")), "boundary basis declared", request.get("selected_command_output_report_artifact_boundary_basis"), "COMMAND_OUTPUT_REPORT_ARTIFACT_BASIS_MISSING"),
        ("command output/report artifact boundary outcome recorded", artifact_boundary.get("outcome") in (None, COMMAND_OUTPUT_REPORT_ARTIFACT_BOUNDARY_OUTCOME), COMMAND_OUTPUT_REPORT_ARTIFACT_BOUNDARY_OUTCOME, artifact_boundary.get("outcome"), "COMMAND_OUTPUT_REPORT_ARTIFACT_NOT_RECORDED"),
        ("command output/report artifact boundary failed check count zero", _to_int(artifact_boundary.get("failed_check_count")) in (None, 0), 0, artifact_boundary.get("failed_check_count"), "COMMAND_OUTPUT_REPORT_ARTIFACT_FAILED_CHECKS_PRESENT"),
        ("command output/report artifact boundary one future report artifact step declared", artifact_boundary.get("one_future_command_output_report_artifact_step_declared") is True, True, artifact_boundary.get("one_future_command_output_report_artifact_step_declared"), "COMMAND_OUTPUT_REPORT_ARTIFACT_BASIS_MISSING"),
        ("command output/report artifact boundary command result not created", artifact_boundary.get("command_result_created") is False, False, artifact_boundary.get("command_result_created"), "COMMAND_OUTPUT_REPORT_ARTIFACT_TREATED_AS_RESULT_AUTHORITY"),
        ("command output/report artifact boundary command success not created", artifact_boundary.get("command_success_created") is False, False, artifact_boundary.get("command_success_created"), "COMMAND_OUTPUT_REPORT_ARTIFACT_ALREADY_CREATED_SUCCESS"),
    )
    ...
    output_capture_checks = (
        ("output capture v2 basis declared", _basis_declared(request.get("selected_output_capture_v2_basis")), "output capture v2 basis declared", request.get("selected_output_capture_v2_basis"), "OUTPUT_CAPTURE_V2_BASIS_MISSING"),
        ("output capture v2 outcome recorded", output_capture_v2.get("outcome") == OUTPUT_CAPTURE_V2_OUTCOME, OUTPUT_CAPTURE_V2_OUTCOME, output_capture_v2.get("outcome"), "OUTPUT_CAPTURE_V2_NOT_RECORDED"),
        ("output capture v2 result version 0.2.0", str(output_capture_v2.get("result_version")) == OUTPUT_CAPTURE_V2_VERSION, OUTPUT_CAPTURE_V2_VERSION, output_capture_v2.get("result_version"), "OUTPUT_CAPTURE_V2_VERSION_NOT_0_2_0"),
        ("output capture v2 failed check count zero", _to_int(output_capture_v2.get("failed_check_count")) == 0, 0, output_capture_v2.get("failed_check_count"), "OUTPUT_CAPTURE_V2_FAILED_CHECKS_PRESENT"),
        ("output capture v2 JSON-safe result preserved", output_capture_v2.get("json_safe_result") is True, True, output_capture_v2.get("json_safe_result"), "OUTPUT_CAPTURE_V2_JSON_SAFE_RESULT_NOT_PRESERVED"),
        ("output capture v2 predecessor failure evidence preserved", output_capture_v2.get("v1_predecessor_failure_preserved") is True, True, output_capture_v2.get("v1_predecessor_failure_preserved"), "OUTPUT_CAPTURE_V2_PREDECESSOR_FAILURE_EVIDENCE_MISSING"),
        ("output capture v2 v1 repaired false", output_capture_v2.get("v1_repaired") is False, False, output_capture_v2.get("v1_repaired"), "OUTPUT_CAPTURE_V2_REPAIRED_V1"),
        ("output capture v2 v1 hidden false", output_capture_v2.get("v1_hidden") is False, False, output_capture_v2.get("v1_hidden"), "OUTPUT_CAPTURE_V2_HID_V1"),
        ("output capture v2 v1 claimed passed false", output_capture_v2.get("v1_claimed_passed") is False, False, output_capture_v2.get("v1_claimed_passed"), "OUTPUT_CAPTURE_V2_CLAIMED_V1_PASSED"),
        ("output capture v2 does not erase v1", output_capture_v2.get("v2_successor_does_not_erase_v1") is True, True, output_capture_v2.get("v2_successor_does_not_erase_v1"), "OUTPUT_CAPTURE_V2_ERASED_V1"),
    )
    ...
    basis_shape_checks = (
        ("post-invocation command execution recorded", _outcome(post_invocation) in (None, POST_INVOCATION_COMMAND_EXECUTION_OUTCOME) and _basis_declared(post_invocation), POST_INVOCATION_COMMAND_EXECUTION_OUTCOME, _outcome(post_invocation), "POST_INVOCATION_COMMAND_EXECUTION_NOT_RECORDED"),
        ("post-invocation command execution failed check count zero", _failed_check_count(post_invocation) in (None, 0), 0, _failed_check_count(post_invocation), "POST_INVOCATION_COMMAND_EXECUTION_FAILED_CHECKS_PRESENT"),
        ("post-invocation command execution trace audit-only", _deep_find(post_invocation, ("execution_trace_audit_only",), True) is True, True, _deep_find(post_invocation, ("execution_trace_audit_only",), True), "POST_INVOCATION_COMMAND_EXECUTION_TRACE_NOT_AUDIT_ONLY"),
        ("consumed request token remains closed", _deep_find(consumed, ("consumed_request_token_remains_closed", "consumption_token_closed"), True) is True and _deep_find(consumed, ("consumed_request_reopened", "consumed_request_is_reopened"), False) is False, "closed consumed request token", _sanitize_reference(consumed), "CONSUMED_REQUEST_REOPENED"),
        ("v2 admitted request failed check count zero", _failed_check_count(v2_admitted) in (None, 0), 0, _failed_check_count(v2_admitted), "V2_ADMITTED_REQUEST_FAILED_CHECKS_PRESENT"),
        ("older command execution boundary lineage not current execution", _deep_find(older_lineage, ("older_command_execution_boundary_lineage_treated_as_current_execution",), False) is False and _deep_find(older_lineage, ("lineage_basis_not_treated_as_current_execution",), True) is True, "prior scaffolding only", _sanitize_reference(older_lineage), "OLDER_COMMAND_EXECUTION_BOUNDARY_LINEAGE_TREATED_AS_CURRENT_EXECUTION"),
        ("command report lineage basis not current report artifact", _deep_find(command_report, ("current_report_artifact",), False) is False and _deep_find(command_report, ("command_report_lineage_basis_treated_as_current_report_artifact",), False) is False, "lineage only", _sanitize_reference(command_report), "COMMAND_REPORT_LINEAGE_BASIS_TREATED_AS_CURRENT_REPORT_ARTIFACT"),
        ("command report lineage basis not command result authority", _deep_find(command_report, ("command_report_lineage_basis_treated_as_command_result_authority", "command_report_lineage_basis_treated_as_command_result"), False) is False, "lineage only, not command result authority", _sanitize_reference(command_report), "COMMAND_REPORT_LINEAGE_BASIS_TREATED_AS_COMMAND_RESULT_AUTHORITY"),
        ("command report lineage basis not command success", _deep_find(command_report, ("command_report_lineage_basis_treated_as_command_success",), False) is False, "lineage only, not command success", _sanitize_reference(command_report), "COMMAND_REPORT_LINEAGE_BASIS_TREATED_AS_COMMAND_SUCCESS"),
    )
...
def _determine_outcome(
    ...
    if first_failure is not None:
        return (
            OUTCOME_BLOCKED,
            _public_block_code(str(first_failure.get("block_code") or first_failure.get("failure_code"))),
...
def _metadata(request: Mapping[str, Any], checks: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    request_id = request.get(
        ...
        "portable_source_body_verification_command_result_v2_request",
    )
    return {
        "portable_source_body_verification_command_result_result_id": (
            f"{request_id}__portable_source_body_verification_command_result_v2_result"
        ),
...
def _assert_public_block_conformance(result: Mapping[str, Any]) -> None:
    block = _mapping(result.get("block"))
    block_code = block.get("block_code")
    if block_code is not None and block_code not in BLOCK_CODES:
        raise PortableSourceBodyVerificationCommandResultV2Error(
            f"undeclared block code emitted: {block_code}"
        )
    for check in result.get("command_result_checks", []):
        ...
        for code_key in ("block_code", "failure_code"):
            ...
            if code is not None and code not in BLOCK_CODES:
                raise PortableSourceBodyVerificationCommandResultV2Error(
                    f"undeclared check {code_key} emitted: {code}"
                )
...
def _build_result(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result["portable_source_body_verification_command_result_summary"] = (
        build_portable_source_body_verification_command_result_v2_summary(result)
    )
    safe_result = _json_safe(result)
    _assert_public_block_conformance(safe_result)
    return safe_result
...
def _malformed_request_result(block_code: str, block_reason: str) -> dict[str, Any]:
    ...
    result["block"] = {
        "blocked": True,
        "block_code": _public_block_code(block_code),
        "block_reason": block_reason,
    }
    result["portable_source_body_verification_command_result_summary"] = (
        build_portable_source_body_verification_command_result_v2_summary(result)
    )
    _assert_public_block_conformance(result)
    return result


def resolve_portable_source_body_verification_command_result_v2(
    declared_command_result_request: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Return one bounded portable source-body verification command result v2."""
...
def resolve_portable_source_body_verification_command_result_v2_from_path(
    declared_command_result_request_path: Path | str,
) -> dict[str, Any]:
    """Load one declared command result request JSON object for v2."""
    ...
    return resolve_portable_source_body_verification_command_result_v2(
...
def _safe_request_id(result: Mapping[str, Any]) -> str:
    ...
    request_id = question.get("command_result_request_id") or metadata.get(
        "portable_source_body_verification_command_result_result_id",
        "portable_source_body_verification_command_result_v2",
    )
    safe = "".join(
        character if character.isalnum() or character in ("-", "_") else "_"
        for character in str(request_id)
    ).strip("_")
    return safe or "portable_source_body_verification_command_result_v2"
...
def write_portable_source_body_verification_command_result_v2_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write a JSON-safe command result v2 artifact additively."""

    if not isinstance(result, Mapping):
        raise PortableSourceBodyVerificationCommandResultV2Error(
            "command result v2 result must be a mapping"
        )
    _assert_public_block_conformance(result)
    ...
    if output_path is None:
        ...
        output_path = OUTPUT_ROOT / (
            f"{request_id}__portable_source_body_verification_command_result_v2_result.json"
        )
...
def build_portable_source_body_verification_command_result_v2_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build a compact JSON-safe summary for one command result v2."""
    ...
    summary = {
        ...
        "artifact_not_result_authority_success_source_authority_currentness_final_completion": (
            non_claims.get("command_output_report_artifact_treated_as_result_authority", False) is False
            and non_claims.get("command_output_report_artifact_treated_as_success", False) is False
            and non_claims.get("command_output_report_artifact_treated_as_source", False) is False
            and non_claims.get("command_output_report_artifact_treated_as_authority", False) is False
            and non_claims.get("command_output_report_artifact_treated_as_currentness", False) is False
            and non_claims.get("command_output_report_artifact_treated_as_final_completion", False) is False
        ),
        "report_body_not_result_authority_success_source_authority_currentness_final_completion": (
            non_claims.get("report_body_treated_as_result_authority", False) is False
            and non_claims.get("report_body_treated_as_success", False) is False
            and non_claims.get("report_body_treated_as_source", False) is False
            and non_claims.get("report_body_treated_as_authority", False) is False
            and non_claims.get("report_body_treated_as_currentness", False) is False
            and non_claims.get("report_body_treated_as_final_completion", False) is False
        ),
        "output_capture_not_result_success_source_authority_currentness_final_completion": (
            non_claims.get("output_capture_treated_as_result", False) is False
            and non_claims.get("output_capture_treated_as_success", False) is False
            and non_claims.get("output_capture_treated_as_source", False) is False
            and non_claims.get("output_capture_treated_as_authority", False) is False
            and non_claims.get("output_capture_treated_as_currentness", False) is False
            and non_claims.get("output_capture_treated_as_final_completion", False) is False
        ),
        "execution_trace_not_result_success_source_authority": (
            non_claims.get("execution_trace_treated_as_result", False) is False
            and non_claims.get("execution_trace_treated_as_success", False) is False
            and non_claims.get("execution_trace_treated_as_source", False) is False
            and non_claims.get("execution_trace_treated_as_authority", False) is False
        ),
        "result_not_authority": non_claims.get("command_result_became_authority", False) is False,
        "success_not_currentness_or_final_completion": (
            non_claims.get("command_success_created_currentness", False) is False
            and non_claims.get("command_success_claimed_final_completion", False) is False
        ),
...
def build_declared_portable_source_body_verification_command_result_v2_request(
    *,
    ...
    **selected_basis: Mapping[str, Any],
    ...
    """Build a declared v2 request without inferring success or next work."""
