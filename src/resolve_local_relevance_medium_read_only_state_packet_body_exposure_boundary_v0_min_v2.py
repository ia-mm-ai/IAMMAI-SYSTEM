"""Resolve one selected-state packet-body-exposure boundary, v2.

This successor preserves the v0 boundary resolver posture and corrects the
closed-token failure line: consumed request reopening and authorization token
reuse now block explicitly. The v0 resolver and test remain preserved failed
lineage evidence.

The resolver records one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_STATE_PACKET_BODY_EXPOSURE_BOUNDARY object from
one clean selected-state result object artifact. It is boundary-shaped, local,
read-only, selected-state-packet-body-exposure-consideration-only, and
non-packet-body-exposure-shaped. It does not expose packet body material,
...
class LocalRelevanceMediumReadOnlyStatePacketBodyExposureBoundaryV0MinV2Error(
    RuntimeError
):
    """Bounded resolver error for state packet body exposure boundary v2."""


LocalRelevanceMediumReadOnlyStatePacketBodyExposureBoundaryV0MinError = (
    LocalRelevanceMediumReadOnlyStatePacketBodyExposureBoundaryV0MinV2Error
)
...
def resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_v2(
    declared_local_relevance_medium_read_only_state_packet_body_exposure_boundary: Mapping[
        str, Any
    ]
    | None = None,
) -> dict:
    """Resolve one v2 state packet body exposure boundary request."""
    ...
    _check(
        ...
        artifact_version == BASIS_STATE_RESULT_OBJECT_RESULT_VERSION
...
def resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_v2_from_path(
    ...
    return resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_v2(
        value
    )


def build_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_v2_summary(
    result: Mapping[str, Any],
) -> dict:
    """Build a compact v2 summary for a resolver result artifact."""
    ...
    def boundary_false(key: str) -> bool:
    ...
    consumed_request_token_closed = non_claim_false("consumed_request_reopened")
    authorization_token_reuse_blocked = non_claim_false("authorization_token_reused")
    ...
    return {
        ...
        "basis_state_result_object_result_version_required": (
            BASIS_STATE_RESULT_OBJECT_RESULT_VERSION
        ),
        "resolver_module": RESOLVER_MODULE,
        "predecessor_resolver_module": metadata.get("predecessor_resolver_module"),
        "predecessor_failure_evidence_preserved": (
            predecessor_failure_evidence_preserved
        ),
        "predecessor_failure_not_repaired": non_claim_false(
            "predecessor_failure_repaired"
        ),
        "predecessor_failure_not_hidden": non_claim_false("predecessor_failure_hidden"),
        "predecessor_failure_not_claimed_passed": non_claim_false(
            "predecessor_failure_claimed_passed"
        ),
        "consumed_request_token_remains_closed": consumed_request_token_closed,
        "authorization_token_reuse_blocked": authorization_token_reuse_blocked,
        ...
        "state_packet_body_not_exposed": boundary_false("state_packet_body_exposed"),
        "raw_full_state_packet_body_not_exposed": boundary_false(
            "raw_full_state_packet_body_exposed"
        ),
        "lookup_not_performed": boundary_false("lookup_performed"),
        "lookup_command_not_executed": boundary_false("lookup_command_executed"),
        "operation_permission_not_created": boundary_false(
            "operation_permission_created"
        ),
        "runtime_permission_not_created": boundary_false("runtime_permission_created"),
        "public_api_not_created": boundary_false("public_api_created"),
        "participant_facing_interface_not_created": boundary_false(
            "participant_facing_interface_created"
        ),
        "distributed_network_behavior_not_created": boundary_false(
            "distributed_network_behavior_created"
        ),
        "general_lookup_permission_not_created": boundary_false(
            "general_lookup_permission_created"
        ),
        "arbitrary_lookup_permission_not_created": boundary_false(
            "arbitrary_lookup_permission_created"
        ),
        "unsupported_commands_not_permitted": boundary_false(
            "unsupported_commands_permitted"
        ),
        "unsupported_lookup_keys_not_permitted": boundary_false(
            "unsupported_lookup_keys_permitted"
        ),
        "no_new_lookup_result_or_entry_created": boundary_false(
            "new_lookup_result_created"
        )
        and boundary_false("new_lookup_entry_created"),
        "no_new_signal_entry_relevance_object_or_index_entry_created": (
            boundary_false("new_signal_accepted")
            and boundary_false("new_entry_accepted")
            and boundary_false("new_relevance_object_created")
            and boundary_false("new_index_entry_created")
        ),
        "filesystem_discovery_not_performed": boundary_false(
            "filesystem_discovery_performed"
        ),
        "registry_search_query_surface_ranking_not_created": (
            boundary_false("registry_created")
            and boundary_false("search_surface_created")
            and boundary_false("query_surface_created")
            and boundary_false("ranking_surface_created")
        ),
        "scoring_priority_validity_truth_authority_currentness_judgment_not_created": (
            boundary_false("scoring_surface_created")
            and boundary_false("priority_surface_created")
            and boundary_false("validity_judgment_created")
            and boundary_false("truth_judgment_created")
            and boundary_false("authority_judgment_created")
            and boundary_false("currentness_judgment_created")
        ),
        "repeated_reception_arbitrary_reception_feed_not_created": (
            boundary_false("repeated_reception_permission_created")
            and boundary_false("arbitrary_reception_created")
            and boundary_false("feed_created")
        ),
        "source_authority_currentness_truth_synchronization_participation_participant_role_not_created": (
            boundary_false("source_transfer_occurred")
            and boundary_false("source_receipt_occurred")
            and non_claim_false("source_created")
            and boundary_false("authority_created")
            and boundary_false("currentness_created")
            and boundary_false("truth_created")
            and boundary_false("synchronization_created")
            and boundary_false("participation_authorized")
            and boundary_false("participant_role_created")
        ),
        "follow_on_not_created": boundary_false("follow_on_work_authorized"),
        "key_non_claims": {
...
def write_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_v2_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write a v2 resolver result artifact without silently overwriting files."""

    if not isinstance(result, Mapping):
        raise LocalRelevanceMediumReadOnlyStatePacketBodyExposureBoundaryV0MinV2Error(
    ...
    filename = (
        f"{boundary_id}__"
        "local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_v2_result.json"
    )
...
def build_declared_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_v2_request(
    *,
    ...
    **overrides: Any,
    ...
    """Build a declared v2 request with canonical false non-claims."""
