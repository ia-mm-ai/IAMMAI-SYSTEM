"""Pure current-line resolver for one bounded RELATION_REVERSIBILITY_OPERATION event."""
...
SOURCE_OUTCOME_ALLOWED = "RELATION_REVERSIBILITY_BOUNDARY_ALLOWED"
SOURCE_RESULT_ALLOWED = "RELATION_REVERSIBILITY_OPERATION_CONSIDERATION_ALLOWED"
...
CURRENT_BOUNDARY_RESULT_REFERENCE = (
    "artifacts/relation_reversibility_boundary_v0_min_v2/"
    "relation_reversibility_boundary_001__"
    "relation_reversibility_boundary_v0_min_v2_result.json"
)
CURRENT_BOUNDARY_RESULT_SHA256 = (
    "70407c854a16dd092cf84af2345745002cb5cb968dc3d6919d2e001b30c1e4e1"
)
...
OPERATION_QUESTION = (
    "May RELATION_REVERSIBILITY_OPERATION, under the exact current "
    "RELATION_REVERSIBILITY_BOUNDARY allowance, record bounded reversibility "
    "support for relation_001 as a historical relation record only, without "
    "lapsing, dissolving, reversing, terminating, erasing, mutating, "
    "invalidating, punishing, or tearing down the relation, and without "
...
RESULT_SUPPORTED = "RELATION_REVERSIBILITY_SUPPORTED"
...
EXPECTED_CONSTITUTIONAL_EVENT_KEY: dict[str, Any] = {
    ...
    "historical_boundary": {
        ...
        "outcome": SOURCE_OUTCOME_ALLOWED,
        "boundary_result": SOURCE_RESULT_ALLOWED,
        "relation_id": RELATION_ID,
    ...
    "historical_operation": {
        ...
        "result_reference": (
            ...
            "relation_reversibility_operation_001__"
            "relation_reversibility_operation_v0_min_result.json"
        ),
        "result_sha256": (
            "d59c4ce570acdf15a06199827d3e6a1432804f708949e73f6c4efab6ee278fbb"
        ),
        "outcome": OUTCOME_RECORDED,
        "operation_result": RESULT_SUPPORTED,
        "relation_id": RELATION_ID,
        "relation_reversibility_id": RELATION_REVERSIBILITY_ID,
    ...
    "non_claim_attribution": {
        ...
        "operation_local": {"owner": OPERATION_TYPE},
    },
}
...
ORDINARY_REVERSIBILITY_SUPPORT_BASIS_PATHS = frozenset()
ORDINARY_REVERSIBILITY_SUPPORT_BASIS = ORDINARY_REVERSIBILITY_SUPPORT_BASIS_PATHS
...
SOURCE_BOUNDARY_LOCAL_NON_CLAIM_PATHS = frozenset(
...
SOURCE_BOUNDARY_REQUIRED_NON_CLAIM_PATHS = (
    SOURCE_RELATION_BOUNDARY_NON_CLAIM_PATHS
    | SOURCE_RELATION_OPERATION_NON_CLAIM_PATHS
    | SOURCE_BOUNDARY_LOCAL_NON_CLAIM_PATHS
)
REQUIRED_NON_CLAIM_PATHS = (
    SOURCE_BOUNDARY_REQUIRED_NON_CLAIM_PATHS | OPERATION_LOCAL_NON_CLAIM_PATHS
)
CLASS_PATHS = {
    ...
    "ORDINARY_REVERSIBILITY_SUPPORT_BASIS": (
        ORDINARY_REVERSIBILITY_SUPPORT_BASIS_PATHS
    ),
...
assert len(CONSTITUTIONAL_EVENT_KEY_PATHS) == 228
assert len(ORDINARY_BOUNDARY_ALLOWANCE_PATHS) == 6
assert len(ORDINARY_REVERSIBILITY_SUPPORT_BASIS_PATHS) == 0
assert len(SOURCE_RELATION_BOUNDARY_NON_CLAIM_PATHS) == 8
assert len(SOURCE_RELATION_OPERATION_NON_CLAIM_PATHS) == 68
assert len(SOURCE_BOUNDARY_LOCAL_NON_CLAIM_PATHS) == 86
assert len(SOURCE_BOUNDARY_REQUIRED_NON_CLAIM_PATHS) == 162
assert len(OPERATION_LOCAL_NON_CLAIM_PATHS) == 96
assert len(REQUIRED_NON_CLAIM_PATHS) == 258
assert len(ALL_REQUIRED_PATHS) == 495
...
def build_declared_relation_reversibility_operation_v0_min_v2_request() -> dict[str, Any]:
...
build_relation_reversibility_operation_v0_min_v2_request = (
    build_declared_relation_reversibility_operation_v0_min_v2_request
)
...
def _operation_material(outcome: str, event_valid: bool) -> dict[str, Any]:
    ...
    return {
        "boundary_allowance_reference": {
            "source_outcome": SOURCE_OUTCOME_ALLOWED,
            "source_boundary_result": SOURCE_RESULT_ALLOWED,
            "relation_reversibility_operation_consideration_allowed": recorded,
            "relation_operation_referenced": recorded,
...
def _build_result(
    ...
    *,
    ...
    return {
        ...
        "outcome": outcome,
        "operation_question": OPERATION_QUESTION,
        "persistent_relation_reversibility_operation_contract": _event_projection(
            event_valid, "next_operation"
        ),
        "current_relation_reversibility_boundary_result_binding": _event_projection(
            event_valid, "current_boundary_result"
        ),
        "current_relation_reversibility_operation_event": _event_projection(
        ...
        "boundary_freshness_and_history": (
            {
        ...
        "operation_freshness_and_history": (
            {
                "freshness": _event_projection(event_valid, "operation_freshness"),
        ...
        "ordinary_boundary_allowance_review": {
            ...
            "ordinary_reversibility_support_input_count": 0,
        ...
        "relation_reversibility_operation_material": _operation_material(
            outcome, event_valid
        ),
        ...
        "source_boundary_local_non_claims": {
        ...
        "downstream_stopping_point": {
            ...
            "historical_realized_immediate_successor": "RELATION_LAPSE_BOUNDARY",
...
def resolve_relation_reversibility_operation_v0_min_v2(
    supplied_envelope: Any,
) -> dict[str, Any]:
    """Resolve one exact current reversibility-operation event without I/O."""
