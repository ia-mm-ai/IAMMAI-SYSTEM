"""Resolve one local read-only selected-state runtime permission.

This resolver reads one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_BOUNDARY artifact and one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION artifact. It records one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION object only.

The object is runtime-permission-shaped, local, read-only, selected-state-only,
closure-token-aware, non-runtime-shaped, non-runtime-hosting-shaped, and
older-runtime-authority-import-refusing. It records one bounded local read-only
runtime permission for selected command state. It creates no runtime, runtime
hosting, runtime loop, daemon behavior, continuation, runtime-held state,
runtime-held re-entry, second operation, metabolic loop, public API,
participant-facing interface, distributed behavior, general or arbitrary
permissions, unsupported command or key permission, new lookup entry,
registry/search/query/ranking surface, source movement, older runtime authority
import, or follow-on work.
"""
...
class LocalRelevanceMediumReadOnlyRuntimePermissionV0MinError(RuntimeError):
    """Bounded resolver error for runtime-permission request/path handling."""


RESULT_VERSION = "0.1.0"
RESOLVER_MODULE = "resolve_local_relevance_medium_read_only_runtime_permission_v0_min"

OUTCOME_RECORDED = "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_RECORDED"
...
OUTCOME_BLOCKED = "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_BLOCKED"
OUTCOME_FAMILY = (
    OUTCOME_RECORDED,
    OUTCOME_NOT_RECORDED,
    OUTCOME_REQUIRES_ADDITIONAL_BASIS,
...
DEFAULT_RUNTIME_PERMISSION_BOUNDARY_ARTIFACT = Path(
    "artifacts/"
    "integrity_host_v0_min_coexistence_local_relevance_medium_read_only_"
...
DEFAULT_RUNTIME_PERMISSION_ID = "local_relevance_medium_read_only_runtime_permission_001"
...
SUPPORTED_RUNTIME_PERMISSION_TYPE_VALUES = (RUNTIME_PERMISSION_TYPE,)
SUPPORTED_RUNTIME_PERMISSION_SCOPE_VALUES = (RUNTIME_PERMISSION_SCOPE,)
SELECTED_COMMAND = "state"

RUNTIME_PERMISSION_BOUNDARY_RECORDED_OUTCOME = (
...
INTENT_RECORD = "RECORD_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION"
INTENT_DO_NOT_RECORD = "DO_NOT_RECORD_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION"
INTENT_BLOCK = "BLOCK_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION"
SUPPORTED_INTENTS = (INTENT_RECORD, INTENT_DO_NOT_RECORD, INTENT_BLOCK)

CORE_QUESTION = (
    "Given one clean LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_BOUNDARY "
    ...
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION be recorded for selected "
    "command state, without creating runtime, creating runtime hosting, creating "
    "runtime loop, creating daemon behavior, creating continuation, creating "
    "runtime-held state, creating runtime-held re-entry, creating second operation, "
    "creating metabolic loop, creating public API, creating participant-facing "
    "interface, creating distributed network behavior, creating general operation "
    "permission, creating general lookup permission, creating arbitrary lookup "
    "permission, permitting unsupported commands, permitting unsupported lookup "
    "keys, creating new lookup entry beyond the already bounded selected-state "
    "lookup result object, accepting new entries, accepting new signals, "
    "performing filesystem discovery, importing older runtime/post-runtime "
    "authority, creating query surface, registry, search, ranking, scoring, "
    "priority, validity judgment, truth judgment, authority, currentness, "
    "synchronization, participation authorization, participant role, repeated "
...
REQUIRED_FALSE_NON_CLAIMS = (
    RUNTIME_PERMISSION_OBJECT_FALSE_FIELDS + REQUEST_ONLY_FALSE_FIELDS
)
...
BLOCK_CODES = (
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_QUESTION_UNDECLARED",
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_INTENT_UNSUPPORTED",
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_BLOCK_REQUESTED",
    "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_PATH_MISSING",
    "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_UNREADABLE",
    "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_NOT_JSON_OBJECT",
    "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_NOT_RECORDED",
    "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_FAILED_CHECKS_PRESENT",
    "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_VERSION_NOT_0_1_0",
    ...
    "RUNTIME_PERMISSION_TYPE_MISSING",
    "RUNTIME_PERMISSION_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION",
    "RUNTIME_PERMISSION_SCOPE_MISSING",
    "RUNTIME_PERMISSION_SCOPE_NOT_SELECTED_RUNTIME_PERMISSION_ONLY",
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_NOT_RECORDED",
    ...
    "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_REQUEST_MALFORMED",
    "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_REQUEST_UNREADABLE",
)
...
_FALSE_FIELD_BLOCK_CODES = {
    ...
    "metabolic_loop_created": "METABOLIC_LOOP_CREATED",
...
HOSTILE_SENTINELS = (
    "RAW_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_BODY_MUST_NOT_RETURN",
    ...
    "RAW_METABOLIC_LOOP_BODY_MUST_NOT_RETURN",
...
def _empty_runtime_permission_boundary_basis(path_value: Any) -> dict[str, Any]:
    return {
        ...
        "future_runtime_permission_may_be_considered": None,
...
def _extract_runtime_permission_boundary_basis(
    artifact: Mapping[str, Any],
    path_value: Any,
) -> dict[str, Any]:
    ...
    statement = _mapping(
        artifact.get("local_relevance_medium_read_only_runtime_permission_boundary_statement")
    )
    return {
        ...
        "boundary_type": _first_present(
            obj.get("boundary_type"),
            summary.get("boundary_type"),
        ),
        "boundary_scope": _first_present(
            obj.get("boundary_scope"),
            summary.get("boundary_scope"),
        ),
        ...
        "selected_operation_execution_recorded": _first_present(
            obj.get("selected_operation_execution_recorded"),
            statement.get("selected_operation_execution_recorded"),
            summary.get("selected_operation_execution_recorded"),
        ),
        ...
        "future_runtime_permission_may_be_considered": _first_present(
            obj.get("future_runtime_permission_may_be_considered"),
            statement.get("future_runtime_permission_may_be_considered"),
            summary.get("future_runtime_permission_may_be_considered"),
        ),
...
def _extract_operation_execution_basis(
    ...
    return {
        ...
        "selected_operation_execution_recorded": _first_present(
            obj.get("selected_operation_execution_recorded"),
            obj.get("local_relevance_medium_read_only_operation_execution_recorded"),
            statement.get("selected_operation_execution_recorded"),
            statement.get("local_relevance_medium_read_only_operation_execution_recorded"),
            summary.get("selected_operation_execution_recorded"),
            summary.get("local_relevance_medium_read_only_operation_execution_recorded"),
        ),
        "operation_execution_created": _first_present(
            obj.get("operation_execution_created"),
            statement.get("operation_execution_created"),
...
def _build_runtime_permission_object(
    declared: Mapping[str, Any],
    runtime_permission_boundary_basis: Mapping[str, Any],
    ...
    runtime_permission: dict[str, Any] = {
        "runtime_permission_id": str(
            declared.get(
                "local_relevance_medium_read_only_runtime_permission_id",
                DEFAULT_RUNTIME_PERMISSION_ID,
            )
            or DEFAULT_RUNTIME_PERMISSION_ID
        ),
        "runtime_permission_type": RUNTIME_PERMISSION_TYPE,
        "runtime_permission_version": RESULT_VERSION,
        "runtime_permission_scope": RUNTIME_PERMISSION_SCOPE,
        "basis_runtime_permission_boundary_artifact": runtime_permission_boundary_basis.get(
            "artifact_path"
        ),
        "basis_runtime_permission_boundary_outcome": runtime_permission_boundary_basis.get(
            "artifact_outcome"
        ),
        "basis_runtime_permission_boundary_result_version": (
            runtime_permission_boundary_basis.get("artifact_result_version")
        ),
        "basis_runtime_permission_boundary_failed_check_count": (
            runtime_permission_boundary_basis.get("artifact_failed_check_count")
        ),
        "basis_operation_execution_artifact": operation_execution_basis.get(
            "artifact_path"
        ),
        "basis_operation_execution_outcome": operation_execution_basis.get(
            "artifact_outcome"
        ),
        ...
        "local_relevance_medium_read_only_runtime_permission_recorded": bool(recorded),
        "runtime_permission_created": bool(recorded),
        "runtime_permission_local_only": bool(recorded),
        "runtime_permission_read_only": bool(recorded),
    }
    for field in RUNTIME_PERMISSION_OBJECT_FALSE_FIELDS:
        runtime_permission[field] = False
    return _sanitize(runtime_permission)
...
def _build_statement(
    ...
    return {
        "local_relevance_medium_read_only_runtime_permission_recorded": bool(recorded),
        "basis_runtime_permission_boundary_artifact_preserved": (
            runtime_permission.get("basis_runtime_permission_boundary_outcome")
            == RUNTIME_PERMISSION_BOUNDARY_RECORDED_OUTCOME
        ),
        "basis_operation_execution_artifact_preserved": (
            runtime_permission.get("basis_operation_execution_outcome")
            == OPERATION_EXECUTION_RECORDED_OUTCOME
        ),
        "selected_command_preserved": runtime_permission.get("selected_command")
        == SELECTED_COMMAND,
        "selected_command_is_state": runtime_permission.get("selected_command_is_state")
        is True,
        "selected_operation_execution_recorded": runtime_permission.get(
            "selected_operation_execution_recorded"
        )
        is True,
        "operation_execution_created": runtime_permission.get("operation_execution_created")
        is True,
        "operation_execution_performed": runtime_permission.get(
            "operation_execution_performed"
        )
        is True,
        "operation_execution_local_only": runtime_permission.get(
            "operation_execution_local_only"
        )
        is True,
        "operation_execution_read_only": runtime_permission.get(
            "operation_execution_read_only"
        )
        is True,
        "runtime_permission_created": runtime_permission.get(
            "runtime_permission_created"
        )
        is True,
        "runtime_permission_local_only": runtime_permission.get(
            "runtime_permission_local_only"
        )
        is True,
        "runtime_permission_read_only": runtime_permission.get(
            "runtime_permission_read_only"
        )
        is True,
...
def _build_non_meaning() -> dict[str, bool]:
    non_meaning = {
        "runtime_permission_is_runtime": False,
        "runtime_permission_is_runtime_hosting": False,
        "runtime_permission_is_runtime_loop": False,
        "runtime_permission_is_daemon_behavior": False,
        "runtime_permission_is_continuation": False,
        "runtime_permission_is_runtime_held_state": False,
        "runtime_permission_is_runtime_held_reentry": False,
        "runtime_permission_is_second_operation": False,
        "runtime_permission_is_metabolic_loop": False,
        "runtime_permission_is_public_api": False,
        "runtime_permission_is_participant_facing_interface": False,
        "runtime_permission_is_distributed_network_behavior": False,
        "runtime_permission_imports_older_runtime_authority": False,
        "runtime_permission_creates_registry_search_query_or_ranking": False,
        "runtime_permission_authorizes_follow_on_work": False,
    }
    for field in REQUIRED_FALSE_NON_CLAIMS:
...
def _what_remains_open() -> list[str]:
    return [
        "local relevance medium read-only runtime permission test",
        "local relevance medium read-only runtime permission live artifact",
        "local relevance medium read-only runtime permission terminal summary",
        "runtime",
        "runtime hosting",
        "runtime loop",
        "daemon behavior",
...
def _build_result_artifact(
    ...
    runtime_permission = _build_runtime_permission_object(
        declared,
        runtime_permission_boundary_basis,
        operation_execution_basis,
        recorded,
    )
    statement = _build_statement(runtime_permission, recorded)
    ...
    result: dict[str, Any] = {
        "local_relevance_medium_read_only_runtime_permission_metadata": {
            "local_relevance_medium_read_only_runtime_permission_id": runtime_permission[
                "runtime_permission_id"
            ],
            "local_relevance_medium_read_only_runtime_permission_type": (
                RUNTIME_PERMISSION_TYPE
            ),
            "local_relevance_medium_read_only_runtime_permission_version": RESULT_VERSION,
        ...
        "declared_local_relevance_medium_read_only_runtime_permission_question": {
            "question": _sanitize(
                declared.get("local_relevance_medium_read_only_runtime_permission_question")
            ),
            "intent": _sanitize(
                declared.get("local_relevance_medium_read_only_runtime_permission_intent")
            ),
            "selected_command": _sanitize(declared.get("selected_command")),
            "runtime_permission_type": _sanitize(declared.get("runtime_permission_type")),
            "runtime_permission_scope": _sanitize(declared.get("runtime_permission_scope")),
        },
        "selected_runtime_permission_boundary_artifact_basis": _sanitize(
            dict(runtime_permission_boundary_basis)
        ),
        ...
        "local_relevance_medium_read_only_runtime_permission_checks": _sanitize(checks),
    ...
    result["local_relevance_medium_read_only_runtime_permission_summary"] = (
        build_local_relevance_medium_read_only_runtime_permission_v0_min_summary(result)
    )
...
def _blocked_malformed_result(code: str, reason: Any) -> dict[str, Any]:
    ...
    _add_check(
        checks,
        "declared_local_relevance_medium_read_only_runtime_permission_request_readable",
        False,
        "mapping JSON object",
        reason,
        code,
    ...
    return _build_result_artifact(
        {},
        _empty_runtime_permission_boundary_basis(None),
...
def resolve_local_relevance_medium_read_only_runtime_permission_v0_min(
    declared_local_relevance_medium_read_only_runtime_permission: Mapping[str, Any]
    | None = None,
) -> dict:
    """Resolve one bounded local read-only selected-state runtime permission."""

    if declared_local_relevance_medium_read_only_runtime_permission is None:
        declared = (
            build_declared_local_relevance_medium_read_only_runtime_permission_v0_min_request()
        )
    elif isinstance(declared_local_relevance_medium_read_only_runtime_permission, MappingABC):
        declared = copy.deepcopy(
            dict(declared_local_relevance_medium_read_only_runtime_permission)
        )
    else:
    ...
    checks: list[dict[str, Any]] = []
    question = declared.get("local_relevance_medium_read_only_runtime_permission_question")
    intent = declared.get("local_relevance_medium_read_only_runtime_permission_intent")

    _add_check(
        checks,
        "runtime_permission_question_declared",
        isinstance(question, str) and bool(question.strip()),
        "declared runtime permission question",
        question,
        "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_QUESTION_UNDECLARED",
    )
    _add_check(
        checks,
        "runtime_permission_intent_supported",
        intent in SUPPORTED_INTENTS and intent != INTENT_BLOCK,
        f"one of {SUPPORTED_INTENTS[:-1]}",
        intent,
        "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_BLOCK_REQUESTED"
        if intent == INTENT_BLOCK
        else "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_INTENT_UNSUPPORTED",
    )

    boundary_path = declared.get("selected_runtime_permission_boundary_artifact")
    boundary_path_declared = (
        boundary_path is not None
        and boundary_path != ""
        and declared.get("runtime_permission_boundary_artifact_missing") is not True
    )
    _add_check(
        checks,
        "runtime_permission_boundary_artifact_path_declared",
        boundary_path_declared,
        "declared runtime permission boundary artifact path",
        boundary_path,
        "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_PATH_MISSING",
    )
    runtime_permission_boundary_basis = _empty_runtime_permission_boundary_basis(
        boundary_path
    )
    if boundary_path_declared:
        ...
        _add_check(
            checks,
            "runtime_permission_boundary_artifact_readable_json_object",
            boundary_read_code is None,
            "readable JSON object",
            boundary_read_code or "readable JSON object",
            boundary_read_code or "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_UNREADABLE",
        )
        if boundary_artifact is not None:
            runtime_permission_boundary_basis = _extract_runtime_permission_boundary_basis(
                boundary_artifact,
                boundary_path,
            )

    boundary_outcome_ok = (
        runtime_permission_boundary_basis.get("artifact_outcome")
        == RUNTIME_PERMISSION_BOUNDARY_RECORDED_OUTCOME
        and declared.get("runtime_permission_boundary_artifact_not_recorded") is not True
    )
    _add_check(
        checks,
        "runtime_permission_boundary_artifact_outcome_recorded",
        boundary_outcome_ok,
        RUNTIME_PERMISSION_BOUNDARY_RECORDED_OUTCOME,
        runtime_permission_boundary_basis.get("artifact_outcome"),
        "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_NOT_RECORDED",
    ...
    boundary_version_ok = (
        runtime_permission_boundary_basis.get("artifact_result_version") == RESULT_VERSION
        and declared.get("runtime_permission_boundary_artifact_version_not_0_1_0")
        is not True
    )
    _add_check(
        checks,
        "runtime_permission_boundary_artifact_result_version_0_1_0",
        boundary_version_ok,
        RESULT_VERSION,
        runtime_permission_boundary_basis.get("artifact_result_version"),
        "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_VERSION_NOT_0_1_0",
    )
    boundary_failed_count = runtime_permission_boundary_basis.get(
        "artifact_failed_check_count"
    )
    boundary_failed_count_ok = (
        boundary_failed_count == 0
        and declared.get("runtime_permission_boundary_artifact_failed_checks_present")
        is not True
    )
    _add_check(
        checks,
        "runtime_permission_boundary_artifact_failed_check_count_zero",
        boundary_failed_count_ok,
        0,
    ...
    _add_check(
        ...
        "selected_operation_execution_recorded",
        operation_execution_basis.get("selected_operation_execution_recorded") is True
        and runtime_permission_boundary_basis.get("selected_operation_execution_recorded")
        is True
        and declared.get("selected_operation_execution_not_recorded") is not True,
        True,
        {
            "operation_execution_artifact": operation_execution_basis.get(
                "selected_operation_execution_recorded"
            ),
            "runtime_permission_boundary_artifact": runtime_permission_boundary_basis.get(
                "selected_operation_execution_recorded"
            ),
        },
        "SELECTED_OPERATION_EXECUTION_NOT_RECORDED",
    ...
    _add_check(
        ...
        "operation_execution_created",
        operation_execution_basis.get("operation_execution_created") is True
        and runtime_permission_boundary_basis.get("operation_execution_created") is True
    ...
    _add_check(
        ...
        "operation_execution_performed",
        operation_execution_basis.get("operation_execution_performed") is True
        and runtime_permission_boundary_basis.get("operation_execution_performed") is True
    ...
    _add_check(
        ...
        "operation_execution_local_only",
        operation_execution_basis.get("operation_execution_local_only") is True
        and runtime_permission_boundary_basis.get("operation_execution_local_only") is True
    ...
    _add_check(
        ...
        "operation_execution_read_only",
        operation_execution_basis.get("operation_execution_read_only") is True
        and runtime_permission_boundary_basis.get("operation_execution_read_only") is True
    ...
    runtime_permission_type = declared.get("runtime_permission_type")
    _add_check(
        checks,
        "runtime_permission_type_declared",
        runtime_permission_type is not None
        and declared.get("runtime_permission_type_missing") is not True,
        RUNTIME_PERMISSION_TYPE,
        runtime_permission_type,
        "RUNTIME_PERMISSION_TYPE_MISSING",
    )
    ...
    runtime_permission_scope = declared.get("runtime_permission_scope")
    _add_check(
        checks,
        "runtime_permission_scope_declared",
        runtime_permission_scope is not None
        and declared.get("runtime_permission_scope_missing") is not True,
    ...
    _add_check(
        checks,
        "runtime_permission_scope_exact",
        runtime_permission_scope == RUNTIME_PERMISSION_SCOPE
        and declared.get("runtime_permission_scope_not_selected_runtime_permission_only")
        is not True,
        RUNTIME_PERMISSION_SCOPE,
        runtime_permission_scope,
        "RUNTIME_PERMISSION_SCOPE_NOT_SELECTED_RUNTIME_PERMISSION_ONLY",
    ...
    declared_runtime_permission_recorded = declared.get(
        "local_relevance_medium_read_only_runtime_permission_recorded",
        True,
    )
    _add_check(
        checks,
        "local_relevance_medium_read_only_runtime_permission_recorded",
        declared_runtime_permission_recorded is True
        and declared.get("local_relevance_medium_read_only_runtime_permission_not_recorded")
        is not True,
        True,
        declared_runtime_permission_recorded,
        "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_NOT_RECORDED",
    )
    declared_runtime_permission_created = declared.get("runtime_permission_created", True)
    _add_check(
        checks,
        "runtime_permission_created",
        declared_runtime_permission_created is True
        and declared.get("runtime_permission_not_created") is not True,
        True,
        declared_runtime_permission_created,
        "RUNTIME_PERMISSION_NOT_CREATED",
    )
    declared_runtime_permission_local_only = declared.get(
        "runtime_permission_local_only",
        True,
    )
    _add_check(
        checks,
        "runtime_permission_local_only",
        declared_runtime_permission_local_only is True
        and declared.get("runtime_permission_local_only_not_true") is not True,
        True,
        declared_runtime_permission_local_only,
        "RUNTIME_PERMISSION_LOCAL_ONLY_NOT_TRUE",
    )
    declared_runtime_permission_read_only = declared.get(
        "runtime_permission_read_only",
        True,
    )
    _add_check(
        checks,
        "runtime_permission_read_only",
        declared_runtime_permission_read_only is True
        and declared.get("runtime_permission_read_only_not_true") is not True,
        True,
        declared_runtime_permission_read_only,
        "RUNTIME_PERMISSION_READ_ONLY_NOT_TRUE",
    )

    _add_check(
        checks,
        "runtime_permission_boundary_future_runtime_permission_consideration",
        runtime_permission_boundary_basis.get("future_runtime_permission_may_be_considered")
        is True,
        True,
        runtime_permission_boundary_basis.get(
    ...
    _add_check(
        checks,
        "runtime_permission_boundary_did_not_create_runtime_permission",
        runtime_permission_boundary_basis.get("runtime_permission_created") is False,
        False,
        runtime_permission_boundary_basis.get("runtime_permission_created"),
        "RUNTIME_PERMISSION_BOUNDARY_ARTIFACT_NOT_RECORDED",
    )
    ...
    if _failed_checks(checks):
        ...
        declared.get("requested_local_relevance_medium_read_only_runtime_permission_outcome")
        == OUTCOME_REQUIRES_ADDITIONAL_BASIS
    ):
...
def resolve_local_relevance_medium_read_only_runtime_permission_v0_min_from_path(
    declared_local_relevance_medium_read_only_runtime_permission_path: Path | str,
) -> dict:
    """Resolve a declared runtime-permission request from one JSON object path."""

    try:
        with Path(declared_local_relevance_medium_read_only_runtime_permission_path).open(
            "r",
            encoding="utf-8",
        ) as handle:
            data = json.load(handle)
    ...
    return resolve_local_relevance_medium_read_only_runtime_permission_v0_min(data)


def build_local_relevance_medium_read_only_runtime_permission_v0_min_summary(
    result: Mapping[str, Any],
) -> dict:
    """Build the bounded runtime-permission summary view."""

    checks = list(
        result.get("local_relevance_medium_read_only_runtime_permission_checks", [])
        if isinstance(
            result.get("local_relevance_medium_read_only_runtime_permission_checks"),
            list,
        )
    ...
    runtime_permission = _mapping(
        result.get("local_relevance_medium_read_only_runtime_permission")
    )
    statement = _mapping(
        result.get("local_relevance_medium_read_only_runtime_permission_statement")
    )
    declared_question = _mapping(
        result.get("declared_local_relevance_medium_read_only_runtime_permission_question")
    )
    ...
    return {
        ...
        "runtime_permission_id": runtime_permission.get("runtime_permission_id"),
        ...
        "result_version": RESULT_VERSION,
        "resolver_module": RESOLVER_MODULE,
        "local_relevance_medium_read_only_runtime_permission_recorded": statement.get(
            "local_relevance_medium_read_only_runtime_permission_recorded"
        )
        is True,
        "basis_runtime_permission_boundary_artifact_preserved": statement.get(
            "basis_runtime_permission_boundary_artifact_preserved"
        )
        is True,
        ...
        is True,
        "selected_command": runtime_permission.get("selected_command"),
        ...
        is True,
        "runtime_permission_object_summary": {
            "runtime_permission_type": runtime_permission.get("runtime_permission_type"),
            "runtime_permission_scope": runtime_permission.get("runtime_permission_scope"),
            "runtime_permission_version": runtime_permission.get(
                "runtime_permission_version"
            ),
            "selected_command": runtime_permission.get("selected_command"),
            "operation_execution_created": runtime_permission.get(
                "operation_execution_created"
            ),
            "operation_execution_performed": runtime_permission.get(
                "operation_execution_performed"
        ...
        "runtime_not_created": runtime_permission.get("runtime_created") is False,
        "runtime_hosting_not_created": runtime_permission.get("runtime_hosting_created")
        is False,
        "runtime_loop_not_created": runtime_permission.get("runtime_loop_created")
        is False,
        "daemon_behavior_not_created": runtime_permission.get("daemon_behavior_created")
        is False,
        "continuation_not_created": runtime_permission.get("continuation_created")
        is False,
        "runtime_held_state_not_created": runtime_permission.get(
            "runtime_held_state_created"
        )
        is False,
        "runtime_held_reentry_not_created": runtime_permission.get(
            "runtime_held_reentry_created"
        )
        is False,
        "second_operation_not_created": runtime_permission.get("second_operation_created")
        is False,
        "metabolic_loop_not_created": runtime_permission.get("metabolic_loop_created")
        is False,
        "public_api_not_created": runtime_permission.get("public_api_created") is False,
        "participant_facing_interface_not_created": runtime_permission.get(
            "participant_facing_interface_created"
        )
        is False,
        "distributed_network_behavior_not_created": runtime_permission.get(
            "distributed_network_behavior_created"
        )
        is False,
        "general_operation_permission_not_created": runtime_permission.get(
            "general_operation_permission_created"
        )
        is False,
        "general_lookup_permission_not_created": runtime_permission.get(
            "general_lookup_permission_created"
        )
        is False,
        "arbitrary_lookup_permission_not_created": runtime_permission.get(
            "arbitrary_lookup_permission_created"
        )
        is False,
        "unsupported_commands_not_permitted": runtime_permission.get(
            "unsupported_commands_permitted"
        )
        is False,
        "unsupported_lookup_keys_not_permitted": runtime_permission.get(
            "unsupported_lookup_keys_permitted"
        )
        is False,
        "no_new_lookup_entry_created_beyond_bounded_lookup_result_object": (
            runtime_permission.get("new_lookup_entry_created") is False
        ),
        "no_new_signal_entry_relevance_object_or_index_entry_created": all(
            runtime_permission.get(field) is False
            for field in (
                "new_signal_accepted",
                "new_entry_accepted",
                "new_relevance_object_created",
        ...
        "filesystem_discovery_not_performed": runtime_permission.get(
            "filesystem_discovery_performed"
        )
        is False,
        ...
        "older_runtime_lineage_not_imported_as_authority": runtime_permission.get(
            "older_runtime_lineage_imported_as_authority"
        )
        is False,
        "older_runtime_permission_not_treated_as_current": runtime_permission.get(
            "older_runtime_permission_treated_as_current"
        )
        is False,
        "runtime_authority_not_imported": runtime_permission.get(
            "runtime_authority_imported"
        )
        is False,
        ...
        is True,
        "follow_on_not_created": runtime_permission.get("follow_on_work_authorized")
        is False,
        "key_non_claims": {
...
def _safe_filename_stem(value: Any) -> str:
    text = str(value or DEFAULT_RUNTIME_PERMISSION_ID)
    safe = "".join(ch if ch.isalnum() or ch in ("-", "_") else "_" for ch in text)
    return safe.strip("_") or DEFAULT_RUNTIME_PERMISSION_ID


def write_local_relevance_medium_read_only_runtime_permission_v0_min_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write a bounded runtime-permission result without overwriting existing JSON."""

    runtime_permission = _mapping(
        result.get("local_relevance_medium_read_only_runtime_permission")
    )
    runtime_permission_id = runtime_permission.get(
        "runtime_permission_id",
        DEFAULT_RUNTIME_PERMISSION_ID,
    )
    if output_path is None:
        output_path = OUTPUT_ROOT / (
            f"{_safe_filename_stem(runtime_permission_id)}__"
...
def build_declared_local_relevance_medium_read_only_runtime_permission_v0_min_request(
    *,
    local_relevance_medium_read_only_runtime_permission_id: str = DEFAULT_RUNTIME_PERMISSION_ID,
    selected_runtime_permission_boundary_artifact: Path | str = (
    ...
    runtime_permission_type: str = RUNTIME_PERMISSION_TYPE,
    runtime_permission_scope: str = RUNTIME_PERMISSION_SCOPE,
    runtime_permission_intent: str = INTENT_RECORD,
    declared_non_claims: Mapping[str, bool] | None = None,
    **extra_fields: Any,
) -> dict[str, Any]:
    """Build a bounded declared request for the selected-state runtime permission."""

    if selected_command != SELECTED_COMMAND:
        raise LocalRelevanceMediumReadOnlyRuntimePermissionV0MinError(
    ...
    request = {
        ...
        "local_relevance_medium_read_only_runtime_permission_question": CORE_QUESTION,
        "local_relevance_medium_read_only_runtime_permission_intent": (
            runtime_permission_intent
        ),
        "selected_runtime_permission_boundary_artifact": str(
