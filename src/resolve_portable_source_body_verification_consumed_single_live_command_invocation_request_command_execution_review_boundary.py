"""Resolve consumed-request command execution review boundary posture.

This resolver answers one question only:

    Can the consumed single live command invocation request basis be bounded for
    one future command execution review without invoking or executing the
    command and without creating command output, command result, command
    success, execution permission, execution approval, standing invocation lane,
    repeat permission, authority, currentness, final completion, continuation,
    reusable permission, derivative reception, vessel relation, another
    reception request, or follow-on work?

The resolver records execution-review-boundary conditions only. It does not
invoke or execute a command, create command output/result/success, create
execution permission or execution approval, reopen the consumed request token,
create a standing invocation lane or repeat permission, repair or hide the v1
predecessor failure line, mutate artifacts, deploy, host, publish, create
authority/currentness, or authorize follow-on work.
"""
...
class PortableSourceBodyVerificationConsumedSingleLiveCommandInvocationRequestCommandExecutionReviewBoundaryError(
    Exception
):
    """Raised for hard execution-review-boundary input/output failures."""


RESOLVER_MODULE = (
    "resolve_portable_source_body_verification_consumed_single_live_command_"
    "invocation_request_command_execution_review_boundary"
)
...
CORE_EXECUTION_REVIEW_BOUNDARY_QUESTION = (
    "Can the consumed single live command invocation request basis be bounded "
    "for one future command execution review without invoking or executing the "
    "command and without creating command output, command result, command "
    "success, execution permission, execution approval, standing invocation lane, "
    "repeat permission, authority, currentness, final completion, continuation, "
    "reusable permission, derivative reception, vessel relation, another "
...
INTENT_RECORD = (
    "RECORD_CONSUMED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_COMMAND_EXECUTION_"
    "REVIEW_BOUNDARY"
)
INTENT_DO_NOT_RECORD = (
    "DO_NOT_RECORD_CONSUMED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_COMMAND_"
    "EXECUTION_REVIEW_BOUNDARY"
)
INTENT_BLOCK = (
    "BLOCK_CONSUMED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_COMMAND_EXECUTION_"
    "REVIEW_BOUNDARY"
)
SUPPORTED_EXECUTION_REVIEW_BOUNDARY_INTENTS = {
    INTENT_RECORD,
    INTENT_DO_NOT_RECORD,
    INTENT_BLOCK,
}
...
SUPPORTED_EXECUTION_REVIEW_BOUNDARY_SCOPE = {
    ...
    "EXECUTION_REVIEW_IS_NOT_EXECUTION",
    "CONSUMED_REQUEST_BASIS_IS_NOT_EXECUTION_PERMISSION",
    "CONSUMED_REQUEST_BASIS_IS_NOT_COMMAND_SUCCESS",
...
FULL_BODY_OMISSION_MARKER = (
    "[OMITTED: full prior artifact body blocked by consumed single live command "
    "invocation request command execution review boundary]"
)
...
COLLAPSE_FIELD_CODES = (
    ...
    ("consumed_request_reopen", "CONSUMED_REQUEST_REOPENED"),
...
BLOCK_REASONS = {
    "DECLARED_EXECUTION_REVIEW_BOUNDARY_REQUEST_MALFORMED": (
        "declared execution-review-boundary request is malformed"
    ),
    "DECLARED_EXECUTION_REVIEW_BOUNDARY_REQUEST_UNREADABLE": (
        "declared execution-review-boundary request path is unreadable"
    ),
    "EXECUTION_REVIEW_BOUNDARY_QUESTION_UNDECLARED": (
        "execution-review boundary question is undeclared"
    ),
    "EXECUTION_REVIEW_BOUNDARY_INTENT_UNSUPPORTED": (
        "execution-review boundary intent is unsupported"
    ),
    "EXECUTION_REVIEW_BOUNDARY_EXPLICITLY_BLOCKED": (
        "execution-review boundary was explicitly blocked"
    ),
    "REQUEST_CONSUMPTION_BASIS_MISSING": (
        "selected request-consumption live artifact basis is missing"
    ),
    "REQUEST_CONSUMPTION_NOT_CONSUMED": (
        "selected request-consumption basis is not consumed outcome"
    ),
    "REQUEST_CONSUMPTION_FAILED_CHECKS_PRESENT": (
        "selected request-consumption basis has failed checks"
    ),
    "REQUEST_CONSUMPTION_TERMINAL_SUMMARY_MISSING": (
        "selected request-consumption terminal summary basis is missing"
    ),
    "CONSUMED_REQUEST_BASIS_MISSING": "consumed request basis is missing",
    "CONSUMED_TOKEN_NOT_CLOSED": "consumed token is not closed",
    "CONSUMED_REQUEST_REOPENED": "consumed request token would be reopened",
    "V2_ADMITTED_REQUEST_BASIS_MISSING": "selected v2 admitted request basis is missing",
    "V2_ADMITTED_REQUEST_NOT_ADMITTED": "selected v2 request basis is not admitted",
    "V2_ADMITTED_REQUEST_VERSION_NOT_0_2_0": (
        "selected v2 admitted request basis is not version 0.2.0"
    ),
    "V2_ADMITTED_REQUEST_FAILED_CHECKS_PRESENT": (
        "selected v2 admitted request basis has failed checks"
    ),
    "V2_SUCCESSOR_METADATA_MISSING": "selected v2 successor metadata is missing",
    "V2_RETURNED_RESULT_CONTAINMENT_MISSING": (
        "selected v2 returned-result containment is missing"
    ),
    "V1_PREDECESSOR_FAILURE_BASIS_MISSING": (
        "selected v1 predecessor failure basis is missing"
    ),
    "V2_TREATED_AS_REPAIRING_V1": "v2 was treated as repairing v1",
    "V1_FAILURE_HIDDEN": "v1 predecessor failure was hidden",
    "V1_CLAIMED_PASSED": "v1 predecessor failure was claimed passed",
    "COMMAND_EXECUTION_BOUNDARY_BASIS_MISSING": (
        "command execution boundary basis is missing"
    ),
    "COMMAND_REPORT_BASIS_MISSING": "command report basis is missing",
    "COMMAND_IMPLEMENTATION_BOUNDARY_BASIS_MISSING": (
        "command implementation boundary basis is missing"
    ),
    "COMMAND_BOUNDARY_BASIS_MISSING": "command boundary basis is missing",
    "ARTIFACT_EMISSION_CONTAINMENT_BASIS_MISSING": (
        "artifact emission containment basis is missing"
    ),
    "EVIDENCE_MANIFEST_BASIS_MISSING": "evidence-manifest basis is missing",
    "PORTABLE_VERIFICATION_BASIS_MISSING": "portable verification basis is missing",
    "ONE_FUTURE_EXECUTION_REVIEW_POSTURE_MISSING": (
        "one future execution review posture is missing"
    ),
    "NO_STANDING_LANE_POSTURE_MISSING": "no-standing-lane posture is missing",
    "NO_REPEAT_PERMISSION_POSTURE_MISSING": "no-repeat-permission posture is missing",
    "NO_EXECUTION_PERMISSION_POSTURE_MISSING": (
        "no-execution-permission posture is missing"
    ),
    "NO_EXECUTION_APPROVAL_POSTURE_MISSING": "no-execution-approval posture is missing",
    "UNSUPPORTED_EXECUTION_REVIEW_BOUNDARY_SCOPE": (
        "execution-review boundary scope is missing or unsupported"
    ),
    "COMMAND_INVOCATION_CREATED": "command invocation was created",
    "COMMAND_EXECUTION_PERFORMED": "command execution was performed",
    "COMMAND_OUTPUT_CREATED": "command output was created",
    "COMMAND_RESULT_CREATED": "command result was created",
    "COMMAND_SUCCESS_CREATED": "command success was created",
    "EXECUTION_PERMISSION_CREATED": "execution permission was created",
    "EXECUTION_APPROVAL_CREATED": "execution approval was created",
    "STANDING_INVOCATION_LANE_CREATED": "standing invocation lane was created",
    "REPEAT_INVOCATION_PERMISSION_CREATED": "repeat invocation permission was created",
    "EXECUTION_REVIEW_TREATED_AS_EXECUTION": "execution review was treated as execution",
    "CONSUMED_REQUEST_BASIS_TREATED_AS_EXECUTION_PERMISSION": (
        "consumed request basis was treated as execution permission"
    ),
    "CONSUMED_REQUEST_BASIS_TREATED_AS_COMMAND_SUCCESS": (
        "consumed request basis was treated as command success"
    ),
    "COMMAND_OUTPUT_TREATED_AS_SOURCE": "command output was treated as source",
    "COMMAND_RESULT_TREATED_AS_AUTHORITY": "command result was treated as authority",
    "COMMAND_SUCCESS_TREATED_AS_CURRENTNESS": (
        "command success was treated as currentness"
    ),
    "COMMAND_SUCCESS_TREATED_AS_FINAL_COMPLETION": (
        "command success was treated as final completion"
    ),
    "FULL_PRIOR_ARTIFACT_BODY_EMITTED": "full prior artifact body was emitted",
    "ARTIFACTS_MUTATED": "artifacts were mutated",
    "DEPLOYMENT_CREATED": "deployment was created",
    "RUNTIME_HOSTING_CREATED": "runtime hosting was created",
    "PUBLIC_RELEASE_CREATED": "public release was created",
    "OPERATION_PERMISSION_CREATED": "operation permission was created",
    "PUBLIC_READINESS_CREATED": "public readiness was created",
    "FINAL_COMPLETION_CLAIMED": "final completion was claimed",
    "CONTINUATION_AUTHORIZED": "continuation was authorized",
    "REUSABLE_PERMISSION_CREATED": "reusable permission was created",
    "DERIVATIVE_RECEPTION_AUTHORIZED": "derivative reception was authorized",
    "VESSEL_RELATION_AUTHORIZED": "vessel relation was authorized",
    "ANOTHER_RECEPTION_REQUEST_AUTHORIZED": (
        "another reception request was authorized"
    ),
    "FOLLOW_ON_WORK_AUTHORIZED": "follow-on work was authorized",
    "MUTATION_REPLAY_OR_MERGE_DETECTED": "mutation, replay, or merge was detected",
    "NON_CLAIM_MISSING_OR_FLIPPED": "required non-claims are missing or flipped",
}
...
def _scope_values(value: Any) -> list[str]:
    if isinstance(value, Mapping):
        for key in (
            ...
            "command_execution_review_boundary_scope",
        ):
...
def _request_consumption_id(request: Mapping[str, Any]) -> Any:
    basis = _request_consumption_basis(request)
    return (
...
def _request_consumption_path(request: Mapping[str, Any]) -> Any:
    basis = _request_consumption_basis(request)
    return (
...
def _consumed_token_closed(request: Mapping[str, Any]) -> bool:
    ...
    request_consumption_basis = _request_consumption_basis(request)
    posture = request.get("consumed_token_closed_posture")
    return (
        _find_truthy(
            basis,
            (
...
def _v2_successor_metadata_preserved(request: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and resolver_module in {None, V2_RESOLVER_MODULE}
    )
    ...
    section.update(
        {
            "selected_request_consumption_result_id": _sanitize(
                _request_consumption_id(request)
            ),
            "selected_request_consumption_result_path": _sanitize(
                _request_consumption_path(request)
            ),
...
def _selected_consumed_request_section(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    reopened = _contains_truthy_key(request, "consumed_request_reopened") or _contains_truthy_key(
    ...
    section.update(
        {
            ...
            "consumed_request_basis_for_one_future_execution_review_only": True,
        }
    )
    return section
...
def _posture_section(value: Any, label: str) -> dict[str, Any]:
    ...
    return {
        ...
        "execution_review_boundary_only": True,
...
def _scope_section(value: Any) -> dict[str, Any]:
    ...
    unsupported = [item for item in values if item not in SUPPORTED_EXECUTION_REVIEW_BOUNDARY_SCOPE]
    ...
    return {
        ...
        "supported_scope_values": sorted(SUPPORTED_EXECUTION_REVIEW_BOUNDARY_SCOPE),
        "command_execution_review_boundary_only": "COMMAND_EXECUTION_REVIEW_BOUNDARY_ONLY"
        in value_set,
        "consumed_request_basis_preserved": "CONSUMED_REQUEST_BASIS_PRESERVED"
        in value_set,
        "consumed_request_token_remains_closed": "CONSUMED_REQUEST_TOKEN_REMAINS_CLOSED"
        in value_set,
        "consumed_request_not_reopened": "CONSUMED_REQUEST_NOT_REOPENED" in value_set,
        "one_future_execution_review_only": "ONE_FUTURE_EXECUTION_REVIEW_ONLY"
        in value_set,
        ...
        "no_standing_invocation_lane_created": "NO_STANDING_INVOCATION_LANE_CREATED"
        in value_set,
        "no_repeat_invocation_permission_created": "NO_REPEAT_INVOCATION_PERMISSION_CREATED"
        in value_set,
        "execution_review_is_not_execution": "EXECUTION_REVIEW_IS_NOT_EXECUTION"
        in value_set,
        "consumed_request_basis_is_not_execution_permission": (
            "CONSUMED_REQUEST_BASIS_IS_NOT_EXECUTION_PERMISSION" in value_set
        ),
        "consumed_request_basis_is_not_command_success": (
            "CONSUMED_REQUEST_BASIS_IS_NOT_COMMAND_SUCCESS" in value_set
        ),
        "v1_predecessor_failure_remains_visible": (
            "V1_PREDECESSOR_FAILURE_REMAINS_VISIBLE" in value_set
        ),
        "v2_successor_does_not_repair_v1": "V2_SUCCESSOR_DOES_NOT_REPAIR_V1"
        in value_set,
        "returned_result_containment_preserved": "RETURNED_RESULT_CONTAINMENT_PRESERVED"
        in value_set,
        "reference_shaped_basis_required": "REFERENCE_SHAPED_BASIS_REQUIRED" in value_set,
        "full_prior_artifact_body_not_emitted": "FULL_PRIOR_ARTIFACT_BODY_NOT_EMITTED"
        in value_set,
...
def _build_checks(
    ...
    if malformed_code:
        return [
            _check(
                "declared execution-review-boundary request is a mapping",
                False,
                "mapping",
                malformed_code,
                malformed_code,
    ...
    question = request.get("execution_review_boundary_question")
    checks.append(
        _check(
            "execution-review boundary question declared",
            _present(question),
            "declared execution-review boundary question",
            question,
            "EXECUTION_REVIEW_BOUNDARY_QUESTION_UNDECLARED",
        )
    )
    intent = request.get("execution_review_boundary_intent")
    checks.append(
        _check(
            "execution-review boundary intent supported",
            intent in SUPPORTED_EXECUTION_REVIEW_BOUNDARY_INTENTS,
            sorted(SUPPORTED_EXECUTION_REVIEW_BOUNDARY_INTENTS),
            intent,
            "EXECUTION_REVIEW_BOUNDARY_INTENT_UNSUPPORTED",
        )
    )
        ...
        [
            ...
            _check(
                ...
                "consumed request token closed",
                {
                    ...
                    "selected_consumed_request_basis": request.get(
                        "selected_consumed_request_basis"
                    ),
                },
                "CONSUMED_TOKEN_NOT_CLOSED",
            ...
            _check(
                ...
                _consumed_token_closed(request)
                and not _contains_truthy_key(request, "consumed_request_reopened")
                and not _contains_truthy_key(request, "consumed_token_reopened"),
                "closed and not reopened",
                {
                    "consumed_token_closed": _consumed_token_closed(request),
                    "consumed_request_reopened": _contains_truthy_key(
                        request, "consumed_request_reopened"
                    ),
                    "consumed_token_reopened": _contains_truthy_key(
                        request, "consumed_token_reopened"
                    ),
                },
                "CONSUMED_TOKEN_NOT_CLOSED",
            ...
            _check(
                ...
                not _contains_truthy_key(request, "consumed_request_reopened")
                and not _contains_truthy_key(request, "consumed_token_reopened"),
                False,
                _first_truthy_key(
                    request,
                    ("consumed_request_reopened", "consumed_token_reopened"),
                ),
                "CONSUMED_REQUEST_REOPENED",
            ),
            _check(
    ...
    scope = _scope_section(request.get("execution_review_boundary_scope"))
    checks.append(
        _check(
            "execution-review-boundary scope supported",
            scope["all_selected_scope_values_supported"],
            sorted(SUPPORTED_EXECUTION_REVIEW_BOUNDARY_SCOPE),
    ...
    checks.append(
        _check(
            ...
            {
                "reference_shaped_input_posture": request.get(
                    "reference_shaped_input_posture"
                ),
                "scope_reference_shaped_basis_required": scope[
                    "reference_shaped_basis_required"
                ],
    ...
    checks.extend(
        [
            ...
            _check(
                ...
                {
                    "scope_command_output_not_created": scope[
                        "command_output_not_created"
                    ],
                    "scope_command_result_not_created": scope[
                        "command_result_not_created"
                    ],
                    "scope_command_success_not_created": scope[
                        "command_success_not_created"
                    ],
                },
                "COMMAND_OUTPUT_CREATED",
...
def _determine_block_code(
    ...
    if request.get("execution_review_boundary_intent") == INTENT_BLOCK:
        return "EXECUTION_REVIEW_BOUNDARY_EXPLICITLY_BLOCKED"
    if request.get("requested_execution_review_boundary_outcome") == OUTCOME_BLOCKED:
        return "EXECUTION_REVIEW_BOUNDARY_EXPLICITLY_BLOCKED"
    requested = request.get("requested_execution_review_boundary_outcome")
    if _present(requested) and requested not in OUTCOME_FAMILY:
        return "DECLARED_EXECUTION_REVIEW_BOUNDARY_REQUEST_MALFORMED"
...
def _determine_outcome(request: Mapping[str, Any], block_code: str | None) -> str:
    ...
    if request.get("execution_review_boundary_intent") == INTENT_DO_NOT_RECORD:
...
def _statement(outcome: str) -> dict[str, Any]:
    ...
    statement.update(
        {
            "command_execution_review_boundary_only": True,
            "consumed_request_not_reopened": True,
            "one_future_execution_review_only": True,
...
def _execution_review_boundary_non_meaning() -> dict[str, bool]:
    return {
        "execution_review_boundary_does_not_mean_command_invocation_created": True,
        "execution_review_boundary_does_not_mean_command_executed": True,
        "execution_review_boundary_does_not_mean_command_output_exists": True,
        "execution_review_boundary_does_not_mean_command_result_exists": True,
        "execution_review_boundary_does_not_mean_command_success_exists": True,
        "execution_review_boundary_does_not_mean_execution_permission_exists": True,
        "execution_review_boundary_does_not_mean_execution_approval_exists": True,
        "execution_review_boundary_does_not_mean_command_success_creates_currentness": True,
        "execution_review_boundary_does_not_mean_command_success_claims_final_completion": True,
        "execution_review_boundary_does_not_mean_command_output_becomes_source": True,
        "execution_review_boundary_does_not_mean_command_result_becomes_authority": True,
        "execution_review_boundary_does_not_mean_standing_invocation_lane_exists": True,
        "execution_review_boundary_does_not_mean_repeat_invocation_permission_exists": True,
        "execution_review_boundary_does_not_mean_consumed_request_token_reopened": True,
        "execution_review_boundary_does_not_mean_v1_was_repaired": True,
        "execution_review_boundary_does_not_mean_v1_was_hidden": True,
        "execution_review_boundary_does_not_mean_v1_passed": True,
        "execution_review_boundary_does_not_mean_deployment_created": True,
        "execution_review_boundary_does_not_mean_runtime_hosting_created": True,
        "execution_review_boundary_does_not_mean_public_release_created": True,
        "execution_review_boundary_does_not_mean_public_readiness_created": True,
        "execution_review_boundary_does_not_mean_final_completion_claimed": True,
        "execution_review_boundary_does_not_mean_continuation_authorized": True,
        "execution_review_boundary_does_not_mean_reusable_permission_created": True,
        "execution_review_boundary_does_not_mean_derivative_reception_authorized": True,
        "execution_review_boundary_does_not_mean_vessel_relation_authorized": True,
        "execution_review_boundary_does_not_mean_another_reception_request_authorized": True,
        "execution_review_boundary_does_not_mean_follow_on_work_authorized": True,
    }
...
def _not_recorded_basis(
    ...
    return {
        ...
        "failed_execution_review_boundary_checks": failed_checks if not_recorded else [],
...
def _metadata(request: Mapping[str, Any]) -> dict[str, Any]:
    request_id = request.get("execution_review_boundary_request_id") or "unidentified"
    return {
        "consumed_single_live_command_invocation_request_command_execution_review_boundary_result_id": (
            ...
            "command_execution_review_boundary_result"
        ),
        "consumed_single_live_command_invocation_request_command_execution_review_boundary_result_type": (
            ...
            "request_command_execution_review_boundary_result"
        ),
        "consumed_single_live_command_invocation_request_command_execution_review_boundary_result_version": RESULT_VERSION,
...
def _declared_question(
    ...
    question = request.get("execution_review_boundary_question")
    return {
        "execution_review_boundary_request_id": request.get(
            "execution_review_boundary_request_id"
        ),
        "execution_review_boundary_question": question,
        "canonical_execution_review_boundary_question": CORE_EXECUTION_REVIEW_BOUNDARY_QUESTION,
        "execution_review_boundary_question_declared": _present(question),
        "execution_review_boundary_intent": request.get("execution_review_boundary_intent"),
        "execution_review_boundary_request_path": str(request_path)
        if request_path is not None
        else None,
        "request_is_execution_review_boundary_only": True,
...
def _read_json_object(path_value: Path | str) -> tuple[dict[str, Any] | None, str | None, str | None]:
    ...
    try:
        ...
        return None, "DECLARED_EXECUTION_REVIEW_BOUNDARY_REQUEST_UNREADABLE", str(exc)
    try:
        value = json.loads(text)
    except json.JSONDecodeError as exc:
        return None, "DECLARED_EXECUTION_REVIEW_BOUNDARY_REQUEST_MALFORMED", str(exc)
...
def _build_result(
    ...
    *,
    ...
    if not isinstance(request_value, Mapping) and malformed_code is None:
        malformed_code = "DECLARED_EXECUTION_REVIEW_BOUNDARY_REQUEST_MALFORMED"
    ...
    result: dict[str, Any] = {
        "consumed_single_live_command_invocation_request_command_execution_review_boundary_metadata": _metadata(
            request
        ),
        "declared_execution_review_boundary_question": _declared_question(
        ...
        "selected_v1_predecessor_failure_basis": _selected_v1_predecessor_section(
            request
        ),
        ...
        "one_future_execution_review_posture": _posture_section(
            request.get("one_future_execution_review_posture"),
            "one_future_execution_review_posture",
        ),
        ...
        "execution_review_separation_posture": _posture_section(
            request.get("execution_review_separation_posture"),
            "execution_review_separation_posture",
        ),
        ...
        "execution_review_boundary_scope": _scope_section(
            request.get("execution_review_boundary_scope")
        ),
        "execution_review_boundary_checks": _checks_summary(checks),
        "execution_review_boundary_statement": statement,
        "execution_review_boundary_non_meaning": _execution_review_boundary_non_meaning(),
...
def resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_execution_review_boundary(
    declared_execution_review_boundary_request: Mapping[str, Any] | None = None,
) -> dict:
    """Resolve one declared consumed-request execution-review-boundary mapping."""

    if declared_execution_review_boundary_request is None:
        return _build_result({})
    if not isinstance(declared_execution_review_boundary_request, Mapping):
        return _build_result(
            {},
            malformed_code="DECLARED_EXECUTION_REVIEW_BOUNDARY_REQUEST_MALFORMED",
        )
    return _build_result(declared_execution_review_boundary_request)


def resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_execution_review_boundary_from_path(
    declared_execution_review_boundary_request_path: Path | str,
) -> dict:
    """Resolve one declared consumed-request execution-review-boundary JSON object."""

    request, error_code, error_reason = _read_json_object(
        declared_execution_review_boundary_request_path
    )
    if error_code:
        return _build_result(
            {},
            request_path=declared_execution_review_boundary_request_path,
...
def build_portable_source_body_verification_consumed_single_live_command_invocation_request_command_execution_review_boundary_summary(
    result: Mapping[str, Any]
) -> dict:
    """Build a compact non-authoritative execution-review-boundary summary."""

    safe_result = _sanitize(result)
    statement = _as_mapping(safe_result.get("execution_review_boundary_statement"))
    checks = _as_mapping(safe_result.get("execution_review_boundary_checks"))
    block = _as_mapping(safe_result.get("block"))
    question = _as_mapping(safe_result.get("declared_execution_review_boundary_question"))
    ...
    return {
        ...
        "execution_review_boundary_request_id": question.get(
            "execution_review_boundary_request_id"
        ),
        "execution_review_boundary_question": question.get(
            "execution_review_boundary_question"
        ),
        "execution_review_boundary_intent": question.get(
            "execution_review_boundary_intent"
        ),
        "passed_check_count": checks.get("passed_check_count", 0),
        "failed_check_count": checks.get("failed_check_count", 0),
        "execution_review_boundary_recorded": statement.get(
            "consumed_request_execution_review_boundary_recorded", False
        ),
        "consumed_request_basis_preserved": statement.get(
            "consumed_request_basis_preserved", False
        ),
        "consumed_request_token_remains_closed": statement.get(
            "consumed_request_token_remains_closed", False
        ),
        "single_execution_review_conditions_declared": statement.get(
            "single_execution_review_conditions_declared", False
        ),
        "execution_review_requires_separate_result": statement.get(
            "execution_review_requires_separate_result", False
        ),
        ...
        "consumed_request_not_reopened": statement.get(
            "consumed_request_not_reopened", True
        ),
        "execution_review_not_execution": statement.get(
            "execution_review_is_not_execution", True
        ),
        "consumed_request_basis_not_execution_permission": statement.get(
            "consumed_request_basis_is_not_execution_permission", True
        ),
        "consumed_request_basis_not_command_success": statement.get(
            "consumed_request_basis_is_not_command_success", True
        ),
...
def _unique_path(path: Path) -> Path:
    ...
    for index in range(1, 10000):
        ...
        if not candidate.exists():
            return candidate
    raise PortableSourceBodyVerificationConsumedSingleLiveCommandInvocationRequestCommandExecutionReviewBoundaryError(
        f"Could not allocate non-overwriting output path for {path}"
    )


def write_portable_source_body_verification_consumed_single_live_command_invocation_request_command_execution_review_boundary_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write one additive bounded execution-review-boundary result JSON file."""

    safe_result = _sanitize(result)
    question = _as_mapping(safe_result.get("declared_execution_review_boundary_question"))
    request_id = _safe_component(
        question.get("execution_review_boundary_request_id"), "unidentified"
    )
    filename = (
        f"{request_id}__consumed_single_live_command_invocation_request_"
        "command_execution_review_boundary_result.json"
    )
...
def build_declared_portable_source_body_verification_consumed_single_live_command_invocation_request_command_execution_review_boundary_request(
    execution_review_boundary_request_id: str,
    execution_review_boundary_question: str,
    ...
    one_future_execution_review_posture: Mapping[str, Any] | str,
    ...
    execution_review_separation_posture: Mapping[str, Any] | str,
    ...
    execution_review_boundary_scope: Sequence[str] | Mapping[str, Any],
    execution_review_boundary_intent: str = INTENT_RECORD,
    *,
    ...
    requested_execution_review_boundary_outcome: str = OUTCOME_RECORDED,
    additional_basis_context: Mapping[str, Any] | None = None,
    not_recorded_basis: Mapping[str, Any] | str | None = None,
) -> dict:
    """Build a declared execution-review-boundary request with false non-claims."""

    request: dict[str, Any] = {
        "execution_review_boundary_request_id": execution_review_boundary_request_id,
        "execution_review_boundary_question": execution_review_boundary_question,
        "execution_review_boundary_intent": execution_review_boundary_intent,
        ...
        "returned_result_containment_posture": _copy(
            returned_result_containment_posture
        ),
        "execution_review_boundary_scope": _copy(execution_review_boundary_scope),
        "requested_execution_review_boundary_outcome": requested_execution_review_boundary_outcome,
