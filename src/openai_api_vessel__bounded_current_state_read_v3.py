...
This module preserves the lawful scope of the earlier bounded vessels:
...
This successor exists only to harden the output-format boundary by using a
strict success-only structured output schema and handling refusal outside that
schema in shell-owned code.
"""
...
class OpenAIDerivativeVesselV3Error(RuntimeError):
    """Raised for bounded vessel v3 setup, source, packet, or output failures."""
...
SUCCESSOR_OF_MODULE = "openai_api_vessel__bounded_current_state_read_v2"
VESSEL_RESULT_TYPE = (
    "IAMMAI_OPENAI_API_DERIVATIVE_VESSEL_BOUNDED_CURRENT_STATE_READ_V3_RESULT"
)
VESSEL_RESULT_VERSION = "0.3.0"
DEFAULT_RESULT_STEM = "bounded_current_state_vessel_v3_result"

STRICT_SUCCESS_OUTPUT_SCHEMA_NAME = "bounded_current_state_read_v3_success"
STRICT_SUCCESS_OUTPUT_SCHEMA = {
    "type": "object",
    "properties": {
        "answer": {"type": "string"},
    },
...
def _require_mapping(value: Any, context: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise OpenAIDerivativeVesselV3Error(
...
def _discover_latest_answered_source() -> tuple[Path | None, dict[str, Any] | None]:
    ...
    for path in reversed(candidates):
        ...
        except OpenAIDerivativeVesselV3Error:
            continue
...
def _initial_api_runtime() -> dict[str, Any]:
    ...
    return {
        ...
        "strict_structured_output_path_used": False,
    }
...
def _client_supports_strict_structured_outputs(client: Any) -> bool:
    responses = getattr(client, "responses", None)
    create = getattr(responses, "create", None)
    if not callable(create):
        return False
...
def _strict_text_format_config() -> dict[str, Any]:
    return {
        "format": {
            "type": "json_schema",
            "name": STRICT_SUCCESS_OUTPUT_SCHEMA_NAME,
            "strict": True,
            "description": (
                "Return only the bounded current-state derivative read success object."
            ),
            "schema": dict(STRICT_SUCCESS_OUTPUT_SCHEMA),
        }
    }
...
def _extract_refusal_signal(response: Any) -> dict[str, Any] | None:
    output = getattr(response, "output", None)
    if not isinstance(output, list):
        return None
    for item in output:
        ...
        for part in content:
            if getattr(part, "type", None) != "refusal":
                continue
            refusal_text = _string_or_none(getattr(part, "refusal", None))
            return {
...
def _call_openai_derivative_read(
    ...
    if not _client_supports_strict_structured_outputs(client):
        raise OpenAIDerivativeVesselV3Error(
            "installed openai client does not expose strict structured output configuration",
            "LOCAL_SETUP_FAILURE",
        )
    ...
    try:
        response = client.responses.create(
            ...
            text=_strict_text_format_config(),
        )
    ...
    if error_detail is not None:
        raise OpenAIDerivativeVesselV3Error(
            f"OpenAI API returned an error response: {error_detail}",
            "API_CALL_FAILURE",
        )

    refusal_signal = _extract_refusal_signal(response)
    raw_output_text = _extract_output_text(response)
    return raw_output_text, refusal_signal, True
...
def _result_id(selected_source_id: str | None, outcome: str) -> str:
    ...
    suffix = (
        "bounded_current_state_read_v3_answered"
        if outcome == OUTCOME_ANSWERED_DERIVATIVE_READ
        else "bounded_current_state_read_v3_refused"
    )
...
def _model_output_section(
    ...
    refusal_signal: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    return {
        "raw_output_text": raw_output_text,
        "parsed_output": dict(parsed_output) if isinstance(parsed_output, Mapping) else None,
        "refusal_signal": (
            dict(refusal_signal) if isinstance(refusal_signal, Mapping) else None
        ),
    }
...
def _answered_result(
    *,
    ...
    result = {
        ...
        "model_output": _model_output_section(raw_output_text, parsed_output, None),
...
def resolve_bounded_current_state_read(
    ...
    try:
        ...
        raw_output_text, refusal_signal, strict_used = _call_openai_derivative_read(
            client,
            vessel_request,
            str(api_runtime["model_name_used"]),
        )
        api_runtime["strict_structured_output_path_used"] = strict_used
        ...
        if refusal_signal is not None:
            ...
            refusal_text = _string_or_none(refusal_signal.get("refusal_text"))
            return _refused_result(
                block_code=MODEL_REFUSAL_CODE,
                block_detail=refusal_text,
        ...
        if raw_output_text is None:
            raise OpenAIDerivativeVesselV3Error(
                "OpenAI API did not return strict structured output text",
                "MODEL_OUTPUT_MALFORMED",
            )
    ...
    except OpenAIDerivativeVesselV3Error as exc:
...
def build_bounded_current_state_vessel_summary(
    ...
    if not isinstance(result, Mapping):
        raise OpenAIDerivativeVesselV3Error("vessel result must be a mapping")
    return _summary_from_result(result)


def _safe_default_output_path(
    result: Mapping[str, Any],
    root: Path | str = OPENAI_API_DERIVATIVE_VESSEL_V3_ROOT,
) -> Path:
    ...
    for index in range(1, 1000):
        ...
        if not candidate.exists():
            return candidate
    raise OpenAIDerivativeVesselV3Error(
        "no bounded vessel v3 filename is available",
        "MALFORMED_VESSEL_PACKET",
    )
...
def write_bounded_current_state_vessel_result(
    ...
    """Write one additive bounded current-state vessel v3 result JSON artifact."""
    ...
    if target.exists():
        raise FileExistsError(f"bounded vessel v3 result already exists: {target}")
