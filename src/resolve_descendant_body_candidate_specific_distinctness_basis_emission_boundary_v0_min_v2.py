"""V2 resolver for the candidate-specific distinctness basis emission boundary.

V1 remains preserved lineage. This successor keeps the same boundary-only
semantics and broadens only the completed differentiation operation terminal
summary marker acceptance so equivalent non-standing candidate-record posture
in the live summary is accepted. It does not define or perform the future
emission operation, emit evidence, rerun distinctness, authorize standing,
repair lineage, scan the repository, or authorize follow-on work.
"""
...
class DescendantBodyCandidateSpecificDistinctnessBasisEmissionBoundaryV0MinV2Error(Exception):
    """Bounded resolver error for malformed local boundary requests."""


RESULT_VERSION = "0.2.0"
RESOLVER_MODULE = "resolve_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2"
...
OUTPUT_ROOT = (
    REPO_ROOT
    / "artifacts"
    / "integrity_host_v0_min_coexistence_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2"
)
...
def _validate_differentiation_marker_file(checks: list[dict[str, Any]], request: Mapping[str, Any]) -> bool:
    ...
    _add_check(
        ...
        "completed_differentiation_operation_terminal_summary_markers_present",
        passed,
        "completed differentiation summary preserves operation result, exactly two non-standing operation-contained candidate records, no descendant bodies, no crossing, and no relation",
        actual,
        "COMPLETED_DIFFERENTIATION_OPERATION_TERMINAL_SUMMARY_MARKER_MISSING",
    )
    return passed
...
def _validate_required_non_claims(checks: list[dict[str, Any]], request: Mapping[str, Any]) -> None:
    ...
    if not isinstance(declared, Mapping):
        ...
        return
    malformed = [key for key in REQUIRED_FALSE_NON_CLAIMS if key not in declared or declared.get(key) is not False]
    _add_check(
        checks,
        "required_non_claims_false",
        not malformed,
...
def _validate_request(checks: list[dict[str, Any]], request: Mapping[str, Any]) -> dict[str, bool]:
    ...
    _add_check(checks, "boundary_type_declared", bool(boundary_type), BOUNDARY_TYPE, boundary_type, "CANDIDATE_SPECIFIC_DISTINCTNESS_BASIS_EMISSION_BOUNDARY_TYPE_MISSING")
    _add_check(checks, "boundary_type_exact", boundary_type == BOUNDARY_TYPE, BOUNDARY_TYPE, boundary_type, "CANDIDATE_SPECIFIC_DISTINCTNESS_BASIS_EMISSION_BOUNDARY_TYPE_NOT_EXPECTED")

    boundary_version = request.get("candidate_specific_distinctness_basis_emission_boundary_version")
    _add_check(checks, "boundary_version_declared", bool(boundary_version), BOUNDARY_VERSION, boundary_version, "CANDIDATE_SPECIFIC_DISTINCTNESS_BASIS_EMISSION_BOUNDARY_VERSION_MISSING")
    _add_check(checks, "boundary_version_exact", boundary_version == BOUNDARY_VERSION, BOUNDARY_VERSION, boundary_version, "CANDIDATE_SPECIFIC_DISTINCTNESS_BASIS_EMISSION_BOUNDARY_VERSION_NOT_0_1_0")
    ...
    _validate_reference(checks, request, "completed_distinctness_operation_terminal_summary_reference", "COMPLETED_DISTINCTNESS_OPERATION_TERMINAL_SUMMARY_REFERENCE_MISSING")
    _validate_reference(checks, request, "completed_differentiation_operation_terminal_summary_reference", "COMPLETED_DIFFERENTIATION_OPERATION_TERMINAL_SUMMARY_REFERENCE_MISSING")
    _validate_reference(checks, request, "completed_distinctness_operation_boundary_terminal_summary_reference", "COMPLETED_DISTINCTNESS_OPERATION_BOUNDARY_TERMINAL_SUMMARY_REFERENCE_MISSING")

    _add_check(
        checks,
        "upstream_distinctness_operation_result_exact",
        request.get("upstream_distinctness_operation_result") == UPSTREAM_DISTINCTNESS_OPERATION_RESULT,
        UPSTREAM_DISTINCTNESS_OPERATION_RESULT,
        request.get("upstream_distinctness_operation_result"),
        "UPSTREAM_DISTINCTNESS_OPERATION_RESULT_NOT_NOT_DISTINCT",
    )
    ...
    for code, aliases in REQUEST_FLAG_ALIASES.items():
        ...
        _add_check(checks, f"{code.lower()}_not_requested", not triggered, "request flag absent or false", {"triggered_aliases": triggered}, code)

    _validate_required_non_claims(checks, request)
    marker_posture = {
        "boundary_spec_markers_present": _validate_marker_file(
            checks, request, "boundary_spec_reference", BOUNDARY_SPEC_MARKERS, "boundary_spec_markers_present", "BOUNDARY_SPEC_MARKER_MISSING"
        ),
        ...
        "completed_differentiation_operation_terminal_summary_markers_present": _validate_differentiation_marker_file(checks, request),
    ...
    _add_check(checks, "result_level_non_claims_canonical_false", True, "final result-level non-claims canonical false", True, "NON_CLAIM_MISSING_OR_FLIPPED")
...
def _build_boundary_object(request: Mapping[str, Any], recorded: bool, marker_posture: Mapping[str, bool]) -> dict[str, Any]:
    return {
        ...
        "candidate_specific_distinctness_basis_emission_boundary_type": _safe_official_value(request, "candidate_specific_distinctness_basis_emission_boundary_type", BOUNDARY_TYPE),
        "candidate_specific_distinctness_basis_emission_boundary_version": _safe_official_value(request, "candidate_specific_distinctness_basis_emission_boundary_version", BOUNDARY_VERSION),
        "future_emission_operation_type": _safe_official_value(request, "future_emission_operation_type", FUTURE_EMISSION_OPERATION_TYPE),
        "admissible_future_basis_route": _safe_official_value(request, "admissible_future_basis_route", ADMISSIBLE_FUTURE_BASIS_ROUTE),
        "rupture_class_blocked": _safe_official_value(request, "rupture_class_blocked", RUPTURE_CLASS_BLOCKED),
        "upstream_distinctness_operation_result": _safe_official_value(request, "upstream_distinctness_operation_result", UPSTREAM_DISTINCTNESS_OPERATION_RESULT),
        "candidate_specific_distinctness_basis_emission_boundary_recorded": recorded,
        "boundary_created": recorded,
        "upstream_distinctness_operation_result_is_not_distinct": request.get("upstream_distinctness_operation_result") == UPSTREAM_DISTINCTNESS_OPERATION_RESULT,
        ...
        "separate_lineage_receipt_material_missing_upstream": request.get("separate_lineage_receipt_material_missing_upstream") is True,
        "separate_digest_material_missing_upstream": request.get("separate_digest_material_missing_upstream") is True,
        "scope_division_route_allowed_for_future_operation_shape": request.get("scope_division_route_allowed_for_future_operation_shape") is True,
        ...
        "completed_distinctness_operation_terminal_summary_markers_present": bool(marker_posture.get("completed_distinctness_operation_terminal_summary_markers_present")),
        "completed_differentiation_operation_terminal_summary_markers_present": bool(marker_posture.get("completed_differentiation_operation_terminal_summary_markers_present")),
        "completed_distinctness_operation_boundary_terminal_summary_markers_present": bool(marker_posture.get("completed_distinctness_operation_boundary_terminal_summary_markers_present")),
    }
...
def _build_result(
    ...
    result: dict[str, Any] = {
        "candidate_specific_distinctness_basis_emission_boundary_metadata": {
            "candidate_specific_distinctness_basis_emission_boundary_id": boundary["candidate_specific_distinctness_basis_emission_boundary_id"],
            "boundary_type": BOUNDARY_TYPE,
            "result_version": RESULT_VERSION,
            "generated_at": _now_iso(),
            "resolver_module": RESOLVER_MODULE,
        },
        "declared_candidate_specific_distinctness_basis_emission_boundary_question": _declared_question_section(request),
        "upstream_basis": {
            ...
            "separate_lineage_receipt_material_missing_upstream": request.get("separate_lineage_receipt_material_missing_upstream") is True,
            "separate_digest_material_missing_upstream": request.get("separate_digest_material_missing_upstream") is True,
            "completed_distinctness_operation_terminal_summary_reference": _sanitize_value("completed_distinctness_operation_terminal_summary_reference", request.get("completed_distinctness_operation_terminal_summary_reference")),
            "completed_differentiation_operation_terminal_summary_reference": _sanitize_value("completed_differentiation_operation_terminal_summary_reference", request.get("completed_differentiation_operation_terminal_summary_reference")),
            "completed_distinctness_operation_boundary_terminal_summary_reference": _sanitize_value("completed_distinctness_operation_boundary_terminal_summary_reference", request.get("completed_distinctness_operation_boundary_terminal_summary_reference")),
        },
        "candidate_specific_distinctness_basis_emission_boundary_basis": {
            "boundary_spec_reference": _sanitize_value("boundary_spec_reference", request.get("boundary_spec_reference")),
            "boundary_spec_markers_present": bool(marker_posture.get("boundary_spec_markers_present")),
            "completed_distinctness_operation_terminal_summary_markers_present": bool(marker_posture.get("completed_distinctness_operation_terminal_summary_markers_present")),
            "completed_differentiation_operation_terminal_summary_markers_present": bool(marker_posture.get("completed_differentiation_operation_terminal_summary_markers_present")),
            "completed_distinctness_operation_boundary_terminal_summary_markers_present": bool(marker_posture.get("completed_distinctness_operation_boundary_terminal_summary_markers_present")),
        ...
        "candidate_specific_distinctness_basis_emission_boundary_statement": {
            "candidate_specific_distinctness_basis_emission_boundary_recorded": recorded,
            "boundary_created": recorded,
            "scope_division_route_allowed_for_future_operation_shape": boundary["scope_division_route_allowed_for_future_operation_shape"],
    ...
    result["candidate_specific_distinctness_basis_emission_boundary_summary"] = (
        build_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2_summary(result)
    )
    return result


def build_declared_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2_request(
    *,
...
def resolve_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2(
    ...
    if declared_candidate_specific_distinctness_basis_emission_boundary is None:
        ...
        request = build_declared_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2_request()
...
def resolve_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2_from_path(
    ...
    if not isinstance(loaded, Mapping):
        ...
        request = build_declared_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2_request()
        return _build_result(request, checks, {}, OUTCOME_BLOCKED)
    return resolve_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2(loaded)


def build_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2_summary(
    ...
    return {
        ...
        "candidate_specific_distinctness_basis_emission_boundary_id": boundary_map.get("candidate_specific_distinctness_basis_emission_boundary_id"),
        ...
        "separate_lineage_receipt_material_missing_upstream": boundary_map.get("separate_lineage_receipt_material_missing_upstream"),
        "separate_digest_material_missing_upstream": boundary_map.get("separate_digest_material_missing_upstream"),
        "scope_division_route_allowed_for_future_operation_shape": boundary_map.get("scope_division_route_allowed_for_future_operation_shape"),
        "divergent_receipt_history_route_authorized": False,
        "carrier_separation_route_authorized": False,
        "cosmetic_substitution_treated_as_basis": False,
        "digest_laundering_treated_as_basis": False,
        ...
        "separate_lineage_receipt_material_not_emitted": boundary_map.get("separate_lineage_receipt_material_not_emitted"),
        ...
        "completed_distinctness_operation_terminal_summary_markers_present": boundary_map.get("completed_distinctness_operation_terminal_summary_markers_present"),
        "completed_differentiation_operation_terminal_summary_markers_present": boundary_map.get("completed_differentiation_operation_terminal_summary_markers_present"),
        "completed_distinctness_operation_boundary_terminal_summary_markers_present": boundary_map.get("completed_distinctness_operation_boundary_terminal_summary_markers_present"),
        "result_level_non_claims_canonical_false": all(non_claim_map.get(key) is False for key in REQUIRED_FALSE_NON_CLAIMS),
    }


def write_descendant_body_candidate_specific_distinctness_basis_emission_boundary_v0_min_v2_result(
    result: Mapping[str, Any],
    ...
    if isinstance(boundary, Mapping):
        boundary_id = str(boundary.get("candidate_specific_distinctness_basis_emission_boundary_id") or DEFAULT_BOUNDARY_ID)
    if output_path is None:
        filename = f"{_safe_filename_part(boundary_id)}__candidate_specific_distinctness_basis_emission_boundary_v0_min_v2_result.json"
