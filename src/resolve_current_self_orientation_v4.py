"""Resolve bounded current self-orientation with body-signal posture.

This successor preserves the v3 self-orientation model and adds one narrow
post-signal recognition surface: body-signal recognition v2 and body-signal
acceptance are recognized only as downstream, non-operative posture.
...
scope a signal, route a signal, create workflow machinery, or create a body
relevance medium.
"""
...
class CurrentSelfOrientationV4Error(RuntimeError):
...
CURRENT_SELF_ORIENTATION_RESULT_VERSION = "0.4.0"
DEFAULT_RESULT_STEM = "current_self_orientation_v4_result"
...
def _require_mapping(value: Any, context: str, block_code: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise CurrentSelfOrientationV4Error(f"{context} must be an object", block_code)
    return value
...
def _read_json_file(
    ...
    *,
    ...
    try:
        ...
        raise CurrentSelfOrientationV4Error(
...
def _self_orientation_v3_identity(
    result: Mapping[str, Any],
    path: Path | str | None,
    selection_mode: str,
) -> dict[str, Any]:
    metadata = _metadata_section(result, "current_self_orientation_v3_metadata")
    return _identity(
        ...
        selection_mode=selection_mode,
        result_family="current_self_orientation_v3_result",
    )
...
def _select_body_pass(
    ...
    for path in _discover_json_files(V0_BODY_PASS_ROOT):
        ...
        except CurrentSelfOrientationV4Error:
            continue
        if artifact.get("outcome") == OUTCOME_BODY_PASS_CONFIRMED:
...
def _validate_self_orientation_v3_result(result: Mapping[str, Any]) -> None:
    metadata = _metadata_section(result, "current_self_orientation_v3_metadata")
    if metadata.get("self_orientation_result_type") != SELF_ORIENTATION_V3_RESULT_TYPE:
        raise CurrentSelfOrientationV4Error(
            "v3 self-orientation result type is not recognized",
            "SOURCE_ARTIFACT_MALFORMED",
        )
    if metadata.get("resolver_module") != SELF_ORIENTATION_V3_RESOLVER_MODULE:
        raise CurrentSelfOrientationV4Error(
            "v3 self-orientation resolver module is not recognized",
            "SOURCE_ARTIFACT_MALFORMED",
        )
    if result.get("outcome") != OUTCOME_SELF_ORIENTED:
        raise CurrentSelfOrientationV4Error(
            "v3 self-orientation result is not SELF_ORIENTED",
            "NO_SELF_ORIENTATION_SOURCE_BASIS",
        )
    _validate_false_non_claims(result.get("non_claims"), "v3 self-orientation")


def _select_matching_self_orientation_v3(
    body_identity: Mapping[str, Any],
) -> tuple[Path, dict[str, Any], str]:
    matches: list[tuple[Path, dict[str, Any]]] = []
    for path in _discover_json_files(CURRENT_SELF_ORIENTATION_V3_ROOT):
        try:
            artifact = _read_json_file(
                path,
                context="candidate current self-orientation v3 result",
                missing_code="REQUIRED_SOURCE_ARTIFACT_UNREADABLE",
                unreadable_code="REQUIRED_SOURCE_ARTIFACT_UNREADABLE",
                malformed_code="SOURCE_ARTIFACT_MALFORMED",
            )
            _validate_self_orientation_v3_result(artifact)
    ...
    if not matches:
        raise CurrentSelfOrientationV4Error(
            "no self-oriented v3 result matches the selected body-pass basis",
            "NO_SELF_ORIENTATION_SOURCE_BASIS",
        )
    path, artifact = matches[-1]
    return path, artifact, "matching_self_orientation_v3_discovery"
...
def _selected_inputs_with_signal_line(
    ...
    selected = copy.deepcopy(
        dict(
            _require_mapping(
                v3_result.get("selected_orientation_inputs"),
                "selected_orientation_inputs",
                "SOURCE_ARTIFACT_MALFORMED",
            )
        )
    )
    selected["selected_self_orientation_v3_result"] = dict(v3_identity)
    ...
    selected["selection_scope"] = {
        **(
        ...
        "self_orientation_v3_is_predecessor_recognition": True,
...
def _result_id(selected_inputs: Mapping[str, Any], outcome: str) -> str:
    ...
    suffix = "current_self_orientation_v4_self_oriented"
    if outcome == OUTCOME_BLOCKED:
        suffix = "current_self_orientation_v4_blocked"
...
def _build_result(
    *,
    ...
    result = {
        "current_self_orientation_v4_metadata": _metadata(selected_inputs, outcome),
...
def _v3_section(v3_result: Mapping[str, Any] | None, key: str) -> Mapping[str, Any]:
    if not isinstance(v3_result, Mapping):
        return {}
    value = v3_result.get(key)
...
def _blocked_result(
    *,
    ...
    selected_inputs: Mapping[str, Any] | None = None,
    checks: Sequence[Mapping[str, Any]] | None = None,
    v3_result: Mapping[str, Any] | None = None,
    recognized_body_signal_surfaces: Mapping[str, Any] | None = None,
    non_claims: Mapping[str, bool] | None = None,
) -> dict[str, Any]:
    ...
    return _build_result(
        ...
        recognized_current_executable_core_line=_v3_section(
            v3_result,
            "recognized_current_executable_core_line",
        ),
        recognized_governing_effective_basis=_v3_section(
            v3_result,
            "recognized_governing_effective_basis",
        ),
        recognized_current_state_surfaces=_v3_section(
            v3_result,
            "recognized_current_state_surfaces",
        ),
        recognized_continuity_surfaces=_v3_section(
            v3_result,
            "recognized_continuity_surfaces",
        ),
        recognized_derivative_surfaces=_v3_section(
            v3_result,
            "recognized_derivative_surfaces",
        ),
        recognized_operator_facing_surfaces=_v3_section(
            v3_result,
            "recognized_operator_facing_surfaces",
        ),
        recognized_reentry_surfaces=_v3_section(
            v3_result,
            "recognized_reentry_surfaces",
        ),
        recognized_body_signal_surfaces=recognized_body_signal_surfaces or {},
        recognized_open_surfaces=_v3_section(v3_result, "recognized_open_surfaces"),
        recognized_blocked_or_refused_surfaces=_v3_section(
            v3_result,
            "recognized_blocked_or_refused_surfaces",
        ),
        recognized_touch_admissibility_surfaces=_v3_section(
            v3_result,
            "recognized_touch_admissibility_surfaces",
        ),
        checks=checks or [],
        ...
        self_orientation_basis={
            "basis_kind": "blocked_v4_self_orientation_preserving_selected_basis",
            "body_signal_surfaces_remain_downstream": True,
...
def _resolve(
    ...
    v3_path, v3_result, v3_selection_mode = _select_matching_self_orientation_v3(
        body_identity
    )
    v3_identity = _self_orientation_v3_identity(v3_result, v3_path, v3_selection_mode)
    v3_selected_inputs = _require_mapping(
        v3_result.get("selected_orientation_inputs"),
        "selected_orientation_inputs",
        "SOURCE_ARTIFACT_MALFORMED",
    )
    selected_inputs = copy.deepcopy(dict(v3_selected_inputs))
    selected_inputs["selected_self_orientation_v3_result"] = dict(v3_identity)
    ...
    checks = [
        *_as_check_list(v3_result.get("bounded_correspondence_checks")),
        _check(
            "v3_self_orientation_is_self_oriented",
            v3_result.get("outcome") == OUTCOME_SELF_ORIENTED,
            expected=OUTCOME_SELF_ORIENTED,
            actual=v3_result.get("outcome"),
            block_code="NO_SELF_ORIENTATION_SOURCE_BASIS",
        ),
        _check(
            "v3_selected_body_pass_matches_anchor",
            _body_pass_matches(_selected_body_pass_from_orientation(v3_result), body_identity),
            expected=body_identity,
            actual=_selected_body_pass_from_orientation(v3_result),
    ...
    return _build_result(
        selected_inputs=selected_inputs,
        recognized_current_executable_core_line=v3_result.get(
            "recognized_current_executable_core_line"
        ),
        recognized_governing_effective_basis=v3_result.get(
            "recognized_governing_effective_basis"
        ),
        recognized_current_state_surfaces=v3_result.get("recognized_current_state_surfaces"),
        recognized_continuity_surfaces=v3_result.get("recognized_continuity_surfaces"),
        recognized_derivative_surfaces=v3_result.get("recognized_derivative_surfaces"),
        recognized_operator_facing_surfaces=v3_result.get(
            "recognized_operator_facing_surfaces"
        ),
        recognized_reentry_surfaces=v3_result.get("recognized_reentry_surfaces"),
        recognized_body_signal_surfaces=body_signal_surfaces,
        recognized_open_surfaces=v3_result.get("recognized_open_surfaces"),
        recognized_blocked_or_refused_surfaces=v3_result.get(
            "recognized_blocked_or_refused_surfaces"
        ),
        recognized_touch_admissibility_surfaces=v3_result.get(
            "recognized_touch_admissibility_surfaces"
        ),
        checks=checks,
        ...
        block_detail=None,
        self_orientation_basis={
            "basis_kind": "v3_self_orientation_plus_downstream_body_signal_acceptance",
            "selected_self_orientation_v3_result_id": v3_identity.get("result_id"),
            "selected_self_orientation_v3_result_path": v3_identity.get("result_path"),
            ...
            "accepted_signal_creates_scope": False,
            "accepted_signal_establishes_presence": False,
            "accepted_signal_meets_threshold": False,
            "accepted_signal_creates_truth": False,
            "accepted_signal_authorizes_action": False,
            "accepted_signal_authorizes_follow_on_work": False,
            "accepted_signal_routes": False,
            "accepted_signal_creates_workflow": False,
            "accepted_signal_creates_body_relevance_medium": False,
            "self_orientation_creates_authority": False,
        },
        non_claims=non_claims,
    )
...
def resolve_current_self_orientation(
    ...
    """Resolve one bounded v4 self-orientation result."""

    try:
        return _resolve(body_pass_result=body_pass_result)
    except CurrentSelfOrientationV4Error as exc:
        return _blocked_result(
...
def resolve_current_self_orientation_from_path(
    ...
    """Resolve one bounded v4 self-orientation result from a body-pass path."""
...
def build_current_self_orientation_summary(result: Mapping[str, Any]) -> dict[str, Any]:
    """Build a bounded v4 self-orientation summary."""
...
def _safe_default_output_path(
    result: Mapping[str, Any],
    root: Path | str = CURRENT_SELF_ORIENTATION_V4_ROOT,
) -> Path:
    selected = result.get("selected_orientation_inputs")
    selected = selected if isinstance(selected, Mapping) else {}
    body = selected.get("selected_body_pass_result")
    ...
    for index in range(1, 1000):
        ...
        if not candidate.exists():
            return candidate
    raise CurrentSelfOrientationV4Error(
        "no bounded current self-orientation v4 filename is available",
        "SOURCE_ARTIFACT_MALFORMED",
    )
...
def write_current_self_orientation_result(
    ...
    """Write one additive v4 self-orientation JSON artifact."""
    ...
    if target.exists():
        raise FileExistsError(f"current self-orientation v4 result already exists: {target}")
