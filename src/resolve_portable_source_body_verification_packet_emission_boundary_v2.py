"""V2 packet emission boundary resolver with preserved v1 failure evidence.

This additive successor preserves
``resolve_portable_source_body_verification_packet_emission_boundary`` as
visible predecessor conformance-failure evidence. V1 compiled and exposed the
expected public API, but its broad sanitizer redacted generated boolean posture
fields whose names contained ``hidden_repo_state`` into the string
``[bounded-redacted-raw-or-hidden-state]``.

V2 fixes that sanitizer/posture typing failure. It contains hostile raw or
hidden-state payload values inside selected basis material without corrupting
generated statement, non-claim, summary, check, or metadata booleans. It records
packet-emission-boundary posture only: no packet is emitted, no emitted packet
is created, no packet is transferred or copied, no second-carrier receipt or
execution occurs, no cross-carrier evidence is created, and no source,
authority, currentness, runtime, final completion, continuation, reusable
permission, or follow-on work is created or authorized.
"""
...
class PortableSourceBodyVerificationPacketEmissionBoundaryV2Error(Exception):
    """Raised for hard v2 packet-emission-boundary input or output failures."""


PortableSourceBodyVerificationPacketEmissionBoundaryError = (
    PortableSourceBodyVerificationPacketEmissionBoundaryV2Error
)

RESOLVER_MODULE = "resolve_portable_source_body_verification_packet_emission_boundary_v2"
PREDECESSOR_RESOLVER_MODULE = "resolve_portable_source_body_verification_packet_emission_boundary"
...
def _any_true(
    ...
    for name in names:
        if request.get(name) is True:
            return True
    keys = (basis_key,) if basis_key else SELECTED_BASIS_KEYS
...
def _check(
    ...
    if code not in BLOCK_CODES:
        raise PortableSourceBodyVerificationPacketEmissionBoundaryV2Error(
...
def _non_claims_from_request(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result = dict(declared) if isinstance(declared, Mapping) else {}
...
def _safe_basis_section(label: str, basis_value: Any, request: Mapping[str, Any]) -> dict[str, Any]:
    basis = _json_safe(_copy(basis_value)) if isinstance(basis_value, Mapping) else {}
    ...
    if label == "selected_packet_artifact_basis":
        ...
        basis.setdefault("packet_artifact_not_emission", True)
...
def _additional_basis_required(request: Mapping[str, Any], outcome: str) -> dict[str, Any]:
    ...
    if isinstance(context, Mapping):
        ...
        if isinstance(supplied_missing, Iterable) and not isinstance(supplied_missing, str):
            missing = [str(item) for item in supplied_missing]
        reason = str(context.get("reason") or reason)
    return {
        "additional_basis_required": True,
        "reason": reason,
        "missing_or_unclear_basis": missing or ["packet_emission_boundary_basis_unclear"],
        "missing_basis_scheduled": False,
        "missing_basis_authorized": False,
        "missing_basis_executed": False,
...
def _build_checks(request: Mapping[str, Any] | None) -> list[dict[str, Any]]:
    ...
    missing_codes = {
        ...
        "selected_packet_artifact_terminal_summary_basis": "PACKET_ARTIFACT_TERMINAL_SUMMARY_BASIS_MISSING",
    ...
    local_artifact_recorded = _value(
        request,
        (
    ...
    _check(checks, "packet artifact version 0.1.0", artifact_version == PACKET_ARTIFACT_RESULT_VERSION, PACKET_ARTIFACT_RESULT_VERSION, artifact_version, "PACKET_ARTIFACT_VERSION_NOT_0_1_0")
    ...
    _check(checks, "packet artifact recorded one local packet artifact", local_artifact_recorded is True, True, local_artifact_recorded, "PACKET_ARTIFACT_DID_NOT_RECORD_LOCAL_PACKET_ARTIFACT")

    for name, names, code in (
        ("packet artifact did not emit packet", ("selected_packet_artifact_packet_emitted", "packet_emitted"), "PACKET_ARTIFACT_ALREADY_EMITTED_PACKET"),
        ("packet artifact did not transfer packet", ("selected_packet_artifact_packet_transferred", "packet_transferred"), "PACKET_ARTIFACT_ALREADY_TRANSFERRED_PACKET"),
        ("packet artifact did not copy packet to another device", ("selected_packet_artifact_copied_packet_to_another_device", "packet_copied_to_another_device"), "PACKET_ARTIFACT_COPIED_PACKET_TO_ANOTHER_DEVICE"),
        ("packet artifact did not authorize source transfer", ("selected_packet_artifact_authorized_source_transfer", "source_transfer_authorized", "source_transfer_occurred"), "PACKET_ARTIFACT_AUTHORIZED_SOURCE_TRANSFER"),
        ("packet artifact did not authorize source receipt", ("selected_packet_artifact_authorized_source_receipt", "source_receipt_authorized", "source_receipt_occurred"), "PACKET_ARTIFACT_AUTHORIZED_SOURCE_RECEIPT"),
        ("packet artifact did not authorize reception", ("selected_packet_artifact_authorized_reception", "reception_authorization_created"), "PACKET_ARTIFACT_AUTHORIZED_RECEPTION"),
        ("packet artifact did not authorize second-carrier execution", ("selected_packet_artifact_authorized_second_carrier_execution", "second_carrier_execution_authorized", "second_carrier_execution_created"), "PACKET_ARTIFACT_AUTHORIZED_SECOND_CARRIER_EXECUTION"),
        ("packet artifact did not create external result", ("selected_packet_artifact_created_external_result", "external_result_created"), "PACKET_ARTIFACT_CREATED_EXTERNAL_RESULT"),
        ("packet artifact did not create cross-carrier evidence", ("selected_packet_artifact_created_cross_carrier_evidence", "cross_carrier_evidence_created"), "PACKET_ARTIFACT_CREATED_CROSS_CARRIER_EVIDENCE"),
        ("packet artifact did not use hidden repo state as emission authority", ("selected_packet_artifact_used_hidden_repo_state_as_emission_authority", "hidden_repo_state_used_as_emission_authority"), "PACKET_ARTIFACT_USED_HIDDEN_REPO_STATE_AS_EMISSION_AUTHORITY"),
        ("packet artifact did not treat repo-local availability as emission authority", ("selected_packet_artifact_treated_repo_local_availability_as_emission_authority", "repo_local_availability_treated_as_emission_authority"), "PACKET_ARTIFACT_TREATED_REPO_LOCAL_AVAILABILITY_AS_EMISSION_AUTHORITY"),
        ("packet artifact did not return raw full prior artifact body", ("selected_packet_artifact_raw_full_prior_artifact_body_returned", "raw_full_prior_artifact_body_returned"), "PACKET_ARTIFACT_RETURNED_RAW_FULL_PRIOR_ARTIFACT_BODY"),
        ("packet artifact did not treat artifact existence as emission authority", ("selected_packet_artifact_treated_artifact_existence_as_emission_authority", "artifact_existence_treated_as_emission_authority"), "PACKET_ARTIFACT_TREATED_ARTIFACT_EXISTENCE_AS_EMISSION_AUTHORITY"),
        ("packet artifact did not treat artifact path as currentness", ("selected_packet_artifact_treated_artifact_path_as_currentness", "artifact_path_treated_as_currentness"), "PACKET_ARTIFACT_TREATED_ARTIFACT_PATH_AS_CURRENTNESS"),
    ):
    ...
    execution_audit_only = _value(request, ("execution_trace_audit_only", "audit_only", "execution_audit_only"), "selected_command_execution_basis", default=True)
    ...
    lineage_only = _value(request, ("command_report_lineage_only", "lineage_only"), "selected_command_report_lineage_basis", default=True)
    ...
    predecessor_visible = _value(request, ("predecessor_failure_evidence_visible", "predecessor_failures_visible"), "selected_predecessor_failure_basis", default=True)
    ...
    repo_local_as_authority = _any_true(request, ("repo_local_availability_treated_as_emission_authority",))
    consumed_request_reopened = _any_true(request, ("consumed_request_reopened",))
    authorization_token_reused = _any_true(request, ("authorization_token_reused",))
    _check(checks, "repo-local availability not emission authority", repo_local_as_authority is False, False, repo_local_as_authority, "REPO_LOCAL_AVAILABILITY_TREATED_AS_EMISSION_AUTHORITY")
    _check(checks, "consumed token remains closed", consumed_request_reopened is False, False, consumed_request_reopened, "CONSUMED_REQUEST_REOPENED")
    _check(checks, "authorization token reuse blocked", authorization_token_reused is False, False, authorization_token_reused, "AUTHORIZATION_TOKEN_REUSED")
...
def _determine_outcome(request: Mapping[str, Any] | None, checks: Sequence[Mapping[str, Any]]) -> str:
    if _failed_code(checks) is not None or not isinstance(request, Mapping):
...
def _block(outcome: str, failed_code: str | None, request: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    reason = request.get("block_reason") if isinstance(request, Mapping) else None
    return {
        "blocked": True,
        "block_code": failed_code or "DECLARED_PACKET_EMISSION_BOUNDARY_REQUEST_MALFORMED",
        "block_reason": reason or "Packet-emission-boundary request is blocked by bounded membrane checks.",
        "packet_emitted": False,
...
def _metadata(request: Mapping[str, Any] | None, checks: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    request_id = "portable_source_body_verification_packet_emission_boundary_v2_request"
    if isinstance(request, Mapping):
        request_id = str(request.get("packet_emission_boundary_request_id") or request_id)
    return {
        ...
        "successor_reason": (
            "V1 sanitizer/posture typing failure preserved: generated hidden-repo-state "
            "boolean posture and non-claim fields were redacted into "
            "[bounded-redacted-raw-or-hidden-state]. V2 preserves boolean posture, "
            "non-claim, check, metadata, and summary values while containing raw or "
            "hidden-state payloads in selected basis material."
        ),
...
def _base_result(request: Mapping[str, Any] | None, outcome: str, checks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "portable_source_body_verification_packet_emission_boundary_metadata": _metadata(request, checks),
        "declared_packet_emission_boundary_question": {
            "request_id": request_map.get("packet_emission_boundary_request_id"),
    ...
    result["portable_source_body_verification_packet_emission_boundary_summary"] = (
        build_portable_source_body_verification_packet_emission_boundary_v2_summary(result)
    )
    return _json_safe(result)


def resolve_portable_source_body_verification_packet_emission_boundary_v2(
    declared_packet_emission_boundary_request: Mapping[str, Any] | None = None,
) -> dict:
    """Resolve one bounded v2 packet-emission-boundary request mapping."""
...
def resolve_portable_source_body_verification_packet_emission_boundary_v2_from_path(
    declared_packet_emission_boundary_request_path: Path | str,
) -> dict:
    """Load a JSON object request from path and resolve v2 packet-emission-boundary posture."""
    ...
    return resolve_portable_source_body_verification_packet_emission_boundary_v2(request)


def build_portable_source_body_verification_packet_emission_boundary_v2_summary(
    result: Mapping[str, Any],
) -> dict:
    """Build a compact v2 summary without promoting boundary posture into emission."""
    ...
    artifact_basis = result.get("selected_packet_artifact_basis", {})
    ...
    summary: dict[str, Any] = {
        ...
        "selected_packet_artifact_outcome": artifact_basis.get("outcome"),
        "selected_packet_artifact_version": artifact_basis.get("result_version"),
        "selected_packet_artifact_failed_check_count": artifact_basis.get("failed_check_count"),
        "successor_of": metadata.get("successor_of"),
        "v1_predecessor_failure_preserved": metadata.get("v1_predecessor_failure_preserved") is True,
        "v1_repaired": metadata.get("v1_repaired") is True,
        "v1_hidden": metadata.get("v1_hidden") is True,
        "v1_claimed_passed": metadata.get("v1_claimed_passed") is True,
        "v2_successor_does_not_erase_v1": metadata.get("v2_successor_does_not_erase_v1") is True,
...
def _request_id_from_result(result: Mapping[str, Any]) -> str:
    metadata = result.get("portable_source_body_verification_packet_emission_boundary_metadata")
    if isinstance(metadata, Mapping) and metadata.get("packet_emission_boundary_request_id"):
        return str(metadata["packet_emission_boundary_request_id"])
    question = result.get("declared_packet_emission_boundary_question")
    if isinstance(question, Mapping) and question.get("request_id"):
        return str(question["request_id"])
    return "portable_source_body_verification_packet_emission_boundary_v2_request"
...
def write_portable_source_body_verification_packet_emission_boundary_v2_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write a bounded v2 JSON result without overwriting an existing artifact."""
...
def build_declared_portable_source_body_verification_packet_emission_boundary_v2_request(
    packet_emission_boundary_request_id: str = (
        "portable_source_body_verification_packet_emission_boundary_v2_reference_review_001"
    ),
    *,
    ...
    """Build a valid bounded v2 request with packet-emission non-claims false."""
...
def resolve_portable_source_body_verification_packet_emission_boundary(
    ...
    """Compatibility alias that uses v2 behavior and v2 metadata."""

    return resolve_portable_source_body_verification_packet_emission_boundary_v2(
        declared_packet_emission_boundary_request
    )
...
def resolve_portable_source_body_verification_packet_emission_boundary_from_path(
    ...
    """Compatibility alias that loads through the v2 path resolver."""

    return resolve_portable_source_body_verification_packet_emission_boundary_v2_from_path(
        declared_packet_emission_boundary_request_path
    )
...
def write_portable_source_body_verification_packet_emission_boundary_result(
    ...
    """Compatibility alias that writes into the v2 output root by default."""

    return write_portable_source_body_verification_packet_emission_boundary_v2_result(
        result, output_path
    )
...
def build_portable_source_body_verification_packet_emission_boundary_summary(
    ...
    """Compatibility alias that builds the v2 summary."""

    return build_portable_source_body_verification_packet_emission_boundary_v2_summary(result)
...
def build_declared_portable_source_body_verification_packet_emission_boundary_request(
    ...
    **kwargs: Any,
    ...
    """Compatibility alias that builds a v2 request."""

    return build_declared_portable_source_body_verification_packet_emission_boundary_v2_request(
        *args, **kwargs
    )
