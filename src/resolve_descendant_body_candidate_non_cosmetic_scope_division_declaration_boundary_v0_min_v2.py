"""Resolve the v2 candidate non-cosmetic scope-division declaration boundary.

This successor preserves the v1 boundary semantics while fixing the v1
canonical false non-claim coverage gaps and accepting clean equivalent live
basis-marker phrasing. It records one boundary result only. It does not define
or implement the future declaration operation, declare candidate scopes, emit
content, rerun upstream operations, authorize standing/runtime/authority, scan,
repair, validate prior unsupported claims, or authorize follow-on work.
"""
...
class DescendantBodyCandidateNonCosmeticScopeDivisionDeclarationBoundaryV0MinV2Error(
    Exception
):
    """Raised when bounded v2 result writing cannot be completed."""
...
BASIS_EMISSION_OPERATION_MARKERS = _marker_groups(
    ...
    (
        ...
        "material_emitted false",
        "material was not emitted",
        "material not emitted",
    ),
    (
...
DIFFERENTIATION_OPERATION_MARKERS = _marker_groups(
    ...
    (
        ...
        "candidate record count emitted 2",
        "two candidate records emitted",
    ),
    (
    ...
    (
        "descendant bodies were not created",
        "descendant_bodies_not_created = true",
        "descendant body creation refused",
    ),
    (
...
def _contains_marker_group(text: str, group: tuple[str, ...]) -> bool:
    haystack = text.lower()
    normalized = " ".join(haystack.replace("`", " ").replace(":", " ").split())
    for marker in group:
        needle = marker.lower()
        normalized_needle = " ".join(needle.replace("`", " ").replace(":", " ").split())
...
def build_declared_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_request(
    **overrides: Any,
) -> dict[str, Any]:
    """Build the default declared request for the v2 repo-local boundary."""
...
def resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2(
    declared_candidate_non_cosmetic_scope_division_declaration_boundary: Mapping[str, Any]
    | None = None,
) -> dict[str, Any]:
    """Resolve one v2 boundary result from a declared request mapping."""
...
def resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_from_path(
    declared_candidate_non_cosmetic_scope_division_declaration_boundary_path: Path | str,
) -> dict[str, Any]:
    """Resolve one v2 boundary result from a JSON request path."""
    ...
    return resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2(
        loaded
    )


def build_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_summary(
    result: Mapping[str, Any],
) -> dict[str, Any]:
    """Build a compact v2 summary without raw Markdown bodies."""
...
def _assert_output_path_allowed(path: Path) -> None:
    ...
    for forbidden in FORBIDDEN_OUTPUT_ROOT_PARTS:
        if forbidden in parts:
            raise DescendantBodyCandidateNonCosmeticScopeDivisionDeclarationBoundaryV0MinV2Error(
                f"Refusing to write boundary result under forbidden prior root: {forbidden}"
            )


def write_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_result(
    result: Mapping[str, Any], output_path: Path | str | None = None
) -> Path:
    """Write a stable v2 JSON result without silently overwriting existing files."""
    ...
    if output_path is None:
        destination = (
            ...
            / f"{boundary_id}__candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_result.json"
        )
    else:
