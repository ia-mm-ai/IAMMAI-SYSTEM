"""Portable source-body verification command output resolver.

This module records one bounded command output event downstream of the
recorded command output boundary line. It does not capture stdout, stderr,
process output, or a raw command output body. It does not create output
capture, a command output/report artifact, command result, command success,
source, authority, currentness, final completion, continuation, reusable
...
class PortableSourceBodyVerificationCommandOutputError(Exception):
    """Raised only for impossible command-output resolver failures."""


RESOLVER_MODULE = "resolve_portable_source_body_verification_command_output"
RESULT_VERSION = "0.1.0"
RESULT_TYPE = "portable_source_body_verification_command_output_result"
OUTPUT_ROOT = Path(
    "artifacts/integrity_host_v0_min_coexistence_portable_source_body_verification_command_output"
)

CORE_QUESTION = (
    "Can the recorded command output boundary basis be used to record one "
    "bounded command output event without creating output capture, command "
    "output/report artifact, command result, command success, source, "
    "authority, currentness, final completion, continuation, reusable "
    "permission, derivative reception, vessel relation, another reception "
    "request, or follow-on work?"
)

OUTCOME_RECORDED = "PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_OUTPUT_RECORDED"
OUTCOME_NOT_RECORDED = "PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_OUTPUT_NOT_RECORDED"
OUTCOME_REQUIRES_ADDITIONAL_BASIS = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_OUTPUT_REQUIRES_ADDITIONAL_BASIS"
)
OUTCOME_BLOCKED = "PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_OUTPUT_BLOCKED"
OUTCOME_FAMILY = (
    OUTCOME_RECORDED,
...
INTENT_RECORD = "RECORD_PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_OUTPUT"
INTENT_DO_NOT_RECORD = "DO_NOT_RECORD_PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_OUTPUT"
INTENT_BLOCK = "BLOCK_PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_OUTPUT"
...
SUPPORTED_COMMAND_OUTPUT_SCOPE = (
    "COMMAND_OUTPUT_ONLY",
    "ONE_BOUNDED_COMMAND_OUTPUT_EVENT_RECORDED",
...
SUPPORTED_COMMAND_OUTPUT_SCOPE_SET = set(SUPPORTED_COMMAND_OUTPUT_SCOPE)
...
TOP_LEVEL_SECTIONS = (
    "portable_source_body_verification_command_output_metadata",
    "declared_command_output_question",
    ...
    "command_output_statement",
    "command_output_non_meaning",
    ...
    "outcome",
    "block",
    "portable_source_body_verification_command_output_summary",
)
    ...
    if isinstance(value, Mapping):
        ...
        for key, nested in value.items():
            ...
            if text_key in FORBIDDEN_FULL_BODY_KEYS:
                ...
                sanitized[text_key] = {
                    "omitted": True,
                    "reason": "raw_command_output_body_not_returned",
                }
            else:
...
def _passed_count(basis: Mapping[str, Any]) -> int | None:
    value = _find(
        basis,
        ("passed_check_count", "passed_checks", "selected_passed_check_count"),
    )
    if isinstance(value, list):
        return len(value)
    return _to_int(value)
...
def _requested_outcome(request: Mapping[str, Any]) -> str | None:
    value = request.get("requested_command_output_outcome")
    if value in OUTCOME_FAMILY:
        return str(value)
    return None


def _selected_command_output_boundary_basis(request: Mapping[str, Any]) -> dict[str, Any]:
    basis = _basis(request, "selected_command_output_boundary_basis")
    command_output_created = _created_flag(
        request,
        basis,
        ("selected_command_output_boundary_output_created",),
        ("command_output_created", "output_created"),
    )
    output_capture_created = _created_flag(
        request,
        basis,
        ("selected_command_output_boundary_output_capture_created",),
        ("output_capture_created",),
    )
    output_report_created = _created_flag(
        request,
        basis,
        ("selected_command_output_boundary_output_report_artifact_created",),
        ("command_output_report_artifact_created", "output_report_artifact_created"),
    )
    command_result_created = _created_flag(
        request,
        basis,
        ("selected_command_output_boundary_result_created",),
        ("command_result_created",),
    )
    command_success_created = _created_flag(
        request,
        basis,
        ("selected_command_output_boundary_success_created",),
        ("command_success_created",),
    )
    return {
        "basis": _sanitize(basis),
        "declared": bool(basis),
        "selected_command_output_boundary_result_path": request.get(
            "selected_command_output_boundary_result_path"
        )
        or basis.get("path")
        or basis.get("artifact_path")
        or basis.get("result_path"),
        "selected_command_output_boundary_result_id": request.get(
            "selected_command_output_boundary_result_id"
        )
        or basis.get("result_id")
        or basis.get("portable_source_body_verification_command_output_boundary_result_id"),
        "outcome": request.get("selected_command_output_boundary_result_outcome")
        or _outcome(basis),
        "failed_check_count": _find(
            request,
            ("selected_command_output_boundary_failed_check_count",),
            _failed_count(basis),
        ),
        "passed_check_count": _passed_count(basis),
        "one_future_command_output_step_declared": _truthy(
            request.get("selected_command_output_boundary_step_declared")
        )
        or _truthy(basis.get("one_future_command_output_step_declared"))
        or _truthy(basis.get("command_output_boundary_recorded")),
...
def _selected_prior_basis(
    ...
    selected: dict[str, Any] = {
        ...
        "failed_check_count": request.get(failed_key, _failed_count(basis)) if failed_key else _failed_count(basis),
        "passed_check_count": _passed_count(basis),
...
def _command_output_event(recorded: bool) -> dict[str, Any]:
    return {
        "command_output_event_recorded": recorded,
        "event_type": "COMMAND_OUTPUT_EVENT_BOUNDED" if recorded else None,
...
def _build_checks(
    ...
    boundary = _selected_command_output_boundary_basis(request)
    ...
    scope_values = _scope_values(request.get("command_output_scope"))
    ...
    question = request.get("command_output_question")
    intent = request.get("command_output_intent")
    checks.extend(
        [
            _check(
                "command output question declared",
                _present(question),
                "declared command output question",
                question,
                "COMMAND_OUTPUT_QUESTION_UNDECLARED",
            ),
            _check(
                "command output intent supported",
                intent in SUPPORTED_INTENTS,
                SUPPORTED_INTENTS,
                intent,
                "COMMAND_OUTPUT_INTENT_UNSUPPORTED",
            ),
            _check(
                "command output boundary terminal summary basis declared",
                _basis_declared(request, "selected_command_output_boundary_terminal_summary_basis"),
                "selected command output boundary terminal summary basis declared",
                request.get("selected_command_output_boundary_terminal_summary_basis"),
                "COMMAND_OUTPUT_BOUNDARY_BASIS_MISSING",
            ),
            _check(
                "command output boundary live artifact basis declared",
            ...
            _check(
                ...
                boundary.get("outcome") == COMMAND_OUTPUT_BOUNDARY_OUTCOME,
                COMMAND_OUTPUT_BOUNDARY_OUTCOME,
                boundary.get("outcome"),
                "COMMAND_OUTPUT_BOUNDARY_NOT_RECORDED",
            ),
            _check(
                "command output boundary failed check count zero",
                _to_int(boundary.get("failed_check_count")) == 0,
                0,
                boundary.get("failed_check_count"),
                "COMMAND_OUTPUT_BOUNDARY_FAILED_CHECKS_PRESENT",
            ),
            _check(
                "command output boundary step declared",
                _truthy(boundary.get("one_future_command_output_step_declared")),
                "one future command output step declared",
                boundary.get("one_future_command_output_step_declared"),
                "COMMAND_OUTPUT_BOUNDARY_STEP_NOT_DECLARED",
            ),
            _check(
                "command output boundary output not created",
                not _truthy(boundary.get("command_output_created")),
                "command output not created by boundary",
                boundary.get("command_output_created"),
                "COMMAND_OUTPUT_BOUNDARY_ALREADY_CREATED_OUTPUT",
            ),
            _check(
                "command output boundary output capture not created",
                not _truthy(boundary.get("output_capture_created")),
                "output capture not created by boundary",
                boundary.get("output_capture_created"),
                "COMMAND_OUTPUT_BOUNDARY_ALREADY_CREATED_OUTPUT_CAPTURE",
            ),
            _check(
                "command output boundary output report artifact not created",
                not _truthy(boundary.get("command_output_report_artifact_created")),
                "command output/report artifact not created by boundary",
                boundary.get("command_output_report_artifact_created"),
                "COMMAND_OUTPUT_BOUNDARY_ALREADY_CREATED_OUTPUT_REPORT_ARTIFACT",
            ),
            _check(
                "command output boundary result not created",
                not _truthy(boundary.get("command_result_created")),
                "command result not created by boundary",
                boundary.get("command_result_created"),
                "COMMAND_OUTPUT_BOUNDARY_ALREADY_CREATED_RESULT",
            ),
            _check(
                "command output boundary success not created",
                ...
                "COMMAND_OUTPUT_BOUNDARY_ALREADY_CREATED_SUCCESS",
            ),
            _check(
    ...
    posture_checks = (
        ("command output only posture declared", "command_output_only_posture", "COMMAND_OUTPUT_ONLY_POSTURE_MISSING"),
        (
            "one bounded command output event posture declared",
            "one_bounded_command_output_event_posture",
            "ONE_BOUNDED_COMMAND_OUTPUT_EVENT_POSTURE_MISSING",
        ),
        (
    ...
    checks.append(
        _check(
            "command output scope supported",
            bool(scope_values) and all(value in SUPPORTED_COMMAND_OUTPUT_SCOPE_SET for value in scope_values),
            SUPPORTED_COMMAND_OUTPUT_SCOPE,
            scope_values,
            "UNSUPPORTED_COMMAND_OUTPUT_SCOPE",
        )
    )
    ...
    collapse_specs = (
        ...
        (("boundary_treated_as_output",), "BOUNDARY_TREATED_AS_OUTPUT", "boundary not command output"),
        (("containment_treated_as_output",), "CONTAINMENT_TREATED_AS_OUTPUT", "containment not command output"),
        (("execution_trace_treated_as_output",), "EXECUTION_TRACE_TREATED_AS_OUTPUT", "execution trace not output"),
    ...
    checks.extend(
        [
            _check(
                ...
                "raw output field present" if _contains_key(request, FORBIDDEN_RAW_OUTPUT_KEYS) else "no raw output field",
                "RAW_OUTPUT_BODY_CAPTURED",
            ),
            _check(
                ...
                "raw prior body field present" if _contains_key(request, FORBIDDEN_FULL_BODY_KEYS) else "no raw prior body field",
...
def _what_remains_open() -> dict[str, Any]:
    return {
        "open_items": [
            "command output test",
            "command output live artifact",
            "output capture boundary or output capture step, if separately specified",
            "output capture",
...
def _result_id(request_id: Any) -> str:
    base = str(request_id).strip() if _present(request_id) else "undeclared_command_output_request"
    return f"{base}__portable_source_body_verification_command_output_result"
...
def _determine_outcome(
    ...
    if intent == INTENT_BLOCK:
        return OUTCOME_BLOCKED, _block_object(
            True,
            "COMMAND_OUTPUT_BLOCKED_BY_REQUEST",
            request.get("block_reason") or "command output block intent declared",
        )
    if failed:
        first_code = failed[0].get("block_code") or "DECLARED_COMMAND_OUTPUT_REQUEST_MALFORMED"
        ...
        return OUTCOME_BLOCKED, _block_object(True, str(first_code), "command output review blocked")
...
def _base_result(
    ...
    request_id = request.get("command_output_request_id")
    ...
    result: dict[str, Any] = {
        "portable_source_body_verification_command_output_metadata": {
            "portable_source_body_verification_command_output_result_id": _result_id(request_id),
            "portable_source_body_verification_command_output_result_type": RESULT_TYPE,
            "portable_source_body_verification_command_output_result_version": RESULT_VERSION,
            "generated_at": _now(),
            "resolver_module": RESOLVER_MODULE,
            "short_resolver_filename": "resolve_portable_source_body_verification_command_output.py",
            "full_upstream_lineage_preserved_inside_selected_basis": True,
        },
        "declared_command_output_question": {
            "command_output_request_id": request_id,
            "question": request.get("command_output_question"),
            "core_question": CORE_QUESTION,
            "intent": request.get("command_output_intent"),
        },
        "selected_command_output_boundary_basis": _selected_command_output_boundary_basis(request),
        "selected_command_output_boundary_terminal_summary_basis": {
            "basis": _sanitize(_basis(request, "selected_command_output_boundary_terminal_summary_basis")),
            "declared": _basis_declared(request, "selected_command_output_boundary_terminal_summary_basis"),
        ...
        "selected_command_output_containment_basis": _selected_command_output_containment_basis(request),
        "selected_command_output_containment_terminal_summary_basis": {
            "basis": _sanitize(_basis(request, "selected_command_output_containment_terminal_summary_basis")),
            "declared": _basis_declared(request, "selected_command_output_containment_terminal_summary_basis"),
            "readability_basis_only": True,
            "does_not_create_output_capture": True,
            "does_not_create_output_report_artifact": True,
            "does_not_create_command_result": True,
        ...
        "selected_post_invocation_command_execution_terminal_summary_basis": {
            "basis": _sanitize(_basis(request, "selected_post_invocation_command_execution_terminal_summary_basis")),
            "declared": _basis_declared(request, "selected_post_invocation_command_execution_terminal_summary_basis"),
            "readability_basis_only": True,
    ...
    result.update(
        {
            "command_output_scope": {
                ...
                "supported_scope": list(SUPPORTED_COMMAND_OUTPUT_SCOPE),
                "unsupported_scope_values": [
                    value for value in scope_values if value not in SUPPORTED_COMMAND_OUTPUT_SCOPE_SET
                ],
            },
            "command_output_checks": checks,
            "command_output_statement": _statement(recorded),
            "command_output_non_meaning": _non_meaning(),
            "command_output_event": _command_output_event(recorded),
            "additional_basis_required": {
            ...
            "not_recorded_basis": {
                ...
                or ("recording declined by declared intent" if outcome == OUTCOME_NOT_RECORDED else None),
            },
    ...
    result["portable_source_body_verification_command_output_summary"] = (
        build_portable_source_body_verification_command_output_summary(result)
    )
    ...
    if not expected.issubset(actual):
        ...
        raise PortableSourceBodyVerificationCommandOutputError(
...
def resolve_portable_source_body_verification_command_output(
    declared_command_output_request: Mapping[str, Any] | None = None,
) -> dict:
    """Resolve one bounded command-output request mapping."""

    if declared_command_output_request is None:
        return _base_result(
            {},
            "COMMAND_OUTPUT_QUESTION_UNDECLARED",
            "declared command output request missing",
        )
    if not isinstance(declared_command_output_request, Mapping):
        return _base_result(
            {},
            "DECLARED_COMMAND_OUTPUT_REQUEST_MALFORMED",
            "declared command output request must be a mapping",
        )
    request = _copy(dict(declared_command_output_request))
    return _base_result(request)


def resolve_portable_source_body_verification_command_output_from_path(
    declared_command_output_request_path: Path | str,
) -> dict:
    """Resolve one bounded command-output request from a JSON object path."""

    path = Path(declared_command_output_request_path)
    try:
        ...
        return _base_result(
            {},
            "DECLARED_COMMAND_OUTPUT_REQUEST_UNREADABLE",
            "declared command output request path unreadable or malformed",
        )
    if not isinstance(parsed, Mapping):
        return _base_result(
            {},
            "DECLARED_COMMAND_OUTPUT_REQUEST_MALFORMED",
            "declared command output request JSON must be an object",
        )
    return resolve_portable_source_body_verification_command_output(parsed)


def build_portable_source_body_verification_command_output_summary(
    result: Mapping[str, Any],
) -> dict:
    """Build a compact summary for a bounded command-output result."""

    checks = list(result.get("command_output_checks", []))
    passed = [check for check in checks if check.get("passed")]
    failed = [check for check in checks if not check.get("passed")]
    metadata = _mapping(result.get("portable_source_body_verification_command_output_metadata"))
    question = _mapping(result.get("declared_command_output_question"))
    statement = _mapping(result.get("command_output_statement"))
    event = _mapping(result.get("command_output_event"))
    boundary = _mapping(result.get("selected_command_output_boundary_basis"))
    ...
    return {
        ...
        "request_id": question.get("command_output_request_id"),
        "question": question.get("question"),
        "intent": question.get("intent"),
        "result_version": metadata.get(
            "portable_source_body_verification_command_output_result_version"
        ),
        "resolver_module": metadata.get("resolver_module"),
        "passed_check_count": len(passed),
        "failed_check_count": len(failed),
        "command_output_recorded": statement.get("command_output_recorded", False),
        "bounded_command_output_event_recorded": statement.get(
        ...
        "selected_command_output_boundary_outcome": boundary.get("outcome"),
        "selected_command_output_boundary_failed_check_count": boundary.get("failed_check_count"),
        ...
        "stdout_not_captured": event.get("stdout_captured") is False,
        "stderr_not_captured": event.get("stderr_captured") is False,
        "process_output_not_captured": event.get("process_output_captured") is False,
        "raw_output_body_not_captured": event.get("raw_output_body_captured") is False,
        ...
        "boundary_not_output": non_claims.get("boundary_treated_as_output") is False,
        "containment_not_output": non_claims.get("containment_treated_as_output") is False,
...
def _safe_filename_part(value: Any) -> str:
    text = str(value).strip() if _present(value) else "undeclared_command_output_request"
    cleaned = []
    for char in text:
        if char.isalnum() or char in {"-", "_", "."}:
            ...
            cleaned.append("_")
    return "".join(cleaned).strip("_") or "undeclared_command_output_request"
...
def write_portable_source_body_verification_command_output_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write one additive command-output result JSON without overwriting."""

    result_copy = _copy(dict(result))
    if output_path is None:
        question = _mapping(result_copy.get("declared_command_output_question"))
        request_id = _safe_filename_part(question.get("command_output_request_id"))
        path = OUTPUT_ROOT / f"{request_id}__portable_source_body_verification_command_output_result.json"
    else:
        path = Path(output_path)
        if path.suffix.lower() != ".json":
            question = _mapping(result_copy.get("declared_command_output_question"))
            request_id = _safe_filename_part(question.get("command_output_request_id"))
            path = path / f"{request_id}__portable_source_body_verification_command_output_result.json"
...
def build_declared_portable_source_body_verification_command_output_request(
    command_output_request_id: str = "portable_source_body_verification_command_output_reference_review_001",
    command_output_question: str = CORE_QUESTION,
    command_output_intent: str = INTENT_RECORD,
    **overrides: Any,
) -> dict[str, Any]:
    """Build a bounded declared command-output request with false non-claims."""

    request: dict[str, Any] = {
        "command_output_request_id": command_output_request_id,
        "command_output_question": command_output_question,
        "command_output_intent": command_output_intent,
        "selected_command_output_boundary_basis": {
        ...
        "selected_command_output_boundary_terminal_summary_basis": {
            **_default_reference_basis("selected_command_output_boundary_terminal_summary_basis"),
        ...
        "selected_command_output_containment_terminal_summary_basis": {
            **_default_reference_basis("selected_command_output_containment_terminal_summary_basis"),
            "terminal_summary_declared": True,
            "terminal_summary_remains_readability_basis_only": True,
        },
        "selected_post_invocation_command_execution_basis": {
        ...
        "selected_post_invocation_command_execution_terminal_summary_basis": {
            **_default_reference_basis("selected_post_invocation_command_execution_terminal_summary_basis"),
            "terminal_summary_declared": True,
            "execution_trace_audit_only_preserved": True,
        },
        "selected_command_invocation_basis": {
        ...
        "command_output_scope": list(SUPPORTED_COMMAND_OUTPUT_SCOPE),
        "declared_non_claims": _default_non_claims(),
        "requested_command_output_outcome": OUTCOME_RECORDED,
    }
    for key in (
        "command_output_only_posture",
        "one_bounded_command_output_event_posture",
