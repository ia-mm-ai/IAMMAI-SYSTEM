"""Resolve one local read-only operation-execution boundary.

This resolver reads one selected
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_PERMISSION artifact as the
selected-state operation-permission basis. It records one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY object only.

The object is boundary-shaped, local, read-only,
selected-state-operation-execution-consideration-only, closure-token-aware,
non-operation-execution-shaped, and non-runtime-shaped. It may record that one
future local read-only operation execution may be considered as a separately
bounded step. It does not create or perform operation execution, create runtime
permission, create public or distributed surfaces, permit unsupported commands
or keys, discover files, accept entries or signals, create registry/search/
query/ranking surfaces, or authorize follow-on work.
"""
...
class LocalRelevanceMediumReadOnlyOperationExecutionBoundaryV0MinError(RuntimeError):
    """Bounded resolver error for operation-execution-boundary handling."""
...
DEFAULT_BOUNDARY_ID = "local_relevance_medium_read_only_operation_execution_boundary_001"
...
INTENT_RECORD = "RECORD_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY"
INTENT_DO_NOT_RECORD = (
    "DO_NOT_RECORD_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY"
)
INTENT_BLOCK = "BLOCK_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY"
SUPPORTED_INTENTS = (INTENT_RECORD, INTENT_DO_NOT_RECORD, INTENT_BLOCK)

CORE_QUESTION = (
    "Given one clean LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_PERMISSION for "
    "selected command state, existing reusable read-only lookup permission, prior "
    "lookup-pair coverage, and prior local carrier command surface, may one "
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY be recorded "
    "that permits a future local read-only operation execution to be considered "
    "as a separately bounded step, without creating operation execution, "
    "performing operation execution, creating runtime permission, creating public "
...
OPERATION_PERMISSION_BASIS_FALSE_FIELDS = REQUIRED_FALSE_NON_CLAIMS
...
BLOCK_CODES = (
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY_QUESTION_UNDECLARED",
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY_INTENT_UNSUPPORTED",
    "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY_BLOCK_REQUESTED",
    ...
    "FUTURE_OPERATION_EXECUTION_MAY_NOT_BE_CONSIDERED",
    "BOUNDARY_TYPE_MISSING",
    "BOUNDARY_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY",
    "BOUNDARY_SCOPE_MISSING",
    "BOUNDARY_SCOPE_NOT_SELECTED_OPERATION_EXECUTION_CONSIDERATION_ONLY",
    "OPERATION_EXECUTION_CREATED",
    "OPERATION_EXECUTION_PERFORMED",
    "RUNTIME_PERMISSION_CREATED",
    "PUBLIC_API_CREATED",
    ...
    "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY_REQUEST_MALFORMED",
    "DECLARED_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY_REQUEST_UNREADABLE",
)
...
def _extract_operation_permission_basis(
    ...
    non_claims = _mapping(artifact.get("non_claims"))
    statement = _mapping(
    ...
    return {
        ...
        "false_posture": {
            field: _first_present(
                obj.get(field),
                summary.get(field),
                non_claims.get(field),
            )
            for field in OPERATION_PERMISSION_BASIS_FALSE_FIELDS
        },
    }
...
def _check_operation_permission_basis_false_posture(
    operation_permission_basis: Mapping[str, Any],
    checks: list[dict[str, Any]],
) -> None:
    false_posture = _mapping(operation_permission_basis.get("false_posture"))
    for field in OPERATION_PERMISSION_BASIS_FALSE_FIELDS:
        actual = false_posture.get(field)
        _add_check(
            checks,
            f"operation_permission_basis_{field}_not_created_or_authorized",
            actual is False or actual is None,
            False,
            actual,
            _FALSE_FIELD_BLOCK_CODES.get(field, "NON_CLAIM_MISSING_OR_FLIPPED"),
        )
...
def _build_boundary_object(
    ...
    boundary: dict[str, Any] = {
        "boundary_id": str(
            declared.get(
                "local_relevance_medium_read_only_operation_execution_boundary_id",
                DEFAULT_BOUNDARY_ID,
            )
        ...
        "basis_operation_permission_artifact": operation_permission_basis.get(
            "artifact_path"
        ),
        "basis_operation_permission_outcome": operation_permission_basis.get(
            "artifact_outcome"
        ),
        ...
        "local_relevance_medium_read_only_operation_execution_boundary_recorded": bool(
            recorded
        ),
        "future_operation_execution_may_be_considered": bool(recorded),
    }
...
def _build_statement(boundary: Mapping[str, Any], recorded: bool) -> dict[str, bool]:
    return {
        ...
        "basis_operation_permission_artifact_preserved": boundary.get(
        ...
        "selected_operation_permission_recorded": boundary.get(
            "selected_operation_permission_recorded"
        )
        is True,
        "operation_permission_created": boundary.get("operation_permission_created") is True,
        "operation_permission_local_only": boundary.get("operation_permission_local_only")
        is True,
        "operation_permission_read_only": boundary.get("operation_permission_read_only")
        is True,
        "future_operation_execution_may_be_considered": boundary.get(
            "future_operation_execution_may_be_considered"
        )
        is True,
...
def _build_non_meaning() -> dict[str, bool]:
    non_meaning = {
        "operation_execution_boundary_is_operation_execution": False,
        "operation_execution_boundary_performs_operation_execution": False,
        "operation_execution_boundary_is_runtime_permission": False,
        "operation_execution_boundary_is_public_api": False,
        "operation_execution_boundary_is_participant_facing_interface": False,
        "operation_execution_boundary_is_distributed_network_behavior": False,
        "operation_execution_boundary_is_general_operation_permission": False,
        "operation_execution_boundary_is_general_lookup_permission": False,
        "operation_execution_boundary_is_arbitrary_lookup_permission": False,
        "operation_execution_boundary_permits_unsupported_commands": False,
        "operation_execution_boundary_permits_unsupported_lookup_keys": False,
        "operation_execution_boundary_creates_new_lookup_entry": False,
        "operation_execution_boundary_performs_filesystem_discovery": False,
        "operation_execution_boundary_creates_registry_search_query_or_ranking": False,
        "operation_execution_boundary_authorizes_follow_on_work": False,
    }
    for field in REQUIRED_FALSE_NON_CLAIMS:
...
def _what_remains_open() -> list[str]:
    return [
        ...
        "local relevance medium read-only operation execution boundary terminal summary",
        "operation execution",
        "runtime permission",
        "public API",
...
def _build_result_artifact(
    ...
    boundary = _build_boundary_object(declared, operation_permission_basis, recorded)
    ...
    result: dict[str, Any] = {
        "local_relevance_medium_read_only_operation_execution_boundary_metadata": {
            "local_relevance_medium_read_only_operation_execution_boundary_id": boundary[
                "boundary_id"
            ],
            "local_relevance_medium_read_only_operation_execution_boundary_type": BOUNDARY_TYPE,
            "local_relevance_medium_read_only_operation_execution_boundary_version": (
                RESULT_VERSION
            ),
        ...
        "declared_local_relevance_medium_read_only_operation_execution_boundary_question": {
            "question": _sanitize(
                declared.get(
                    "local_relevance_medium_read_only_operation_execution_boundary_question"
                )
            ),
            "intent": _sanitize(
                declared.get(
                    "local_relevance_medium_read_only_operation_execution_boundary_intent"
                )
            ),
        ...
        "local_relevance_medium_read_only_operation_execution_boundary_checks": _sanitize(
            checks
        ),
        "local_relevance_medium_read_only_operation_execution_boundary_statement": statement,
    ...
    result["local_relevance_medium_read_only_operation_execution_boundary_summary"] = (
...
def _blocked_malformed_result(code: str, reason: Any) -> dict[str, Any]:
    ...
    _add_check(
        checks,
        "declared_local_relevance_medium_read_only_operation_execution_boundary_request_readable",
        False,
        "mapping JSON object",
        reason,
        code,
...
def resolve_local_relevance_medium_read_only_operation_execution_boundary_v0_min(
    declared_local_relevance_medium_read_only_operation_execution_boundary: Mapping[
        str, Any
    ]
    | None = None,
) -> dict:
    """Resolve one bounded local read-only operation-execution boundary."""

    if declared_local_relevance_medium_read_only_operation_execution_boundary is None:
        declared = (
            build_declared_local_relevance_medium_read_only_operation_execution_boundary_v0_min_request()
        )
    elif isinstance(
        declared_local_relevance_medium_read_only_operation_execution_boundary,
        MappingABC,
    ):
        declared = copy.deepcopy(
            dict(declared_local_relevance_medium_read_only_operation_execution_boundary)
        )
    else:
    ...
    _add_check(
        checks,
        "operation_execution_boundary_question_declared",
        isinstance(question, str) and bool(question.strip()),
        "declared operation execution boundary question",
        question,
        "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY_QUESTION_UNDECLARED",
    )
    _add_check(
        checks,
        "operation_execution_boundary_intent_supported",
        intent in SUPPORTED_INTENTS and intent != INTENT_BLOCK,
        f"one of {SUPPORTED_INTENTS[:-1]}",
        intent,
        "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY_BLOCK_REQUESTED"
        if intent == INTENT_BLOCK
        else "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_BOUNDARY_INTENT_UNSUPPORTED",
    )
    ...
    operation_permission_outcome_ok = (
        operation_permission_basis.get("artifact_outcome") == OPERATION_PERMISSION_RECORDED_OUTCOME
    ...
    _check_operation_permission_basis_false_posture(operation_permission_basis, checks)

    future_operation_execution_may_be_considered = declared.get(
        "future_operation_execution_may_be_considered",
        True,
    )
    _add_check(
        checks,
        "future_operation_execution_may_be_considered",
        future_operation_execution_may_be_considered is True
        and declared.get("future_operation_execution_may_not_be_considered") is not True,
        True,
        future_operation_execution_may_be_considered,
        "FUTURE_OPERATION_EXECUTION_MAY_NOT_BE_CONSIDERED",
    )
    ...
    _add_check(
        ...
        boundary_type is not None and declared.get("boundary_type_missing") is not True,
        BOUNDARY_TYPE,
        boundary_type,
        "BOUNDARY_TYPE_MISSING",
    )
    ...
    _add_check(
        ...
        boundary_scope is not None and declared.get("boundary_scope_missing") is not True,
        BOUNDARY_SCOPE,
        boundary_scope,
        "BOUNDARY_SCOPE_MISSING",
    )
    _add_check(
        ...
        boundary_scope == BOUNDARY_SCOPE
        and declared.get("boundary_scope_not_selected_operation_execution_consideration_only")
        is not True,
        BOUNDARY_SCOPE,
        boundary_scope,
        "BOUNDARY_SCOPE_NOT_SELECTED_OPERATION_EXECUTION_CONSIDERATION_ONLY",
    ...
    if _failed_checks(checks):
        ...
        declared.get(
            "requested_local_relevance_medium_read_only_operation_execution_boundary_outcome"
        )
        == OUTCOME_REQUIRES_ADDITIONAL_BASIS
    ):
...
def resolve_local_relevance_medium_read_only_operation_execution_boundary_v0_min_from_path(
    declared_local_relevance_medium_read_only_operation_execution_boundary_path: Path | str,
) -> dict:
    """Resolve a declared operation-execution-boundary request from JSON."""

    try:
        with Path(
            declared_local_relevance_medium_read_only_operation_execution_boundary_path
    ...
    return resolve_local_relevance_medium_read_only_operation_execution_boundary_v0_min(data)


def build_local_relevance_medium_read_only_operation_execution_boundary_v0_min_summary(
    result: Mapping[str, Any],
) -> dict:
    """Build the bounded operation-execution-boundary summary view."""

    checks = list(
        result.get("local_relevance_medium_read_only_operation_execution_boundary_checks", [])
        if isinstance(
            result.get("local_relevance_medium_read_only_operation_execution_boundary_checks"),
            list,
        )
    ...
    boundary = _mapping(
        result.get("local_relevance_medium_read_only_operation_execution_boundary")
    )
    statement = _mapping(
        result.get("local_relevance_medium_read_only_operation_execution_boundary_statement")
    )
    ...
    return {
        ...
        "resolver_module": RESOLVER_MODULE,
        "local_relevance_medium_read_only_operation_execution_boundary_recorded": (
            statement.get("local_relevance_medium_read_only_operation_execution_boundary_recorded")
            is True
        ),
        ...
        is True,
        "future_operation_execution_may_be_considered": statement.get(
            "future_operation_execution_may_be_considered"
        )
        is True,
        ...
        "operation_execution_not_created": boundary.get("operation_execution_created") is False,
        "operation_execution_not_performed": boundary.get("operation_execution_performed")
        is False,
        "runtime_permission_not_created": boundary.get("runtime_permission_created") is False,
        ...
        is False,
        "unsupported_commands_not_permitted": boundary.get("unsupported_commands_permitted")
        is False,
        ...
        "key_non_claims": {
            ...
            "operation_execution_performed": non_claims.get("operation_execution_performed"),
...
def write_local_relevance_medium_read_only_operation_execution_boundary_v0_min_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write one operation-execution-boundary result artifact without overwrite."""

    boundary = _mapping(
        result.get("local_relevance_medium_read_only_operation_execution_boundary")
    )
    boundary_id = _safe_filename_stem(boundary.get("boundary_id", DEFAULT_BOUNDARY_ID))
...
def build_declared_local_relevance_medium_read_only_operation_execution_boundary_v0_min_request(
    *,
    local_relevance_medium_read_only_operation_execution_boundary_id: str = DEFAULT_BOUNDARY_ID,
    ...
    **overrides: Any,
    ...
    """Build a declared request for one selected-state operation-execution boundary."""

    if selected_command != SELECTED_COMMAND:
        raise LocalRelevanceMediumReadOnlyOperationExecutionBoundaryV0MinError(
    ...
    request: dict[str, Any] = {
        ...
        "local_relevance_medium_read_only_operation_execution_boundary_intent": intent,
        "selected_operation_permission_artifact": str(selected_operation_permission_artifact),
        "selected_command": selected_command,
        "boundary_type": boundary_type,
        "boundary_scope": boundary_scope,
