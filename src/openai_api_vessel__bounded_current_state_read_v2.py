"""Run one lineage-preserving successor bounded OpenAI API derivative read.

This module preserves the lawful scope of the original bounded vessel:
...
This successor exists only to harden the output-format boundary. It does not
reopen vessel scope, body law, source-family scope, or model authority.
"""
...
class OpenAIDerivativeVesselV2Error(RuntimeError):
    """Raised for bounded vessel v2 setup, source, packet, or output failures."""
...
OPENAI_API_DERIVATIVE_VESSEL_V2_ROOT = Path(
...
RESOLVER_MODULE = "openai_api_vessel__bounded_current_state_read_v2"
SUCCESSOR_OF_MODULE = "openai_api_vessel__bounded_current_state_read"
VESSEL_RESULT_TYPE = (
    "IAMMAI_OPENAI_API_DERIVATIVE_VESSEL_BOUNDED_CURRENT_STATE_READ_V2_RESULT"
)
VESSEL_RESULT_VERSION = "0.2.0"
DEFAULT_RESULT_STEM = "bounded_current_state_vessel_v2_result"

STRICT_OUTPUT_SCHEMA_NAME = "bounded_current_state_read_v2_output"
...
def _require_mapping(value: Any, context: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise OpenAIDerivativeVesselV2Error(
...
def _validate_source_artifact(artifact: Mapping[str, Any]) -> None:
    family = _source_family_from_artifact(artifact)
...
def _discover_latest_answered_source() -> tuple[Path | None, dict[str, Any] | None]:
    ...
    for path in reversed(candidates):
        ...
        except OpenAIDerivativeVesselV2Error:
            continue
...
def _initial_api_runtime() -> dict[str, Any]:
    ...
    return {
        ...
        "strict_format_enforcement_used": False,
        "fallback_extraction_path_used": False,
    }
...
def _strict_text_format_config() -> dict[str, Any]:
    return {
        "format": {
            "type": "json_schema",
            "name": STRICT_OUTPUT_SCHEMA_NAME,
            "strict": True,
            "description": (
                "Return only the bounded vessel output contract for the current-state "
                "derivative read."
            ),
            "schema": dict(STRICT_OUTPUT_SCHEMA),
        }
    }


def _client_supports_strict_format_enforcement(client: Any) -> bool:
    responses = getattr(client, "responses", None)
    create = getattr(responses, "create", None)
    if not callable(create):
        return False
...
def _call_openai_derivative_read(
    ...
    use_strict = _client_supports_strict_format_enforcement(client)
    ...
    if use_strict:
        create_kwargs["text"] = _strict_text_format_config()
    ...
    return raw_text, use_strict, not use_strict


def _extract_single_top_level_json_object_text(raw_text: str) -> str:
    ...
    if start is not None or depth != 0:
        raise OpenAIDerivativeVesselV2Error(
            "model output does not contain a recoverable single JSON object",
            "MODEL_OUTPUT_MALFORMED",
        )
    if not spans:
        raise OpenAIDerivativeVesselV2Error(
    ...
    if len(spans) != 1:
        raise OpenAIDerivativeVesselV2Error(
            "model output contains multiple top-level JSON objects",
            "MODEL_OUTPUT_MALFORMED",
        )
...
def _parse_json_object_text(raw_text: str) -> dict[str, Any]:
    try:
        ...
        extracted_text = _extract_single_top_level_json_object_text(raw_text)
        try:
...
def _validate_model_output(raw_text: str) -> dict[str, Any]:
    ...
    parsed = _parse_json_object_text(raw_text.strip())
    keys = set(parsed)
    if keys == {"answer"}:
    ...
    if not keys:
        raise OpenAIDerivativeVesselV2Error(
            "model output is empty within the bounded contract",
            "MODEL_OUTPUT_OUTSIDE_CONTRACT",
        )
    raise OpenAIDerivativeVesselV2Error(
...
def _result_id(selected_source_id: str | None, outcome: str) -> str:
    ...
    suffix = (
        "bounded_current_state_read_v2_answered"
        if outcome == OUTCOME_ANSWERED_DERIVATIVE_READ
        else "bounded_current_state_read_v2_refused"
    )
...
def _refused_result(
    *,
    ...
    result = {
        "openai_api_derivative_vessel_v2_metadata": metadata,
...
def resolve_bounded_current_state_read(
    ...
    try:
        ...
        raw_output_text, strict_used, fallback_used = _call_openai_derivative_read(
            client,
            vessel_request,
            str(api_runtime["model_name_used"]),
        )
        api_runtime["strict_format_enforcement_used"] = strict_used
        api_runtime["fallback_extraction_path_used"] = fallback_used
    ...
    except OpenAIDerivativeVesselV2Error as exc:
...
def build_bounded_current_state_vessel_summary(
    ...
    if not isinstance(result, Mapping):
        raise OpenAIDerivativeVesselV2Error("vessel result must be a mapping")
    return _summary_from_result(result)


def _safe_default_output_path(
    result: Mapping[str, Any],
    root: Path | str = OPENAI_API_DERIVATIVE_VESSEL_V2_ROOT,
) -> Path:
    ...
    for index in range(1, 1000):
        ...
        if not candidate.exists():
            return candidate
    raise OpenAIDerivativeVesselV2Error(
        "no bounded vessel v2 filename is available",
        "MALFORMED_VESSEL_PACKET",
    )
...
def write_bounded_current_state_vessel_result(
    ...
    """Write one additive bounded current-state vessel v2 result JSON artifact."""
    ...
    if target.exists():
        raise FileExistsError(f"bounded vessel v2 result already exists: {target}")
