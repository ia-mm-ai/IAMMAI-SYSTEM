"""Resolve portable source-body verification cross-carrier evidence.

This module is downstream of the cross-carrier-evidence-boundary line. It may
record one bounded cross-carrier evidence posture from clean boundary basis
only. Cross-carrier evidence here is not portable verification closure, source
...
RESOLVER_MODULE = "resolve_portable_source_body_verification_cross_carrier_evidence"
RESULT_TYPE = "portable_source_body_verification_cross_carrier_evidence_result"
OUTPUT_ROOT = Path(
    "artifacts/"
...
OUTCOME_REQUIRES_ADDITIONAL_BASIS = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_CROSS_CARRIER_EVIDENCE_"
    "REQUIRES_ADDITIONAL_BASIS"
)
OUTCOME_BLOCKED = (
    "PORTABLE_SOURCE_BODY_VERIFICATION_CROSS_CARRIER_EVIDENCE_BLOCKED"
)
OUTCOME_FAMILY = (
    OUTCOME_RECORDED,
    OUTCOME_NOT_RECORDED,
...
INTENT_DO_NOT_RECORD = (
    "DO_NOT_RECORD_PORTABLE_SOURCE_BODY_VERIFICATION_CROSS_CARRIER_EVIDENCE"
)
...
NON_MEANING = (
    ...
    "cross-carrier evidence became portable verification closure",
    "cross-carrier evidence became source transfer/source receipt/reception authorization",
    "cross-carrier evidence became source/authority/currentness",
    "cross-carrier evidence artifact became portable verification closure",
    "returned capture became cross-carrier proof by itself",
    "zero exit code became cross-carrier evidence automatically",
    'string "0" became cross-carrier evidence automatically',
    'string "0" became doctrine',
    "OK became cross-carrier evidence automatically",
    "Ran 7 tests became cross-carrier evidence automatically",
...
OVERREACH_FLAG_CODES: tuple[tuple[set[str], str], ...] = (
    ...
    (
        {"zero_exit_code_treated_as_cross_carrier_evidence_standalone", "zero_exit_code_treated_as_cross_carrier_evidence"},
        "ZERO_EXIT_CODE_TREATED_AS_CROSS_CARRIER_EVIDENCE",
    ),
    (
        {"string_zero_treated_as_cross_carrier_evidence_standalone", "string_zero_treated_as_cross_carrier_evidence"},
        "STRING_ZERO_TREATED_AS_CROSS_CARRIER_EVIDENCE",
    ),
    ...
    (
        {"ran_7_tests_treated_as_cross_carrier_evidence_standalone", "ran_7_tests_treated_as_cross_carrier_evidence"},
        "RAN_7_TESTS_TREATED_AS_CROSS_CARRIER_EVIDENCE",
    ),
    (
        {"returned_capture_treated_as_cross_carrier_evidence_standalone", "returned_capture_treated_as_cross_carrier_evidence"},
        "RETURNED_CAPTURE_TREATED_AS_CROSS_CARRIER_EVIDENCE",
    ),
...
def _result_id(request: Mapping[str, Any]) -> str:
    ...
    if isinstance(value, str) and value.strip():
        return value.strip()
    return "portable_source_body_verification_cross_carrier_evidence_request_001"
...
def _build_result(
    ...
    result["cross_carrier_evidence_scope"] = _scope_values(
        request.get("cross_carrier_evidence_scope")
    )
    result["cross_carrier_evidence_checks"] = checks
    result["cross_carrier_evidence_statement"] = statement
    result["cross_carrier_evidence_non_meaning"] = {
        "preserved_non_meanings": list(NON_MEANING),
        "does_not_create_portable_verification_closure": True,
        "does_not_authorize_source_transfer_source_receipt_or_reception": True,
        "does_not_create_source_authority_currentness_runtime_final_completion": True,
...
def resolve_portable_source_body_verification_cross_carrier_evidence(
    ...
    if declared_cross_carrier_evidence_request is None:
        declared_cross_carrier_evidence_request = (
...
def resolve_portable_source_body_verification_cross_carrier_evidence_from_path(
    ...
    if not isinstance(loaded, Mapping):
        raise PortableSourceBodyVerificationCrossCarrierEvidenceError(
            "declared cross-carrier evidence request path must contain a JSON object"
        )
...
def write_portable_source_body_verification_cross_carrier_evidence_result(
    ...
    metadata = result.get("portable_source_body_verification_cross_carrier_evidence_metadata", {})
    request_id = "portable_source_body_verification_cross_carrier_evidence_request_001"
...
def build_declared_portable_source_body_verification_cross_carrier_evidence_request(
    **overrides: Any,
    ...
    request: dict[str, Any] = {
        "cross_carrier_evidence_request_id": (
