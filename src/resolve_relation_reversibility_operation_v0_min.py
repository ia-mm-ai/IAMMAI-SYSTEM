"""Resolve one bounded relation-reversibility operation result.

The resolver may record only a historical-relation reversibility support result
after a completed boundary allowance. It never performs lapse, dissolution,
reversal, termination, mutation, presence, runtime, repair, discovery, or
...
class RelationReversibilityOperationV0MinError(Exception):
    """Raised when a relation-reversibility operation result cannot be written."""
...
PRIOR_RELATION_REVERSIBILITY_OPERATION_CONSIDERATION_ALLOWED_REQUIRED = True
...
PRIOR_RELATION_REVERSIBILITY_AUTHORIZED_REQUIRED = False
PRIOR_RELATION_LAPSE_AUTHORIZED_REQUIRED = False
PRIOR_RELATION_DISSOLUTION_AUTHORIZED_REQUIRED = False
PRIOR_RELATION_REVERSIBILITY_PERFORMED_REQUIRED = False
...
OUTCOME_RECORDED = "RELATION_REVERSIBILITY_OPERATION_RECORDED"
OUTCOME_REQUIRES_BOUNDARY_ALLOWANCE = "RELATION_REVERSIBILITY_OPERATION_REQUIRES_BOUNDARY_ALLOWANCE"
OUTCOME_BLOCKED = "RELATION_REVERSIBILITY_OPERATION_BLOCKED"
OUTCOME_NOT_RECORDED = "RELATION_REVERSIBILITY_OPERATION_NOT_RECORDED"
OUTCOME_FAMILY = (
    OUTCOME_RECORDED,
...
INTENT_RECORD = "RECORD_RELATION_REVERSIBILITY_OPERATION"
INTENT_DO_NOT_RECORD = "DO_NOT_RECORD_RELATION_REVERSIBILITY_OPERATION"
INTENT_BLOCK = "BLOCK_RELATION_REVERSIBILITY_OPERATION"
SUPPORTED_INTENTS = (INTENT_RECORD, INTENT_DO_NOT_RECORD, INTENT_BLOCK)

REPO_ROOT = Path(__file__).resolve().parent.parent
OUTPUT_ROOT = Path("artifacts/integrity_host_v0_min_coexistence_relation_reversibility_operation_v0_min")
DETERMINISTIC_FILENAME = "relation_reversibility_operation_001__relation_reversibility_operation_v0_min_result.json"

DEFAULT_RELATION_REVERSIBILITY_OPERATION_SPEC_REFERENCE = "spec/RELATION_REVERSIBILITY_OPERATION_V0_MIN_SPEC.md"
DEFAULT_RELATION_REVERSIBILITY_BOUNDARY_TERMINAL_SUMMARY_REFERENCE = "spec/RELATION_REVERSIBILITY_BOUNDARY_TERMINAL_SUMMARY_V0.md"
...
EXPECTED_REQUEST_VALUES = {
    ...
    "relation_reversibility_operation_id": OPERATION_ID,
    "relation_reversibility_operation_type": OPERATION_TYPE,
    "relation_reversibility_operation_version": OPERATION_VERSION,
    "relation_reversibility_operation_scope": OPERATION_SCOPE,
    "prior_relation_reversibility_boundary_type": PRIOR_RELATION_REVERSIBILITY_BOUNDARY_TYPE,
    "prior_relation_reversibility_boundary_outcome_required": PRIOR_RELATION_REVERSIBILITY_BOUNDARY_OUTCOME_REQUIRED,
    "prior_relation_reversibility_boundary_result_required": PRIOR_RELATION_REVERSIBILITY_BOUNDARY_RESULT_REQUIRED,
    "prior_relation_reversibility_operation_consideration_allowed_required": PRIOR_RELATION_REVERSIBILITY_OPERATION_CONSIDERATION_ALLOWED_REQUIRED,
    ...
    "relation_reversibility_scope": RELATION_REVERSIBILITY_SCOPE,
    ...
    "relation_reversibility_result": "NOT_EVALUATED",
}

TARGET_SPEC_MARKER_CLASSES = (
    ("operation identity", (("Relation Reversibility Operation V0 Minimum Specification", OPERATION_TYPE, OPERATION_ID, OPERATION_SCOPE),)),
    ("boundary allowance", ((
        "RELATION_REVERSIBILITY_BOUNDARY_ALLOWED",
        "RELATION_REVERSIBILITY_OPERATION_CONSIDERATION_ALLOWED",
        "relation_reversibility_operation_consideration_allowed = true",
    ...
    ("operation support", ((
        ...
        "RELATION_REVERSIBILITY_FOR_RELATION_RECORD_ONLY",
    ),)),
    ("operation non-conversion", (
        (
            "Relation reversibility operation is not relation lapse",
            "Relation reversibility operation is not relation dissolution",
            "Relation reversibility operation is not relation reversal",
            "Relation reversibility operation is not relation termination",
            "Relation reversibility operation is not relation erasure",
            "Relation reversibility operation is not relation mutation",
            "Relation reversibility operation is not relation invalidation",
            "Relation reversibility operation is not punishment",
            "Relation reversibility operation is not teardown",
            "Relation reversibility support is not relation lapse",
            "Relation reversibility support is not relation dissolution",
            "Relation reversibility support is not relation reversal",
            "Relation reversibility support is not relation termination",
            "Relation reversibility support is not presence boundary authorization",
            "Relation reversibility support is not presence",
            "Relation reversibility support is not identity",
            "Relation reversibility support is not coupling",
            "Relation reversibility support is not FIELD machinery",
            "Relation reversibility support is not runtime",
            "Relation reversibility support is not currentness",
            "Relation reversibility support is not authority",
            "Relation reversibility support is not follow-on authorization",
            "Relation reversibility support is not follow-on work",
        ),
        (
            "Relation reversibility operation is not relation lapse, dissolution, reversal, termination, erasure, mutation, invalidation, punishment, or teardown",
            "Relation reversibility support is not relation lapse, dissolution, reversal, termination, presence boundary authorization, presence, identity, coupling, FIELD machinery, runtime, currentness, authority, follow-on authorization, or follow-on work",
        ),
    )),
    ("historical relation record", (
        (
            "Historical relation record is not living relation state",
            "Relation record may be confirmed historical-only without preserving historical receipt",
            "Historical receipt preservation requires a separately bounded operation",
            "Living relation state may not be created by relation reversibility operation",
            "Living relation state may lapse only by separately bounded operation",
            "Relation lapse is not punishment",
            "Relation dissolution is not erasure",
            "Relation reversibility is not teardown",
        ),
        (
            "Historical relation record is not living relation state",
            "Relation record may be confirmed historical-only without preserving historical receipt",
            "Historical receipt preservation requires a separately bounded operation",
            "Living relation state may not be created by relation reversibility operation and may lapse only by separately bounded operation",
            "Relation lapse is not punishment",
            "Relation dissolution is not erasure",
            "Relation reversibility is not teardown",
        ),
    ...
    ("permitted route", ((
        ...
        "Only after a future relation reversibility operation records RELATION_REVERSIBILITY_SUPPORTED may a separately bounded relation lapse boundary or presence boundary be considered",
        "No relation lapse, dissolution, presence boundary, or later operation is authorized by this operation specification alone",
    ),)),
    ...
    ("blocked routes", ((
        "direct relation reversibility operation spec to relation reversibility completion",
        "direct relation reversibility boundary allowance to relation lapse",
        "direct relation reversibility boundary allowance to relation dissolution",
        "direct relation reversibility boundary allowance to relation reversal",
        "direct relation reversibility boundary allowance to relation termination",
        "direct relation reversibility boundary allowance to relation erasure",
        "direct relation reversibility boundary allowance to relation mutation",
        "direct relation reversibility boundary allowance to relation invalidation",
        "direct relation reversibility boundary allowance to punitive lapse interpretation",
        "direct relation reversibility boundary allowance to teardown logic",
        "direct relation reversibility boundary allowance to living relation state",
        "direct relation reversibility boundary allowance to historical receipt preservation",
        "direct relation reversibility boundary allowance to presence boundary authorization",
        "direct relation reversibility boundary allowance to presence establishment",
        "direct relation reversibility operation to relation lapse without lapse boundary",
        "direct relation reversibility operation to relation dissolution without dissolution boundary",
        "direct relation reversibility operation to presence boundary without separate boundary consideration",
        ...
        "direct relation reversibility operation to follow-on work",
    ...
    ("closing lock", (
        (
            "This operation spec defines only a future relation reversibility operation shape",
            "It does not itself execute relation reversibility or record an operation result",
            ...
            "Relation reversibility operation is not relation lapse, relation dissolution, relation reversal, relation termination, relation erasure, relation mutation, relation invalidation, punishment, or teardown",
            "Relation reversibility support is not presence boundary authorization, presence, identity, coupling, FIELD machinery, runtime, currentness, authority, follow-on authorization, or follow-on work",
...
UPSTREAM_REQUIREMENTS = (
    (
        "relation_reversibility_boundary_terminal_summary_reference",
        DEFAULT_RELATION_REVERSIBILITY_BOUNDARY_TERMINAL_SUMMARY_REFERENCE,
        "RELATION_REVERSIBILITY_BOUNDARY_TERMINAL_SUMMARY_REFERENCE_MISSING",
        "RELATION_REVERSIBILITY_BOUNDARY_TERMINAL_SUMMARY_MARKER_MISSING",
        ((
            "RELATION_REVERSIBILITY_BOUNDARY_ALLOWED",
            "failed_check_count = 0",
            "passed_check_count = 307",
            "RELATION_REVERSIBILITY_OPERATION_CONSIDERATION_ALLOWED",
            "relation_reversibility_operation_consideration_allowed = true",
            "relation_operation_referenced = true",
            "relation_record_referenced = true",
        ...
        "completed_relation_reversibility_boundary_terminal_summary_markers_present",
        True,
    ),
    (
...
WHAT_REMAINS_OPEN = (
    "relation lapse boundary, if separately bounded after reversibility support",
    "relation lapse operation, if separately bounded",
    "relation dissolution boundary, if separately bounded",
    "relation dissolution operation, if separately bounded",
    "presence boundary, if separately bounded after reversibility support or lapse consideration",
    "presence",
    "FIELD machinery",
    "runtime",
    "API",
...
def _validate_target_spec(checks: list[dict[str, Any]], request: Mapping[str, Any]) -> str | None:
    reference = request.get("relation_reversibility_operation_spec_reference")
    text = _read_text(reference)
    _add_check(
        checks,
        "relation-reversibility operation specification reference readable",
        text is not None,
    ...
    if text is None:
        return "RELATION_REVERSIBILITY_OPERATION_SPEC_REFERENCE_MISSING"
    failure: str | None = None
    for name, variants in TARGET_SPEC_MARKER_CLASSES:
        passed = _markers_present(text, variants)
        _add_check(
            checks,
            f"relation-reversibility operation specification {name} markers present",
            passed,
        ...
        if not passed and failure is None:
            failure = "RELATION_REVERSIBILITY_OPERATION_SPEC_MARKER_MISSING"
    return failure
...
def _operation_result(outcome: str) -> str:
    if outcome == OUTCOME_RECORDED:
        return "RELATION_REVERSIBILITY_SUPPORTED"
...
def _operation_object(outcome: str, flags: Mapping[str, bool]) -> dict[str, Any]:
    ...
    operation: dict[str, Any] = {**EXPECTED_REQUEST_VALUES, "relation_reversibility_result": _operation_result(outcome)}
...
def _operation_material(outcome: str) -> dict[str, Any]:
    ...
    return {
        "boundary_allowance_reference": {
            "prior_relation_reversibility_boundary_type": PRIOR_RELATION_REVERSIBILITY_BOUNDARY_TYPE,
            "prior_relation_reversibility_boundary_outcome": PRIOR_RELATION_REVERSIBILITY_BOUNDARY_OUTCOME_REQUIRED,
            "prior_relation_reversibility_boundary_result": PRIOR_RELATION_REVERSIBILITY_BOUNDARY_RESULT_REQUIRED,
            "prior_relation_reversibility_operation_consideration_allowed": recorded,
            "prior_relation_operation_referenced": recorded,
            "prior_relation_record_referenced": recorded,
            "prior_relation_basis_referenced": recorded,
            "prior_relation_reversibility_authorized": False,
            "prior_relation_lapse_authorized": False,
            "prior_relation_dissolution_authorized": False,
            "prior_relation_reversibility_performed": False,
        ...
        "reversibility_result_evaluation": {
            "relation_reversibility_result": _operation_result(outcome),
            "relation_reversibility_supported": recorded,
            "relation_reversibility_authorized": recorded,
            "relation_reversibility_performed": recorded,
            "relation_reversibility_recorded": recorded,
            "relation_reversibility_id": RELATION_REVERSIBILITY_ID,
            "relation_reversibility_scope": RELATION_REVERSIBILITY_SCOPE,
            "relation_lapse_authorized": False,
            "relation_lapse_performed": False,
...
def _declared_basis_projection(request: Mapping[str, Any]) -> dict[str, Any]:
    keys = (
        "intent",
        *EXPECTED_REQUEST_VALUES,
        "relation_reversibility_operation_spec_reference",
...
def _build_summary(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "selected_relation_reversibility_operation_spec_path": upstream_map.get("relation_reversibility_operation_spec_reference"),
        "completed_relation_reversibility_boundary_terminal_summary_path": upstream_map.get("relation_reversibility_boundary_terminal_summary_reference"),
    ...
    summary["relation_reversibility_result"] = operation_map.get("relation_reversibility_result")
...
def build_relation_reversibility_operation_v0_min_summary(result: Mapping[str, Any]) -> dict[str, Any]:
...
def _build_result(
    ...
    result: dict[str, Any] = {
        ...
        "declared_relation_reversibility_operation_basis": _declared_basis_projection(request),
        "upstream_basis": {
            "relation_reversibility_operation_spec_reference": request.get("relation_reversibility_operation_spec_reference"),
            **{field: request.get(field) for field, *_ in UPSTREAM_REQUIREMENTS},
            **flags,
        },
        "relation_reversibility_operation": operation,
        "relation_reversibility_operation_material": _operation_material(outcome),
        "relation_reversibility_operation_checks": checks,
        "relation_reversibility_operation_statement": {
            "outcome": outcome,
            **{field: operation.get(field) for field in ALLOWED_TRUE_RECORDED_FIELDS},
            "relation_reversibility_result": operation["relation_reversibility_result"],
        ...
        "permitted_future_route": {
            "admissible_future_route": ADMISSIBLE_FUTURE_ROUTE,
            "relation_lapse_or_presence_boundary_requires_separate_bounded_step": True,
            "presence_boundary_authorized": False,
        },
    ...
    result["relation_reversibility_operation_summary"] = _build_summary(result)
...
def build_relation_reversibility_operation_v0_min_request(**overrides: Any) -> dict[str, Any]:
    """Build one explicit operation request without repository discovery."""
    request: dict[str, Any] = {
        "intent": INTENT_RECORD,
        **EXPECTED_REQUEST_VALUES,
        "relation_reversibility_operation_spec_reference": DEFAULT_RELATION_REVERSIBILITY_OPERATION_SPEC_REFERENCE,
...
build_declared_relation_reversibility_operation_v0_min_request = build_relation_reversibility_operation_v0_min_request


def resolve_relation_reversibility_operation_v0_min(
    declared_relation_reversibility_operation: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Resolve one relation-reversibility operation without converting relation."""
    if declared_relation_reversibility_operation is None:
        request = build_relation_reversibility_operation_v0_min_request()
    elif not isinstance(declared_relation_reversibility_operation, Mapping):
        checks: list[dict[str, Any]] = []
        _add_check(checks, "request mapping", False, "mapping", type(declared_relation_reversibility_operation).__name__, "REQUEST_NOT_MAPPING")
        return _blocked_result({}, checks, "REQUEST_NOT_MAPPING", "request is not a mapping")
    else:
        request = copy.deepcopy(dict(declared_relation_reversibility_operation))
...
def resolve_relation_reversibility_operation_v0_min_from_path(
    declared_relation_reversibility_operation_path: Path | str,
) -> dict[str, Any]:
    """Resolve one explicit JSON request path without filesystem discovery."""
    path = Path(declared_relation_reversibility_operation_path)
    try:
    ...
    return resolve_relation_reversibility_operation_v0_min(payload)


def _next_output_path(path: Path) -> Path:
    if path.exists() and path.is_dir():
        raise RelationReversibilityOperationV0MinError("WRITE_REFUSED: output path is a directory")
    if not path.exists():
        return path
    for index in range(1, 10000):
        ...
        if not candidate.exists():
            return candidate
    raise RelationReversibilityOperationV0MinError("WRITE_REFUSED: no deterministic output suffix available")


def write_relation_reversibility_operation_v0_min_result(
    result: Mapping[str, Any],
    output_path: Path | str | None = None,
) -> Path:
    """Write one operation result without overwriting a prior result."""
    if not isinstance(result, Mapping):
        raise RelationReversibilityOperationV0MinError("WRITE_REFUSED: result must be a mapping")
    ...
    try:
        ...
        raise RelationReversibilityOperationV0MinError(f"WRITE_REFUSED: {error}") from error
    return destination
