"""Bounded portable source-body verification cross-carrier evidence resolver v2.
...
The predecessor cross-carrier evidence resolver remains preserved conformance-
failure evidence. This v2 resolver keeps the same public shape while blocking
boundary overreads where string "0" was promoted into doctrine or returned
capture was promoted into cross-carrier evidence/proof.
"""
...
RESOLVER_MODULE = "resolve_portable_source_body_verification_cross_carrier_evidence_v2"
...
def _boundary_overreach_truthy(
    ...
    for section in BOUNDARY_OVERREACH_SECTIONS:
        found = _find_key(request.get(section), nested_keys)
        if _truthy(found):
            return True
