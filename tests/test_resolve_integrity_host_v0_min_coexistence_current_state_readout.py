"""Bounded tests for the v0-min current-state readout resolver.
...
resolutions, and current-work-operation results emitted into temporary roots.

The suite verifies one bounded downstream readout over an explicit completed
current-work-operation result only. It does not test replay, merge, persistence
architecture, registry integration, distributed continuity, CLI behavior,
dashboard behavior, or broad reporting frameworks.
"""
...
EXPECTED_TOP_LEVEL_KEYS = {
    "readout_metadata",
    "selected_current_work_operation",
    "effective_readout_inputs",
    "checks",
    "outcome",
    "block",
    "readout_output",
...
class IntegrityHostV0MinCoexistenceCurrentStateReadoutTests(unittest.TestCase):
    def emit_source_run(
        self,
        temp_root: Path,
    ...
    def valid_proposal(
        ...
        return {
            "transition_proposal_id": "proposal_accepted_for_current_state_readout",
            ...
            "proposal_basis_ref": "bounded explicit current-state-readout test basis",
    ...
    def resolve_readout_default(self, temp_root: Path) -> dict[str, Any]:
        with mock.patch.object(readout_resolver, "_repo_root", return_value=temp_root):
            return readout_resolver.resolve_current_state_readout()

    def resolve_readout_object(
        self,
        temp_root: Path,
        current_work_operation: dict[str, Any],
    ) -> dict[str, Any]:
        with mock.patch.object(readout_resolver, "_repo_root", return_value=temp_root):
            return readout_resolver.resolve_current_state_readout(
    ...
    def resolve_readout_from_path(
        self,
        temp_root: Path,
        operation_path: Path,
    ) -> dict[str, Any]:
        with mock.patch.object(readout_resolver, "_repo_root", return_value=temp_root):
            return readout_resolver.resolve_current_state_readout_from_path(
                operation_path
            )
    ...
    def assert_result_shape(self, result: dict[str, Any]) -> None:
        ...
        self.assertIsInstance(result["readout_metadata"], dict)
        self.assertIsInstance(result["selected_current_work_operation"], dict)
        self.assertIsInstance(result["effective_readout_inputs"], dict)
        self.assertIsInstance(result["checks"], list)
        self.assertIsInstance(result["block"], dict)
        self.assertIsInstance(result["readout_summary"], dict)
    ...
    def assert_emitted_result(self, result: dict[str, Any]) -> None:
        ...
        self.assertEqual(result["outcome"], readout_resolver.OUTCOME_EMITTED)
        self.assertIsNone(result["block"]["block_code"])
        self.assertIsNone(result["block"]["block_reason"])
        self.assertIsInstance(result["readout_output"], dict)
        self.assertEqual(set(result["effective_readout_inputs"]), READOUT_INPUT_KEYS)
        for value in result["effective_readout_inputs"].values():
    ...
    def assert_blocked_result(
        ...
        *,
        ...
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], readout_resolver.OUTCOME_BLOCKED)
        ...
        self.assertIsNone(result["readout_output"])
        if expect_non_claims_false:
            self.assert_non_claims_false(result)

    def assert_readout_inputs_match_operation(
        self,
        temp_root: Path,
        result: dict[str, Any],
        operation: dict[str, Any],
    ) -> None:
        for key in READOUT_INPUT_KEYS:
            self.assert_paths_same(
                temp_root,
                result["effective_readout_inputs"][key],
                operation["effective_inputs"][key],
            )

    def assert_under_readout_root(self, temp_root: Path, path: Path) -> None:
        root = (temp_root / readout_resolver.CURRENT_STATE_READOUT_ROOT).resolve()
        try:
            path.resolve().relative_to(root)
        except ValueError as exc:
            self.fail(f"{path} was not written under {root}: {exc}")

    def parse_effective_readout_files(
        self,
        temp_root: Path,
        result: dict[str, Any],
    ...
    def test_real_emitted_current_state_readout_default_selection(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_readout_default(temp_root)

            self.assert_emitted_result(result)
            selected = result["selected_current_work_operation"]
            self.assert_paths_same(
                temp_root,
                selected["current_work_operation_result_path"],
                stack["operation_path"],
            )
            self.assertEqual(
                selected["current_work_operation_result_id"],
                stack["operation_result"]["work_operation_metadata"]["work_result_id"],
            )
            self.assertEqual(selected["outcome"], operation_resolver.OUTCOME_COMPLETED)
            self.assertEqual(
                set(selected),
                {
            ...
            self.assertIsNone(selected["selected_current_work_input_id"])
            self.assert_readout_inputs_match_operation(
                temp_root,
                result,
                stack["operation_result"],
            )
            metadata = result["readout_metadata"]
            for key in (
                "readout_result_id",
                "readout_result_type",
    ...
    def test_resolution_from_path_and_object_are_consistent(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            object_result = self.resolve_readout_object(
                temp_root,
                stack["operation_result"],
            )
            path_result = self.resolve_readout_from_path(
                temp_root,
                stack["operation_path"],
            )
            ...
            self.assertIsNone(
                object_result["selected_current_work_operation"][
                    "current_work_operation_result_path"
                ]
            )
            self.assert_paths_same(
                temp_root,
                path_result["selected_current_work_operation"][
                    "current_work_operation_result_path"
                ],
                stack["operation_path"],
            )
            ...
            self.assertEqual(
                object_result["effective_readout_inputs"],
                path_result["effective_readout_inputs"],
            )
            self.assertEqual(
                object_result["readout_output"],
                path_result["readout_output"],
            )
            self.assertEqual(
                object_result["readout_summary"],
                path_result["readout_summary"],
            )

    def test_effective_inputs_checks_readout_output_and_summary(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_readout_object(
                temp_root,
                stack["operation_result"],
            )
            ...
            self.assertEqual(set(result["readout_output"]), READOUT_OUTPUT_KEYS)
            output = result["readout_output"]
            operation_output = stack["operation_result"]["work_output"]
            self.assert_paths_same(
                temp_root,
                output["current_governing_source_run_path"],
                operation_output["current_governing_source_run_path"],
            )
            self.assert_paths_same(
                temp_root,
                output["current_governing_ingress_run_path"],
                operation_output["current_governing_ingress_run_path"],
            )
            for output_key, input_key in (
                ...
                self.assert_paths_same(
                    temp_root,
                    output[output_key],
                    result["effective_readout_inputs"][input_key],
                )
            ...
            self.assertEqual(
                output["current_work_basis"],
                operation_output["current_work_basis"],
            )

            summary = readout_resolver.build_current_state_readout_summary(result)
            for key in (
                "readout_result_id",
                "outcome",
                "block_code",
                "block_reason",
                "selected_current_work_operation_id",
                "effective_authority_artifact_path",
                "effective_family_packet_path",
                "effective_status_packet_path",
                "effective_current_governing_packet_path",
            ...
            self.assertEqual(summary["outcome"], readout_resolver.OUTCOME_EMITTED)
            self.assertIsNone(summary["block_code"])
            self.assertIsNone(summary["block_reason"])
            self.assertEqual(
                summary["selected_current_work_operation_id"],
                stack["operation_result"]["work_operation_metadata"]["work_result_id"],
            )
    ...
    def test_emitted_semantics_preserve_prior_family(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            ...
            for effective_key, prior_key in (
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        result["effective_readout_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
            ...
            parsed = self.parse_effective_readout_files(temp_root, result)
            ...
            self.assertEqual(
                result["readout_output"]["readout_basis"],
                "completed_current_work_operation",
            )
            self.assertEqual(
                result["readout_output"]["current_work_basis"],
                effective_resolver.BASIS_ADOPTED_SUCCESSOR_FAMILY_EFFECTIVE,
            )

    def test_no_current_work_operation_result_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()

            result = self.resolve_readout_default(temp_root)

            self.assert_blocked_result(result, "NO_CURRENT_WORK_OPERATION_RESULT")
            selected = result["selected_current_work_operation"]
            self.assertIsNone(selected["current_work_operation_result_id"])
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            stack = self.build_multi_run_stack(
                temp_root,
                write_work_operation_artifact=False,
            )
            blocked_operation = self.copy_payload(stack["operation_result"])
            blocked_operation["outcome"] = operation_resolver.OUTCOME_BLOCKED
            blocked_operation["block"] = {
                "block_code": "TEST_BLOCKED_OPERATION",
                "block_reason": "Blocked current-work-operation fixture.",
            }
            blocked_operation["work_output"] = None
            root = temp_root / operation_resolver.CURRENT_WORK_OPERATION_ROOT
            self.write_json(root / "blocked_current_work_operation.json", blocked_operation)

            result = self.resolve_readout_default(temp_root)

            self.assert_blocked_result(result, "NO_CURRENT_WORK_OPERATION_RESULT")

    def test_current_work_operation_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            missing_path = temp_root / "missing_current_work_operation.json"

            result = self.resolve_readout_from_path(temp_root, missing_path)

            self.assert_blocked_result(result, "CURRENT_WORK_OPERATION_UNREADABLE")

        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            malformed_path = temp_root / "malformed_current_work_operation.json"
            malformed_path.write_text("[1, 2, 3]\n", encoding="utf-8")

            with self.assertRaises(readout_resolver.CurrentStateReadoutError):
                self.resolve_readout_from_path(temp_root, malformed_path)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = self.resolve_readout_object(temp_root, blocked_operation)

            self.assert_blocked_result(result, "CURRENT_WORK_OPERATION_NOT_COMPLETED")

    def test_effective_readout_artifact_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(
                temp_root,
            ...
            operation = self.copy_payload(stack["operation_result"])
            missing_path = self.resolve_display_path(
                temp_root,
                operation["effective_inputs"]["effective_status_packet_path"],
            )
            missing_path.unlink()

            result = self.resolve_readout_object(temp_root, operation)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            malformed_path = self.resolve_display_path(
                temp_root,
                operation["effective_inputs"]["effective_authority_artifact_path"],
            )
            malformed_path.write_text("{not valid json\n", encoding="utf-8")

            with self.assertRaises(readout_resolver.CurrentStateReadoutError):
                self.resolve_readout_object(temp_root, operation)
    ...
    def test_canonical_execution_line_mismatch_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            family_path = self.resolve_display_path(
                temp_root,
                operation["effective_inputs"]["effective_family_packet_path"],
            )
    ...
    def test_stale_prior_family_fallback_is_not_used(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            emitted = self.resolve_readout_object(
                temp_root,
                stack["operation_result"],
            )
            self.assert_emitted_result(emitted)
            for effective_key, prior_key in (
                ("effective_authority_artifact_path", "prior_authority_artifact_path"),
                ("effective_family_packet_path", "prior_family_packet_path"),
                ("effective_status_packet_path", "prior_status_packet_path"),
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        emitted["effective_readout_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
                        stack["effective_result"]["prior_current_family"][prior_key],
            ...
            stale = self.copy_payload(stack["operation_result"])
            stale["effective_inputs"].update(
                {
            ...
            blocked = self.resolve_readout_object(temp_root, stale)
    ...
    def test_non_claim_shortcuts_block_readout(self) -> None:
        cases = (
    ...
    def test_multiple_current_work_operation_result_conflict_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            conflict = self.copy_payload(stack["operation_result"])
            conflict["work_operation_metadata"]["work_result_id"] = (
                "conflicting_current_work_operation"
            )
            conflict["effective_inputs"][
                "effective_current_governing_packet_path"
            ] = "artifacts/conflicting_effective_governing_packet.json"
            conflict_path = (
                temp_root
                / operation_resolver.CURRENT_WORK_OPERATION_ROOT
                / "conflicting_current_work_operation.json"
            )
            ...
            self.assert_blocked_result(
                result,
                "MULTIPLE_CURRENT_WORK_OPERATION_RESULTS_CONFLICT_UNRESOLVED",
            )

    def test_hard_malformed_artifact_failures(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            root_file = temp_root / operation_resolver.CURRENT_WORK_OPERATION_ROOT
            root_file.parent.mkdir(parents=True, exist_ok=True)
            root_file.write_text("not a directory\n", encoding="utf-8")

            with self.assertRaises(readout_resolver.CurrentStateReadoutError):
                self.resolve_readout_default(temp_root)

        malformed_operation_cases = (
            ("non_mapping_input", "not a mapping"),
            (
                "missing_effective_inputs",
                {"outcome": operation_resolver.OUTCOME_COMPLETED},
            ),
        )
        for label, payload in malformed_operation_cases:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    with self.assertRaises(readout_resolver.CurrentStateReadoutError):
                        with mock.patch.object(
                            ...
                            readout_resolver.resolve_current_state_readout(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            operation = self.copy_payload(stack["operation_result"])
            operation["effective_inputs"]["effective_family_packet_path"] = 42
    ...
    def test_write_behavior_and_default_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_path = temp_root / "nested" / "current_state_readout" / "result.json"

            with mock.patch.object(readout_resolver, "_repo_root", return_value=temp_root):
                written = readout_resolver.write_current_state_readout_result(
                    result,
                    output_path,
                )
            ...
            with mock.patch.object(readout_resolver, "_repo_root", return_value=temp_root):
                with self.assertRaises(FileExistsError):
                    readout_resolver.write_current_state_readout_result(
                        result,
                        output_path,
                    )

            full_readout_id = result["readout_metadata"]["readout_result_id"]
            with mock.patch.object(readout_resolver, "_repo_root", return_value=temp_root):
                default_first = readout_resolver.write_current_state_readout_result(
                    result
                )
                default_second = readout_resolver.write_current_state_readout_result(
                    result
                )
            ...
            self.assert_under_readout_root(temp_root, default_first)
            self.assert_under_readout_root(temp_root, default_second)
            self.assertEqual(default_first.name, "current_state_readout_result.json")
            self.assertEqual(default_second.name, "current_state_readout_result_001.json")
            self.assertLess(len(default_first.name), 255)
            self.assertLess(len(default_second.name), 255)
            self.assertNotIn(full_readout_id, default_first.name)
            self.assertNotIn(full_readout_id, default_second.name)

            parsed_default = self.read_json(default_first)
            self.assertEqual(set(parsed_default), EXPECTED_TOP_LEVEL_KEYS)
            self.assertEqual(
                parsed_default["readout_metadata"]["readout_result_id"],
                full_readout_id,
            )
            self.assertEqual(
                parsed_default["readout_metadata"]["readout_result_version"],
                readout_resolver.READOUT_RESULT_VERSION,
            )
    ...
    def test_non_mutation_posture(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = self.resolve_readout_object(
                temp_root,
                stack["operation_result"],
            )
            after_first = {
            ...
            second = self.resolve_readout_from_path(
                temp_root,
                stack["operation_path"],
            )
            after_second = {
            ...
            self.assertEqual(
                first["selected_current_work_operation"][
                    "current_work_operation_result_id"
                ],
                second["selected_current_work_operation"][
                    "current_work_operation_result_id"
                ],
            )
            self.assertEqual(
                first["effective_readout_inputs"],
                second["effective_readout_inputs"],
            )
            self.assertEqual(first["readout_output"], second["readout_output"])
            self.assertEqual(first["readout_summary"], second["readout_summary"])
