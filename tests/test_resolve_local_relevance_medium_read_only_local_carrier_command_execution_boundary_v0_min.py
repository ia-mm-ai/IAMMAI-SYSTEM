"""Tests for the local read-only local carrier command execution boundary resolver.

The suite keeps the target boundary-shaped, local, read-only, single-command,
and non-execution-shaped. It verifies that one clean local carrier command
surface artifact may support one future command execution consideration boundary
without executing commands, returning state, performing lookup, or creating any
operation/runtime/API/distributed/follow-on posture.
"""
...
class LocalCarrierCommandExecutionBoundaryV0MinTests(unittest.TestCase):
    ...
    def assert_recorded_result_core(
        ...
        self.assertEqual(
            Path(boundary["basis_local_carrier_command_surface_artifact"]),
            surface_path,
        )
        self.assertEqual(
    ...
    def test_public_api_constants_and_default_builder(self) -> None:
        ...
        self.assertEqual(resolver.RESOLVER_MODULE, Path(__file__).stem.removeprefix("test_"))
