"""Tests for the local read-only runtime-held-re-entry boundary resolver.

This suite is bounded to one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY object. It
verifies that the resolver reads one clean runtime-held-state artifact, one
clean runtime-held-state boundary artifact, one clean runtime v3 artifact, one
clean runtime boundary v2 artifact, one clean runtime permission artifact, and
one clean operation execution artifact, then records one local read-only
selected-state runtime-held-re-entry boundary object only.

The boundary remains selected-state-runtime-held-reentry-consideration-only,
raw-state-body-excluding, state-mutation-refusing, state-update-refusing,
closure-token-aware, non-reentry-shaped, non-prior-result-cycle-shaped,
non-second-operation-shaped, non-continuation-shaped, non-hosting-shaped,
...
import resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min as resolver  # noqa: E402
...
EXPECTED_WRAPPER_SECTIONS = (
    "local_relevance_medium_read_only_runtime_held_reentry_boundary_metadata",
    "declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_question",
    ...
    "local_relevance_medium_read_only_runtime_held_reentry_boundary_checks",
    "local_relevance_medium_read_only_runtime_held_reentry_boundary_statement",
    "local_relevance_medium_read_only_runtime_held_reentry_boundary_non_meaning",
    ...
    "outcome",
    "block",
    "local_relevance_medium_read_only_runtime_held_reentry_boundary_summary",
)
...
class LocalRelevanceMediumReadOnlyRuntimeHeldReentryBoundaryV0MinTests(
    unittest.TestCase
):
    ...
    def summary(self, result: Mapping[str, Any]) -> Mapping[str, Any]:
        summary = resolver.build_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_summary(
            result
        )
    ...
    def assert_runtime_held_reentry_boundary_non_claims(
        ...
        self.assertIs(result["non_claims"]["runtime_held_reentry_created"], False)
        self.assertIs(result["non_claims"]["prior_result_reentry_cycle_created"], False)
        self.assertIs(result["non_claims"]["second_operation_created"], False)
        self.assertIs(result["non_claims"]["continuation_created"], False)
        self.assertIs(result["non_claims"]["runtime_hosting_created"], False)
        self.assertIs(result["non_claims"]["runtime_loop_created"], False)
        self.assertIs(result["non_claims"]["daemon_behavior_created"], False)
        self.assertIs(result["non_claims"]["raw_state_body_embedded"], False)
        self.assertIs(result["non_claims"]["state_mutation_performed"], False)
        self.assertIs(result["non_claims"]["state_update_performed"], False)
        self.assertIs(
            result["non_claims"]["older_runtime_lineage_imported_as_authority"],
            False,
        )
        self.assertIs(
            result["non_claims"]["older_runtime_permission_treated_as_current"],
            False,
        )
    ...
    def assert_blocked_with_public_code(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_runtime_held_reentry_boundary_non_claims(result)
    ...
    def assert_recorded_boundary_posture(self, result: Mapping[str, Any]) -> None:
        ...
        self.assertEqual(
            ...
            "local_relevance_medium_read_only_runtime_held_reentry_boundary_001",
        )
        ...
        self.assertEqual(boundary["basis_runtime_held_state_outcome"], RUNTIME_HELD_STATE_OUTCOME)
        self.assertEqual(boundary["basis_runtime_held_state_result_version"], "0.1.0")
        self.assertEqual(boundary["basis_runtime_held_state_failed_check_count"], 0)
        self.assertEqual(
            boundary["basis_runtime_held_state_boundary_outcome"],
            RUNTIME_HELD_STATE_BOUNDARY_OUTCOME,
        )
        self.assertEqual(
            boundary["basis_runtime_held_state_boundary_result_version"], "0.1.0"
        )
        self.assertEqual(
            boundary["basis_runtime_held_state_boundary_failed_check_count"], 0
        )
        ...
        self.assertEqual(boundary["basis_runtime_boundary_outcome"], RUNTIME_BOUNDARY_OUTCOME)
    ...
    def base_false_fields(self) -> dict[str, bool]:
        return {key: False for key in BOUNDARY_FALSE_FIELDS}
    ...
    def build_request_for_paths(self, paths: Mapping[str, Path]) -> dict[str, Any]:
        return resolver.build_declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_request(
            local_relevance_medium_read_only_runtime_held_reentry_boundary_id=(
                "local_relevance_medium_read_only_runtime_held_reentry_boundary_001"
            ),
    ...
    def resolve_valid_synthetic(self, root: Path) -> dict[str, Any]:
        ...
        return resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
            request
        )
    ...
    def test_public_api_constants_and_default_builder(self) -> None:
        for name in (
            "resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min",
            "resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_from_path",
            "write_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_result",
            "build_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_summary",
            "build_declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_request",
        ):
        ...
        self.assertTrue(
            str(resolver.OUTPUT_ROOT).endswith(
                "artifacts/integrity_host_v0_min_coexistence_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min"
            )
        )
        ...
        request = resolver.build_declared_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_request()
        self.assertTrue(
    ...
    def test_successful_recorded_result_from_synthetic_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
                request
            )
        ...
        self.assert_same_or_stable_artifact_path(
            boundary["basis_runtime_held_state_artifact"], paths["held_state"]
        )
        self.assert_same_or_stable_artifact_path(
            boundary["basis_runtime_held_state_boundary_artifact"],
            paths["held_state_boundary"],
        )
        self.assert_same_or_stable_artifact_path(
            boundary["basis_runtime_artifact"], paths["runtime"]
        )
        self.assert_same_or_stable_artifact_path(
            boundary["basis_runtime_boundary_artifact"], paths["runtime_boundary"]
        )
        self.assert_same_or_stable_artifact_path(
    ...
    def test_successful_recorded_result_from_default_artifacts_if_present(self) -> None:
        ...
        self.assert_canonical_false_non_claims(result)
        self.assert_same_or_stable_artifact_path(
            boundary["basis_runtime_held_state_artifact"],
        ...
        self.assert_same_or_stable_artifact_path(
            boundary["basis_runtime_artifact"],
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for index, (key, expected_code) in enumerate(EXACT_TOP_LEVEL_BLOCKS.items()):
                with self.subTest(top_level=key):
                    request, _, _ = self.build_valid_synthetic_request(base_root / f"top_{index}")
                    request[key] = True
                    result = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
                        request
                    )
    ...
    def test_representative_blocking_behavior(self) -> None:
        def no_mutation(request: dict[str, Any], _root: Path) -> dict[str, Any]:
            return request
        ...
        cases: list[tuple[str, Callable[[dict[str, Any], Path], dict[str, Any]]]] = [
            (
                ...
                lambda request, _root: {
                    **request,
                    "local_relevance_medium_read_only_runtime_held_reentry_boundary_intent": (
                        "BLOCK_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_REENTRY_BOUNDARY"
                    ),
                },
            ),
            ("unsupported intent", lambda request, _root: {**request, "local_relevance_medium_read_only_runtime_held_reentry_boundary_intent": "UNSUPPORTED"}),
            ("runtime-held-state artifact path missing", lambda request, _root: {**request, "selected_runtime_held_state_artifact": ""}),
            ("runtime-held-state artifact unreadable", lambda request, root: {**request, "selected_runtime_held_state_artifact": str(root / "absent.json")}),
            ("runtime-held-state boundary artifact path missing", lambda request, _root: {**request, "selected_runtime_held_state_boundary_artifact": ""}),
            ("runtime artifact path missing", lambda request, _root: {**request, "selected_runtime_artifact": ""}),
            ("runtime boundary artifact path missing", lambda request, _root: {**request, "selected_runtime_boundary_artifact": ""}),
            ("runtime permission artifact path missing", lambda request, _root: {**request, "selected_runtime_permission_artifact": ""}),
            ("operation execution artifact path missing", lambda request, _root: {**request, "selected_operation_execution_artifact": ""}),
            ("selected command missing", lambda request, _root: {**request, "selected_command": ""}),
            ("selected command not state", lambda request, _root: {**request, "selected_command": "lookup"}),
            ("boundary type missing", lambda request, _root: {key: value for key, value in request.items() if key != "boundary_type"}),
            ("boundary type wrong", lambda request, _root: {**request, "boundary_type": "LOCAL_RELEVANCE_MEDIUM_RUNTIME_HOSTING"}),
            ("boundary scope missing", lambda request, _root: {key: value for key, value in request.items() if key != "boundary_scope"}),
            ("boundary scope wrong", lambda request, _root: {**request, "boundary_scope": "SELECTED_RUNTIME_LOOP_ONLY"}),
        ]
        shortcut_cases = (
            ...
            "consumed_request_reopened",
            "authorization_token_reused",
            "artifact_existence_treated_as_runtime_held_reentry_boundary_authority",
            "latest_file_posture_treated_as_runtime_held_reentry_boundary_authority",
            "repo_local_availability_treated_as_runtime_held_reentry_boundary_authority",
            "hidden_repo_state_used_as_runtime_held_reentry_boundary_content",
            "hidden_repo_state_used_as_runtime_held_reentry_boundary_authority",
            "predecessor_failure_repaired",
            "predecessor_failure_hidden",
            "predecessor_failure_claimed_passed",
        )
                    ...
                    lambda request, _root: {
                        **request,
                        "declared_non_claims": {
                            ...
                            if key != "runtime_held_reentry_created"
                        },
                    },
                ),
                (
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            malformed_results = [
                resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
                    None
                ),
                resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
            ...
            for index, (name, mutate_request) in enumerate(cases):
                with self.subTest(name=name):
                    ...
                    request = mutate_request(copy.deepcopy(request), case_root)
                    result = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
                        request
                    )
                    self.assert_blocked_with_public_code(result)
            ...
            artifact_cases = [
                ("held_state", "outcome", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE_NOT_RECORDED"),
                ("held_state_boundary", "outcome", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE_BOUNDARY_NOT_RECORDED"),
                ("runtime", "outcome", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_NOT_RECORDED"),
                ("runtime_boundary", "outcome", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_BOUNDARY_NOT_RECORDED"),
                ("runtime_permission", "outcome", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_PERMISSION_NOT_RECORDED"),
                ("operation_execution", "outcome", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_OPERATION_EXECUTION_NOT_RECORDED"),
                ("held_state", "failed_check_count", 1),
                ("held_state_boundary", "failed_check_count", 1),
                ("runtime", "failed_check_count", 1),
                ("runtime_boundary", "failed_check_count", 1),
                ("runtime_permission", "failed_check_count", 1),
                ("operation_execution", "failed_check_count", 1),
                ("held_state", "result_version", "9.9.9"),
                ("held_state_boundary", "result_version", "9.9.9"),
                ("runtime", "result_version", "9.9.9"),
                ("runtime_boundary", "result_version", "9.9.9"),
                ("runtime_permission", "result_version", "9.9.9"),
                ("operation_execution", "result_version", "9.9.9"),
            ]
            ...
            for index, request_key in enumerate(
                (
    ...
    def test_raw_hidden_older_runtime_hostile_content_containment(self) -> None:
        def inject_sentinels(artifacts: dict[str, dict[str, Any]]) -> None:
            ...
            for index, (artifact_key, sensitive_key) in enumerate(sensitive_keys.items()):
                ...
                object_key = next(
                    key
                    for key in artifacts[artifact_key]
                    if key.startswith("local_relevance_medium_read_only_")
                    and not key.endswith("_statement")
                    and not key.endswith("_summary")
                    and not key.endswith("_checks")
                )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            request["raw_runtime_held_reentry_boundary_body"] = HOSTILE_SENTINELS[0]
            request["raw_runtime_held_reentry_body"] = HOSTILE_SENTINELS[1]
            request["raw_prior_result_reentry_body"] = HOSTILE_SENTINELS[14]
            request["hidden_repo_state"] = HOSTILE_SENTINELS[-1]
            request["older_runtime_authority_probe"] = HOSTILE_SENTINELS[-2]
            result = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
                request
            )
        ...
        self.assertEqual(request, original_request | {
            "raw_runtime_held_reentry_boundary_body": HOSTILE_SENTINELS[0],
            "raw_runtime_held_reentry_body": HOSTILE_SENTINELS[1],
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_from_path(
                request_path
            )
            self.assertEqual(result["outcome"], resolver.OUTCOME_RECORDED)
            ...
            malformed = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_from_path(
                malformed_path
            )
            self.assert_blocked_with_public_code(malformed)
            array_path = self.write_json(root / "array_request.json", [])
            array_result = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_from_path(
                array_path
            )
            self.assert_blocked_with_public_code(array_result)
            missing_result = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_from_path(
            ...
            with mock.patch.object(resolver, "OUTPUT_ROOT", output_root):
                output_path = resolver.write_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_result(
                    result
                )
                output_path_second = resolver.write_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min_result(
                    result
                )
            ...
            self.assertIn(
                "local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min",
    ...
    def test_non_mutation(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            request["raw_runtime_loop_body"] = HOSTILE_SENTINELS[9]
            original_request = copy.deepcopy(request)
        ...
        for key, expected_code in (
            ...
            with self.subTest(key=key):
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    request[key] = True
                    blocked = resolver.resolve_local_relevance_medium_read_only_runtime_held_reentry_boundary_v0_min(
                        request
                    )
