"""V2 tests for the candidate non-cosmetic scope-division declaration boundary.

V1 is preserved as failed lineage: it exposed canonical false non-claim gaps
and a brittle default-live marker acceptance failure. This suite targets only
the additive v2 resolver. It verifies one boundary result, clean
REQUIRES_ADDITIONAL_BASIS preservation, missing upstream scope classes, future
declaration-operation shape only, no scope declaration, no emission, no reruns,
no standing/runtime/authority/follow-on, canonical false non-claims, sanitizer
posture, and v2 output/write behavior.
"""
...
import resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2 as resolver


class CandidateNonCosmeticScopeDivisionDeclarationBoundaryV2Tests(unittest.TestCase):
    V2_REPAIR_KEYS = (
        "standing_authorized",
        "crossing_authorized",
    ...
    def assert_v2_repair_keys_false(self, result: dict) -> None:
        non_claims = result.get("non_claims")
        self.assertIsInstance(non_claims, dict)
        summary = self.summary(result)
        for key in self.V2_REPAIR_KEYS:
            self.assertIn(key, non_claims)
            self.assertIs(non_claims[key], False, key)
    ...
    def assert_blocked_with_public_code(self, result: dict) -> None:
        ...
        self.assert_v2_repair_keys_false(result)
    ...
    def valid_request(self, base: Path, include_sentinels: bool = False) -> dict:
        ...
        return resolver.build_declared_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_request(
    ...
    def test_public_api_and_constants_v2(self) -> None:
        for name in (
            "resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2",
            "resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_from_path",
            "write_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_result",
            "build_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_summary",
            "build_declared_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_request",
        ):
        ...
        for key in self.REQUIRED_FALSE_KEYS:
        ...
        for code in self.PUBLIC_BLOCK_CODES:
            self.assertIn(code, resolver.BLOCK_CODES, code)

    def test_default_synthetic_basis_records_boundary_v2(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            request = self.valid_request(Path(temp_dir))
            result = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2(request)
    ...
    def test_records_default_live_target_if_present_v2(self) -> None:
        required = [
        ...
        request = resolver.build_declared_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_request()
        result = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2(request)
        self.assert_recorded_boundary_posture(result)

    def test_do_not_record_intent_does_not_record_boundary_v2(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            request = self.valid_request(Path(temp_dir))
    ...
    def test_block_intent_blocks_v2(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            request = self.valid_request(Path(temp_dir))
            request["candidate_non_cosmetic_scope_division_declaration_boundary_intent"] = (
    ...
    def test_request_shape_and_blocking_behavior_v2(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base_request = self.valid_request(Path(temp_dir))
            non_mapping = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2([])
    ...
    def test_required_false_top_level_posture_blocks_v2(self) -> None:
        fields = tuple(dict.fromkeys((*self.REQUIRED_FALSE_KEYS, "relation_authorized", "field_machinery_authorized")))
        with tempfile.TemporaryDirectory() as temp_dir:
            base_request = self.valid_request(Path(temp_dir))
            for field in fields:
    ...
    def test_sanitizer_behavior_v2(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            recorded = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2(request)
            ...
            blocked = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2(blocked_request)
    ...
    def test_from_path_and_write_behavior_v2(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            request = self.valid_request(base)
            request_path = base / "request.json"
            request_path.write_text(json.dumps(request, indent=2, sort_keys=True), encoding="utf-8")
            result = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_from_path(request_path)
            self.assert_recorded_boundary_posture(result)
            missing = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_from_path(base / "missing.json")
            self.assert_blocked_with_public_code(missing)
            malformed = base / "malformed.json"
            malformed.write_text("{", encoding="utf-8")
            malformed_result = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_from_path(malformed)
            self.assert_blocked_with_public_code(malformed_result)
            array_path = base / "array.json"
            array_path.write_text("[]", encoding="utf-8")
            array_result = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_from_path(array_path)
            self.assert_blocked_with_public_code(array_result)
            output_dir = base / "integrity_host_v0_min_coexistence_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2"
            first = resolver.write_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_result(result, output_dir)
            second = resolver.write_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_result(result, output_dir)
            self.assertTrue(first.exists())
            self.assertTrue(second.exists())
            self.assertNotEqual(first, second)
            self.assertIn("candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_result", first.name)
            self.assertIn(
    ...
    def test_summary_behavior_v2(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            request = self.valid_request(Path(temp_dir))
            result = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2(request)
            summary = resolver.build_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_summary(result)
    ...
    def test_smoke_behavior_default_boundary_recorded_v2(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            request = self.valid_request(Path(temp_dir))
            result = resolver.resolve_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2(request)
            summary = resolver.build_descendant_body_candidate_non_cosmetic_scope_division_declaration_boundary_v0_min_v2_summary(result)
