"""V2 tests for the local relevance medium second bounded relevance reception resolver.

This suite preserves the substantive v1 test intent while correcting one
over-broad output-root assertion. V1 remains preserved over-strict failed test
evidence: it rejected the lawful second reception output root because that root
contains the substring ``bounded_relevance_reception_v0_min``.

The v2 suite is bounded to one second reception object. It verifies that the
...
class LocalRelevanceMediumSecondBoundedRelevanceReceptionTests(unittest.TestCase):
    ...
    def assert_output_root_is_second_reception_only(self) -> None:
        ...
        if output_root_path.is_absolute():
            artifacts_root = output_root_path.parent
            for forbidden_root in FORBIDDEN_EXACT_OUTPUT_ROOTS:
                forbidden_sibling = artifacts_root / forbidden_root.name
                self.assertNotEqual(_path_text(output_root_path), _path_text(forbidden_sibling))

    def assert_path_under_second_reception_output_root_only(
        self,
        output_path: Path,
        output_root: Path,
    ) -> None:
    ...
    def test_public_api_and_constants(self) -> None:
        ...
        self.assert_output_root_is_second_reception_only()
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assert_path_under_second_reception_output_root_only(first_path, patched_root)
            self.assert_path_under_second_reception_output_root_only(second_path, patched_root)
