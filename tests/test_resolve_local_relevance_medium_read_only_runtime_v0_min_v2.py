"""Tests for the local read-only selected-state runtime v2 successor.

This suite treats v2 as additive successor evidence. It preserves the runtime
v0 default-live-artifact extraction mismatch and the runtime-boundary v0
selected-operation-execution recorded-alias ambiguity as failed lineage.

The suite verifies that v2 records one LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME
...
import resolve_local_relevance_medium_read_only_runtime_v0_min_v2 as resolver  # noqa: E402
...
class LocalRelevanceMediumReadOnlyRuntimeV2Tests(unittest.TestCase):
    ...
    def assert_v0_lineage_preserved(self, result: Mapping[str, Any]) -> None:
        runtime = self.runtime(result)
    ...
    def assert_recorded_runtime(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_canonical_false_non_claims(result)
        self.assert_v0_lineage_preserved(result)
    ...
    def clean_request(
        ...
        return resolver.build_declared_local_relevance_medium_read_only_runtime_v0_min_v2_request(
    ...
    def test_public_api_constants_and_default_builder(self) -> None:
        for name in (
            ...
            "resolve_local_relevance_medium_read_only_runtime_v0_min_v2_from_path",
            "write_local_relevance_medium_read_only_runtime_v0_min_v2_result",
            "build_local_relevance_medium_read_only_runtime_v0_min_v2_summary",
            "build_declared_local_relevance_medium_read_only_runtime_v0_min_v2_request",
        ):
        ...
        request = resolver.build_declared_local_relevance_medium_read_only_runtime_v0_min_v2_request()
        self.assertTrue(
    ...
    def test_successful_recorded_result_from_synthetic_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = resolver.resolve_local_relevance_medium_read_only_runtime_v0_min_v2(
                request
            )
    ...
    def test_successful_recorded_result_from_default_artifacts_if_present(self) -> None:
        ...
        if not all(path.exists() for path in defaults):
            self.skipTest("default live runtime v2 basis artifacts are not present")
        request = resolver.build_declared_local_relevance_medium_read_only_runtime_v0_min_v2_request()
        result = resolver.resolve_local_relevance_medium_read_only_runtime_v0_min_v2(
            request
        )
    ...
    def test_closure_runtime_import_and_v0_lineage_blocking(self) -> None:
        exact_cases = (
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = resolver.resolve_local_relevance_medium_read_only_runtime_v0_min_v2_from_path(
                request_path
            )
            self.assert_recorded_runtime(result)
            malformed_path = tmp_path / "malformed.json"
            malformed_path.write_text("{", encoding="utf-8")
            malformed = resolver.resolve_local_relevance_medium_read_only_runtime_v0_min_v2_from_path(
                malformed_path
            )
            self.assert_blocked_with_public_code(malformed)
            array_path = self.write_json(tmp_path / "array.json", [])
            array_result = resolver.resolve_local_relevance_medium_read_only_runtime_v0_min_v2_from_path(
                array_path
            )
            self.assert_blocked_with_public_code(array_result)
            missing_result = resolver.resolve_local_relevance_medium_read_only_runtime_v0_min_v2_from_path(
                tmp_path / "missing.json"
            )
            self.assert_blocked_with_public_code(missing_result)
            output_root = tmp_path / "local_relevance_medium_read_only_runtime_v0_min_v2"
            with mock.patch.object(resolver, "OUTPUT_ROOT", output_root):
                written = resolver.write_local_relevance_medium_read_only_runtime_v0_min_v2_result(
                    result
                )
                written_again = resolver.write_local_relevance_medium_read_only_runtime_v0_min_v2_result(
                    result
                )
            self.assertTrue(written.exists())
            self.assertTrue(written_again.exists())
            self.assertNotEqual(written, written_again)
            self.assertIn("local_relevance_medium_read_only_runtime_v0_min_v2", written.as_posix())
    ...
    def test_runtime_v0_and_runtime_boundary_v0_failure_preservation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            request = self.clean_request(Path(tmp))
            result = resolver.resolve_local_relevance_medium_read_only_runtime_v0_min_v2(
                request
            ...
            for key, expected_code in (
                ...
                with self.subTest(flip=key):
                    ...
                    blocked = resolver.resolve_local_relevance_medium_read_only_runtime_v0_min_v2(
                        blocked_request
                    )
