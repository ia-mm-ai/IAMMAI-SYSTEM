"""Tests for command result boundary only.

This suite is downstream of the recorded command output/report artifact line.
It proves that one future command result boundary may be recorded without
creating command result, inventing result body, creating command success,
inferring pass, inferring fail, source, authority, currentness, final
completion, deployment, continuation, reusable permission, derivative
...
import resolve_portable_source_body_verification_command_result_boundary as resolver
from resolve_portable_source_body_verification_command_result_boundary import (
    build_declared_portable_source_body_verification_command_result_boundary_request,
    build_portable_source_body_verification_command_result_boundary_summary,
    resolve_portable_source_body_verification_command_result_boundary,
    resolve_portable_source_body_verification_command_result_boundary_from_path,
    write_portable_source_body_verification_command_result_boundary_result,
)
...
SUPPORTED_SCOPE = tuple(resolver.SUPPORTED_COMMAND_RESULT_BOUNDARY_SCOPE)
REQUIRED_FALSE_NON_CLAIMS = tuple(resolver.REQUIRED_FALSE_NON_CLAIMS)
RAW_FULL_BODY_SENTINEL = "RAW_COMMAND_RESULT_BOUNDARY_BODY_MUST_NOT_RETURN_" * 6

TOP_LEVEL_SECTIONS = (
    "portable_source_body_verification_command_result_boundary_metadata",
    ...
    "command_result_boundary_checks",
    "command_result_boundary_statement",
    "command_result_boundary_non_meaning",
    ...
    "outcome",
    "block",
    "portable_source_body_verification_command_result_boundary_summary",
)
...
def _terminal_summary_basis(label: str, **overrides: Any) -> dict[str, Any]:
    basis = _reference_basis(
        ...
        terminal_summary_does_not_invent_result_body=True,
        terminal_summary_does_not_create_command_success=True,
        terminal_summary_does_not_infer_pass=True,
        terminal_summary_does_not_infer_fail=True,
...
def _posture(label: str, **overrides: Any) -> dict[str, Any]:
    posture: dict[str, Any] = {
        ...
        "posture": label,
        "command_result_boundary_only": True,
...
def _valid_request(**overrides: Any) -> dict[str, Any]:
    request: dict[str, Any] = {
        "command_result_boundary_request_id": "command_result_boundary_reference_review_001",
        "command_result_boundary_question": QUESTION,
        "command_result_boundary_intent": resolver.INTENT_RECORD,
        ...
        "selected_command_output_report_artifact_terminal_summary_basis": _terminal_summary_basis(
            "command_output_report_artifact_terminal_summary"
        ),
        "selected_command_output_report_artifact_boundary_basis": (
        ...
        "selected_command_report_lineage_basis": _reference_basis(
            ...
            command_report_lineage_basis_not_command_result=True,
        ...
        "command_result_boundary_scope": sorted(SUPPORTED_SCOPE),
        "requested_command_result_boundary_outcome": RECORDED,
...
def _resolve(request: Mapping[str, Any]) -> dict[str, Any]:
    return resolve_portable_source_body_verification_command_result_boundary(
        declared_command_result_boundary_request=request
    )


class CommandResultBoundaryTests(unittest.TestCase):
    def assertRecordedBoundary(self, result: Mapping[str, Any]) -> None:
        self.assertIsInstance(result, dict)
        self.assertEqual(result["outcome"], RECORDED)
        self.assertIsNone(result["block"]["block_code"])
        self.assertIsNone(result["block"]["block_reason"])
        metadata = result["portable_source_body_verification_command_result_boundary_metadata"]
        self.assertEqual(metadata["failed_check_count"], 0)
        self.assertGreater(metadata["passed_check_count"], 0)
        self.assertEqual(
            metadata["portable_source_body_verification_command_result_boundary_result_version"],
            "0.1.0",
        )
        self.assertEqual(
        ...
        self.assertIs(metadata["creates_command_result"], False)
        self.assertIs(metadata["invents_result_body"], False)
        self.assertIs(metadata["creates_command_success"], False)
        self.assertIs(metadata["infers_pass"], False)
        self.assertIs(metadata["infers_fail"], False)
        for section in TOP_LEVEL_SECTIONS:
            self.assertIn(section, result)

        statement = result["command_result_boundary_statement"]
    ...
    def assertBlocked(self, result: Mapping[str, Any]) -> None:
        ...
        self.assertIs(statement["result_body_invented"], False)
        self.assertIs(statement["command_success_created"], False)
        self.assertIs(statement["pass_inference_made"], False)
        self.assertIs(statement["fail_inference_made"], False)
        self.assertIs(result["non_claims"]["follow_on_work_authorized"], False)

    def test_public_api_and_constants_are_present(self) -> None:
        self.assertTrue(callable(resolve_portable_source_body_verification_command_result_boundary))
        self.assertTrue(callable(resolve_portable_source_body_verification_command_result_boundary_from_path))
        self.assertTrue(callable(write_portable_source_body_verification_command_result_boundary_result))
        self.assertTrue(callable(build_portable_source_body_verification_command_result_boundary_summary))
        self.assertTrue(callable(build_declared_portable_source_body_verification_command_result_boundary_request))
        self.assertEqual(
            {
        ...
        self.assertEqual(set(SUPPORTED_SCOPE), set(resolver.SUPPORTED_COMMAND_RESULT_BOUNDARY_SCOPE))
        self.assertIn("command_result_created", REQUIRED_FALSE_NON_CLAIMS)
        self.assertIn("result_body_invented", REQUIRED_FALSE_NON_CLAIMS)
        self.assertIn("pass_inference_made", REQUIRED_FALSE_NON_CLAIMS)
        self.assertIn("fail_inference_made", REQUIRED_FALSE_NON_CLAIMS)

    def test_successful_recorded_result_preserves_boundary_only(self) -> None:
        ...
        self.assertIs(artifact["command_result_created"], False)
        ...
        checks = result["command_result_boundary_checks"]
        ...
        non_meaning = result["command_result_boundary_non_meaning"]
        self.assertIs(non_meaning["command_result_exists"], False)
        self.assertIs(non_meaning["result_body_exists"], False)
        self.assertIs(non_meaning["command_success_exists"], False)
        self.assertIs(non_meaning["pass_was_inferred"], False)
        self.assertIs(non_meaning["fail_was_inferred"], False)
        self.assertIs(non_meaning["boundary_is_authority"], False)
        self.assertIs(non_meaning["boundary_is_final_completion"], False)
    ...
    def test_selected_basis_lineage_preserves_artifact_output_capture_and_execution(self) -> None:
        ...
        self.assertIs(
            result["selected_command_output_report_artifact_boundary_terminal_summary_basis"][
                "basis_declared"
            ],
            True,
        )
        ...
        self.assertIs(command_report["command_report_lineage_basis_treated_as_command_result"], False)
    ...
    def test_path_resolution_and_json_writing_are_bounded(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = resolve_portable_source_body_verification_command_result_boundary_from_path(
                request_path
            )
            self.assertRecordedBoundary(result)
            ...
            with patch.object(resolver, "OUTPUT_ROOT", patched_output_root):
                output_path_1 = write_portable_source_body_verification_command_result_boundary_result(
                    result
                )
                output_path_2 = write_portable_source_body_verification_command_result_boundary_result(
                    result
                )
    ...
    def test_not_recorded_and_requires_additional_basis_outcomes_are_bounded(self) -> None:
        not_recorded = _resolve(
            _copy_request_with(
                lambda request: request.update(
                    {
                        "command_result_boundary_intent": resolver.INTENT_DO_NOT_RECORD,
                        "requested_command_result_boundary_outcome": NOT_RECORDED,
                        "not_recorded_basis": {
        ...
        self.assertIs(not_recorded["command_result_boundary_statement"]["command_result_created"], False)
        self.assertIs(not_recorded["command_result_boundary_statement"]["result_body_invented"], False)
        self.assertIs(not_recorded["command_result_boundary_statement"]["command_success_created"], False)

        requires = _resolve(
            _copy_request_with(
                lambda request: request.update(
                    {
                        "requested_command_result_boundary_outcome": REQUIRES_ADDITIONAL_BASIS,
                        "additional_basis_context": {
                            "result_body_non_invention_posture_needs_more_basis": True,
    ...
    def test_blocking_request_shape_artifact_and_scope_cases(self) -> None:
        self.assertBlocked(resolve_portable_source_body_verification_command_result_boundary(None))
        self.assertBlocked(
            resolve_portable_source_body_verification_command_result_boundary(
                declared_command_result_boundary_request=[]
            )
        )
        cases: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ("explicit block intent", lambda request: request.update({"command_result_boundary_intent": resolver.INTENT_BLOCK})),
            ("unsupported intent", lambda request: request.update({"command_result_boundary_intent": "UNSUPPORTED"})),
            ("unsupported scope", lambda request: request.update({"command_result_boundary_scope": ["UNSUPPORTED_SCOPE"]})),
            ("missing question", lambda request: request.update({"command_result_boundary_question": ""})),
            ...
            ("artifact created result", lambda request: request["selected_command_output_report_artifact_basis"].update({"command_result_created": True})),
    ...
    def test_blocking_artifact_boundary_and_report_body_collapse_cases(self) -> None:
        cases: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ("missing artifact boundary basis", lambda request: request.update({"selected_command_output_report_artifact_boundary_basis": {}})),
            ("artifact boundary not recorded", lambda request: request["selected_command_output_report_artifact_boundary_basis"].update({"outcome": "NOT_RECORDED"})),
            ("artifact boundary failed checks", lambda request: request["selected_command_output_report_artifact_boundary_basis"].update({"failed_check_count": 1})),
            ("artifact boundary step not declared", lambda request: request["selected_command_output_report_artifact_boundary_basis"].update({"one_future_command_output_report_artifact_step_declared": False})),
            ("artifact boundary created result", lambda request: request["selected_command_output_report_artifact_boundary_basis"].update({"command_result_created": True})),
            ("artifact boundary created success", lambda request: request["selected_command_output_report_artifact_boundary_basis"].update({"command_success_created": True})),
            ("artifact treated as result", lambda request: request.update({"command_output_report_artifact_treated_as_result": True})),
            ...
            ("report body treated as result", lambda request: request.update({"report_body_treated_as_result": True})),
    ...
    def test_blocking_prior_lineage_execution_and_token_cases(self) -> None:
        cases: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ...
            ("earlier v2 treated as repairing v1 admission", lambda request: request.update({"v2_treated_as_repairing_v1_request_admission": True})),
            ("earlier v1 request-admission failure hidden", lambda request: request.update({"v1_request_admission_failure_hidden": True})),
            ...
            ("command report lineage command result", lambda request: request["selected_command_report_lineage_basis"].update({"command_report_lineage_basis_treated_as_command_result": True})),
        )
        for label, mutator in cases:
            with self.subTest(label=label):
                self.assertBlocked(_resolve(_copy_request_with(mutator)))
    ...
    def test_input_selected_basis_posture_and_scope_are_not_mutated(self) -> None:
        request = _valid_request()
        ...
        scope_original = copy.deepcopy(request["command_result_boundary_scope"])
        ...
        for key, original in posture_originals.items():
            self.assertEqual(request[key], original, key)
        self.assertEqual(request["command_result_boundary_scope"], scope_original)

    def test_summary_and_builder_preserve_short_name_lineage_and_non_claims(self) -> None:
        request = _valid_request()
        built = build_declared_portable_source_body_verification_command_result_boundary_request(
            command_result_boundary_request_id="builder_request",
        ...
        built.update({"requested_command_result_boundary_outcome": RECORDED})
        result = _resolve(built)
        self.assertRecordedBoundary(result)
        summary = build_portable_source_body_verification_command_result_boundary_summary(result)
        self.assertEqual(summary["outcome"], RECORDED)
        self.assertEqual(summary["request_id"], "builder_request")
        self.assertIs(summary["command_result_boundary_recorded"], True)
        self.assertIs(summary["one_future_command_result_step_declared"], True)
        ...
        self.assertIs(summary["pass_inference_blocked"], True)
        self.assertIs(summary["fail_inference_blocked"], True)
        self.assertIs(summary["command_report_lineage_not_current_report_artifact"], True)
        self.assertIs(summary["command_report_lineage_not_command_result"], True)
