"""Tests for the admitted single live command invocation request consumption resolver.

This test surface is bounded to request consumption only. Request consumption
may close the already-admitted one-shot request token as consumed basis for a
later execution review. It is not command invocation, command execution,
command output, command result, command success, execution permission, a
standing invocation lane, repeat permission, authority, currentness, final
...
import resolve_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption as resolver
from resolve_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption import (
    ...
    resolve_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption,
    resolve_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_from_path,
    write_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_result,
)


CONSUMED = resolver.OUTCOME_CONSUMED
NOT_CONSUMED = resolver.OUTCOME_NOT_CONSUMED
REQUIRES_ADDITIONAL_BASIS = resolver.OUTCOME_REQUIRES_ADDITIONAL_BASIS
BLOCKED = resolver.OUTCOME_BLOCKED
ADMITTED_REQUEST_OUTCOME = resolver.SELECTED_ADMISSION_OUTCOME
ADMITTED_REQUEST_VERSION = resolver.SELECTED_ADMISSION_VERSION
V2_RESOLVER_MODULE = resolver.SELECTED_ADMISSION_RESOLVER_MODULE
V1_RESOLVER_MODULE = resolver.PREDECESSOR_RESOLVER_MODULE
CONSUMPTION_BOUNDARY_RECORDED = resolver.CONSUMPTION_BOUNDARY_RECORDED_OUTCOME
SUCCESSOR_REASON = (
...
QUESTION = (
    ...
    "command result, command success, standing invocation lane, repeat permission, "
    "execution permission, authority, currentness, final completion, continuation, "
    "reusable permission, derivative reception, vessel relation, another reception "
    "request, or follow-on work?"
)
SENTINEL = "SENTINEL_REQUEST_CONSUMPTION_FULL_BODY_SHOULD_NOT_RETURN_" * 6
OMISSION_MARKER = resolver.FULL_BODY_OMISSION_MARKER

SUPPORTED_SCOPE = tuple(sorted(resolver.SUPPORTED_REQUEST_CONSUMPTION_SCOPE))
REQUIRED_NON_CLAIMS = tuple(resolver.REQUIRED_FALSE_NON_CLAIMS)
OUTCOME_FAMILY = {CONSUMED, NOT_CONSUMED, REQUIRES_ADDITIONAL_BASIS, BLOCKED}
...
TOP_LEVEL_SECTIONS = {
    ...
    "request_consumption_checks",
    "request_consumption_statement",
    "request_consumption_non_meaning",
...
def _assert_forbidden_values_sanitized(
...
def _selected_v2_admitted_request_basis(**extra: Any) -> dict[str, Any]:
    basis = {
        "selected_result_id": (
            "portable_source_body_verification_single_live_command_invocation_"
            "request_admission_reference_review_001_v2"
        ),
        "selected_result_path": (
            "artifacts/integrity_host_v0_min_coexistence_portable_source_body_"
            "verification_single_live_command_invocation_request_admission_boundary_v2/"
            "portable_source_body_verification_single_live_command_invocation_request_"
            "admission_reference_review_001_v2__single_live_command_invocation_"
            "request_admission_result_v2.json"
        ),
        "outcome": ADMITTED_REQUEST_OUTCOME,
        "result_version": ADMITTED_REQUEST_VERSION,
        ...
        "admitted_request_basis_remains_lineage_evidence": True,
        "admitted_request_consumed_exactly_once_only_in_consumed_outcome": True,
...
def _selected_consumption_boundary_basis(**extra: Any) -> dict[str, Any]:
    basis = {
        "selected_result_id": (
            "admitted_single_live_command_invocation_request_consumption_reference_"
            "review_001"
        ),
        "selected_result_path": (
            "artifacts/integrity_host_v0_min_coexistence_portable_source_body_"
            "verification_admitted_single_live_command_invocation_request_"
            "consumption_boundary/admitted_single_live_command_invocation_request_"
            "consumption_reference_review_001__admitted_single_live_command_"
            "invocation_request_consumption_result.json"
        ),
        "outcome": CONSUMPTION_BOUNDARY_RECORDED,
        "failed_check_count": 0,
        "passed_check_count": 48,
...
def _selected_consumption_boundary_terminal_summary_basis(
    **extra: Any,
) -> dict[str, Any]:
    basis = {
        ...
        "terminal_summary_path": (
            ...
            "INVOCATION_REQUEST_CONSUMPTION_BOUNDARY_TERMINAL_SUMMARY_V0.md"
        ),
...
def _selected_v1_predecessor_failure_basis(**extra: Any) -> dict[str, Any]:
    basis = {
        ...
        "selected_v1_predecessor_artifact_id": (
            "historical_v1_returned_result_containment_failure"
        ),
        "selected_v1_predecessor_outcome": (
            "SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_REVIEW_BLOCKED"
        ),
...
def _posture(label: str, **extra: Any) -> dict[str, Any]:
    posture = {
        "posture_label": label,
        "declared": True,
        "one_shot_consumption_declared": label == "one_shot_consumption",
        "one_shot_consumption_preserved": label == "one_shot_consumption",
        "admitted_request_consumed_exactly_once_only_in_consumed_outcome": True,
        "consumption_token_closed_only_in_consumed_outcome": True,
        "consumed_request_basis_recorded": label == "consumed_request_basis",
...
def _valid_request(**extra: Any) -> dict[str, Any]:
    request = {
        "request_consumption_request_id": "admitted_request_consumption_review_001",
        "request_consumption_question": QUESTION,
        "request_consumption_intent": resolver.INTENT_RECORD,
        "selected_v2_admitted_request_basis": _selected_v2_admitted_request_basis(),
        "selected_consumption_boundary_basis": _selected_consumption_boundary_basis(),
        "selected_consumption_boundary_terminal_summary_basis": (
            _selected_consumption_boundary_terminal_summary_basis()
        ),
        "selected_v1_predecessor_failure_basis": (
            _selected_v1_predecessor_failure_basis()
        ),
        "one_shot_consumption_posture": _posture("one_shot_consumption"),
        "consumed_request_basis": _posture("consumed_request_basis"),
        ...
        "request_consumption_scope": list(SUPPORTED_SCOPE),
        "requested_request_consumption_outcome": CONSUMED,
...
def _resolve(request: dict[str, Any] | None) -> dict[str, Any]:
    return resolve_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption(
        declared_request_consumption_request=request
    )
...
def _assert_shape(testcase: unittest.TestCase, result: dict[str, Any]) -> None:
...
def _assert_common_non_claims_false(
    testcase: unittest.TestCase,
    result: dict[str, Any],
) -> None:
...
def _assert_no_command_or_follow_on_created(
    testcase: unittest.TestCase,
    result: dict[str, Any],
) -> None:
    statement = result["request_consumption_statement"]
    for name in REQUIRED_NON_CLAIMS:
    ...
    for name in (
        ...
        testcase.assertIn(name, statement)
        testcase.assertFalse(statement[name], name)
    testcase.assertTrue(statement["no_execution_permission_created"])
    _assert_common_non_claims_false(testcase, result)


class PortableSourceBodyVerificationAdmittedRequestConsumptionTests(unittest.TestCase):
    def test_successful_request_consumption_consumed_result(self) -> None:
        ...
        self.assertEqual(CONSUMED, result["outcome"])
        self.assertIsNone(result["block"]["block_code"])
        self.assertIsNone(result["block"]["block_reason"])
        self.assertEqual(0, result["request_consumption_checks"]["failed_check_count"])
        self.assertGreater(result["request_consumption_checks"]["passed_check_count"], 0)
        ...
        for name in (
            ...
            "no_execution_permission_created",
            "consumption_not_execution_permission",
            "consumption_not_command_success",
        ):
            self.assertTrue(statement[name], name)
    ...
    def test_metadata_lineage_and_boundary_basis_are_preserved(self) -> None:
        ...
        self.assertEqual(
            ADMITTED_REQUEST_VERSION,
            selected_v2["selected_v2_admitted_request_version"],
        )
        ...
        self.assertTrue(
            selected_v2[
                "admitted_request_basis_is_consumed_once_only_in_consumed_outcome"
            ]
        )
        self.assertTrue(selected_v2["command_invocation_not_created"])
        self.assertTrue(selected_v2["command_execution_not_performed"])
        self.assertTrue(selected_v2["command_output_result_success_not_created"])
        self.assertTrue(selected_v2["no_standing_invocation_lane"])
        self.assertTrue(selected_v2["no_repeat_invocation_permission"])
        self.assertTrue(selected_v2["no_execution_permission"])

        boundary = result["selected_consumption_boundary_basis"]
        self.assertEqual(
            CONSUMPTION_BOUNDARY_RECORDED,
            boundary["selected_consumption_boundary_result_outcome"],
        )
        self.assertEqual(0, boundary["selected_consumption_boundary_failed_check_count"])
        self.assertTrue(boundary["selected_consumption_boundary_outcome_recorded"])
        self.assertTrue(boundary["selected_consumption_boundary_failed_check_count_zero"])
        self.assertTrue(boundary["boundary_remains_pre_consumption_pre_execution_basis_only"])
        self.assertTrue(boundary["boundary_did_not_authorize_execution"])
        ...
        self.assertTrue(selected_v1["v2_successor_does_not_erase_v1"])
        self.assertTrue(selected_v1["predecessor_failure_evidence_is_lineage_evidence_only"])

    def test_request_consumption_checks_and_non_meaning_are_explicit(self) -> None:
        result = _resolve(_valid_request())
        checks = result["request_consumption_checks"]
        ...
        non_meaning = result["request_consumption_non_meaning"]
        for name in (
            "request_consumption_does_not_mean_command_invocation_created",
            "request_consumption_does_not_mean_command_executed",
            "request_consumption_does_not_mean_command_output_exists",
            "request_consumption_does_not_mean_command_result_exists",
            "request_consumption_does_not_mean_command_success_exists",
            "request_consumption_does_not_mean_command_success_creates_currentness",
            "request_consumption_does_not_mean_command_success_claims_final_completion",
            "request_consumption_does_not_mean_command_output_becomes_source",
            "request_consumption_does_not_mean_command_result_becomes_authority",
            "request_consumption_does_not_mean_standing_invocation_lane_exists",
            "request_consumption_does_not_mean_repeat_invocation_permission_exists",
            "request_consumption_does_not_mean_execution_permission_exists",
            "request_consumption_does_not_mean_v1_was_repaired",
            "request_consumption_does_not_mean_v1_was_hidden",
            "request_consumption_does_not_mean_v1_passed",
            "request_consumption_does_not_mean_deployment_created",
            "request_consumption_does_not_mean_runtime_hosting_created",
            "request_consumption_does_not_mean_public_release_created",
            "request_consumption_does_not_mean_public_readiness_created",
            "request_consumption_does_not_mean_final_completion_claimed",
            "request_consumption_does_not_mean_continuation_authorized",
            "request_consumption_does_not_mean_reusable_permission_created",
            "request_consumption_does_not_mean_derivative_reception_authorized",
            "request_consumption_does_not_mean_vessel_relation_authorized",
            "request_consumption_does_not_mean_another_reception_request_authorized",
            "request_consumption_does_not_mean_follow_on_work_authorized",
        ):
            self.assertTrue(non_meaning[name], name)

    def test_additional_basis_and_not_consumed_outcomes_are_bounded(self) -> None:
        additional_context = {
            "missing_basis": "selected consumption-boundary live artifact basis unclear",
            "missing_basis_is_not_scheduled": True,
        }
        additional_request = _valid_request(
            requested_request_consumption_outcome=REQUIRES_ADDITIONAL_BASIS,
        ...
        additional_basis = additional_result["additional_basis_required"]
        self.assertTrue(additional_basis["additional_basis_required"])
        self.assertIn(
            "selected consumption-boundary live artifact basis unclear",
            _json_text(additional_basis),
        )
        self.assertTrue(additional_basis["additional_basis_not_scheduled"])
        self.assertTrue(additional_basis["additional_basis_not_authorized"])
        self.assertTrue(additional_basis["additional_basis_not_executed"])
        self.assertTrue(additional_basis["execution_permission_not_created"])
        _assert_no_command_or_follow_on_created(self, additional_result)

        not_consumed_basis = {
            "reason": "request consumption cannot be separated from execution permission",
            "not_consumed_does_not_authorize_repair_or_next_work": True,
        }
        not_consumed_request = _valid_request(
            requested_request_consumption_outcome=NOT_CONSUMED,
            not_consumed_basis=not_consumed_basis,
        )
        not_consumed_result = _resolve(not_consumed_request)

        self.assertEqual(NOT_CONSUMED, not_consumed_result["outcome"])
        self.assertTrue(not_consumed_result["not_consumed_basis"]["not_consumed"])
        self.assertIn(
            "request consumption cannot be separated from execution permission",
            _json_text(not_consumed_result["not_consumed_basis"]),
        )
        self.assertTrue(not_consumed_result["not_consumed_basis"]["not_consumed_does_not_repair"])
        self.assertTrue(
            not_consumed_result["not_consumed_basis"][
                "not_consumed_does_not_authorize_follow_on_work"
            ]
        )
        self.assertTrue(
            not_consumed_result["not_consumed_basis"][
                "not_consumed_does_not_create_execution_permission"
            ]
        )
        _assert_no_command_or_follow_on_created(self, not_consumed_result)
    ...
    def test_what_remains_open_and_summary_helper(self) -> None:
        ...
        expected_summary_values = {
            ...
            "selected_v2_admitted_request_version": ADMITTED_REQUEST_VERSION,
            "selected_v2_failed_check_count": 0,
            "selected_consumption_boundary_outcome": CONSUMPTION_BOUNDARY_RECORDED,
            "selected_consumption_boundary_failed_check_count": 0,
            "command_invocation_not_created": True,
            "command_execution_not_performed": True,
        ...
        for name, expected in expected_summary_values.items():
            self.assertEqual(expected, summary[name], name)
        self.assertGreater(summary["passed_check_count"], 0)
        for name in REQUIRED_NON_CLAIMS:
            self.assertFalse(summary["key_non_claims"][name], name)

    def test_request_builder_helper_builds_resolvable_request(self) -> None:
        request = (
            build_declared_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_request(
                request_consumption_request_id="builder_request_consumption_review_001",
                request_consumption_question=QUESTION,
                selected_v2_admitted_request_basis=_selected_v2_admitted_request_basis(),
                selected_consumption_boundary_basis=_selected_consumption_boundary_basis(),
                selected_consumption_boundary_terminal_summary_basis=(
                ...
                request_consumption_scope=SUPPORTED_SCOPE,
                selected_v2_admitted_request_artifact_path=(
                    "artifacts/synthetic/v2_admission.json"
                ),
                selected_v2_admitted_request_artifact_id="builder_v2_admission",
                selected_v2_admitted_request_outcome=ADMITTED_REQUEST_OUTCOME,
                selected_v2_admitted_request_version=ADMITTED_REQUEST_VERSION,
                selected_v2_failed_check_count=0,
                selected_consumption_boundary_result_path=(
                    "artifacts/synthetic/consumption_boundary.json"
                ),
                selected_consumption_boundary_result_id="builder_consumption_boundary",
                selected_consumption_boundary_result_outcome=(
                    CONSUMPTION_BOUNDARY_RECORDED
                ),
                selected_consumption_boundary_failed_check_count=0,
                requested_request_consumption_outcome=CONSUMED,
                additional_basis_context={"missing_basis": "none for valid request"},
                not_consumed_basis={"reason": "none for valid request"},
            )
        )

        self.assertEqual(
            "builder_request_consumption_review_001",
            request["request_consumption_request_id"],
        )
        self.assertEqual(QUESTION, request["request_consumption_question"])
        for key in (
            "selected_v2_admitted_request_basis",
            "selected_consumption_boundary_basis",
            "selected_consumption_boundary_terminal_summary_basis",
        ...
        self.assertEqual(SUPPORTED_SCOPE, tuple(request["request_consumption_scope"]))
        self.assertEqual(
            "artifacts/synthetic/v2_admission.json",
            request["selected_v2_admitted_request_artifact_path"],
        )
        ...
        self.assertEqual(
            ADMITTED_REQUEST_VERSION,
            request["selected_v2_admitted_request_version"],
        )
        self.assertEqual(0, request["selected_v2_failed_check_count"])
        self.assertEqual(
            "artifacts/synthetic/consumption_boundary.json",
            request["selected_consumption_boundary_result_path"],
        )
        self.assertEqual(
            "builder_consumption_boundary",
            request["selected_consumption_boundary_result_id"],
        )
        self.assertEqual(
            CONSUMPTION_BOUNDARY_RECORDED,
            request["selected_consumption_boundary_result_outcome"],
        )
        self.assertEqual(0, request["selected_consumption_boundary_failed_check_count"])
        self.assertEqual(CONSUMED, request["requested_request_consumption_outcome"])
        self.assertIn("none for valid request", _json_text(request["additional_basis_context"]))
        self.assertIn("none for valid request", _json_text(request["not_consumed_basis"]))
        for name in REQUIRED_NON_CLAIMS:
    ...
    def test_path_based_request_and_write_behavior(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            request_path = temp_root / "input" / "declared_request_consumption.json"
            ...
            self.assertEqual(CONSUMED, path_result["outcome"])
            self.assertEqual(set(mapping_result), set(path_result))
            self.assertEqual(
                str(request_path),
                path_result["declared_request_consumption_question"][
            ...
            default_root = temp_root / "bounded_request_consumption_root"
            with patch.object(
                resolver,
            ...
            self.assertIn(default_root, default_written.parents)
            default_text = str(default_written)
            self.assertNotIn("consumption_boundary", default_text)
            self.assertNotIn("admission_boundary", default_text)
    ...
    def test_reference_shaped_containment_blocks_and_sanitizes_raw_full_body(
        self,
    ) -> None:
        ...
        self.assertEqual("FULL_PRIOR_ARTIFACT_BODY_EMITTED", _block_code(v2_result))
        _assert_no_sentinel(self, v2_result)
        _assert_forbidden_values_sanitized(self, v2_result)
        _assert_no_command_or_follow_on_created(self, v2_result)

        boundary_request = _valid_request()
        boundary_request["selected_consumption_boundary_basis"][
            "full_artifact_body"
        ] = SENTINEL
        boundary_original = copy.deepcopy(boundary_request)
        boundary_result = _resolve(boundary_request)
        self.assertEqual(boundary_original, boundary_request)
        self.assertEqual(BLOCKED, boundary_result["outcome"])
        self.assertEqual(
            "FULL_PRIOR_ARTIFACT_BODY_EMITTED",
            _block_code(boundary_result),
        )
        _assert_no_sentinel(self, boundary_result)
        _assert_forbidden_values_sanitized(self, boundary_result)
        _assert_no_command_or_follow_on_created(self, boundary_result)
    ...
    def test_resolver_does_not_mutate_declared_inputs(self) -> None:
        ...
        self.assertEqual(original, request)
        self.assertEqual(CONSUMED, first["outcome"])
        self.assertEqual(CONSUMED, second["outcome"])
        for key in (
            "selected_v2_admitted_request_basis",
            "selected_consumption_boundary_basis",
            "selected_consumption_boundary_terminal_summary_basis",
    ...
    def test_blocking_basis_scope_and_lineage_conditions(self) -> None:
        ...
        self.assertEqual("REQUEST_CONSUMPTION_QUESTION_UNDECLARED", _block_code(direct_missing))

        non_mapping = resolve_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption(
            declared_request_consumption_request=["not", "a", "mapping"]  # type: ignore[arg-type]
        )
        ...
        cases = [
            (
                ...
                lambda request: request.update(request_consumption_intent=resolver.INTENT_BLOCK),
            ...
            (
                "missing v2 returned-result containment",
                lambda request: [
            ...
            (
                "missing consumption-boundary basis",
                lambda request: request.pop("selected_consumption_boundary_basis"),
                "CONSUMPTION_BOUNDARY_BASIS_MISSING",
            ),
            (
                "consumption-boundary not recorded",
                lambda request: request["selected_consumption_boundary_basis"].update(
                    outcome="ADMITTED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_CONSUMPTION_BOUNDARY_NOT_RECORDED"
                ),
                "CONSUMPTION_BOUNDARY_NOT_RECORDED",
            ),
            (
                "consumption-boundary failed checks",
                lambda request: request["selected_consumption_boundary_basis"].update(
                    failed_check_count=1
                ),
            ...
            (
                "missing consumption-boundary terminal summary",
            ...
            (
                ...
                lambda request: request.pop("consumed_request_basis"),
            ...
            (
                "unsupported scope",
                lambda request: request["request_consumption_scope"].append(
                    "UNSUPPORTED_SCOPE"
                ),
    ...
    def test_blocking_collapse_flags_and_non_claims(self) -> None:
        collapse_cases = [
        ...
        missing_non_claim_request["declared_non_claims"].pop("command_invocation_created")
