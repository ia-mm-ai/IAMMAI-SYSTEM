"""Tests for portable source-body verification command success boundary.

This suite is downstream of recorded command result v2. It proves only that one
future command-success-step boundary may be recorded. It does not create command
success, success body, success/currentness/final-completion/public-readiness/
deployment-readiness/authority/follow-on/unbounded-pass-fail inference,
deployment, runtime hosting, public release, final completion, or follow-on
work. Command result v2 remains command-result posture only, output capture v2
remains output-capture posture only, v1 predecessor failures remain visible,
command report lineage remains lineage only, execution trace remains audit-only,
authorization token reuse remains blocked, and the consumed request token
remains closed.
"""
...
import resolve_portable_source_body_verification_command_success_boundary as resolver
...
RAW_BODY_SENTINEL = "RAW_COMMAND_SUCCESS_BOUNDARY_BODY_MUST_NOT_RETURN_" * 8
...
def _request(**overrides: Any) -> dict[str, Any]:
    request = resolver.build_declared_portable_source_body_verification_command_success_boundary_request()
...
class CommandSuccessBoundaryResolverTests(unittest.TestCase):
    ...
    def test_public_api_and_constants_exist(self) -> None:
        for name in (
    ...
    def test_successful_command_success_boundary_recorded_result(self) -> None:
        request = _request()
        result = resolver.resolve_portable_source_body_verification_command_success_boundary(
        ...
        self.assertEqual(
            result["portable_source_body_verification_command_success_boundary_metadata"][
                "failed_check_count"
            ],
            0,
        )
        ...
        self.assertTrue(metadata["command_success_boundary_is_not_command_success"])
        self.assertTrue(metadata["command_success_still_requires_separately_bounded_step"])

        selected_v2 = result["selected_command_result_v2_basis"]
        self.assertEqual(selected_v2["outcome"], "PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_RESULT_RECORDED")
        ...
        self.assertEqual(selected_v2["successor_of"], "resolve_portable_source_body_verification_command_result")
        ...
        non_meaning = result["command_success_boundary_non_meaning"]
        for key in (
            "command_success_exists",
            "success_body_exists",
            "success_was_inferred",
            "currentness_was_inferred",
            "final_completion_was_inferred",
            "public_readiness_was_inferred",
            "deployment_readiness_was_inferred",
            "authority_was_inferred",
            "follow_on_work_was_inferred",
            "unbounded_pass_fail_was_inferred",
            ...
            self.assertIs(non_meaning.get(key), False, key)
    ...
    def test_not_recorded_and_requires_additional_basis_outcomes_are_bounded(self) -> None:
        not_recorded_request = _request(requested_command_success_boundary_outcome=NOT_RECORDED)
        not_recorded = resolver.resolve_portable_source_body_verification_command_success_boundary(
            declared_command_success_boundary_request=not_recorded_request
        )
        self.assertEqual(not_recorded["outcome"], NOT_RECORDED)
        self.assertIn(not_recorded["outcome"], OUTCOME_FAMILY)
        self.assertIsNone((not_recorded.get("block") or {}).get("block_code"))
        self.assert_no_success_or_follow_on(not_recorded)

        additional_request = _request(additional_basis_context=["need clearer basis"])
        additional = resolver.resolve_portable_source_body_verification_command_success_boundary(
            declared_command_success_boundary_request=additional_request
        )
        ...
        self.assert_no_success_or_follow_on(additional)

    def test_path_and_write_behavior_is_bounded_to_success_boundary_root(self) -> None:
        request = _request(command_success_boundary_request_id="success_boundary_path_test")
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            request_path = temp_root / "request.json"
            request_path.write_text(json.dumps(request, indent=2, sort_keys=True), encoding="utf-8")
            result = resolver.resolve_portable_source_body_verification_command_success_boundary_from_path(
                request_path
            )
            self.assertEqual(result["outcome"], RECORDED)
            ...
            malformed = resolver.resolve_portable_source_body_verification_command_success_boundary_from_path(
                malformed_path
            )
            ...
            array_result = resolver.resolve_portable_source_body_verification_command_success_boundary_from_path(
                array_path
            )
            self.assertEqual(array_result["outcome"], BLOCKED)

            missing_result = resolver.resolve_portable_source_body_verification_command_success_boundary_from_path(
            ...
            output_root = (
                temp_root
                / "artifacts"
                / "integrity_host_v0_min_coexistence_portable_source_body_verification_command_success_boundary"
            )
            with patch.object(resolver, "OUTPUT_ROOT", output_root):
                first_path = resolver.write_portable_source_body_verification_command_success_boundary_result(result)
                second_path = resolver.write_portable_source_body_verification_command_success_boundary_result(result)
            ...
            self.assertIn("command_success_boundary", str(first_path))
            self.assertNotIn("command_result_v2", str(first_path))
            self.assertNotIn("command_result_boundary", str(first_path))
    ...
    def test_summary_helper_preserves_boundary_only_posture(self) -> None:
        request = _request(command_success_boundary_request_id="summary_request")
        result = resolver.resolve_portable_source_body_verification_command_success_boundary(
            declared_command_success_boundary_request=request
        )
        summary = resolver.build_portable_source_body_verification_command_success_boundary_summary(result)

        self.assertEqual(summary["outcome"], RECORDED)
        self.assertIsNone(summary["block_code"])
        self.assertEqual(summary["request_id"], "summary_request")
        ...
        for key in (
            ...
            self.assertIs(summary.get(key), True, key)
        self.assertEqual(summary["selected_command_result_v2_outcome"], "PORTABLE_SOURCE_BODY_VERIFICATION_COMMAND_RESULT_RECORDED")
        ...
        self.assertEqual(summary["selected_output_capture_v2_outcome"], "PORTABLE_SOURCE_BODY_VERIFICATION_OUTPUT_CAPTURE_RECORDED")
    ...
    def test_resolver_does_not_mutate_input_or_return_raw_full_body(self) -> None:
        request = _request()
        ...
        self.assertNotIn(RAW_BODY_SENTINEL, _json_text(result))

        raw_request = _request()
        raw_request["selected_command_report_lineage_basis"]["full_prior_artifact_body"] = RAW_BODY_SENTINEL
        blocked = resolver.resolve_portable_source_body_verification_command_success_boundary(
            declared_command_success_boundary_request=raw_request
        )
        self.assertEqual(blocked["outcome"], BLOCKED)
        self.assertNotIn(RAW_BODY_SENTINEL, _json_text(blocked))
