"""Tests for the local read-only runtime-held-state resolver.

This suite is bounded to one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE object. It verifies that the
resolver reads one clean runtime-held-state boundary artifact, one clean runtime
v3 artifact, one clean runtime boundary v2 artifact, one clean runtime
permission artifact, and one clean operation execution artifact, then records
one local read-only selected-state runtime-held-state object only.

The held-state object is selected-state-only, basis-reference-only,
raw-state-body-excluding, state-mutation-refusing, state-update-refusing,
non-continuation-shaped, non-re-entry-shaped, non-second-operation-shaped,
...
import resolve_local_relevance_medium_read_only_runtime_held_state_v0_min as resolver  # noqa: E402
...
DEFAULT_RUNTIME_HELD_STATE_BOUNDARY_ARTIFACT = REPO_ROOT / (
    "artifacts/"
...
EXPECTED_WRAPPER_SECTIONS = (
    "local_relevance_medium_read_only_runtime_held_state_metadata",
    "declared_local_relevance_medium_read_only_runtime_held_state_question",
    ...
    "local_relevance_medium_read_only_runtime_held_state_checks",
    "local_relevance_medium_read_only_runtime_held_state_statement",
    "local_relevance_medium_read_only_runtime_held_state_non_meaning",
    ...
    "outcome",
    "block",
    "local_relevance_medium_read_only_runtime_held_state_summary",
)

FORBIDDEN_HELD_STATE_WRAPPER_FIELDS = (
    "outcome",
    "block",
    "local_relevance_medium_read_only_runtime_held_state_checks",
    "non_claims",
...
HELD_STATE_OBJECT_FALSE_FIELDS = (
...
class LocalRelevanceMediumReadOnlyRuntimeHeldStateTests(unittest.TestCase):
    """Executable membrane for the runtime-held-state resolver."""
    ...
    def held_state(self, result: Mapping[str, Any]) -> Mapping[str, Any]:
        held_state = result.get("local_relevance_medium_read_only_runtime_held_state")
        self.assertIsInstance(held_state, Mapping)
        return held_state
    ...
    def summary(self, result: Mapping[str, Any]) -> Mapping[str, Any]:
        summary = result.get("local_relevance_medium_read_only_runtime_held_state_summary")
    ...
    def assert_held_state_object_not_wrapper(self, result: Mapping[str, Any]) -> None:
        held_state = self.held_state(result)
        for field in FORBIDDEN_HELD_STATE_WRAPPER_FIELDS:
            self.assertNotIn(field, held_state)

    def assert_runtime_held_state_non_claims(self, result: Mapping[str, Any]) -> None:
        held_state = self.held_state(result)
        for field in HELD_STATE_OBJECT_FALSE_FIELDS:
            self.assertIn(field, held_state)
            self.assertIs(held_state[field], False)
    ...
    def assert_blocked_with_public_code(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_canonical_false_non_claims(result)
        self.assert_runtime_held_state_non_claims(result)
    ...
    def assert_lineage_preserved(self, result: Mapping[str, Any]) -> None:
        ...
        self.assertIs(held_state["runtime_v0_failure_evidence_preserved"], True)
        self.assertIs(held_state["runtime_v2_failure_evidence_preserved"], True)
        self.assertIs(held_state["runtime_boundary_v0_failure_evidence_preserved"], True)
    ...
    def assert_recorded_held_state(
        ...
        self.assertEqual(
            ...
            "resolve_local_relevance_medium_read_only_runtime_held_state_v0_min",
        )
        ...
        held_state = self.held_state(result)
        self.assertEqual(
            held_state["held_state_id"],
            "local_relevance_medium_read_only_runtime_held_state_001",
        )
        self.assertEqual(held_state["held_state_type"], HELD_STATE_TYPE)
        self.assertEqual(held_state["held_state_version"], "0.1.0")
        self.assertEqual(held_state["held_state_scope"], HELD_STATE_SCOPE)
        self.assertEqual(
            held_state["basis_runtime_held_state_boundary_outcome"],
            RUNTIME_HELD_STATE_BOUNDARY_OUTCOME,
        )
        self.assertEqual(held_state["basis_runtime_held_state_boundary_result_version"], "0.1.0")
        self.assertEqual(held_state["basis_runtime_held_state_boundary_failed_check_count"], 0)
        self.assertEqual(held_state["basis_runtime_outcome"], RUNTIME_OUTCOME)
        self.assertEqual(held_state["basis_runtime_result_version"], "0.1.0")
        self.assertEqual(held_state["basis_runtime_failed_check_count"], 0)
        self.assertEqual(held_state["basis_runtime_boundary_outcome"], RUNTIME_BOUNDARY_OUTCOME)
        self.assertEqual(held_state["basis_runtime_boundary_result_version"], "0.1.0")
        self.assertEqual(held_state["basis_runtime_boundary_failed_check_count"], 0)
        self.assertEqual(
            held_state["basis_runtime_permission_outcome"], RUNTIME_PERMISSION_OUTCOME
        )
        self.assertEqual(held_state["basis_runtime_permission_result_version"], "0.1.0")
        self.assertEqual(held_state["basis_runtime_permission_failed_check_count"], 0)
        self.assertEqual(
            held_state["basis_operation_execution_outcome"], OPERATION_EXECUTION_OUTCOME
        )
        self.assertEqual(held_state["basis_operation_execution_result_version"], "0.1.0")
        self.assertEqual(held_state["basis_operation_execution_failed_check_count"], 0)

        if expected_paths is not None:
            self.assert_same_or_stable_artifact_path(
                held_state["basis_runtime_held_state_boundary_artifact"],
                expected_paths["held_boundary"],
            )
            self.assert_same_or_stable_artifact_path(
                held_state["basis_runtime_artifact"], expected_paths["runtime"]
            )
            self.assert_same_or_stable_artifact_path(
                held_state["basis_runtime_boundary_artifact"],
                expected_paths["runtime_boundary"],
            )
            self.assert_same_or_stable_artifact_path(
                held_state["basis_runtime_permission_artifact"],
                expected_paths["runtime_permission"],
            )
            self.assert_same_or_stable_artifact_path(
                held_state["basis_operation_execution_artifact"],
                expected_paths["operation_execution"],
            )

        self.assertEqual(held_state["selected_command"], SELECTED_COMMAND)
        for field in HELD_STATE_TRUE_FIELDS:
            self.assertIn(field, held_state)
            self.assertIs(held_state[field], True)
        for field in HELD_STATE_OBJECT_FALSE_FIELDS:
            self.assertIn(field, held_state)
            self.assertIs(held_state[field], False)

        self.assert_held_state_object_not_wrapper(result)

        statement = self.statement(result)
        for field in STATEMENT_TRUE_FIELDS:
            self.assertIn(field, statement)
            self.assertIs(statement[field], True)
        for field in ("raw_state_body_embedded", "state_mutation_performed", "state_update_performed"):
            self.assertIn(field, statement)
            self.assertIs(statement[field], False)
        ...
        for key in (
            ...
            self.assertIs(held_state[key], True)
    ...
    def synthetic_runtime_held_state_boundary_artifact(self) -> dict[str, Any]:
        boundary = {
            "boundary_id": "local_relevance_medium_read_only_runtime_held_state_boundary_001",
        ...
        boundary["runtime_held_state_created"] = False
        return self.layered_artifact(
            outcome=RUNTIME_HELD_STATE_BOUNDARY_OUTCOME,
            metadata_key="local_relevance_medium_read_only_runtime_held_state_boundary_metadata",
            object_key="local_relevance_medium_read_only_runtime_held_state_boundary",
            statement_key="local_relevance_medium_read_only_runtime_held_state_boundary_statement",
            summary_key="local_relevance_medium_read_only_runtime_held_state_boundary_summary",
    ...
    def synthetic_runtime_artifact(self) -> dict[str, Any]:
        ...
        for field in HELD_STATE_OBJECT_FALSE_FIELDS:
            runtime[field] = False
        runtime["runtime_held_state_created"] = False
        return self.layered_artifact(
    ...
    def synthetic_runtime_permission_artifact(self) -> dict[str, Any]:
        ...
        permission["runtime_held_state_created"] = False
        return self.layered_artifact(
    ...
    def synthetic_operation_execution_artifact(self) -> dict[str, Any]:
        ...
        execution["runtime_held_state_created"] = False
        return self.layered_artifact(
    ...
    def synthetic_artifacts(self) -> dict[str, dict[str, Any]]:
        return {
            "held_boundary": self.synthetic_runtime_held_state_boundary_artifact(),
    ...
    def write_synthetic_basis(
        ...
        paths = {
            "held_boundary": root / f"{name}_runtime_held_state_boundary.json",
    ...
    def clean_request(self, paths: Mapping[str, Path]) -> dict[str, Any]:
        return resolver.build_declared_local_relevance_medium_read_only_runtime_held_state_v0_min_request(
            selected_runtime_held_state_boundary_artifact=paths["held_boundary"],
    ...
    def mutate_artifact_top_level(
        self, artifacts: dict[str, Any], paths: dict[str, Path], artifact_key: str, field: str, value: Any
    ) -> None:
        artifacts[artifact_key][field] = value
        self.write_json(paths[artifact_key], artifacts[artifact_key])

    def test_public_api_constants_and_default_builder(self) -> None:
        for name in (
            "resolve_local_relevance_medium_read_only_runtime_held_state_v0_min",
            "resolve_local_relevance_medium_read_only_runtime_held_state_v0_min_from_path",
            "write_local_relevance_medium_read_only_runtime_held_state_v0_min_result",
            "build_local_relevance_medium_read_only_runtime_held_state_v0_min_summary",
            "build_declared_local_relevance_medium_read_only_runtime_held_state_v0_min_request",
        ):
        ...
        for name in (
            ...
            "RESOLVER_MODULE",
            "OUTPUT_ROOT",
            "SUPPORTED_HELD_STATE_TYPE_VALUES",
            "SUPPORTED_HELD_STATE_SCOPE_VALUES",
            "SELECTED_COMMAND",
        ...
        self.assertIn(HELD_STATE_TYPE, resolver.SUPPORTED_HELD_STATE_TYPE_VALUES)
        self.assertIn(HELD_STATE_SCOPE, resolver.SUPPORTED_HELD_STATE_SCOPE_VALUES)
        ...
        for outcome in (
            ...
            "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE_BLOCKED",
        ):
            self.assertIn(outcome, resolver.OUTCOME_FAMILY)

        request = (
            resolver.build_declared_local_relevance_medium_read_only_runtime_held_state_v0_min_request()
        )
        self.assertTrue(
        ...
        self.assertEqual(request["held_state_type"], HELD_STATE_TYPE)
        self.assertEqual(request["held_state_scope"], HELD_STATE_SCOPE)
    ...
    def test_successful_recorded_result_from_synthetic_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = resolver.resolve_local_relevance_medium_read_only_runtime_held_state_v0_min(
                request
            )
            self.assert_recorded_held_state(result, paths)

    def test_successful_recorded_result_from_default_artifacts_if_present(self) -> None:
        default_paths = {
            "held_boundary": DEFAULT_RUNTIME_HELD_STATE_BOUNDARY_ARTIFACT,
        ...
        self.assert_recorded_held_state(result, default_paths)

    def test_closure_older_runtime_raw_mutation_and_failure_lineage_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            base_request, _paths, _artifacts = self.clean_request_with_artifacts(Path(tmp))
            for key, expected_code in TOP_LEVEL_EXACT_BLOCK_CODES:
                with self.subTest(key=key):
                    request = copy.deepcopy(base_request)
                    request[key] = True
                    result = (
                        resolver.resolve_local_relevance_medium_read_only_runtime_held_state_v0_min(
                            request
                        )
                    )
            ...
            for key, _expected_code in TOP_LEVEL_EXACT_BLOCK_CODES:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for key in resolver.REQUIRED_FALSE_NON_CLAIMS:
                with self.subTest(key=key):
                    ...
                    self.assert_runtime_held_state_non_claims(result)
                    held_state = self.held_state(result)
                    self.assertIs(held_state["raw_state_body_embedded"], False)
                    self.assertIs(held_state["state_mutation_performed"], False)
                    self.assertIs(held_state["state_update_performed"], False)
                    self.assertIs(held_state["older_runtime_lineage_imported_as_authority"], False)
                    self.assertIs(held_state["older_runtime_permission_treated_as_current"], False)
                    self.assertIs(held_state["runtime_authority_imported"], False)
                    self.assertIs(held_state["follow_on_work_authorized"], False)
    ...
    def representative_cases(
        ...
        def artifact_outcome(
            ...
            def mutate(request: dict[str, Any], artifacts: dict[str, Any], paths: dict[str, Path]) -> None:
                self.mutate_artifact_top_level(artifacts, paths, artifact_key, "outcome", value)
        ...
        def artifact_version(
            ...
            def mutate(request: dict[str, Any], artifacts: dict[str, Any], paths: dict[str, Path]) -> None:
                self.mutate_artifact_top_level(artifacts, paths, artifact_key, "result_version", value)
        ...
        def artifact_failed_checks(
            ...
            def mutate(request: dict[str, Any], artifacts: dict[str, Any], paths: dict[str, Path]) -> None:
                self.mutate_artifact_top_level(artifacts, paths, artifact_key, "failed_check_count", value)
        ...
        def artifact_unreadable(
            ...
            request_key = {
                "held_boundary": "selected_runtime_held_state_boundary_artifact",
        ...
        cases: list[
            ...
            (
                ...
                request_set(
                    "local_relevance_medium_read_only_runtime_held_state_intent",
                    "BLOCK_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE",
                ),
            ),
            (
                "missing question",
                "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE_QUESTION_UNDECLARED",
                request_pop("local_relevance_medium_read_only_runtime_held_state_question"),
            ),
            (
                "unsupported intent",
            ...
            ("runtime-held-state boundary artifact path missing", "RUNTIME_HELD_STATE_BOUNDARY_ARTIFACT_PATH_MISSING", artifact_path_missing("selected_runtime_held_state_boundary_artifact")),
            ("runtime-held-state boundary artifact unreadable", "RUNTIME_HELD_STATE_BOUNDARY_ARTIFACT_UNREADABLE", artifact_unreadable("held_boundary")),
            ("runtime-held-state boundary artifact JSON array", "RUNTIME_HELD_STATE_BOUNDARY_ARTIFACT_NOT_JSON_OBJECT", artifact_array("held_boundary")),
            ("runtime-held-state boundary artifact not recorded", "RUNTIME_HELD_STATE_BOUNDARY_ARTIFACT_NOT_RECORDED", artifact_outcome("held_boundary", "WRONG")),
            ("runtime-held-state boundary artifact failed checks", "RUNTIME_HELD_STATE_BOUNDARY_ARTIFACT_FAILED_CHECKS_PRESENT", artifact_failed_checks("held_boundary", 1)),
            ("runtime-held-state boundary artifact version mismatch", "RUNTIME_HELD_STATE_BOUNDARY_ARTIFACT_VERSION_NOT_0_1_0", artifact_version("held_boundary", "9.9.9")),
            ...
            ("held state type missing", "HELD_STATE_TYPE_MISSING", request_set("held_state_type", "")),
            ("held state type wrong", "HELD_STATE_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE", request_set("held_state_type", "REGISTRY")),
            ("held state scope missing", "HELD_STATE_SCOPE_MISSING", request_set("held_state_scope", "")),
            ("held state scope wrong", "HELD_STATE_SCOPE_NOT_SELECTED_RUNTIME_HELD_STATE_ONLY", request_set("held_state_scope", "SEARCH_VIEW")),
            ("required non-claim missing", "NON_CLAIM_MISSING_OR_FLIPPED", lambda request, artifacts, paths: request["declared_non_claims"].pop("raw_state_body_embedded", None)),
        ]
        ...
        shortcut_flag_codes = (
            ...
            ("local_relevance_medium_read_only_runtime_held_state_not_recorded", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RUNTIME_HELD_STATE_NOT_RECORDED"),
        ...
        top_level_flag_codes = (
            ...
            ("consumed_request_reopened", "CONSUMED_REQUEST_REOPENED"),
            ("authorization_token_reused", "AUTHORIZATION_TOKEN_REUSED"),
            ("artifact_existence_treated_as_runtime_held_state_authority", "ARTIFACT_EXISTENCE_TREATED_AS_RUNTIME_HELD_STATE_AUTHORITY"),
            ("latest_file_posture_treated_as_runtime_held_state_authority", "LATEST_FILE_POSTURE_TREATED_AS_RUNTIME_HELD_STATE_AUTHORITY"),
            ("repo_local_availability_treated_as_runtime_held_state_authority", "REPO_LOCAL_AVAILABILITY_TREATED_AS_RUNTIME_HELD_STATE_AUTHORITY"),
            ("hidden_repo_state_used_as_runtime_held_state_content", "HIDDEN_REPO_STATE_USED_AS_RUNTIME_HELD_STATE_CONTENT"),
            ("hidden_repo_state_used_as_runtime_held_state_authority", "HIDDEN_REPO_STATE_USED_AS_RUNTIME_HELD_STATE_AUTHORITY"),
        ...
        for key, code in shortcut_flag_codes + top_level_flag_codes:
    ...
    def test_representative_blocking_behavior(self) -> None:
        for malformed in (None, ["not", "a", "mapping"]):
            with self.subTest(malformed=type(malformed).__name__):
                result = resolver.resolve_local_relevance_medium_read_only_runtime_held_state_v0_min(  # type: ignore[arg-type]
                    malformed
                )
                self.assert_blocked_with_public_code(result)
                self.assertEqual(
    ...
    def test_official_values_are_preserved(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assert_recorded_held_state(result)
    ...
    def test_raw_hidden_and_older_runtime_hostile_content_containment(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            request["raw_runtime_held_state_body"] = HOSTILE_SENTINELS[0]
            request["raw_state_body"] = HOSTILE_SENTINELS[6]
            ...
            serialized = json.dumps(result, ensure_ascii=False, sort_keys=True)
            self.assertIn(HELD_STATE_TYPE, serialized)
            self.assertIn(HELD_STATE_SCOPE, serialized)
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = resolver.resolve_local_relevance_medium_read_only_runtime_held_state_v0_min_from_path(
                request_path
            )
            self.assert_recorded_held_state(result)
            ...
            malformed_result = resolver.resolve_local_relevance_medium_read_only_runtime_held_state_v0_min_from_path(
                malformed_path
            )
            self.assert_blocked_with_public_code(malformed_result)

            array_path = self.write_json(root / "array.json", ["not", "object"])
            array_result = resolver.resolve_local_relevance_medium_read_only_runtime_held_state_v0_min_from_path(
                array_path
            )
            self.assert_blocked_with_public_code(array_result)

            missing_result = resolver.resolve_local_relevance_medium_read_only_runtime_held_state_v0_min_from_path(
            ...
            with mock.patch.object(resolver, "OUTPUT_ROOT", output_root):
                first_path = resolver.write_local_relevance_medium_read_only_runtime_held_state_v0_min_result(
                    result
                )
                second_path = resolver.write_local_relevance_medium_read_only_runtime_held_state_v0_min_result(
                    result
                )
    ...
    def test_non_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            request["raw_runtime_loop_body"] = HOSTILE_SENTINELS[8]
    ...
    def test_failure_lineage_and_predecessor_preservation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assert_recorded_held_state(result)
            summary = resolver.build_local_relevance_medium_read_only_runtime_held_state_v0_min_summary(
                result
            )
            ...
            for key, expected_code in TOP_LEVEL_EXACT_BLOCK_CODES[8:]:
                with self.subTest(key=key):
