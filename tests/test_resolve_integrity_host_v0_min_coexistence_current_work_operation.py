"""Bounded tests for the v0-min current-work-operation resolver.

These tests lock the current behavior of
``src/resolve_integrity_host_v0_min_coexistence_current_work_operation.py``.
...
current-work-input references only. It does not test replay, merge, persistence
...
import resolve_integrity_host_v0_min_coexistence_current_work_operation as operation_resolver  # noqa: E402
...
class IntegrityHostV0MinCoexistenceCurrentWorkOperationTests(unittest.TestCase):
    def emit_source_run(
        self,
        temp_root: Path,
    ...
    def valid_proposal(
        ...
        return {
            ...
            "proposed_by_surface": (
                "tests/test_resolve_integrity_host_v0_min_coexistence_current_work_operation.py"
            ),
        }
    ...
    def test_write_behavior_and_default_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            short_result = self.copy_payload(result)
            short_result["work_operation_metadata"]["work_result_id"] = (
                "current_work_operation_test_result"
            )
