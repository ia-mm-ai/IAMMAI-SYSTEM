"""Tests for the consumed-request command invocation authorization resolver.

This suite is bounded to command invocation authorization only. Authorization
may record one one-shot authorization token as basis for a future separately
bounded command invocation step, but these tests do not create command
invocation, command execution, command output, command result, command success,
...
import resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization as resolver
from resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization import (
    build_declared_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_request,
    ...
    resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization,
    resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_from_path,
    write_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_result,
)
...
QUESTION = resolver.CORE_COMMAND_INVOCATION_AUTHORIZATION_QUESTION
...
SUPPORTED_SCOPE = tuple(resolver.SUPPORTED_COMMAND_INVOCATION_AUTHORIZATION_SCOPE)
...
TOP_LEVEL_SECTIONS = (
    ...
    "command_invocation_authorization_scope",
    "command_invocation_authorization_checks",
    "command_invocation_authorization_statement",
    "command_invocation_authorization_non_meaning",
...
def _authorization_boundary_basis(**extra: Any) -> dict[str, Any]:
...
def _authorization_boundary_terminal_summary_basis(**extra: Any) -> dict[str, Any]:
    basis = _reference_shape(
        "command_invocation_authorization_boundary_terminal_summary",
        ...
        terminal_summary_path=(
            ...
            "COMMAND_INVOCATION_AUTHORIZATION_BOUNDARY_TERMINAL_SUMMARY_V0.md"
        ),
...
def _valid_request(**overrides: Any) -> dict[str, Any]:
    request = {
        "command_invocation_authorization_request_id": "command_invocation_authorization_request_001",
        "command_invocation_authorization_question": QUESTION,
        "command_invocation_authorization_intent": resolver.INTENT_RECORD,
        "selected_command_invocation_authorization_boundary_basis": _authorization_boundary_basis(),
        "selected_command_invocation_authorization_boundary_terminal_summary_basis": _authorization_boundary_terminal_summary_basis(),
        ...
        "authorization_only_posture": _posture("authorization_only_posture"),
        "one_shot_authorization_posture": _posture("one_shot_authorization_posture"),
        "authorization_token_posture": _posture("authorization_token_posture"),
        ...
        "command_invocation_authorization_scope": list(SUPPORTED_SCOPE),
        "declared_non_claims": _false_non_claims(),
        "requested_command_invocation_authorization_outcome": RECORDED,
...
def _resolve(request: dict[str, Any]) -> dict[str, Any]:
    return resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization(
        declared_command_invocation_authorization_request=request
    )
...
class CommandInvocationAuthorizationResolverTests(unittest.TestCase):
    def assert_blocked(self, request: Any, expected_code: str) -> dict[str, Any]:
        if isinstance(request, dict):
            result = _resolve(request)
        else:
            result = resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization(
                declared_command_invocation_authorization_request=request
            )
        self.assertEqual(BLOCKED, result["outcome"])
        self.assertEqual(expected_code, result["block"]["code"])
        self._assert_no_invocation_execution_success(result)
        return result

    def _assert_no_invocation_execution_success(self, result: dict[str, Any]) -> None:
        statement = result["command_invocation_authorization_statement"]
    ...
    def test_recorded_result_is_bounded_authorization_only(self) -> None:
        ...
        self.assertEqual(0, result["command_invocation_authorization_checks"]["failed_check_count"])
    ...
    def test_metadata_is_preserved(self) -> None:
        ...
        for key in (
            "consumed_single_live_command_invocation_request_command_invocation_authorization_result_id",
            "consumed_single_live_command_invocation_request_command_invocation_authorization_result_type",
            "consumed_single_live_command_invocation_request_command_invocation_authorization_result_version",
            "generated_at",
            "resolver_module",
        ):
            self.assertTrue(metadata[key], key)
        self.assertEqual("0.1.0", metadata["consumed_single_live_command_invocation_request_command_invocation_authorization_result_version"])
        self.assertEqual(
            "resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization",
    ...
    def test_selected_basis_lineage_is_preserved_as_reference_shape(self) -> None:
        result = _resolve(_valid_request())
        boundary = result["selected_command_invocation_authorization_boundary_basis"]
        self.assertEqual(COMMAND_INVOCATION_AUTHORIZATION_BOUNDARY_OUTCOME, boundary["outcome"])
        self.assertEqual(0, boundary["failed_check_count"])
        self.assertIs(boundary["authorization_boundary_did_not_create_command_invocation_authorization"], True)
        self.assertIs(boundary["authorization_boundary_did_not_create_invocation_execution_output_result_success"], True)
        self.assertIs(boundary["authorization_boundary_did_not_create_execution_permission_or_approval"], True)
        self.assertIs(boundary["authorization_boundary_did_not_create_standing_lane_or_repeat_permission"], True)
        self.assertIs(boundary["reference_shaped_basis"], True)
        ...
        self.assertIs(consumed["consumed_request_basis_only"], True)
    ...
    def test_checks_are_explicit_and_pass_for_recorded_case(self) -> None:
        checks = _resolve(_valid_request())["command_invocation_authorization_checks"]
        ...
        expected_names = {
            ...
            "authorization-only posture declared",
            "one-shot-authorization posture declared",
            "authorization-token posture declared",
            ...
            "authorization not invocation",
            "authorization not execution",
            "authorization not command success",
    ...
    def test_command_invocation_authorization_non_meaning(self) -> None:
        non_meaning = _resolve(_valid_request())["command_invocation_authorization_non_meaning"]
        for suffix in (
            "command_invocation_created",
            "command_executed",
            ...
            self.assertIs(non_meaning[f"command_invocation_authorization_does_not_mean_{suffix}"], True)

    def test_requires_additional_basis_and_not_recorded_are_bounded(self) -> None:
        additional = {"missing_basis": ["authorization-token posture unclear"], "reason": "needs bounded basis"}
        requires_request = _valid_request(
            requested_command_invocation_authorization_outcome=REQUIRES_ADDITIONAL_BASIS,
        ...
        self._assert_no_invocation_execution_success(requires)

        not_recorded_basis = {"reason": "authorization basis cannot be bounded"}
        not_recorded_request = _valid_request(
            requested_command_invocation_authorization_outcome=NOT_RECORDED,
        ...
        self._assert_no_invocation_execution_success(not_recorded)
    ...
    def test_summary_helper_preserves_key_authorization_and_non_claim_fields(self) -> None:
        result = _resolve(_valid_request())
        summary = build_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_summary(
            result
        )
        self.assertEqual(RECORDED, summary["outcome"])
        self.assertIsNone(summary["block_code"])
        self.assertIsNone(summary["block_reason"])
        self.assertEqual("command_invocation_authorization_request_001", summary["command_invocation_authorization_request_id"])
        self.assertEqual(QUESTION, summary["command_invocation_authorization_question"])
        self.assertEqual(resolver.INTENT_RECORD, summary["command_invocation_authorization_intent"])
        ...
        for key in (
            ...
            "authorization_not_invocation",
            "authorization_not_execution",
            "authorization_not_command_success",
    ...
    def test_request_builder_helper_preserves_inputs_and_resolves(self) -> None:
        ...
        request = build_declared_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_request(
            "builder_request_001",
            QUESTION,
            _authorization_boundary_basis(),
            _authorization_boundary_terminal_summary_basis(),
            ...
            _posture("authorization_only_posture"),
            _posture("one_shot_authorization_posture"),
            _posture("authorization_token_posture"),
        ...
        self.assertEqual("builder_request_001", request["command_invocation_authorization_request_id"])
        self.assertEqual(QUESTION, request["command_invocation_authorization_question"])
        ...
        self.assertEqual(list(SUPPORTED_SCOPE), request["command_invocation_authorization_scope"])
    ...
    def test_path_based_resolution_and_write_behavior(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path_result = resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_from_path(
                request_path
            )
            self.assertEqual(RECORDED, path_result["outcome"])
            self.assertEqual(set(mapping_result), set(path_result))
            self.assertEqual(str(request_path), path_result["declared_command_invocation_authorization_question"]["request_path"])

            malformed_path = tmp_path / "malformed.json"
            malformed_path.write_text("{not json", encoding="utf-8")
            malformed = resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_from_path(
                malformed_path
            )
            self.assertEqual(BLOCKED, malformed["outcome"])
            self.assertEqual("DECLARED_COMMAND_INVOCATION_AUTHORIZATION_REQUEST_UNREADABLE", malformed["block"]["code"])

            array_path = tmp_path / "array.json"
            array_path.write_text("[]", encoding="utf-8")
            array_result = resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_from_path(
                array_path
            )
            self.assertEqual(BLOCKED, array_result["outcome"])
            self.assertEqual("DECLARED_COMMAND_INVOCATION_AUTHORIZATION_REQUEST_MALFORMED", array_result["block"]["code"])

            missing = resolve_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_from_path(
                tmp_path / "missing.json"
            )
            self.assertEqual(BLOCKED, missing["outcome"])
            self.assertEqual("DECLARED_COMMAND_INVOCATION_AUTHORIZATION_REQUEST_UNREADABLE", missing["block"]["code"])

            output_path = tmp_path / "nested" / "authorization_result.json"
            written = write_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_result(
            ...
            second = write_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_result(
                mapping_result, output_path
            )
            self.assertNotEqual(written, second)
            self.assertTrue(second.name.endswith("_001.json"))

            default_root = tmp_path / "bounded_command_invocation_authorization"
            with patch.object(
                resolver,
                "PORTABLE_SOURCE_BODY_VERIFICATION_CONSUMED_SINGLE_LIVE_COMMAND_INVOCATION_REQUEST_COMMAND_INVOCATION_AUTHORIZATION_ROOT",
                default_root,
            ):
                default_written = write_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_result(
                    mapping_result
                )
    ...
    def test_reference_shaped_containment_blocks_full_prior_artifact_body(self) -> None:
        boundary_request = _set_nested(
            _valid_request(),
            "selected_command_invocation_authorization_boundary_basis",
            full_artifact_body=RAW_FULL_BODY_SENTINEL,
        )
        boundary_result = self.assert_blocked(boundary_request, "FULL_PRIOR_ARTIFACT_BODY_EMITTED")
        self.assertNotIn(RAW_FULL_BODY_SENTINEL, _json_text(boundary_result))
        self._assert_no_invocation_execution_success(boundary_result)
        ...
        self._assert_no_invocation_execution_success(consumption_result)
    ...
    def test_resolver_does_not_mutate_inputs(self) -> None:
        ...
        self.assertEqual(before["selected_command_invocation_authorization_boundary_terminal_summary_basis"], request["selected_command_invocation_authorization_boundary_terminal_summary_basis"])
        ...
        self.assertEqual(before["authorization_only_posture"], request["authorization_only_posture"])
        self.assertEqual(before["command_invocation_authorization_scope"], request["command_invocation_authorization_scope"])

        with tempfile.TemporaryDirectory() as tmp:
            output = write_portable_source_body_verification_consumed_single_live_command_invocation_request_command_invocation_authorization_result(
                first, Path(tmp) / "authorization" / "result.json"
            )
            self.assertTrue(output.exists())
            self.assertTrue(str(output).startswith(tmp))

    def test_blocking_missing_or_malformed_request_inputs(self) -> None:
        self.assert_blocked(None, "COMMAND_INVOCATION_AUTHORIZATION_QUESTION_UNDECLARED")
        self.assert_blocked(["not", "mapping"], "DECLARED_COMMAND_INVOCATION_AUTHORIZATION_REQUEST_MALFORMED")
        explicit = _valid_request(command_invocation_authorization_intent=resolver.INTENT_BLOCK)
        blocked = _resolve(explicit)
        self.assertEqual(BLOCKED, blocked["outcome"])
        self.assertEqual("COMMAND_INVOCATION_AUTHORIZATION_BLOCKED_BY_REQUEST", blocked["block"]["code"])
    ...
    def test_blocking_basis_and_posture_failures(self) -> None:
        cases = (
            ...
            (_drop(_valid_request(), "selected_command_invocation_authorization_boundary_terminal_summary_basis"), "COMMAND_INVOCATION_AUTHORIZATION_BOUNDARY_TERMINAL_SUMMARY_MISSING"),
            ...
            (_drop(_valid_request(), "authorization_only_posture"), "AUTHORIZATION_ONLY_POSTURE_MISSING"),
            (_drop(_valid_request(), "one_shot_authorization_posture"), "ONE_SHOT_AUTHORIZATION_POSTURE_MISSING"),
            (_drop(_valid_request(), "authorization_token_posture"), "AUTHORIZATION_TOKEN_POSTURE_MISSING"),
            ...
            (_valid_request(command_invocation_authorization_scope=["UNSUPPORTED_SCOPE"]), "UNSUPPORTED_COMMAND_INVOCATION_AUTHORIZATION_SCOPE"),
        )
