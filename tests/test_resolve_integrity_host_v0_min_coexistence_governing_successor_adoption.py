"""Bounded tests for the v0-min governing successor-adoption resolver.

These tests lock the current additive successor-adoption surface in
``src/resolve_integrity_host_v0_min_coexistence_governing_successor_adoption.py``
...
They verify adopted and blocked outcomes, successor-family references, explicit
checks, non-claims, write behavior, malformed input failures, and read-only
posture. They do not test replay, merge, persistence architecture, registry
integration, distributed continuity, CLI behavior, or broad governance
frameworks.
"""
...
import resolve_integrity_host_v0_min_coexistence_governing_successor_adoption as adoption_resolver  # noqa: E402
...
class IntegrityHostV0MinCoexistenceGoverningSuccessorAdoptionTests(
    unittest.TestCase
):
    def emit_source_run(
        self,
    ...
    def emit_reresolution_result_from_object(
        ...
        result = self.adoption_ready_reresolution_result(result)
    ...
    def valid_proposal(
        ...
        return {
            ...
            "proposed_by_surface": (
                "tests/test_resolve_integrity_host_v0_min_coexistence_governing_successor_adoption.py"
            ),
        }
    ...
    def adoption_ready_reresolution_result(
        self,
        payload: dict[str, Any],
    ) -> dict[str, Any]:
        result = self.copy_payload(payload)
        if not isinstance(result.get("selected_reresolution_result"), dict):
    ...
    def test_blocked_no_successor_projection_available_or_only_blocked(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            blocked_reresolution = self.adoption_ready_reresolution_result(
