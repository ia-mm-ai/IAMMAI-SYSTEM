"""Tests for command-success-boundary v3 containment and summary surface.

This suite treats v1 as preserved raw-body containment failure evidence and v2
as preserved summary-surface failure evidence. V3 is an additive successor that
keeps v2 raw full-body containment, exposes the previously missing
``bounded_command_output_report_artifact_preserved`` summary field, and records
one future command success boundary only. It does not create command success,
success body, authority, currentness, final completion, public readiness,
deployment readiness, operation permission, continuation, reusable permission,
or follow-on work.
"""
...
import resolve_portable_source_body_verification_command_success_boundary_v3 as resolver
...
def _request(**overrides: Any) -> dict[str, Any]:
    request = (
        resolver.build_declared_portable_source_body_verification_command_success_boundary_v3_request()
    )
...
class CommandSuccessBoundaryV3ResolverTests(unittest.TestCase):
    ...
    def assert_v3_metadata(self, result: Mapping[str, Any]) -> None:
        metadata = result["portable_source_body_verification_command_success_boundary_metadata"]
        self.assertEqual(metadata["result_version"], "0.3.0")
        self.assertEqual(
        ...
        self.assertIn("v1", successor_reason.lower())
        self.assertIn("raw", successor_reason.lower())
        self.assertIn("containment", successor_reason.lower())
        self.assertIn("v2", successor_reason.lower())
        self.assertIn("summary-surface", successor_reason.lower())
        self.assertIn("bounded_command_output_report_artifact_preserved", successor_reason)
        ...
        self.assertIs(metadata["v2_predecessor_failure_preserved"], True)
        self.assertIs(metadata["v2_repaired"], False)
        self.assertIs(metadata["v2_hidden"], False)
        self.assertIs(metadata["v2_claimed_passed"], False)
        self.assertIs(metadata["v3_successor_does_not_erase_v1"], True)
        self.assertIs(metadata["v3_successor_does_not_erase_v2"], True)
        ...
        self.assertIs(metadata["summary_surface_conformance_preserved"], True)
    ...
    def assert_summary_recorded_surface(self, summary: Mapping[str, Any]) -> None:
        self.assertIn("bounded_command_output_report_artifact_preserved", summary)
    ...
    def test_public_api_constants_and_block_codes(self) -> None:
        ...
        self.assert_v3_metadata(alias_result)
        ...
        self.assertIs(alias_summary["bounded_command_output_report_artifact_preserved"], True)

    def test_successful_v3_command_success_boundary_recorded_result(self) -> None:
        request = _request()
        ...
        result = resolver.resolve_portable_source_body_verification_command_success_boundary_v3(
        ...
        self.assert_v3_metadata(result)
        ...
        embedded_summary = result["portable_source_body_verification_command_success_boundary_summary"]
        summary = resolver.build_portable_source_body_verification_command_success_boundary_v3_summary(result)
        for key in TRUE_RECORDED_FIELDS:
            self.assertIs(statement.get(key), True, key)
            self.assertIs(embedded_summary.get(key), True, key)
            self.assertIs(summary.get(key), True, key)
        self.assert_summary_recorded_surface(summary)
    ...
    def test_raw_full_body_sentinel_is_contained_without_mutating_input(self) -> None:
        request = _request(command_success_boundary_request_id="raw_body_containment_v3")
    ...
    def test_representative_blocking_behavior_preserves_v3_metadata(self) -> None:
    ...
    def test_path_and_write_behavior_is_bounded_to_v3_root(self) -> None:
        request = _request(command_success_boundary_request_id="success_boundary_v3_path_test")
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            request_path = temp_root / "request.json"
            request_path.write_text(json.dumps(request, indent=2, sort_keys=True), encoding="utf-8")

            result = resolver.resolve_portable_source_body_verification_command_success_boundary_v3_from_path(
                request_path
            )
            self.assertEqual(result["outcome"], RECORDED)
            self.assert_v3_metadata(result)
            ...
            malformed = resolver.resolve_portable_source_body_verification_command_success_boundary_v3_from_path(
                malformed_path
            )
            self.assertEqual(malformed["outcome"], BLOCKED)
            self.assert_v3_metadata(malformed)
            self.assert_public_block_codes(malformed)

            array_path = temp_root / "array.json"
            array_path.write_text("[]", encoding="utf-8")
            array_result = resolver.resolve_portable_source_body_verification_command_success_boundary_v3_from_path(
                array_path
            )
            self.assertEqual(array_result["outcome"], BLOCKED)
            self.assert_v3_metadata(array_result)

            missing_result = resolver.resolve_portable_source_body_verification_command_success_boundary_v3_from_path(
                temp_root / "missing.json"
            )
            self.assertEqual(missing_result["outcome"], BLOCKED)
            self.assert_v3_metadata(missing_result)

            output_root = (
                temp_root
                / "artifacts"
                / "integrity_host_v0_min_coexistence_portable_source_body_verification_command_success_boundary_v3"
            )
            v1_root_name = (
                "integrity_host_v0_min_coexistence_portable_source_body_verification_"
            ...
            with patch.object(resolver, "OUTPUT_ROOT", output_root):
                first_path = resolver.write_portable_source_body_verification_command_success_boundary_v3_result(result)
                second_path = resolver.write_portable_source_body_verification_command_success_boundary_v3_result(result)
            ...
            self.assertEqual(first_path.parent.name, f"{v1_root_name}_v3")
            self.assertNotEqual(first_path.parent.name, v1_root_name)
            self.assertNotEqual(first_path.parent.name, f"{v1_root_name}_v2")
    ...
    def test_summary_helper_preserves_v3_successor_and_required_summary_surface(self) -> None:
        request = _request(command_success_boundary_request_id="summary_v3_request")
        result = resolver.resolve_portable_source_body_verification_command_success_boundary_v3(
            declared_command_success_boundary_request=request
        )
        summary = resolver.build_portable_source_body_verification_command_success_boundary_v3_summary(result)
        ...
        self.assertEqual(summary["request_id"], "summary_v3_request")
        ...
        self.assertIn(
            "resolve_portable_source_body_verification_command_success_boundary",
            summary["predecessor_chain"],
        )
        self.assertIn(
            "resolve_portable_source_body_verification_command_success_boundary_v2",
            summary["predecessor_chain"],
        )
        self.assertIn("summary-surface", summary["successor_reason"].lower())
        self.assertIn("bounded_command_output_report_artifact_preserved", summary["successor_reason"])
        ...
        self.assertIs(summary["v2_predecessor_failure_preserved"], True)
        self.assertIs(summary["v2_repaired"], False)
        self.assertIs(summary["v2_hidden"], False)
        self.assertIs(summary["v2_claimed_passed"], False)
        self.assertIs(summary["v3_successor_does_not_erase_v1"], True)
        self.assertIs(summary["v3_successor_does_not_erase_v2"], True)
        ...
        self.assertIs(summary["summary_surface_conformance_preserved"], True)
        self.assert_summary_recorded_surface(summary)
        self.assertEqual(
