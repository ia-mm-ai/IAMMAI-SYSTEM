"""Successor tests for the admitted request consumption-boundary resolver.

The predecessor test file is preserved as historical failed-lineage evidence.
This successor keeps the same boundary law while asserting the resolver's
actual additional-basis shape: missing basis is not scheduled, not authorized,
and not executed through three explicit fields.

Consumption boundary is not request consumption. The admitted request is not
consumed here. Command invocation, command execution, command output, command
result, command success, standing invocation lane, repeat permission,
authority, currentness, final completion, continuation, reusable permission,
derivative reception, vessel relation, another reception request, and follow-on
work are not created.
"""
...
def _valid_request(**extra: Any) -> dict[str, Any]:
    request = {
        "consumption_boundary_request_id": "admitted_consumption_boundary_reference_review_001_v2",
...
def _assert_no_request_or_command_created(
    testcase: unittest.TestCase,
    result: dict[str, Any],
) -> None:
...
class PortableSourceBodyVerificationAdmittedRequestConsumptionBoundaryV2Tests(
    unittest.TestCase
):
    ...
    def test_metadata_v2_basis_and_v1_lineage_are_preserved(self) -> None:
    ...
    def test_checks_and_non_meaning_preserve_consumption_boundary_only(self) -> None:
        result = _resolve(_valid_request())
        records = result["consumption_boundary_checks"]["records"]
    ...
    def test_additional_basis_and_not_recorded_outcomes_are_bounded(self) -> None:
        ...
        self.assertIn("selected v2 successor metadata unclear", _json_text(additional_basis))
    ...
    def test_what_remains_open_and_summary_helpers(self) -> None:
        result = _resolve(_valid_request())
        ...
        expected_summary_values = {
            ...
            "consumption_boundary_request_id": (
                "admitted_consumption_boundary_reference_review_001_v2"
            ),
            "consumption_boundary_question": QUESTION,
            "failed_check_count": 0,
    ...
    def test_request_builder_helper_builds_resolvable_request(self) -> None:
        request = (
            build_declared_portable_source_body_verification_admitted_single_live_command_invocation_request_consumption_request(
                consumption_boundary_request_id="builder_consumption_review_001_v2",
                consumption_boundary_question=QUESTION,
                selected_v2_admitted_request_basis=_selected_v2_admitted_request_basis(),
                selected_v2_terminal_summary_basis=_selected_v2_terminal_summary_basis(),
                selected_v1_predecessor_failure_basis=_selected_v1_predecessor_failure_basis(),
        ...
        self.assertEqual("builder_consumption_review_001_v2", request["consumption_boundary_request_id"])
