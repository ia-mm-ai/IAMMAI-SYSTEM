"""Executable checks for cross-carrier evidence v3 posture only.

This suite is downstream of the cross-carrier-evidence-boundary line. It
verifies that the v3 resolver records one bounded cross-carrier evidence
posture only and blocks direct and nested predecessor/v2 overreach conditions
where the boundary treats string "0" as doctrine or promotes returned capture
into cross-carrier evidence/proof.

The predecessor cross-carrier evidence resolvers and the v2 test remain
preserved conformance-failure evidence. This suite does not repair, hide,
rename, delete, or claim passed those predecessor surfaces. It also preserves
predecessor external-result v1, first success-boundary, first result-boundary,
and v1 packet-emission failure surfaces as preserved predecessor evidence.
Official enum, scope, and block-code strings are checked for exact preservation
...
import resolve_portable_source_body_verification_cross_carrier_evidence_v3 as resolver
...
def set_deep_nested(section: str, name: str, value: Any) -> Callable[[dict[str, Any]], None]:
    def mutate(request: dict[str, Any]) -> None:
        target = request.setdefault(section, {})
        if not isinstance(target, dict):
            target = {}
            request[section] = target
        target.setdefault("nested_boundary_probe", []).append(
            {"first_layer": [{"second_layer": {name: value}}]}
        )
...
class CrossCarrierEvidenceV3Test(unittest.TestCase):
    ...
    def assert_predecessors_not_repaired_hidden_or_claimed_passed(
        ...
        claims = non_claims(result)
        predecessor = result.get("selected_predecessor_failure_basis", {})
        self.assertIs(
            predecessor.get("predecessor_cross_carrier_evidence_resolver_not_repaired"),
            True,
        )
        self.assertIs(
            predecessor.get("predecessor_cross_carrier_evidence_resolver_not_hidden"),
            True,
        )
        self.assertIs(
            predecessor.get("predecessor_cross_carrier_evidence_resolver_not_claimed_passed"),
            True,
        )
        self.assertIs(
            predecessor.get("predecessor_cross_carrier_evidence_v2_not_repaired"), True
        )
        self.assertIs(
            predecessor.get("predecessor_cross_carrier_evidence_v2_not_hidden"), True
        )
        self.assertIs(
            predecessor.get("predecessor_cross_carrier_evidence_v2_not_claimed_passed"),
            True,
        )
        for key in (
    ...
    def test_public_api_and_constants(self) -> None:
        ...
        self.assertEqual(
            ...
            "resolve_portable_source_body_verification_cross_carrier_evidence_v3",
        )
        self.assertEqual(
    ...
    def test_predecessor_and_v2_missed_boundary_overreach_cases_block(self) -> None:
        cases = (
            (
            ...
            (
                "nested returned capture evidence under boundary basis",
                set_deep_nested(
                    "selected_cross_carrier_evidence_boundary_basis",
                    "returned_capture_treated_as_cross_carrier_evidence",
                    True,
            ...
            (
                "nested returned capture proof under boundary basis",
                set_deep_nested(
                    "selected_cross_carrier_evidence_boundary_basis",
            ...
            (
                "nested returned capture evidence under terminal summary basis",
                set_deep_nested(
                    "selected_cross_carrier_evidence_boundary_terminal_summary_basis",
                    "returned_capture_treated_as_cross_carrier_evidence",
                    True,
            ...
            (
                "nested returned capture proof under terminal summary basis",
                set_deep_nested(
                    "selected_cross_carrier_evidence_boundary_terminal_summary_basis",
                    "returned_capture_treated_as_cross_carrier_proof",
                    True,
            ...
            (
                "nested returned capture promoted to evidence",
                set_deep_nested(
                    "selected_cross_carrier_evidence_boundary_basis",
            ...
            (
                "nested returned capture became evidence",
                set_deep_nested(
                    "selected_cross_carrier_evidence_boundary_basis",
    ...
    def test_representative_blocking_behavior(self) -> None:
        ...
        cases: tuple[
            ...
            ("explicit block intent", set_field("cross_carrier_evidence_intent", "BLOCK_PORTABLE_SOURCE_BODY_VERIFICATION_CROSS_CARRIER_EVIDENCE"), None),
            ("undeclared question", set_field("cross_carrier_evidence_question", ""), None),
            ("unsupported intent", set_field("cross_carrier_evidence_intent", "UNSUPPORTED_INTENT"), None),
            ("unsupported scope", set_field("cross_carrier_evidence_scope", ["UNSUPPORTED_CROSS_CARRIER_EVIDENCE_SCOPE_VALUE"]), None),
            ("missing cross-carrier evidence boundary basis", clear_field("selected_cross_carrier_evidence_boundary_basis"), None),
            ("cross-carrier evidence boundary not recorded", set_field("selected_cross_carrier_evidence_boundary_result_outcome", "NOT_RECORDED"), None),
            ("cross-carrier evidence boundary failed checks", set_field("selected_cross_carrier_evidence_boundary_failed_check_count", 1), None),
            ("cross-carrier evidence boundary version wrong", set_field("selected_cross_carrier_evidence_boundary_result_version", "9.9.9"), None),
            ("cross-carrier evidence boundary did not declare future review", set_field("selected_cross_carrier_evidence_boundary_declared_future_cross_carrier_evidence_review_step", False), None),
            ("cross-carrier evidence boundary created evidence", set_field("selected_cross_carrier_evidence_boundary_already_created_cross_carrier_evidence", True), None),
            ("cross-carrier evidence boundary created closure", set_field("selected_cross_carrier_evidence_boundary_already_created_portable_verification_closure", True), None),
            ("cross-carrier evidence boundary treated boundary as evidence", set_field("selected_cross_carrier_evidence_boundary_treated_boundary_as_cross_carrier_evidence", True), None),
            ("cross-carrier evidence boundary treated boundary as closure", set_field("selected_cross_carrier_evidence_boundary_treated_boundary_as_portable_verification_closure", True), None),
            ("cross-carrier evidence boundary treated zero exit code as evidence", set_field("selected_cross_carrier_evidence_boundary_zero_exit_code_not_cross_carrier_evidence", False), None),
            ("cross-carrier evidence boundary treated string zero as evidence", set_field("selected_cross_carrier_evidence_boundary_string_zero_not_cross_carrier_evidence", False), None),
            ("cross-carrier evidence boundary treated string zero as doctrine", set_field("selected_cross_carrier_evidence_boundary_treated_string_zero_as_doctrine", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_STRING_ZERO_AS_DOCTRINE"),
            ("cross-carrier evidence boundary treated returned capture as evidence", set_field("selected_cross_carrier_evidence_boundary_treated_returned_capture_as_cross_carrier_evidence", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_RETURNED_CAPTURE_AS_CROSS_CARRIER_EVIDENCE"),
            ("cross-carrier evidence boundary treated returned capture as proof", set_field("selected_cross_carrier_evidence_boundary_treated_returned_capture_as_cross_carrier_proof", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_RETURNED_CAPTURE_AS_CROSS_CARRIER_EVIDENCE"),
            ("nested boundary basis returned capture evidence", set_deep_nested("selected_cross_carrier_evidence_boundary_basis", "returned_capture_treated_as_cross_carrier_evidence", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_RETURNED_CAPTURE_AS_CROSS_CARRIER_EVIDENCE"),
            ("nested boundary basis returned capture proof", set_deep_nested("selected_cross_carrier_evidence_boundary_basis", "returned_capture_treated_as_cross_carrier_proof", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_RETURNED_CAPTURE_AS_CROSS_CARRIER_EVIDENCE"),
            ("nested terminal summary returned capture evidence", set_deep_nested("selected_cross_carrier_evidence_boundary_terminal_summary_basis", "returned_capture_treated_as_cross_carrier_evidence", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_RETURNED_CAPTURE_AS_CROSS_CARRIER_EVIDENCE"),
            ("nested terminal summary returned capture proof", set_deep_nested("selected_cross_carrier_evidence_boundary_terminal_summary_basis", "returned_capture_treated_as_cross_carrier_proof", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_RETURNED_CAPTURE_AS_CROSS_CARRIER_EVIDENCE"),
            ("nested boundary basis returned capture promoted", set_deep_nested("selected_cross_carrier_evidence_boundary_basis", "returned_capture_promoted_to_cross_carrier_evidence", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_RETURNED_CAPTURE_AS_CROSS_CARRIER_EVIDENCE"),
            ("nested boundary basis returned capture became", set_deep_nested("selected_cross_carrier_evidence_boundary_basis", "returned_capture_became_cross_carrier_evidence", True), "CROSS_CARRIER_EVIDENCE_BOUNDARY_TREATED_RETURNED_CAPTURE_AS_CROSS_CARRIER_EVIDENCE"),
            ("cross-carrier evidence boundary treated OK as evidence", set_field("selected_cross_carrier_evidence_boundary_ok_not_cross_carrier_evidence", False), None),
            ("cross-carrier evidence boundary treated Ran 7 tests as evidence", set_field("selected_cross_carrier_evidence_boundary_ran_7_tests_not_cross_carrier_evidence", False), None),
            ("cross-carrier evidence boundary redacted official strings", set_field("selected_cross_carrier_evidence_boundary_official_enum_scope_strings_redacted", True), None),
            ("predecessor external-result v1 failure missing", set_field("selected_cross_carrier_evidence_boundary_predecessor_external_result_v1_failure_preserved", False), None),
            ("predecessor external-result v1 repaired", set_field("predecessor_external_result_v1_repaired", True), None),
            ("second-carrier external result basis missing", clear_field("selected_second_carrier_external_result_basis"), None),
            ("second-carrier external result not recorded", set_field("selected_second_carrier_external_result_result_outcome", "NOT_RECORDED"), None),
            ("second-carrier external result failed checks", set_field("selected_second_carrier_external_result_failed_check_count", 1), None),
            ("second-carrier external result created evidence", set_field("selected_second_carrier_external_result_already_created_cross_carrier_evidence", True), None),
            ("second-carrier external result created closure", set_field("selected_second_carrier_external_result_already_created_portable_verification_closure", True), None),
            ("second-carrier external result treated external result as evidence", set_field("selected_second_carrier_external_result_treated_external_result_as_cross_carrier_evidence", True), None),
            ("second-carrier external result treated external result as closure", set_field("selected_second_carrier_external_result_treated_external_result_as_portable_verification_closure", True), None),
            ("second-carrier external result treated string zero as doctrine", set_field("selected_second_carrier_external_result_treated_string_zero_as_doctrine", True), None),
            ("second-carrier external-result boundary basis missing", clear_field("selected_second_carrier_external_result_boundary_basis"), None),
            ("second-carrier verification basis missing", clear_field("selected_second_carrier_verification_basis"), None),
            ("second-carrier verification not recorded", set_field("selected_second_carrier_verification_result_outcome", "NOT_RECORDED"), None),
            ("second-carrier verification failed checks", set_field("selected_second_carrier_verification_failed_check_count", 1), None),
            ("second-carrier success basis missing", clear_field("selected_second_carrier_success_basis"), None),
            ("second-carrier success not recorded", set_field("selected_second_carrier_success_result_outcome", "NOT_RECORDED"), None),
            ("second-carrier success failed checks", set_field("selected_second_carrier_success_failed_check_count", 1), None),
            ("second-carrier result basis missing", clear_field("selected_second_carrier_result_basis"), None),
            ("second-carrier result not recorded", set_field("selected_second_carrier_result_result_outcome", "NOT_RECORDED"), None),
            ("second-carrier result failed checks", set_field("selected_second_carrier_result_failed_check_count", 1), None),
            ("returned capture intake basis missing", clear_field("selected_returned_second_carrier_live_capture_intake_basis"), None),
            ("returned capture intake not preserved", set_field("selected_returned_capture_intake_preserved", False), None),
            ("returned capture treated as cross-carrier evidence", set_field("returned_capture_treated_as_cross_carrier_evidence", True), None),
            ("zero exit code treated as evidence", set_field("zero_exit_code_treated_as_cross_carrier_evidence", True), None),
            ("string zero treated as evidence", set_field("string_zero_treated_as_cross_carrier_evidence", True), None),
            ("OK output treated as evidence", set_field("ok_output_treated_as_cross_carrier_evidence", True), None),
            ("Ran 7 tests treated as evidence", set_field("ran_7_tests_treated_as_cross_carrier_evidence", True), None),
            ("placeholder carrier fields repaired", set_field("selected_returned_capture_placeholder_fields_unrepaired", False), None),
            ("returned capture material missing", clear_field("selected_returned_capture_material_basis"), None),
            ("returned zip path missing", set_field("selected_returned_capture_zip_path", ""), None),
            ("returned hash path missing", set_field("selected_returned_capture_hash_path", ""), None),
            ("returned extracted directory missing", set_field("selected_returned_capture_extracted_directory_path", ""), None),
            ("returned combined terminal log missing", set_field("selected_returned_capture_combined_terminal_log_path", ""), None),
            ("returned exit code missing", clear_field("selected_returned_capture_exit_code"), None),
            ("returned command text missing", set_field("selected_returned_capture_command_text", ""), None),
            ("returned timestamps missing", set_field("selected_returned_capture_started_at", ""), None),
            ("second-carrier output capture basis missing", clear_field("selected_second_carrier_output_capture_basis"), None),
            ("second-carrier output capture not recorded", set_field("selected_second_carrier_output_capture_result_outcome", "NOT_RECORDED"), None),
            ("second-carrier output capture failed checks", set_field("selected_second_carrier_output_capture_failed_check_count", 1), None),
            ("cross-carrier evidence treated as closure", set_non_claim("cross_carrier_evidence_treated_as_portable_verification_closure"), None),
            ("cross-carrier evidence treated as source transfer", set_non_claim("cross_carrier_evidence_treated_as_source_transfer"), None),
            ("cross-carrier evidence treated as source receipt", set_non_claim("cross_carrier_evidence_treated_as_source_receipt"), None),
            ("cross-carrier evidence treated as reception authorization", set_non_claim("cross_carrier_evidence_treated_as_reception_authorization"), None),
            ("cross-carrier evidence treated as source", set_non_claim("cross_carrier_evidence_treated_as_source"), None),
            ("cross-carrier evidence treated as authority", set_non_claim("cross_carrier_evidence_treated_as_authority"), None),
            ("cross-carrier evidence treated as currentness", set_non_claim("cross_carrier_evidence_treated_as_currentness"), None),
            ("cross-carrier evidence treated as final completion", set_non_claim("cross_carrier_evidence_treated_as_final_completion"), None),
            ("cross-carrier evidence treated as runtime", set_non_claim("cross_carrier_evidence_treated_as_runtime"), None),
            ("cross-carrier evidence treated as continuation", set_non_claim("cross_carrier_evidence_treated_as_continuation"), None),
            ("cross-carrier evidence treated as reusable permission", set_non_claim("cross_carrier_evidence_treated_as_reusable_permission"), None),
            ("cross-carrier evidence treated as follow-on work", set_non_claim("cross_carrier_evidence_treated_as_follow_on_work"), None),
            ("portable verification closure created", set_non_claim("portable_verification_closure_created"), None),
            ...
            ("final completion claimed", set_non_claim("final_completion_claimed"), None),
            ("runtime hosting created", set_non_claim("runtime_hosting_created"), None),
            ...
            ("continuation authorized", set_non_claim("continuation_authorized"), None),
            ...
            ("receiving carrier treated as authority", set_non_claim("receiving_carrier_treated_as_authority"), None),
            ("artifact existence treated as authority", set_non_claim("artifact_existence_treated_as_cross_carrier_evidence_authority"), None),
            ("artifact path treated as currentness", set_non_claim("artifact_path_treated_as_currentness"), None),
            ("repo-local availability treated as authority", set_non_claim("repo_local_availability_treated_as_cross_carrier_evidence_authority"), None),
            ("hidden repo state used as evidence content", set_non_claim("hidden_repo_state_used_as_cross_carrier_evidence_content"), None),
            ("hidden repo state used as authority", set_non_claim("hidden_repo_state_used_as_cross_carrier_evidence_authority"), None),
            ("selected basis not reference-shaped", set_field("reference_shaped_input_posture", False), None),
            ("raw full prior artifact body returned", set_non_claim("raw_full_prior_artifact_body_returned"), None),
            ("predecessor failure evidence repaired", set_field("v1_packet_emission_repaired", True), None),
            ("first success-boundary hidden", set_field("first_success_boundary_test_hidden", True), None),
            ("first result-boundary claimed passed", set_field("first_result_boundary_resolver_claimed_passed", True), None),
            ("command report lineage current report artifact", set_field("command_report_lineage_treated_as_current_report_artifact", True), None),
            ("command report lineage source", set_field("command_report_lineage_treated_as_source", True), None),
            ("command report lineage authority", set_field("command_report_lineage_treated_as_authority", True), None),
            ("command report lineage currentness", set_field("command_report_lineage_treated_as_currentness", True), None),
            ...
            ("full prior artifact body emitted outside bounded posture", set_field("full_prior_artifact_body_emitted_outside_bounded_cross_carrier_evidence", True), None),
            ("string zero representation turned into doctrine", set_field("string_zero_representation_turned_into_doctrine", True), None),
            ("artifacts mutated", set_non_claim("prior_artifacts_mutated"), None),
            ("returned capture material mutated", set_non_claim("returned_capture_material_mutated"), None),
            ("required non-claim missing", remove_non_claim("source_created"), None),
        )
        for label, mutate, expected_code in cases:
            with self.subTest(label=label):
                request = build_request()
    ...
    def test_predecessor_failure_preservation(self) -> None:
        ...
        self.assertIs(compact["predecessor_cross_carrier_evidence_v2_resolver_failure_preserved"], True)
        self.assertIs(compact["predecessor_cross_carrier_evidence_v2_test_failure_preserved"], True)
        self.assertIs(
            compact["predecessor_cross_carrier_evidence_v2_not_repaired_hidden_or_claimed_passed"],
            True,
        )
        ...
        self.assertIs(predecessor["predecessor_cross_carrier_evidence_v2_resolver_failure_preserved"], True)
        self.assertIs(predecessor["predecessor_cross_carrier_evidence_v2_test_failure_preserved"], True)
        self.assertIs(predecessor["predecessor_cross_carrier_evidence_v2_not_repaired"], True)
        self.assertIs(predecessor["predecessor_cross_carrier_evidence_v2_not_hidden"], True)
        self.assertIs(predecessor["predecessor_cross_carrier_evidence_v2_not_claimed_passed"], True)
