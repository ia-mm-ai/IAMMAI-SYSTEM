"""Successor tests for the local read-only lookup command execution boundary.

This v2 suite preserves the v1 lookup-command-execution-boundary coverage while
leaving the v1 test as failed-lineage evidence for its synthetic fixture
filename path-shaping bug. Representative slash-bearing case labels are never
allowed to control directory structure here.

The target remains one bounded
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_LOOKUP_COMMAND_EXECUTION_BOUNDARY object:
local, read-only, selected-state-lookup-command-execution-consideration-only,
closure-token-aware, non-lookup-command-execution-shaped,
non-lookup-performed-shaped, non-lookup-result-shaped, and non-operation-shaped.
"""
...
import resolve_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min as resolver  # noqa: E402
...
class LookupCommandExecutionBoundaryResolverV2Tests(unittest.TestCase):
    ...
    def write_synthetic_artifact(
        ...
        filename: str = "synthetic_raw_full_state_packet_body_exposure.json",
    ) -> Path:
        self.assertNotIn("/", filename)
        self.assertNotIn("\\", filename)
    ...
    def build_request(self, artifact_path: Path | str) -> dict[str, Any]:
        return resolver.build_declared_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_request(
            selected_raw_full_state_packet_body_exposure_artifact=str(artifact_path)
        )

    def summary(self, result: Mapping[str, Any]) -> Mapping[str, Any]:
        return result["local_relevance_medium_read_only_lookup_command_execution_boundary_summary"]

    def checks(self, result: Mapping[str, Any]) -> list[Mapping[str, Any]]:
        return result["local_relevance_medium_read_only_lookup_command_execution_boundary_checks"]

    def boundary(self, result: Mapping[str, Any]) -> Mapping[str, Any]:
        return result["local_relevance_medium_read_only_lookup_command_execution_boundary"]

    def statement(self, result: Mapping[str, Any]) -> Mapping[str, Any]:
        return result["local_relevance_medium_read_only_lookup_command_execution_boundary_statement"]
    ...
    def assert_lookup_command_execution_boundary_non_claims(
    ...
    def assert_blocked_with_public_code(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_lookup_command_execution_boundary_non_claims(result)
    ...
    def assert_clean_recorded_boundary(
        ...
        self.assertEqual(
            ...
            "SELECTED_LOOKUP_COMMAND_EXECUTION_CONSIDERATION_ONLY",
        )
        self.assert_same_or_stable_artifact_path(
            boundary["basis_raw_full_state_packet_body_exposure_artifact"],
            artifact_path,
        )
        self.assertEqual(
            boundary["basis_raw_full_state_packet_body_exposure_outcome"],
            "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RAW_FULL_STATE_PACKET_BODY_EXPOSURE_RECORDED",
        )
        self.assertEqual(
            boundary["basis_raw_full_state_packet_body_exposure_result_version"],
            "0.1.0",
        )
        self.assertEqual(
            boundary["basis_raw_full_state_packet_body_exposure_failed_check_count"],
            0,
        )
        self.assertEqual(boundary["selected_command"], "state")
        self.assertIs(boundary["selected_command_is_state"], True)
        self.assertIs(
            boundary["selected_raw_full_state_packet_body_exposure_recorded"], True
        )
        self.assertIs(boundary["raw_full_state_packet_body_exposed"], True)
        self.assertIs(boundary["raw_full_state_packet_body_exposure_local_only"], True)
        self.assertIs(boundary["raw_full_state_packet_body_exposure_read_only"], True)
        self.assertIs(
            boundary["future_lookup_command_execution_may_be_considered"], True
        )
        self.assert_lookup_command_execution_boundary_non_claims(result)
    ...
    def assert_path_blocks_or_raises(self, path: Path) -> None:
        try:
            result = resolver.resolve_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_from_path(
                path
            )
        except resolver.LocalRelevanceMediumReadOnlyLookupCommandExecutionBoundaryV0MinError:
            return
    ...
    def test_public_api_constants_and_default_builder(self) -> None:
        for name in (
            ...
            "resolve_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_from_path",
            "write_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_result",
            "build_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_summary",
            "build_declared_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_request",
        ):
        ...
        request = resolver.build_declared_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_request()
        self.assertTrue(
        ...
        self.assertEqual(
            self.safe_json_filename("selected raw/full not recorded", index=7),
            "007_selected_raw_full_not_recorded.json",
        )
    ...
    def test_successful_recorded_result_from_synthetic_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            artifact_path = self.write_synthetic_artifact(Path(tmp))
            result = resolver.resolve_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min(
    ...
    def test_successful_recorded_result_from_default_artifact_if_present(self) -> None:
        if not DEFAULT_RAW_FULL_STATE_PACKET_BODY_EXPOSURE_ARTIFACT.exists():
            self.skipTest("default raw/full state packet body exposure artifact is absent")
        request = resolver.build_declared_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_request()
        result = resolver.resolve_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min(
            request
        )
        self.assert_clean_recorded_boundary(
            result, DEFAULT_RAW_FULL_STATE_PACKET_BODY_EXPOSURE_ARTIFACT
        )
        boundary = self.boundary(result)
        self.assertIs(boundary["future_lookup_command_execution_may_be_considered"], True)
        for key in (
            "lookup_command_executed",
            "lookup_performed",
            "lookup_result_created",
    ...
    def test_representative_blocking_behavior_uses_safe_filenames(self) -> None:
        def set_artifact_summary(artifact: dict[str, Any], key: str, value: Any) -> None:
            artifact[
                "local_relevance_medium_read_only_raw_full_state_packet_body_exposure_summary"
            ][key] = value

        def set_artifact_exposure(artifact: dict[str, Any], key: str, value: Any) -> None:
            artifact[
                "local_relevance_medium_read_only_raw_full_state_packet_body_exposure"
            ][key] = value
            artifact[
            ...
            set_artifact_summary(artifact, key, value)
        ...
        cases: tuple[
            ...
            ("explicit block intent", None, lambda r: r.__setitem__("local_relevance_medium_read_only_lookup_command_execution_boundary_intent", "BLOCK_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_LOOKUP_COMMAND_EXECUTION_BOUNDARY"), None),
            ("missing request", None, None, "missing_request"),
            ("non-mapping request", None, None, "non_mapping"),
            ("unsupported intent", None, lambda r: r.__setitem__("local_relevance_medium_read_only_lookup_command_execution_boundary_intent", "UNSUPPORTED_INTENT"), None),
            ("artifact path missing", None, lambda r: r.pop("selected_raw_full_state_packet_body_exposure_artifact"), None),
            ("artifact unreadable", None, lambda r: r.__setitem__("selected_raw_full_state_packet_body_exposure_artifact", str(Path(r["_tmpdir"]) / "missing.json")), None),
            ("artifact JSON array", None, None, "artifact_array"),
            ("artifact not recorded", lambda a: (a.__setitem__("outcome", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RAW_FULL_STATE_PACKET_BODY_EXPOSURE_NOT_RECORDED"), set_artifact_summary(a, "outcome", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_RAW_FULL_STATE_PACKET_BODY_EXPOSURE_NOT_RECORDED")), None, None),
            ("artifact failed checks present", lambda a: set_artifact_summary(a, "failed_check_count", 1), None, None),
            ("artifact version not 0.1.0", lambda a: set_artifact_summary(a, "result_version", "9.9.9"), None, None),
            ...
            ("selected raw/full not recorded", lambda a: set_artifact_exposure(a, "local_relevance_medium_read_only_raw_full_state_packet_body_exposure_recorded", False), None, None),
            ("raw/full packet body not exposed", lambda a: set_artifact_exposure(a, "raw_full_state_packet_body_exposed", False), None, None),
            ("raw/full local only not true", lambda a: set_artifact_exposure(a, "raw_full_state_packet_body_exposure_local_only", False), None, None),
            ("raw/full read only not true", lambda a: set_artifact_exposure(a, "raw_full_state_packet_body_exposure_read_only", False), None, None),
            ("future lookup command execution may not be considered", None, lambda r: r.__setitem__("future_lookup_command_execution_may_not_be_considered", True), None),
            ("boundary type missing", None, lambda r: r.pop("boundary_type"), None),
            ("boundary type unsupported", None, lambda r: r.__setitem__("boundary_type", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_LOOKUP_COMMAND_EXECUTION"), None),
            ("boundary scope missing", None, lambda r: r.pop("boundary_scope"), None),
            ("boundary scope unsupported", None, lambda r: r.__setitem__("boundary_scope", "LOOKUP_COMMAND_EXECUTION_ONLY"), None),
            ("lookup command executed", None, lambda r: r.__setitem__("lookup_command_executed", True), None),
            ("lookup performed", None, lambda r: r.__setitem__("lookup_performed", True), None),
            ("lookup result created", None, lambda r: r.__setitem__("lookup_result_created", True), None),
            ("operation permission created", None, lambda r: r.__setitem__("operation_permission_created", True), None),
            ("runtime permission created", None, lambda r: r.__setitem__("runtime_permission_created", True), None),
            ("public API created", None, lambda r: r.__setitem__("public_api_created", True), None),
            ("participant-facing interface created", None, lambda r: r.__setitem__("participant_facing_interface_created", True), None),
            ("distributed network behavior created", None, lambda r: r.__setitem__("distributed_network_behavior_created", True), None),
            ("general lookup permission created", None, lambda r: r.__setitem__("general_lookup_permission_created", True), None),
            ("arbitrary lookup permission created", None, lambda r: r.__setitem__("arbitrary_lookup_permission_created", True), None),
            ("unsupported commands permitted", None, lambda r: r.__setitem__("unsupported_commands_permitted", True), None),
            ("unsupported lookup keys permitted", None, lambda r: r.__setitem__("unsupported_lookup_keys_permitted", True), None),
            ("new lookup entry created", None, lambda r: r.__setitem__("new_lookup_entry_created", True), None),
            ("new signal accepted", None, lambda r: r.__setitem__("new_signal_accepted", True), None),
            ("new entry accepted", None, lambda r: r.__setitem__("new_entry_accepted", True), None),
            ("new relevance object created", None, lambda r: r.__setitem__("new_relevance_object_created", True), None),
            ("new index entry created", None, lambda r: r.__setitem__("new_index_entry_created", True), None),
            ("filesystem discovery performed", None, lambda r: r.__setitem__("filesystem_discovery_performed", True), None),
            ("registry created", None, lambda r: r.__setitem__("registry_created", True), None),
            ("search surface created", None, lambda r: r.__setitem__("search_surface_created", True), None),
            ("query surface created", None, lambda r: r.__setitem__("query_surface_created", True), None),
            ("ranking surface created", None, lambda r: r.__setitem__("ranking_surface_created", True), None),
            ("scoring surface created", None, lambda r: r.__setitem__("scoring_surface_created", True), None),
            ("priority surface created", None, lambda r: r.__setitem__("priority_surface_created", True), None),
            ("validity judgment created", None, lambda r: r.__setitem__("validity_judgment_created", True), None),
            ("truth judgment created", None, lambda r: r.__setitem__("truth_judgment_created", True), None),
            ("authority judgment created", None, lambda r: r.__setitem__("authority_judgment_created", True), None),
            ("currentness judgment created", None, lambda r: r.__setitem__("currentness_judgment_created", True), None),
            ("repeated reception permission created", None, lambda r: r.__setitem__("repeated_reception_permission_created", True), None),
            ("arbitrary reception created", None, lambda r: r.__setitem__("arbitrary_reception_created", True), None),
            ("feed created", None, lambda r: r.__setitem__("feed_created", True), None),
            ("source transfer occurred", None, lambda r: r.__setitem__("source_transfer_occurred", True), None),
            ("source receipt occurred", None, lambda r: r.__setitem__("source_receipt_occurred", True), None),
            ("source created", None, lambda r: r.__setitem__("source_created", True), None),
            ("authority created", None, lambda r: r.__setitem__("authority_created", True), None),
            ("currentness created", None, lambda r: r.__setitem__("currentness_created", True), None),
            ("truth created", None, lambda r: r.__setitem__("truth_created", True), None),
            ("synchronization created", None, lambda r: r.__setitem__("synchronization_created", True), None),
            ("participation authorized", None, lambda r: r.__setitem__("participation_authorized", True), None),
            ("participant role created", None, lambda r: r.__setitem__("participant_role_created", True), None),
            ("deployment created", None, lambda r: r.__setitem__("deployment_created", True), None),
            ("public release created", None, lambda r: r.__setitem__("public_release_created", True), None),
            ("broader reusable permission created", None, lambda r: r.__setitem__("broader_reusable_permission_created", True), None),
            ("follow-on work authorized", None, lambda r: r.__setitem__("follow_on_work_authorized", True), None),
            ("consumed request reopened", None, lambda r: r.__setitem__("consumed_request_reopened", True), None),
            ("authorization token reused", None, lambda r: r.__setitem__("authorization_token_reused", True), None),
            ("artifact existence treated as authority", None, lambda r: r.__setitem__("artifact_existence_treated_as_lookup_command_execution_boundary_authority", True), None),
            ("latest file posture treated as authority", None, lambda r: r.__setitem__("latest_file_posture_treated_as_lookup_command_execution_boundary_authority", True), None),
            ("repo-local availability treated as authority", None, lambda r: r.__setitem__("repo_local_availability_treated_as_lookup_command_execution_boundary_authority", True), None),
            ("hidden repo state used as content", None, lambda r: r.__setitem__("hidden_repo_state_used_as_lookup_command_execution_boundary_content", True), None),
            ("hidden repo state used as authority", None, lambda r: r.__setitem__("hidden_repo_state_used_as_lookup_command_execution_boundary_authority", True), None),
            ("predecessor failure repaired", None, lambda r: r.__setitem__("predecessor_failure_repaired", True), None),
            ("predecessor failure hidden", None, lambda r: r.__setitem__("predecessor_failure_hidden", True), None),
            ("predecessor failure claimed passed", None, lambda r: r.__setitem__("predecessor_failure_claimed_passed", True), None),
            ("required non-claim flipped", None, lambda r: r["declared_non_claims"].__setitem__("lookup_performed", True), None),
        )
        slash_cases = {
            "selected raw/full not recorded",
            "raw/full packet body not exposed",
            "raw/full local only not true",
            "raw/full read only not true",
        }
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            for index, (name, artifact_mutator, request_mutator, direct) in enumerate(cases):
                with self.subTest(name=name):
                    ...
                    if direct == "missing_request":
                        ...
                        artifact = self.synthetic_raw_full_state_packet_body_exposure_artifact()
    ...
    def test_official_values_are_preserved(self) -> None:
        ...
        self.assertIn("SELECTED_LOOKUP_COMMAND_EXECUTION_CONSIDERATION_ONLY", serialized)
    ...
    def test_raw_hidden_hostile_content_containment(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            artifact = self.synthetic_raw_full_state_packet_body_exposure_artifact()
            artifact["raw_full_state_packet_body_exposure_body"] = HOSTILE_SENTINELS[5]
            artifact["hidden_repo_state"] = HOSTILE_SENTINELS[-1]
            artifact_path = self.write_synthetic_artifact(Path(tmp), artifact)
            request = self.build_request(artifact_path)
            request["raw_lookup_command_execution_boundary_body"] = HOSTILE_SENTINELS[0]
        ...
        serialized = json.dumps(result, sort_keys=True)
        self.assertIn("LOCAL_RELEVANCE_MEDIUM_READ_ONLY_LOOKUP_COMMAND_EXECUTION_BOUNDARY", serialized)
        self.assertIn("SELECTED_LOOKUP_COMMAND_EXECUTION_CONSIDERATION_ONLY", serialized)
        self.assert_canonical_false_non_claims(result)
        self.assert_lookup_command_execution_boundary_non_claims(result)
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            artifact_path = self.write_synthetic_artifact(tmp_path)
            request_path = tmp_path / "request.json"
            request_path.write_text(
                json.dumps(self.build_request(artifact_path), indent=2, sort_keys=True),
                encoding="utf-8",
            )
            result = resolver.resolve_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_from_path(
                request_path
            ...
            with mock.patch.object(resolver, "OUTPUT_ROOT", output_root):
                written = resolver.write_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_result(
                    result
                )
                written_again = resolver.write_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min_result(
                    result
                )
            ...
            self.assertIn(
                "integrity_host_v0_min_coexistence_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min",
                written.parts,
            )
    ...
    def test_non_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            artifact = self.synthetic_raw_full_state_packet_body_exposure_artifact()
            artifact["nested_raw"] = {
                "raw_lookup_command_execution_body": HOSTILE_SENTINELS[2]
            }
        ...
        self.assertEqual(request["boundary_type"], before["boundary_type"])
        self.assertEqual(request["boundary_scope"], before["boundary_scope"])
