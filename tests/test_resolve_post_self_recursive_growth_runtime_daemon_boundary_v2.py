"""Corrected v2 tests for the runtime-daemon-boundary resolver.

This suite preserves the intended post-self-recursive-growth
runtime-daemon-boundary coverage while correcting the v1 assertion overreach:
flipped incoming non-claims may appear as validation evidence in failed check
records, but they must never become final result-level ``non_claims`` posture
or promote statement posture. Hostile raw-body and hidden-state sentinel
strings remain prohibited anywhere in emitted results.

This is a runtime-daemon-boundary test only. It does not create runtime daemon,
runtime loop, public API, participant-facing interface, distributed network
...
class RuntimeDaemonBoundaryV2Tests(unittest.TestCase):
    """Corrected runtime-daemon-boundary v2 test surface."""
    ...
    def assert_flipped_non_claim_not_final_posture(self, result: Mapping[str, Any], key: str) -> None:
        ...
        for path in true_paths:
            with self.subTest(flipped_key=key, path=path):
                ...
                self.assertNotEqual(path[:1], ("runtime_daemon_boundary_statement",))
                self.assertIn(path[0], ("runtime_daemon_boundary_checks", "block"))
    ...
    def test_representative_blocking_behavior(self) -> None:
        ...
        mutation_cases: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ...
            ("required non-claim missing", _remove_declared_non_claim("runtime_daemon_created")),
        )
    ...
    def test_missing_or_incomplete_declared_non_claims_still_emit_canonical_false(self) -> None:
        variants: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ...
            ("remove one required non-claim", _remove_declared_non_claim("runtime_daemon_created")),
            (
