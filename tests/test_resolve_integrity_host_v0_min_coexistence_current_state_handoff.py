"""Bounded tests for the v0-min current-state handoff resolver.
...
resolutions, current-work-operation results, and current-state readout results
emitted into temporary roots.

The suite verifies one bounded downstream handoff over an explicit emitted
current-state readout result only. It does not test replay, merge, persistence
...
EXPECTED_TOP_LEVEL_KEYS = {
    "handoff_metadata",
    "selected_current_state_readout",
...
class IntegrityHostV0MinCoexistenceCurrentStateHandoffTests(unittest.TestCase):
    def emit_source_run(
        self,
        temp_root: Path,
    ...
    def valid_proposal(
        ...
        return {
            "transition_proposal_id": "proposal_accepted_for_current_state_handoff",
            ...
            "proposal_basis_ref": "bounded explicit current-state-handoff test basis",
    ...
    def resolve_handoff_default(self, temp_root: Path) -> dict[str, Any]:
        with mock.patch.object(handoff_resolver, "_repo_root", return_value=temp_root):
            return handoff_resolver.resolve_current_state_handoff()

    def resolve_handoff_object(
        self,
        temp_root: Path,
        current_state_readout: dict[str, Any],
    ) -> dict[str, Any]:
        with mock.patch.object(handoff_resolver, "_repo_root", return_value=temp_root):
            return handoff_resolver.resolve_current_state_handoff(
                current_state_readout
            )

    def resolve_handoff_from_path(
        self,
        temp_root: Path,
        readout_path: Path,
    ) -> dict[str, Any]:
        with mock.patch.object(handoff_resolver, "_repo_root", return_value=temp_root):
            return handoff_resolver.resolve_current_state_handoff_from_path(
                readout_path
            )
    ...
    def assert_result_shape(self, result: dict[str, Any]) -> None:
        ...
        self.assertIsInstance(result["handoff_metadata"], dict)
        self.assertIsInstance(result["selected_current_state_readout"], dict)
        self.assertIsInstance(result["effective_handoff_inputs"], dict)
        self.assertIsInstance(result["checks"], list)
        self.assertIsInstance(result["block"], dict)
        self.assertIsInstance(result["handoff_summary"], dict)
    ...
    def assert_handed_off_result(self, result: dict[str, Any]) -> None:
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], handoff_resolver.OUTCOME_HANDED_OFF)
        self.assertIsNone(result["block"]["block_code"])
        self.assertIsNone(result["block"]["block_reason"])
        self.assertIsInstance(result["handoff_output"], dict)
        self.assertEqual(set(result["effective_handoff_inputs"]), HANDOFF_INPUT_KEYS)
        for value in result["effective_handoff_inputs"].values():
    ...
    def assert_blocked_result(
        ...
        *,
        ...
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], handoff_resolver.OUTCOME_BLOCKED)
        ...
        self.assertIsNone(result["handoff_output"])
        if expect_non_claims_false:
            self.assert_non_claims_false(result)

    def assert_handoff_inputs_match_readout(
        self,
        temp_root: Path,
        result: dict[str, Any],
        readout: dict[str, Any],
    ) -> None:
        for key in HANDOFF_INPUT_KEYS:
            self.assert_paths_same(
                temp_root,
                result["effective_handoff_inputs"][key],
                readout["effective_readout_inputs"][key],
            )

    def assert_under_handoff_root(self, temp_root: Path, path: Path) -> None:
        root = (temp_root / handoff_resolver.CURRENT_STATE_HANDOFF_ROOT).resolve()
        try:
            path.resolve().relative_to(root)
        except ValueError as exc:
            self.fail(f"{path} was not written under {root}: {exc}")

    def parse_effective_handoff_files(
        self,
        temp_root: Path,
        result: dict[str, Any],
    ...
    def test_real_handed_off_current_state_handoff_default_selection(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_handoff_default(temp_root)

            self.assert_handed_off_result(result)
            selected = result["selected_current_state_readout"]
            self.assert_paths_same(
                temp_root,
                selected["current_state_readout_result_path"],
                stack["readout_path"],
            )
            self.assertEqual(
                selected["current_state_readout_result_id"],
                stack["readout_result"]["readout_metadata"]["readout_result_id"],
            )
            self.assertEqual(selected["outcome"], readout_resolver.OUTCOME_EMITTED)
            self.assertEqual(
                selected["selected_current_work_operation_id"],
                stack["operation_result"]["work_operation_metadata"]["work_result_id"],
            )
            self.assert_handoff_inputs_match_readout(
                temp_root,
                result,
                stack["readout_result"],
            )
            metadata = result["handoff_metadata"]
            for key in (
                "handoff_result_id",
                "handoff_result_type",
                "handoff_result_version",
                "generated_at",
                "resolver_module",
            ):
    ...
    def test_resolution_from_path_and_object_are_consistent(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            object_result = self.resolve_handoff_object(
                temp_root,
                stack["readout_result"],
            )
            path_result = self.resolve_handoff_from_path(
                temp_root,
                stack["readout_path"],
            )

            self.assert_handed_off_result(object_result)
            self.assert_handed_off_result(path_result)
            self.assertIsNone(
                object_result["selected_current_state_readout"][
                    "current_state_readout_result_path"
                ]
            )
            self.assert_paths_same(
                temp_root,
                path_result["selected_current_state_readout"][
                    "current_state_readout_result_path"
                ],
                stack["readout_path"],
            )
            ...
            self.assertEqual(
                object_result["effective_handoff_inputs"],
                path_result["effective_handoff_inputs"],
            )
            self.assertEqual(
                object_result["handoff_output"],
                path_result["handoff_output"],
            )
            self.assertEqual(
                object_result["handoff_summary"],
                path_result["handoff_summary"],
            )

    def test_effective_inputs_checks_handoff_output_and_summary(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_handoff_object(
                temp_root,
                stack["readout_result"],
            )
            ...
            self.assertEqual(set(result["handoff_output"]), HANDOFF_OUTPUT_KEYS)
            output = result["handoff_output"]
            readout_output = stack["readout_result"]["readout_output"]
            self.assert_paths_same(
                temp_root,
                output["current_governing_source_run_path"],
                readout_output["current_governing_source_run_path"],
            )
            self.assert_paths_same(
                temp_root,
                output["current_governing_ingress_run_path"],
                readout_output["current_governing_ingress_run_path"],
            )
            for output_key, input_key in (
                ...
                self.assert_paths_same(
                    temp_root,
                    output[output_key],
                    result["effective_handoff_inputs"][input_key],
                )
            ...
            self.assertEqual(
                ...
                readout_output["readout_basis"],
            )
            self.assertEqual(output["handoff_basis"], "emitted_current_state_readout")

            summary = handoff_resolver.build_current_state_handoff_summary(result)
            for key in (
                "handoff_result_id",
                "outcome",
                "block_code",
            ...
            self.assertEqual(summary["outcome"], handoff_resolver.OUTCOME_HANDED_OFF)
            self.assertIsNone(summary["block_code"])
            self.assertIsNone(summary["block_reason"])
            self.assertEqual(
                summary["selected_current_state_readout_id"],
                stack["readout_result"]["readout_metadata"]["readout_result_id"],
            )
    ...
    def test_handed_off_semantics_preserve_prior_family(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            ...
            for effective_key, prior_key in (
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        result["effective_handoff_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
            ...
            parsed = self.parse_effective_handoff_files(temp_root, result)
            ...
            self.assertEqual(
                result["handoff_output"]["handoff_basis"],
                "emitted_current_state_readout",
            )
            self.assertEqual(
                result["handoff_output"]["readout_basis"],
                "completed_current_work_operation",
            )

    def test_no_current_state_readout_result_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()

            result = self.resolve_handoff_default(temp_root)

            self.assert_blocked_result(result, "NO_CURRENT_STATE_READOUT_RESULT")
            selected = result["selected_current_state_readout"]
            self.assertIsNone(selected["current_state_readout_result_id"])
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            stack = self.build_multi_run_stack(
                temp_root,
                write_readout_artifact=False,
            )
            blocked_readout = self.copy_payload(stack["readout_result"])
            blocked_readout["outcome"] = readout_resolver.OUTCOME_BLOCKED
            blocked_readout["block"] = {
                "block_code": "TEST_BLOCKED_READOUT",
                "block_reason": "Blocked current-state readout fixture.",
            }
            blocked_readout["readout_output"] = None
            root = temp_root / readout_resolver.CURRENT_STATE_READOUT_ROOT
            self.write_json(root / "blocked_current_state_readout.json", blocked_readout)

            result = self.resolve_handoff_default(temp_root)

            self.assert_blocked_result(result, "NO_CURRENT_STATE_READOUT_RESULT")

    def test_current_state_readout_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            missing_path = temp_root / "missing_current_state_readout.json"

            result = self.resolve_handoff_from_path(temp_root, missing_path)

            self.assert_blocked_result(result, "CURRENT_STATE_READOUT_UNREADABLE")

        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            malformed_path = temp_root / "malformed_current_state_readout.json"
            malformed_path.write_text("[1, 2, 3]\n", encoding="utf-8")

            with self.assertRaises(handoff_resolver.CurrentStateHandoffError):
                self.resolve_handoff_from_path(temp_root, malformed_path)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = self.resolve_handoff_object(temp_root, blocked_readout)

            self.assert_blocked_result(result, "CURRENT_STATE_READOUT_NOT_EMITTED")

    def test_effective_handoff_artifact_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(
                temp_root,
            ...
            readout = self.copy_payload(stack["readout_result"])
            missing_path = self.resolve_display_path(
                temp_root,
                readout["effective_readout_inputs"]["effective_status_packet_path"],
            )
            missing_path.unlink()

            result = self.resolve_handoff_object(temp_root, readout)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            malformed_path = self.resolve_display_path(
                temp_root,
                readout["effective_readout_inputs"][
                    "effective_authority_artifact_path"
                ],
            )
            ...
            with self.assertRaises(handoff_resolver.CurrentStateHandoffError):
                self.resolve_handoff_object(temp_root, readout)
    ...
    def test_canonical_execution_line_mismatch_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            family_path = self.resolve_display_path(
                temp_root,
                readout["effective_readout_inputs"]["effective_family_packet_path"],
            )
    ...
    def test_stale_prior_family_fallback_is_not_used(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            handed_off = self.resolve_handoff_object(
                temp_root,
                stack["readout_result"],
            )
            self.assert_handed_off_result(handed_off)
            for effective_key, prior_key in (
                ("effective_authority_artifact_path", "prior_authority_artifact_path"),
                ("effective_family_packet_path", "prior_family_packet_path"),
                ("effective_status_packet_path", "prior_status_packet_path"),
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        handed_off["effective_handoff_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
                        stack["effective_result"]["prior_current_family"][prior_key],
            ...
            stale = self.copy_payload(stack["readout_result"])
            stale["effective_readout_inputs"].update(
                {
            ...
            blocked = self.resolve_handoff_object(temp_root, stale)
    ...
    def test_non_claim_shortcuts_block_handoff(self) -> None:
        cases = (
    ...
    def test_multiple_current_state_readout_result_conflict_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            conflict = self.copy_payload(stack["readout_result"])
            conflict["readout_metadata"]["readout_result_id"] = (
                "conflicting_current_state_readout"
            )
            conflict["effective_readout_inputs"][
                "effective_current_governing_packet_path"
            ] = "artifacts/conflicting_effective_governing_packet.json"
            conflict_path = (
                temp_root
                / readout_resolver.CURRENT_STATE_READOUT_ROOT
                / "conflicting_current_state_readout.json"
            )
    ...
    def test_hard_malformed_artifact_failures(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            root_file = temp_root / readout_resolver.CURRENT_STATE_READOUT_ROOT
            root_file.parent.mkdir(parents=True, exist_ok=True)
            root_file.write_text("not a directory\n", encoding="utf-8")

            with self.assertRaises(handoff_resolver.CurrentStateHandoffError):
                self.resolve_handoff_default(temp_root)

        malformed_readout_cases = (
            ("non_mapping_input", "not a mapping"),
            (
                "missing_effective_readout_inputs",
                {"outcome": readout_resolver.OUTCOME_EMITTED},
            ),
        )
        for label, payload in malformed_readout_cases:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    with self.assertRaises(handoff_resolver.CurrentStateHandoffError):
                        with mock.patch.object(
                            ...
                            handoff_resolver.resolve_current_state_handoff(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            readout = self.copy_payload(stack["readout_result"])
            readout["effective_readout_inputs"]["effective_family_packet_path"] = 42
    ...
    def test_write_behavior_and_default_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_path = temp_root / "nested" / "current_state_handoff" / "result.json"

            with mock.patch.object(handoff_resolver, "_repo_root", return_value=temp_root):
                written = handoff_resolver.write_current_state_handoff_result(
                    result,
                    output_path,
                )
            ...
            with mock.patch.object(handoff_resolver, "_repo_root", return_value=temp_root):
                with self.assertRaises(FileExistsError):
                    handoff_resolver.write_current_state_handoff_result(
                        result,
                        output_path,
                    )

            full_handoff_id = result["handoff_metadata"]["handoff_result_id"]
            with mock.patch.object(handoff_resolver, "_repo_root", return_value=temp_root):
                default_first = handoff_resolver.write_current_state_handoff_result(
                    result
                )
                default_second = handoff_resolver.write_current_state_handoff_result(
                    result
                )
            ...
            self.assert_under_handoff_root(temp_root, default_first)
            self.assert_under_handoff_root(temp_root, default_second)
            self.assertEqual(default_first.name, "current_state_handoff_result.json")
            self.assertEqual(default_second.name, "current_state_handoff_result_001.json")
            self.assertLess(len(default_first.name), 255)
            self.assertLess(len(default_second.name), 255)
            self.assertNotIn(full_handoff_id, default_first.name)
            self.assertNotIn(full_handoff_id, default_second.name)

            parsed_default = self.read_json(default_first)
            self.assertEqual(set(parsed_default), EXPECTED_TOP_LEVEL_KEYS)
            self.assertEqual(
                parsed_default["handoff_metadata"]["handoff_result_id"],
                full_handoff_id,
            )
            self.assertEqual(
                parsed_default["handoff_metadata"]["handoff_result_version"],
                handoff_resolver.HANDOFF_RESULT_VERSION,
            )
    ...
    def test_non_mutation_posture(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = self.resolve_handoff_object(
                temp_root,
                stack["readout_result"],
            )
            after_first = {
            ...
            second = self.resolve_handoff_from_path(
                temp_root,
                stack["readout_path"],
            )
            after_second = {
            ...
            self.assertEqual(
                first["selected_current_state_readout"][
                    "current_state_readout_result_id"
                ],
                second["selected_current_state_readout"][
                    "current_state_readout_result_id"
                ],
            )
            self.assertEqual(
                first["effective_handoff_inputs"],
                second["effective_handoff_inputs"],
            )
            self.assertEqual(first["handoff_output"], second["handoff_output"])
            self.assertEqual(first["handoff_summary"], second["handoff_summary"])
