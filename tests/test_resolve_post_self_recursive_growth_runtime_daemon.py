"""Executable tests for the post-self-recursive-growth runtime-daemon resolver.

This suite is bounded to runtime-daemon posture only. Runtime-daemon-boundary
and self-recursive-growth are upstream basis, runtime-daemon-boundary v1 remains
preserved over-strict failed test evidence, and runtime-hosting-boundary v1
remains preserved predecessor failure lineage.

These tests do not create runtime loop, public API, participant-facing
...
import resolve_post_self_recursive_growth_runtime_daemon as resolver  # noqa: E402


EXPECTED_OUTPUT_ROOT_SUFFIX = (
    "artifacts/"
    "integrity_host_v0_min_coexistence_post_self_recursive_growth_runtime_daemon"
)
...
TOP_LEVEL_SECTIONS = (
    ...
    "runtime_daemon_scope",
    "runtime_daemon_checks",
    "runtime_daemon_statement",
    "runtime_daemon_non_meaning",
    "bounded_runtime_daemon_envelope",
...
def _valid_request() -> dict[str, Any]:
    return resolver.build_declared_post_self_recursive_growth_runtime_daemon_request()
...
def _non_meaning(result: Mapping[str, Any]) -> Mapping[str, Any]:
    non_meaning = result.get("runtime_daemon_non_meaning")
...
def _summary(result: Mapping[str, Any]) -> Mapping[str, Any]:
    return resolver.build_post_self_recursive_growth_runtime_daemon_summary(result)
...
class RuntimeDaemonResolverTests(unittest.TestCase):
    """Bounded runtime-daemon resolver test surface."""
    ...
    def assert_generated_booleans(self, result: Mapping[str, Any]) -> None:
        for section_name in ("runtime_daemon_statement", "runtime_daemon_non_meaning", "non_claims"):
    ...
    def assert_runtime_non_meaning_safe(self, result: Mapping[str, Any]) -> None:
        for key, value in _non_meaning(result).items():
            with self.subTest(non_meaning=key):
                self.assertIs(value, False)

    def assert_safe_runtime_posture(self, result: Mapping[str, Any]) -> None:
        statement = _statement(result)
        ...
        self.assert_runtime_non_meaning_safe(result)
    ...
    def assert_flipped_non_claim_not_final_posture(self, result: Mapping[str, Any], key: str) -> None:
        ...
        for path in true_paths:
            with self.subTest(flipped_key=key, path=path):
                ...
                self.assertNotEqual(path[:1], ("runtime_daemon_statement",))
                self.assertIn(path[0], ("runtime_daemon_checks", "block"))
    ...
    def assert_blocked_safely(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_safe_runtime_posture(result)

    def assert_envelope_bounded(self, result: Mapping[str, Any]) -> None:
        envelope = result.get("bounded_runtime_daemon_envelope")
        self.assertIsInstance(envelope, Mapping)
        self.assertIn("bounded_runtime_daemon_envelope", json.dumps(envelope, sort_keys=True))
        self.assertIs(envelope.get("does_not_authorize_arbitrary_runtime_activity"), True)
        self.assertIs(envelope.get("anything_outside_requires_fresh_admission"), True)
    ...
    def test_public_api_and_constants(self) -> None:
        for name in (
            "resolve_post_self_recursive_growth_runtime_daemon",
            "resolve_post_self_recursive_growth_runtime_daemon_from_path",
            "write_post_self_recursive_growth_runtime_daemon_result",
            "build_post_self_recursive_growth_runtime_daemon_summary",
            "build_declared_post_self_recursive_growth_runtime_daemon_request",
        ):
        ...
        for name in (
            ...
            "SUPPORTED_RUNTIME_DAEMON_SCOPE",
        ...
        self.assertEqual(resolver.RESOLVER_MODULE, "resolve_post_self_recursive_growth_runtime_daemon")
        self.assertEqual(resolver.SUPPORTED_RUNTIME_DAEMON_SCOPE, resolver.SUPPORTED_SCOPE_VALUES)
    ...
    def test_successful_recorded_result_from_no_argument_builder(self) -> None:
        ...
        result = resolver.resolve_post_self_recursive_growth_runtime_daemon(request)
        ...
        self.assertEqual(summary["request_id"], request["runtime_daemon_request_id"])
        ...
        self.assert_envelope_bounded(result)
        self.assert_canonical_non_claims(result)
        self.assert_public_codes(result)
        self.assert_generated_booleans(result)
    ...
    def test_representative_blocking_behavior(self) -> None:
        ...
        mutation_cases: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ("explicit block intent", _mutate_field("runtime_daemon_intent", "BLOCK_POST_SELF_RECURSIVE_GROWTH_RUNTIME_DAEMON")),
            ("unsupported intent", _mutate_field("runtime_daemon_intent", "UNSUPPORTED_INTENT")),
            ("unsupported scope", _mutate_field("runtime_daemon_scope", ["UNSUPPORTED_SCOPE"])),
            ("missing runtime-daemon-boundary basis", _remove_field("selected_runtime_daemon_boundary_basis")),
            ("runtime-daemon-boundary not recorded", _mutate_basis("selected_runtime_daemon_boundary_basis", "outcome", "NOT_RECORDED")),
            ("runtime-daemon-boundary failed checks present", _mutate_basis("selected_runtime_daemon_boundary_basis", "failed_check_count", 1)),
            ("runtime-daemon-boundary version not 0.1.0", _mutate_basis("selected_runtime_daemon_boundary_basis", "result_version", "9.9.9")),
            ("runtime-daemon-boundary did not declare future runtime-daemon review", _mutate_field("selected_runtime_daemon_boundary_declared_future_review", False)),
            ("runtime-daemon-boundary already created runtime daemon", _mutate_field("selected_runtime_daemon_boundary_already_created_runtime_daemon", True)),
            ("runtime-daemon-boundary already created runtime loop", _mutate_field("selected_runtime_daemon_boundary_already_created_runtime_loop", True)),
            ("runtime-daemon-boundary already created public API", _mutate_field("selected_runtime_daemon_boundary_already_created_public_api", True)),
            ("runtime-daemon-boundary already created participant-facing interface", _mutate_field("selected_runtime_daemon_boundary_already_created_participant_facing_interface", True)),
            ("runtime-daemon-boundary already created distributed network behavior", _mutate_field("selected_runtime_daemon_boundary_already_created_distributed_network_behavior", True)),
            ("runtime-daemon-boundary treated boundary as runtime daemon", _mutate_field("selected_runtime_daemon_boundary_treated_as_runtime_daemon", True)),
            ("runtime-daemon-boundary treated boundary as runtime loop", _mutate_field("selected_runtime_daemon_boundary_treated_as_runtime_loop", True)),
            ("runtime-daemon-boundary treated boundary as public API", _mutate_field("selected_runtime_daemon_boundary_treated_as_public_api", True)),
            ("runtime-daemon-boundary authorized future work", _mutate_field("selected_runtime_daemon_boundary_authorized_future_work", True)),
            ...
            ("self-recursive-growth basis missing", _remove_field("selected_self_recursive_growth_basis")),
            ("self-recursive growth not recorded", _mutate_basis("selected_self_recursive_growth_basis", "outcome", "NOT_RECORDED")),
            ("self-recursive growth already created runtime loop", _mutate_field("selected_self_recursive_growth_already_created_runtime_loop", True)),
            ("self-recursive growth already created public API", _mutate_field("selected_self_recursive_growth_already_created_public_api", True)),
            ("self-recursive growth already created participant-facing interface", _mutate_field("selected_self_recursive_growth_already_created_participant_facing_interface", True)),
            ("self-recursive growth already created distributed network behavior", _mutate_field("selected_self_recursive_growth_already_created_distributed_network_behavior", True)),
            ("bounded self-recursive-growth envelope treated as runtime daemon before review", _mutate_field("selected_bounded_self_recursive_growth_envelope_treated_as_runtime_daemon_before_review", True)),
            ("bounded self-recursive-growth envelope authorized arbitrary runtime activity before review", _mutate_field("selected_bounded_self_recursive_growth_envelope_authorized_arbitrary_runtime_activity_before_review", True)),
            ...
            ("runtime daemon recorded before review", _mutate_field("runtime_daemon_recorded_before_review", True)),
            ("runtime daemon treated as runtime loop", _mutate_field("runtime_daemon_treated_as_runtime_loop", True)),
            ("runtime daemon treated as public API", _mutate_field("runtime_daemon_treated_as_public_api", True)),
            ("runtime daemon treated as participant-facing interface", _mutate_field("runtime_daemon_treated_as_participant_facing_interface", True)),
            ("runtime daemon treated as distributed network behavior", _mutate_field("runtime_daemon_treated_as_distributed_network_behavior", True)),
            ("runtime daemon treated as source transfer", _mutate_field("runtime_daemon_treated_as_source_transfer", True)),
            ("runtime daemon treated as source receipt", _mutate_field("runtime_daemon_treated_as_source_receipt", True)),
            ("runtime daemon treated as reception authorization", _mutate_field("runtime_daemon_treated_as_reception_authorization", True)),
            ("runtime daemon treated as source", _mutate_field("runtime_daemon_treated_as_source", True)),
            ("runtime daemon treated as authority", _mutate_field("runtime_daemon_treated_as_authority", True)),
            ("runtime daemon treated as currentness", _mutate_field("runtime_daemon_treated_as_currentness", True)),
            ("runtime daemon treated as deployment", _mutate_field("runtime_daemon_treated_as_deployment", True)),
            ("runtime daemon treated as public release", _mutate_field("runtime_daemon_treated_as_public_release", True)),
            ("runtime daemon treated as operation permission", _mutate_field("runtime_daemon_treated_as_operation_permission", True)),
            ("runtime daemon treated as broader reusable permission", _mutate_field("runtime_daemon_treated_as_broader_reusable_permission", True)),
            ("runtime daemon treated as follow-on work", _mutate_field("runtime_daemon_treated_as_follow_on_work", True)),
            ...
            ("artifact existence treated as runtime-daemon authority", _mutate_field("artifact_existence_treated_as_runtime_daemon_authority", True)),
            ("artifact path treated as currentness", _mutate_field("artifact_path_treated_as_currentness", True)),
            ("latest file posture treated as runtime-daemon authority", _mutate_field("latest_file_posture_treated_as_runtime_daemon_authority", True)),
            ("repo-local availability treated as runtime-daemon authority", _mutate_field("repo_local_availability_treated_as_runtime_daemon_authority", True)),
            ("hidden repo state used as runtime-daemon content", _mutate_field("hidden_repo_state_used_as_runtime_daemon_content", True)),
            ("hidden repo state used as runtime-daemon authority", _mutate_field("hidden_repo_state_used_as_runtime_daemon_authority", True)),
            ...
            ("required non-claim flipped", _mutate_field("declared_non_claims", {key: False for key in resolver.REQUIRED_FALSE_NON_CLAIMS} | {resolver.REQUIRED_FALSE_NON_CLAIMS[1]: True})),
        )
    ...
    def test_official_enum_strings_are_preserved(self) -> None:
        result = resolver.resolve_post_self_recursive_growth_runtime_daemon(_valid_request())
        self.assertEqual(result["outcome"], resolver.OUTCOME_RECORDED)
        self.assert_official_scope_preserved(result)

        request = _valid_request()
        request["runtime_daemon_scope"] = list(resolver.SUPPORTED_RUNTIME_DAEMON_SCOPE)
        custom = resolver.resolve_post_self_recursive_growth_runtime_daemon(request)
    ...
    def test_raw_hidden_hostile_content_is_contained_without_mutating_request(self) -> None:
        ...
        for section in SELECTED_BASIS_SECTIONS:
            for key in SENSITIVE_KEYS:
    ...
    def test_path_and_write_behavior(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            result = resolver.resolve_post_self_recursive_growth_runtime_daemon_from_path(request_path)
            self.assertEqual(result["outcome"], resolver.OUTCOME_RECORDED)
            summary = _summary(result)
            self.assertEqual(summary["result_version"], "0.1.0")
            self.assertEqual(summary["resolver_module"], resolver.RESOLVER_MODULE)
            ...
            malformed = resolver.resolve_post_self_recursive_growth_runtime_daemon_from_path(malformed_path)
            self.assert_blocked_safely(malformed)

            array_path = root / "array.json"
            array_path.write_text("[]", encoding="utf-8")
            array_result = resolver.resolve_post_self_recursive_growth_runtime_daemon_from_path(array_path)
            self.assert_blocked_safely(array_result)

            missing_result = resolver.resolve_post_self_recursive_growth_runtime_daemon_from_path(root / "missing.json")
            self.assert_blocked_safely(missing_result)

            redirected_root = root / "artifacts" / "integrity_host_v0_min_coexistence_post_self_recursive_growth_runtime_daemon"
            with mock.patch.object(resolver, "OUTPUT_ROOT", redirected_root):
                first = resolver.write_post_self_recursive_growth_runtime_daemon_result(result)
                second = resolver.write_post_self_recursive_growth_runtime_daemon_result(result)
            ...
            self.assertIn("post_self_recursive_growth_runtime_daemon", first_text)
    ...
    def test_resolver_does_not_mutate_request_or_selected_basis(self) -> None:
        ...
        nested_originals = {
            ...
            "runtime_daemon_scope": copy.deepcopy(request["runtime_daemon_scope"]),
            "requested_bounded_runtime_daemon_envelope": copy.deepcopy(
                request["requested_bounded_runtime_daemon_envelope"]
            ),
        }
        for section in SELECTED_BASIS_SECTIONS + POSTURE_SECTIONS:
            nested_originals[section] = copy.deepcopy(request[section])

        resolver.resolve_post_self_recursive_growth_runtime_daemon(request)
    ...
    def test_predecessor_failure_preservation(self) -> None:
        ...
        self.assertTrue(boundary_v1["preserved_as_failed_test_evidence"])
        ...
        self.assertTrue(hosting_v1["preserved_as_predecessor_failure_evidence"])
        ...
        self.assertIs(summary["selected_runtime_daemon_boundary_v1_failure_lineage_preserved"], True)
