"""Tests for v2 descendant-body candidate-record distinctness boundary.

This suite verifies one boundary result only. It preserves the v1 test
posture while exercising the v2 correction that accepts equivalent
non-standing candidate-record wording in the completed-operation terminal
summary. It does not create scanner, repair, validation enforcement,
distinctness operation, candidate standing, descendant body, runtime,
authority, currentness, or follow-on behavior.
"""
...
import resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2 as resolver
...
ACCEPTED_NON_STANDING_MARKERS = (
    ...
    "Candidate records are not descendant bodies, not standing descendants, not crossing authorization",
    "Candidate records are not descendant bodies, not standing descendants, not crossing authorization, not relation",
    "Candidate records are not standing",
    "not standing descendants",
    "non-standing candidate records",
)

INSUFFICIENT_NON_STANDING_SUBSTITUTES = (
    ("only_candidate_record_ids", "candidate_record_ids = descendant_body_basis_candidate_a_001, descendant_body_basis_candidate_b_001"),
    ("only_candidate_roles", "candidate_role = CANDIDATE_A\ncandidate_role = CANDIDATE_B"),
    ("only_created_by_operation", "candidate_record_created_by_operation = true"),
    ("only_descendant_body_false", "descendant_body_created = false"),
    ("only_not_descendant_bodies", "Candidate records are not descendant bodies"),
    ("only_not_crossing", "Candidate records are not crossing authorization"),
    ("only_operation_evidenced", "operation-evidenced records"),
    ("only_result_contained", "result-contained records"),
)
...
class DescendantBodyCandidateRecordDistinctnessOperationBoundaryV2Tests(
    unittest.TestCase
):
    ...
    def write_synthetic_basis(
        ...
        *,
        ...
        extra_text: str = "",
        completed_summary_markers: tuple[str, ...] | None = None,
    ) -> dict[str, Path]:
        boundary_markers = tuple(
            marker for marker in BOUNDARY_SPEC_MARKERS if marker != remove_boundary_marker
        )
        source_completed_summary_markers = (
            completed_summary_markers or COMPLETED_OPERATION_SUMMARY_MARKERS
        )
        completed_summary_markers = tuple(
            marker
            for marker in source_completed_summary_markers
    ...
    def valid_request(self, paths: dict[str, Path]) -> dict[str, Any]:
        return resolver.build_declared_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_request(
    ...
    def test_public_api_and_constants(self) -> None:
        for name in (
            "resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2",
            "resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_from_path",
            "write_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_result",
            "build_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_summary",
            "build_declared_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_request",
        ):
        ...
        for alias_name, v2_name in alias_pairs:
            self.assertTrue(hasattr(resolver, alias_name), alias_name)
            self.assertIs(getattr(resolver, alias_name), getattr(resolver, v2_name))
        for name in (
            "RESULT_VERSION",
            "RESOLVER_MODULE",
            "OUTCOME_RECORDED",
    ...
    def test_records_boundary_from_synthetic_basis_without_exact_v1_phrase(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            paths = self.write_synthetic_basis(Path(temp_dir))
            request = self.valid_request(paths)
            result = resolver.resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2(
                request
            )
            completed_summary_text = paths["completed_summary"].read_text(
                encoding="utf-8"
            )
        self.assertIsInstance(result, dict)
        self.assert_recorded_boundary_posture(result)
        summary = resolver.build_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_summary(
            result
        )
        ...
        self.assertNotIn(
            "Candidate records are not standing descendants", completed_summary_text
        )
        self.assertIn(
            "Candidate records are result-contained, non-standing, operation-evidenced records only",
    ...
    def test_equivalent_non_standing_marker_acceptance_variants(self) -> None:
        base_markers = COMPLETED_OPERATION_SUMMARY_MARKERS
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            for index, marker in enumerate(ACCEPTED_NON_STANDING_MARKERS):
                with self.subTest(marker=marker):
                    summary_markers = tuple(dict.fromkeys(base_markers + (marker,)))
                    paths = self.write_synthetic_basis(
                        root / self.safe_json_filename(marker, index),
                        completed_summary_markers=summary_markers,
                    )
                    request = self.valid_request(paths)
                    result = resolver.resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2(
                        request
                    ...
                    boundary = self.boundary(result)
                    self.assertIs(
                        boundary["completed_operation_terminal_summary_markers_present"],
                        True,
                    )
                    self.assertIs(boundary["distinctness_operation_not_created"], True)
                    self.assertIs(result["non_claims"]["distinctness_supported"], False)
                    self.assertIs(result["non_claims"]["candidate_records_distinct"], False)
                    self.assertIs(boundary["candidate_standing_authorized"], False)
                    self.assertIs(boundary["descendant_body_created"], False)
                    self.assertIs(boundary["enumeration_not_treated_as_distinction"], True)
                    self.assertIs(
                        boundary[
                            "id_and_role_difference_alone_not_treated_as_distinctness"
                        ],
    ...
    def test_non_standing_marker_absence_still_blocks(self) -> None:
        base_without_non_standing = tuple(
            marker
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            for index, (name, substitute) in enumerate(
                INSUFFICIENT_NON_STANDING_SUBSTITUTES
            ):
                with self.subTest(case=name):
                    summary_markers = base_without_non_standing + (substitute,)
                    paths = self.write_synthetic_basis(
    ...
    def test_records_default_live_target_if_present(self) -> None:
        ...
        request = resolver.build_declared_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_request()
        result = resolver.resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2(
            request
        )
        self.assert_recorded_boundary_posture(result)
    ...
    def test_from_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = resolver.resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_from_path(
                request_path
            )
            self.assert_recorded_boundary_posture(result)
            summary = resolver.build_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_summary(
            ...
            missing = resolver.resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_from_path(
                base / "missing.json"
            )
            self.assert_blocked_with_public_code(missing)
            malformed_path = base / "malformed.json"
            malformed_path.write_text("{", encoding="utf-8")
            malformed = resolver.resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_from_path(
                malformed_path
            )
            self.assert_blocked_with_public_code(malformed)
            array_path = self._write_json(base / "array.json", [])
            array_result = resolver.resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_from_path(
                array_path
            )
            ...
            output_dir.mkdir()
            first_path = resolver.write_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_result(
                result, output_dir
            )
            second_path = resolver.write_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_result(
                result, output_dir
            )
            ...
            self.assertIn(
                "descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2_result",
                first_path.name,
            )
    ...
    def test_non_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            resolver.resolve_descendant_body_candidate_record_distinctness_operation_boundary_v0_min_v2(
                request
            )
