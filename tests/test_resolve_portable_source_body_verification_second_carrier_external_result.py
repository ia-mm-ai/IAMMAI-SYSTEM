"""Executable checks for second-carrier external-result posture only.

This suite is downstream of the second-carrier-external-result-boundary line.
It verifies that the resolver records one bounded second-carrier external
result posture only: external result is not cross-carrier evidence, portable
...
import resolve_portable_source_body_verification_second_carrier_external_result as resolver
...
def scope_values(result: dict[str, Any]) -> list[str]:
    ...
    return list(scope.get("declared_scope_values", []))
...
class PortableSourceBodyVerificationSecondCarrierExternalResultTest(unittest.TestCase):
    ...
    def test_representative_blocking_behavior(self) -> None:
        block_cases: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ...
            ("verification already external result", set_basis_flag("selected_second_carrier_verification_basis", "already_created_external_result", True)),
            ("verification already cross carrier", set_basis_flag("selected_second_carrier_verification_basis", "already_created_cross_carrier_evidence", True)),
            ("verification already closure", set_basis_flag("selected_second_carrier_verification_basis", "already_created_portable_verification_closure", True)),
            ...
            ("returned capture treated as external result", mutate_key("selected_returned_capture_intake_capture_only", False)),
            ("returned capture treated as proof", flip_non_claim("returned_capture_treated_as_cross_carrier_evidence")),
            ("zero exit treated as external", flip_non_claim("zero_exit_code_treated_as_external_result_standalone")),
            ("string zero treated as external", flip_non_claim("string_zero_treated_as_external_result_standalone")),
            ("ok treated as external", flip_non_claim("ok_output_treated_as_external_result_standalone")),
            ("ran seven treated as proof", flip_non_claim("ran_7_tests_treated_as_cross_carrier_proof")),
            ...
            ("command lineage current report", mutate_key("command_report_lineage_treated_as_current_report_artifact", True)),
            ("command lineage source", mutate_key("command_report_lineage_treated_as_source", True)),
            ("command lineage authority", mutate_key("command_report_lineage_treated_as_authority", True)),
            ("command lineage currentness", mutate_key("command_report_lineage_treated_as_currentness", True)),
    ...
    def test_non_mutation_of_request_and_selected_inputs(self) -> None:
        ...
        request["second_carrier_external_result_scope"] = {
            value: True for value in resolver.SUPPORTED_SECOND_CARRIER_EXTERNAL_RESULT_SCOPE
        }
