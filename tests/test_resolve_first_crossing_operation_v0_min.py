"""Tests for one bounded first-crossing operation result only.
...
records. The tests keep first crossing distinct from relation, coupling,
presence, identity, runtime, standing, repair, discovery, and follow-on work.
"""
...
import resolve_first_crossing_operation_v0_min as resolver


class FirstCrossingOperationV0MinTests(unittest.TestCase):
    """Exercise first crossing without converting it into a relation surface."""
    ...
    def _build_valid_synthetic_request(
        ...
        return resolver.build_first_crossing_operation_v0_min_request(**references)
    ...
    def test_public_api_constants_and_default_request(self) -> None:
        for name in (
            ...
            "resolve_first_crossing_operation_v0_min_from_path",
            "write_first_crossing_operation_v0_min_result",
            "build_first_crossing_operation_v0_min_summary",
        ...
        exact_values = {
            ...
            "RESOLVER_MODULE": "resolve_first_crossing_operation_v0_min",
        ...
        self.assertTrue(
            str(resolver.OUTPUT_ROOT).endswith(
                "artifacts/integrity_host_v0_min_coexistence_first_crossing_operation_v0_min"
            )
        )
        ...
        request = resolver.build_first_crossing_operation_v0_min_request()
    ...
    def test_default_synthetic_complete_operation_records_recorded(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = resolver.resolve_first_crossing_operation_v0_min(request)
    ...
    def test_first_crossing_material_is_two_separate_records_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            result = resolver.resolve_first_crossing_operation_v0_min(
    ...
    def test_intents_and_support_not_found_do_not_record(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = resolver.resolve_first_crossing_operation_v0_min(do_not_record)
            ...
            result = resolver.resolve_first_crossing_operation_v0_min(unsupported)
            self.assert_not_recorded_not_blocked(result)
            ...
            result = resolver.resolve_first_crossing_operation_v0_min(explicit_block)
        self.assert_blocked_with_public_code(result)
    ...
    def test_request_shape_and_exact_value_mismatches_block(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for case_name, payload in cases:
                with self.subTest(case=case_name):
                    result = resolver.resolve_first_crossing_operation_v0_min(payload)
                    self.assert_blocked_with_public_code(result)
                    self.assert_final_false_posture(result)
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = resolver.resolve_first_crossing_operation_v0_min_from_path(request_path)
            self.assert_recorded_not_blocked(result)
            self.assertEqual(self._failed_check_count(result), 0)
            self.assertEqual(result["result_version"], resolver.RESULT_VERSION)
            self.assertEqual(result["resolver_module"], resolver.RESOLVER_MODULE)
            for name, content, expected_code in (
                ...
                with self.subTest(path_case=name):
                    ...
                    malformed = resolver.resolve_first_crossing_operation_v0_min_from_path(path)
            ...
            first_written = resolver.write_first_crossing_operation_v0_min_result(result, output)
            second_written = resolver.write_first_crossing_operation_v0_min_result(result, output)
            ...
            self.assertIn("first_crossing_operation_v0_min_result", first_written.name)
    ...
    def test_summary_and_direct_smoke_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            request = self._build_valid_synthetic_request(Path(directory))
            result = resolver.resolve_first_crossing_operation_v0_min(request)
            summary = resolver.build_first_crossing_operation_v0_min_summary(result)
        self.assert_recorded_not_blocked(result)
