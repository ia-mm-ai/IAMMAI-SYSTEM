"""Bounded tests for body-signal recognition.

This suite exercises ``src/resolve_body_signal_recognition.py`` as one
single-candidate recognition surface:
...
import resolve_body_signal_recognition as recognition
...
def category_source(category: str) -> dict[str, Any]:
    cases = {
        ...
        "TOUCH_ADMISSIBILITY_SIGNAL": dict(
            family="current_state_admissibility_and_touch_permission_result",
        ...
        "DERIVATIVE_VESSEL_SIGNAL": dict(
            family="openai_api_derivative_vessel_result",
        ...
        "OPERATOR_FACING_SIGNAL": dict(
            family="operator_facing_terminal_brief_result",
...
def candidate_signal(
    ...
    *,
    ...
    carried = {
        ...
        "source_artifact_family": source["result_family"],
    ...
    return {
        "candidate_signal_metadata": {
            ...
            "candidate_signal_version": "0.1.0",
            "declared_at": "2026-04-25T00:00:00Z",
            "declared_by_surface": "tests/test_resolve_body_signal_recognition.py",
        },
        ...
        "claimed_signal": {
            ...
            "claimed_signal_reason": "selected source posture is visible as bounded signal",
...
class BodySignalRecognitionTests(unittest.TestCase):
    maxDiff = None
    ...
    def test_successful_mapping_based_recognition(self) -> None:
        source = source_artifact()
        candidate = candidate_signal(source)
    ...
    def test_successful_path_based_recognition(self) -> None:
    ...
    def test_metadata_selected_sections_and_recognized_signal(self) -> None:
        ...
        metadata = result["body_signal_recognition_metadata"]
        for key in REQUIRED_METADATA_KEYS:
            self.assertIsInstance(metadata.get(key), str)
            self.assertTrue(metadata[key])
        self.assertEqual(metadata["resolver_module"], "resolve_body_signal_recognition")
    ...
    def test_default_output_path_behavior_uses_bounded_suffix(self) -> None:
        result = self.recognized_result()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "body_signal_recognition"

            def temp_default_path(value: Mapping[str, Any]) -> Path:
                candidate = value["selected_candidate_signal"]
                stem = candidate["candidate_signal_metadata"]["candidate_signal_id"]
                first = root / f"{stem}__body_signal_recognition_result.json"
                if not first.exists():
                    return first
                return root / f"{stem}__body_signal_recognition_result_001.json"

            with mock.patch.object(
                recognition,
                "_safe_default_output_path",
                side_effect=temp_default_path,
            ):
    ...
    def test_category_specific_valid_pairs_are_recognized(self) -> None:
        ...
        for category in categories:
            with self.subTest(category=category):
                ...
                self.assertEqual(result["body_signal_recognition_summary"]["failed_check_count"], 0)
