"""Bounded tests for the v0-min source-to-receiving comparison runner.
...
using fresh source scenario runs emitted through the current local scenario
runner.

They verify discovery, manifest validation, direct source/receiving-packet
...
import compare_integrity_host_v0_min_coexistence_source_and_receiving_packet as comparator  # noqa: E402
...
class IntegrityHostV0MinCoexistenceSourceReceivingComparisonTests(unittest.TestCase):
    ...
    def assert_count_entry(self, entry: dict[str, Any]) -> None:
        self.assertEqual(set(entry), {"source", "receiving", "matched"})
        self.assertIsInstance(entry["source"], int)
        self.assertIsInstance(entry["receiving"], int)
        self.assertIsInstance(entry["matched"], bool)

    def assert_visibility_entry(self, entry: dict[str, Any]) -> None:
        self.assertEqual(set(entry), {"source", "receiving", "matched"})
        self.assertIn(type(entry["source"]), {list, dict})
        self.assertIn(type(entry["receiving"]), {list, dict})
        self.assertIsInstance(entry["matched"], bool)
    ...
    def test_real_source_receiving_comparison_shape_and_coverage(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            source_run_dir = self.emit_run(Path(temp_dir) / "run_compare")

            comparison = comparator.compare_source_run_and_receiving_packet(source_run_dir)
            ...
            self.assertEqual(
                comparison["comparison_metadata"]["comparison_version"],
                comparator.COMPARISON_VERSION,
            )
            self.assertEqual(
    ...
    def test_per_scenario_comparison_content(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for scenario_comparison in comparison["scenario_comparisons"]:
                ...
                status_visibility = visibility["receiving_status_flags"]
                self.assertEqual(status_visibility["source"], comparator.EXPECTED_RECEIVING_STATUS)
                self.assertEqual(
                    status_visibility["receiving"],
                    comparator.EXPECTED_RECEIVING_STATUS,
                )
    ...
    def test_aggregate_comparison_clean_match(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("receiving_totals", aggregate)
            ...
            self.assertEqual(
                aggregate["source_totals"]["scenario_count"],
                aggregate["receiving_totals"]["scenario_count"],
            )
            self.assertEqual(
    ...
    def test_scenario_specific_comparison_signals(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            by_id = self.comparison_by_id(
                comparator.compare_source_run_and_receiving_packet(source_run_dir)
            )
            ...
            self.assertEqual(
                hold["visibility"]["receiving_status_flags"]["receiving"],
                comparator.EXPECTED_RECEIVING_STATUS,
            )
    ...
    def test_mismatch_visibility(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def build_with_missing_object(path: Path) -> dict[str, Any]:
                packet = copy.deepcopy(builder.build_receiving_packet(path))
                if (
                    packet["source_provenance"]["scenario_id"]
                    == "lawful_distinct_matter_coexistence"
                ):
                    packet["imported_packet"]["objects"] = packet["imported_packet"][
                        "objects"
                    ][:-1]
                    packet["import_summary"]["object_count"] = len(
                        packet["imported_packet"]["objects"]
                    )
                return packet
            ...
            with mock.patch.object(
                ...
                side_effect=build_with_missing_object,
            ):
                comparison = comparator.compare_source_run_and_receiving_packet(
                    source_run_dir
                )
    ...
    def test_error_posture(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(RuntimeError):
                comparator.compare_source_run_and_receiving_packet(
            ...
            with self.assertRaises(RuntimeError):
                comparator.compare_source_run_and_receiving_packet(missing_artifact_run)
            ...
            with self.assertRaises(RuntimeError):
                comparator.compare_source_run_and_receiving_packet(bad_artifact_run)

            receiving_failure_run = self.emit_run(temp_root / "run_receiving_failure")
            with mock.patch.object(
                comparator,
                "build_receiving_packet",
                side_effect=RuntimeError("forced receiving failure"),
            ):
                with self.assertRaises(RuntimeError):
                    comparator.compare_source_run_and_receiving_packet(
                        receiving_failure_run
                    )
    ...
    def test_non_mutation_posture(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = comparator.compare_source_run_and_receiving_packet(source_run_dir)
            second = comparator.compare_source_run_and_receiving_packet(source_run_dir)
