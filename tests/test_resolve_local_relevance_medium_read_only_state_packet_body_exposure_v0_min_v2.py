"""Successor tests for selected-state local read-only packet body exposure.

This v2 suite preserves the v1 packet-body-exposure coverage while treating the
v1 test as over-strict failed-lineage evidence. Top-level closure-token
violations still require exact closure-token block codes. Declared non-claim
malformed, flipped, or missing closure-token posture is allowed to block with
``NON_CLAIM_MISSING_OR_FLIPPED`` and is not required to use the top-level
closure-token block codes.

The target remains one bounded
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_STATE_PACKET_BODY_EXPOSURE object: local,
read-only, selected-state-only, non-raw-full-packet-body-exposure, and
non-operation-shaped.
"""
...
class LocalRelevanceMediumReadOnlyStatePacketBodyExposureV2Tests(unittest.TestCase):
    ...
    def failed_check_count(self, result: Mapping[str, Any]) -> int:
        return int(self.summary(result).get("failed_check_count", 0))
    ...
    def assert_blocked_with_public_code(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_packet_body_exposure_non_claims(result)
    ...
    def assert_packet_body_exposure_non_claims(self, result: Mapping[str, Any]) -> None:
        self.assert_canonical_false_non_claims(result)
    ...
    def test_closure_token_blocking_behavior(self) -> None:
        cases: list[tuple[str, set[str], Callable[[dict[str, Any]], None]]] = [
            (
            ...
            (
                "declared non-claim consumed request reopened",
                {"NON_CLAIM_MISSING_OR_FLIPPED", "CONSUMED_REQUEST_REOPENED"},
                lambda request: _set(
            ...
            (
                "declared non-claim authorization token reused",
                {"NON_CLAIM_MISSING_OR_FLIPPED", "AUTHORIZATION_TOKEN_REUSED"},
                lambda request: _set(
                    request["declared_non_claims"],
                    "authorization_token_reused",
                    True,
        ...
        declared_non_claim_cases = {
            name for name, allowed, _mutator in cases if "non-claim" in name
        }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            _write_json(bundle["boundary_path"], bundle["boundary_artifact"])
            _write_json(bundle["result_object_path"], bundle["result_object_artifact"])
            for name, allowed_codes, mutator in cases:
                with self.subTest(case=name):
                    ...
                    self.assertIn(self.block_code(result), allowed_codes)
                    if name in declared_non_claim_cases:
                        self.assertIn("NON_CLAIM_MISSING_OR_FLIPPED", allowed_codes)
                    self.assert_closure_token_final_posture_false(result)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for key in resolver.REQUIRED_FALSE_NON_CLAIMS:
                with self.subTest(non_claim=key):
                    ...
                    for non_claim_key in resolver.REQUIRED_FALSE_NON_CLAIMS:
                        self.assertIs(result["non_claims"][non_claim_key], False)
                    self.assert_packet_body_exposure_non_claims(result)
    ...
    def test_representative_blocking_behavior(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assert_blocked_with_public_code(boundary_array_result)
            ...
            self.assert_blocked_with_public_code(result_object_array_result)
        ...
        self.assert_blocked_with_public_code(non_mapping_result)
    ...
    def test_predecessor_failure_preservation(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assert_blocked_with_public_code(consumed)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assert_blocked_with_public_code(reused)
