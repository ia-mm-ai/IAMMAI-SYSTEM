"""Bounded tests for the v0-min effective-family consumption resolver.
...
governing re-resolution results, successor-adoption results, and
effective-family resolution artifacts emitted into temporary roots.

The suite verifies consumption of an explicit effective-family resolution only.
...
CONSUMED_REFERENCE_KEYS = EFFECTIVE_INPUT_KEYS


class IntegrityHostV0MinCoexistenceEffectiveFamilyConsumptionTests(
    unittest.TestCase
):
    def emit_source_run(
        self,
    ...
    def build_multi_run_stack(
        ...
        *,
        ...
        if emit_effective:
            effective_path, effective_result = (
                self.emit_effective_family_result_from_object(
                    temp_root,
                    adoption_result,
    ...
    def valid_proposal(
        ...
        return {
            "transition_proposal_id": "proposal_accepted_for_consumption",
            ...
            "proposal_basis_ref": "bounded explicit consumption test basis",
    ...
    def resolve_consumption_default(self, temp_root: Path) -> dict[str, Any]:
        with mock.patch.object(
            consumption_resolver,
            "_repo_root",
            return_value=temp_root,
        ):
            return consumption_resolver.resolve_effective_family_consumption()

    def resolve_consumption_object(
        self,
        temp_root: Path,
        effective_family_resolution: dict[str, Any],
    ) -> dict[str, Any]:
        with mock.patch.object(
            consumption_resolver,
            "_repo_root",
            ...
            return consumption_resolver.resolve_effective_family_consumption(
                effective_family_resolution
            )

    def resolve_consumption_from_path(
        self,
        temp_root: Path,
        effective_path: Path,
    ) -> dict[str, Any]:
        with mock.patch.object(
            ...
            return consumption_resolver.resolve_effective_family_consumption_from_path(
                effective_path
            )
    ...
    def assert_result_shape(self, result: dict[str, Any]) -> None:
        ...
        self.assertIsInstance(result["consumption_metadata"], dict)
        self.assertIsInstance(result["selected_effective_family_resolution"], dict)
        self.assertIsInstance(result["effective_inputs"], dict)
        self.assertIsInstance(result["checks"], list)
        self.assertIsInstance(result["block"], dict)
        self.assertIsInstance(result["consumed_family_references"], dict)
        self.assertIsInstance(result["consumption_summary"], dict)
    ...
    def assert_consumed_result(self, result: dict[str, Any]) -> None:
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], consumption_resolver.OUTCOME_CONSUMED)
        self.assertIsNone(result["block"]["block_code"])
        self.assertIsNone(result["block"]["block_reason"])
        self.assertEqual(set(result["effective_inputs"]), EFFECTIVE_INPUT_KEYS)
        self.assertEqual(
            set(result["consumed_family_references"]),
            CONSUMED_REFERENCE_KEYS,
        )
        for value in result["effective_inputs"].values():
            self.assert_non_empty_string(value)
        for value in result["consumed_family_references"].values():
            self.assert_non_empty_string(value)
    ...
    def assert_blocked_result(
        ...
        *,
        ...
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], consumption_resolver.OUTCOME_BLOCKED)
    ...
    def assert_effective_inputs_match_resolution(
        self,
        temp_root: Path,
        result: dict[str, Any],
        ...
        for key in EFFECTIVE_INPUT_KEYS:
            self.assert_paths_same(
                ...
                effective_resolution["effective_family"][key],
            )
            self.assert_paths_same(
                temp_root,
                result["consumed_family_references"][key],
                effective_resolution["effective_family"][key],
            )

    def assert_under_consumption_root(self, temp_root: Path, path: Path) -> None:
        root = (
            temp_root / consumption_resolver.EFFECTIVE_FAMILY_CONSUMPTION_ROOT
        ).resolve()
        try:
    ...
    def test_real_accepted_effective_family_consumption_default_selection(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_consumption_default(temp_root)

            self.assert_consumed_result(result)
            selected = result["selected_effective_family_resolution"]
            self.assert_paths_same(
                temp_root,
                selected["effective_family_resolution_artifact_path"],
                stack["effective_path"],
            )
            self.assertEqual(
                selected["effective_family_resolution_id"],
            ...
            self.assertEqual(
                selected["selected_adoption_result_id"],
                stack["adoption_result"]["adoption_metadata"]["adoption_result_id"],
            )
            self.assert_effective_inputs_match_resolution(
                temp_root,
                result,
                stack["effective_result"],
            )
            metadata = result["consumption_metadata"]
            for key in (
                "consumption_result_id",
                "consumption_result_type",
                "consumption_result_version",
                "generated_at",
                "resolver_module",
            ):
                self.assert_non_empty_string(metadata[key])

    def test_consumption_from_path_and_object_are_consistent(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            object_result = self.resolve_consumption_object(
                temp_root,
                stack["effective_result"],
            )
            path_result = self.resolve_consumption_from_path(
                temp_root,
                stack["effective_path"],
            )

            self.assert_consumed_result(object_result)
            self.assert_consumed_result(path_result)
            self.assertIsNone(
                object_result["selected_effective_family_resolution"][
                    "effective_family_resolution_artifact_path"
                ]
            )
            self.assert_paths_same(
                temp_root,
                path_result["selected_effective_family_resolution"][
                    "effective_family_resolution_artifact_path"
                ],
                stack["effective_path"],
            )
            ...
            self.assertEqual(
                object_result["consumed_family_references"],
                path_result["consumed_family_references"],
            )

    def test_no_effective_family_resolution_available_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()

            result = self.resolve_consumption_default(temp_root)

            self.assert_blocked_result(result, "NO_EFFECTIVE_FAMILY_RESOLUTION")
            selected = result["selected_effective_family_resolution"]
            self.assertIsNone(selected["effective_family_resolution_id"])
            self.assertIsNone(selected["outcome"])
            for value in result["effective_inputs"].values():
                self.assertIsNone(value)

    def test_selected_resolution_effective_inputs_checks_and_summary(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_consumption_object(
                temp_root,
                stack["effective_result"],
            )
            ...
            summary = consumption_resolver.build_effective_family_consumption_summary(
                result
            )
            for key in (
                "consumption_result_id",
            ...
            self.assertEqual(summary["outcome"], consumption_resolver.OUTCOME_CONSUMED)
            self.assertIsNone(summary["block_code"])
            self.assertIsNone(summary["block_reason"])
            self.assertGreater(summary["passed_check_count"], 0)
            self.assertEqual(summary["failed_check_count"], 0)
            self.assertEqual(
                summary["selected_effective_family_resolution_id"],
                stack["effective_result"]["resolution_metadata"][
                    "effective_family_resolution_id"
                ],
            )
    ...
    def test_consumed_references_and_semantics_preserve_prior_family(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            ...
            self.assertTrue(
                result["consumption_summary"]["prior_family_remained_preserved"]
            )
            self.assertEqual(
                result["consumption_summary"]["consumed_current_family_basis"],
                effective_resolver.BASIS_ADOPTED_SUCCESSOR_FAMILY_EFFECTIVE,
            )

    def test_blocked_or_malformed_effective_family_resolution(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            malformed_path = temp_root / "malformed_effective_family_resolution.json"
            malformed_path.write_text("[1, 2, 3]\n", encoding="utf-8")

            with self.assertRaises(
                consumption_resolver.EffectiveFamilyConsumptionError
            ):
                self.resolve_consumption_from_path(temp_root, malformed_path)

        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root, write_effective_artifact=False)
            blocked_resolution = self.copy_payload(stack["effective_result"])
            blocked_resolution["outcome"] = "BLOCKED"

            result = self.resolve_consumption_object(temp_root, blocked_resolution)
    ...
    def test_effective_family_artifact_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root, write_effective_artifact=False)
            effective = self.copy_payload(stack["effective_result"])
            missing_path = self.resolve_display_path(
                temp_root,
                effective["effective_family"]["effective_status_packet_path"],
            )
            missing_path.unlink()

            result = self.resolve_consumption_object(temp_root, effective)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            malformed_path = self.resolve_display_path(
                temp_root,
                effective["effective_family"]["effective_authority_artifact_path"],
            )
            ...
            with self.assertRaises(
                ...
                self.resolve_consumption_object(temp_root, effective)
    ...
    def test_canonical_execution_line_mismatch_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            family_path = self.resolve_display_path(
                temp_root,
                effective["effective_family"]["effective_family_packet_path"],
            )
    ...
    def test_stale_prior_family_fallback_is_refused(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root, write_effective_artifact=False)

            consumed = self.resolve_consumption_object(
                temp_root,
                stack["effective_result"],
            )
            self.assert_consumed_result(consumed)
            for effective_key, prior_key in (
                ("effective_authority_artifact_path", "prior_authority_artifact_path"),
                ("effective_family_packet_path", "prior_family_packet_path"),
                ("effective_status_packet_path", "prior_status_packet_path"),
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        consumed["effective_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
                        stack["effective_result"]["prior_current_family"][prior_key],
            ...
            stale = self.copy_payload(stack["effective_result"])
            stale["effective_family"] = {
                "effective_authority_artifact_path": stale["prior_current_family"][
                    "prior_authority_artifact_path"
                ],
                "effective_family_packet_path": stale["prior_current_family"][
                    "prior_family_packet_path"
                ],
                "effective_status_packet_path": stale["prior_current_family"][
                    "prior_status_packet_path"
                ],
                "effective_current_governing_packet_path": stale[
            ...
            blocked = self.resolve_consumption_object(temp_root, stale)

            self.assert_blocked_result(blocked, "STALE_PRIOR_FAMILY_FALLBACK_REFUSED")

    def test_non_claim_shortcuts_block_consumption(self) -> None:
        cases = (
            ...
            with self.subTest(non_claim_key=non_claim_key):
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    stack = self.build_multi_run_stack(
                        temp_root,
                        write_effective_artifact=False,
                    )
                    effective = self.copy_payload(stack["effective_result"])
                    governing_path = self.resolve_display_path(
                        temp_root,
                        effective["effective_family"][
                            "effective_current_governing_packet_path"
                        ],
                    )
    ...
    def test_multiple_effective_family_resolution_conflict_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            conflict = self.copy_payload(stack["effective_result"])
            conflict["resolution_metadata"]["effective_family_resolution_id"] = (
                "conflicting_effective_family_resolution"
            )
            conflict["effective_family"][
                "effective_current_governing_packet_path"
            ] = "artifacts/conflicting_effective_governing_packet.json"
            conflict_path = (
                temp_root
                / consumption_resolver.EFFECTIVE_FAMILY_RESOLUTION_ROOT
                / "conflicting_effective_family_resolution.json"
            )
    ...
    def test_hard_malformed_artifact_failures(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            root_file = (
                temp_root / consumption_resolver.EFFECTIVE_FAMILY_RESOLUTION_ROOT
            )
            ...
            with self.assertRaises(
                ...
                self.resolve_consumption_default(temp_root)

        malformed_resolution_cases = (
            ("non_mapping_input", "not a mapping"),
            ("missing_effective_family", {"outcome": "CURRENT_FAMILY_EFFECTIVE"}),
        )
        for label, payload in malformed_resolution_cases:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    with self.assertRaises(
                        ...
                        with mock.patch.object(
                            ...
                            consumption_resolver.resolve_effective_family_consumption(
    ...
    def test_write_behavior_and_default_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_path = temp_root / "nested" / "consumption" / "result.json"
            ...
            self.assert_under_consumption_root(temp_root, default_first)
            self.assert_under_consumption_root(temp_root, default_second)
            self.assertIn(
                result["consumption_metadata"]["consumption_result_id"],
                default_first.name,
            )
    ...
    def test_non_mutation_posture(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = self.resolve_consumption_object(
                temp_root,
                stack["effective_result"],
            )
            after_first = {
            ...
            second = self.resolve_consumption_from_path(temp_root, stack["effective_path"])
            after_second = {
                label: self.json_texts(root) for label, root in roots.items()
            }
            ...
            self.assertEqual(
                first["selected_effective_family_resolution"][
                    "effective_family_resolution_id"
                ],
                second["selected_effective_family_resolution"][
                    "effective_family_resolution_id"
                ],
            )
            self.assertEqual(first["effective_inputs"], second["effective_inputs"])
            self.assertEqual(
                first["consumed_family_references"],
                second["consumed_family_references"],
            )
            self.assertEqual(first["consumption_summary"], second["consumption_summary"])
