"""Tests for the v4 derivative participant participation-motion boundary.

The v4 resolver records one boundary object only. V1, v2, and v3 participation
motion boundary resolvers remain preserved predecessor evidence. These tests
exercise the v4 terminal-summary-derived role-admission v1 evidence path and
the blocked-result canonicalization posture: incoming illegal true posture may
block, but final boundary, statement, and result-level non-claims stay false.
"""
...
import resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4 as resolver  # noqa: E402
...
BOUNDARY_ONLY_FALSE_FIELDS = tuple(REQ_FALSE)
...
class LocalRelevanceMediumReadOnlyDerivativeParticipantParticipationMotionBoundaryV0MinV4Tests(
    unittest.TestCase
):
    ...
    def write_role_terminal_summary(self, path: Path, text: str | None = None) -> Path:
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            text
            or (
                "The v1 role admission resolver is preserved predecessor evidence only. "
                "It is not repaired, hidden, or claimed passed."
            ),
            encoding="utf-8",
        )
        return path
    ...
    def clean_role_artifact(
        ...
        *,
        ...
        include_exact_role_v1_evidence: bool = False,
        ...
        if include_exact_role_v1_evidence:
            role_object["role_admission_v1_failure_evidence_preserved"] = True
        ...
        for key in ROLE_POSITIVE_FIELDS + ("predecessor_failure_evidence_preserved",):
            statement.setdefault(key, True)
        if include_exact_role_v1_evidence:
            statement["role_admission_v1_failure_evidence_preserved"] = True
        ...
        if include_exact_role_v1_evidence:
            summary["role_admission_v1_failure_evidence_preserved"] = True
        ...
        if include_exact_role_v1_evidence:
            non_meaning["role_admission_v1_failure_evidence_preserved"] = True
    ...
    def request_for_paths(
        self,
        role_summary_path: Path | str,
        role_path: Path | str,
        continuation_path: Path | str,
        **overrides: Any,
    ) -> dict[str, Any]:
        request = (
            resolver.build_declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_request(
                selected_role_admission_terminal_summary_path=role_summary_path,
    ...
    def result_from_artifacts(
        ...
        terminal_summary_text: str | None = None,
        **request_overrides: Any,
        ...
        role_summary_path = self.write_role_terminal_summary(
            tmp_path / "role_admission_terminal_summary.md", terminal_summary_text
        )
        ...
        request = self.request_for_paths(
            role_summary_path, role_path, continuation_path, **request_overrides
        )
        result = resolver.resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4(
            request
        )
        return result, request, role_summary_path, role_path, continuation_path
    ...
    def assert_boundary_forbidden_posture_false(self, result: Mapping[str, Any]) -> None:
    ...
    def assert_statement_forbidden_posture_false(self, result: Mapping[str, Any]) -> None:
    ...
    def assert_boundary_only(self, result: Mapping[str, Any]) -> None:
        self.assert_boundary_forbidden_posture_false(result)
        boundary = self.boundary(result)
    ...
    def assert_blocked_with_public_code(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_boundary_forbidden_posture_false(result)
        self.assert_statement_forbidden_posture_false(result)
    ...
    def assert_path_block_or_bounded_error(self, func: Any) -> None:
        try:
            result = func()
        except resolver.LocalRelevanceMediumReadOnlyDerivativeParticipantParticipationMotionBoundaryV0MinV4Error:
            return
        self.assert_blocked_with_public_code(result)

    def test_public_api_constants_and_builder_defaults(self) -> None:
        for name in (
            "resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4",
            "resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_from_path",
            "write_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_result",
            "build_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_summary",
            "build_declared_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_request",
        ):
        ...
        self.assertTrue(str(resolver.OUTPUT_ROOT).endswith("participation_motion_boundary_v0_min_v4"))
        ...
        for key in BOUNDARY_POSITIVE_FIELDS + ROLE_POSITIVE_FIELDS + CONTINUATION_POSITIVE_FIELDS + EVIDENCE_FIELDS + ("result_level_non_claims_canonical_false",):
        ...
        self.assertTrue(
            request["selected_role_admission_terminal_summary_path"].endswith(
                "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_DERIVATIVE_PARTICIPANT_ROLE_ADMISSION_TERMINAL_SUMMARY_V0.md"
            )
        )
        self.assertTrue(
    ...
    def test_successful_recorded_result_from_synthetic_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            result, request, _, _, _ = self.result_from_artifacts(Path(tmp))
    ...
    def test_successful_recorded_result_from_default_live_artifacts_if_present(self) -> None:
        ...
        paths = [
            REPO_ROOT / request["selected_role_admission_terminal_summary_path"],
            REPO_ROOT / request["selected_role_admission_artifact"],
            REPO_ROOT / request["selected_continuation_artifact"],
        ]
        missing = [path for path in paths if not path.exists()]
        if missing:
            self.skipTest("default live basis files are not present")
        result = resolver.resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4(
            request
        )
        self.assert_recorded_clean_result(result)
        self.assert_success_boundary_fields(result)
        self.assertTrue(
            self.boundary(result)["basis_role_admission_artifact"].endswith(paths[1].name)
        )
        self.assertTrue(
            self.boundary(result)["basis_continuation_artifact"].endswith(paths[2].name)
        )

    def test_role_admission_v1_evidence_derives_from_terminal_summary(self) -> None:
        role = self.clean_role_artifact(include_exact_role_v1_evidence=False)
        with tempfile.TemporaryDirectory() as tmp:
            result, _, _, _, _ = self.result_from_artifacts(Path(tmp), role_artifact=role)
        self.assert_recorded_clean_result(result)
        self.assertIs(self.boundary(result)["role_admission_v1_failure_evidence_preserved"], True)
        self.assertIs(self.summary(result)["role_admission_v1_failure_evidence_preserved"], True)
        self.assertIs(self.boundary(result)["participation_motion_created"], False)
        self.assertIs(self.boundary(result)["role_admission_treated_as_participation_permission"], False)

        blocked_variants = (
            ("missing_terminal_summary", {"selected_role_admission_terminal_summary_path": ""}, None),
            ("unreadable_terminal_summary", {"selected_role_admission_terminal_summary_path": "missing-summary.md"}, None),
            ("terminal_summary_lacks_v1_text", {}, "This summary has no bounded v1 predecessor-evidence posture."),
        )
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp)
            for name, request_overrides, text in blocked_variants:
                with self.subTest(name=name):
                    result, request, _, _, _ = self.result_from_artifacts(
                        base / name,
                        role_artifact=self.clean_role_artifact(include_exact_role_v1_evidence=False),
                        terminal_summary_text=text,
                    )
    ...
    def test_declared_non_claim_flips_block_and_canonicalize_final_posture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            _, request, _, _, _ = self.result_from_artifacts(Path(tmp))
            for key in REQ_FALSE:
                with self.subTest(key=key):
                    ...
                    self.assertIs(self.statement(result)[key], False)
    ...
    def test_representative_blocking_behavior(self) -> None:
        cases: list[tuple[str, dict[str, Any]]] = [
            ...
            ("role_terminal_summary_missing", {"request": {"selected_role_admission_terminal_summary_path": ""}}),
            ...
            ("role_v1_evidence_not_preserved", {"role_object": {"predecessor_failure_evidence_preserved": False}}),
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name, config in cases:
                with self.subTest(name=name):
                    ...
                    if "role" in config:
                        role_artifact = self.clean_role_artifact(**config["role"])
                        result, request, _, _, _ = self.result_from_artifacts(
                            base / f"{name}_role_header", role_artifact=role_artifact
                        )
                    if "continuation" in config:
                        ...
                        result, request, _, _, _ = self.result_from_artifacts(
                            base / f"{name}_continuation_header",
                            continuation_artifact=continuation_artifact,
                        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for key in role_omits:
                with self.subTest(role_key=key):
                    ...
                    result, _, _, _, _ = self.result_from_artifacts(Path(tmp) / key, role_artifact=role)
                    self.assert_recorded_clean_result(result)
            for key in continuation_omits:
                with self.subTest(continuation_key=key):
                    ...
                    result, _, _, _, _ = self.result_from_artifacts(
                        Path(tmp) / key, continuation_artifact=continuation
                    )
                    self.assert_recorded_clean_result(result)
    ...
    def test_exact_forbidden_true_blocks_and_canonicalizes_final_posture(self) -> None:
        exact_keys = (
            "participation_motion_created",
            "participant_output_authorized",
            "action_authorization_created",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for key in exact_keys:
                with self.subTest(selected_object_key=key):
                    ...
                    result, _, _, _, _ = self.result_from_artifacts(Path(tmp) / f"object_{key}", role_artifact=role)
                    self.assert_blocked_with_public_code(result)
                ...
                with self.subTest(top_level_non_claim_key=key):
                    ...
                    result, _, _, _, _ = self.result_from_artifacts(Path(tmp) / f"nonclaim_{key}", role_artifact=role)
                    self.assert_blocked_with_public_code(result)
                    self.assert_expected_code_present(result, FIELD_CODES[key])
                    self.assertIs(self.boundary(result)[key], False)
                    self.assertIs(self.statement(result)[key], False)
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            result, request, _, _, _ = self.result_from_artifacts(root)
            request_path = self.write_json(root / "request.json", request)
            from_path = resolver.resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_from_path(
                request_path
            )
            ...
            self.assert_path_block_or_bounded_error(
                lambda: resolver.resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_from_path(
                    malformed
                )
            )
            ...
            output_root = root / "artifacts" / "integrity_host_v0_min_coexistence_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4"
            with mock.patch.object(resolver, "OUTPUT_ROOT", output_root):
                first = resolver.write_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_result(
                    result
                )
                second = resolver.write_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4_result(
                    result
                )
            ...
            self.assertIn("participation_motion_boundary_v0_min_v4", str(first))
            for forbidden in (
                "participation_motion_boundary_v0_min/",
                "participation_motion_boundary_v0_min_v2",
                "participation_motion_boundary_v0_min_v3",
    ...
    def test_non_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            role_before = _json_round_trip(role)
            continuation_before = _json_round_trip(continuation)
            result, request, role_summary_path, role_path, continuation_path = self.result_from_artifacts(
                root, role_artifact=role, continuation_artifact=continuation
            )
            request_before = _json_round_trip(request)
            resolver.resolve_local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_v0_min_v4(
                request
            )
        self.assert_recorded_clean_result(result)
        self.assertEqual(role, role_before)
        self.assertEqual(continuation, continuation_before)
        self.assertEqual(request, request_before)
        self.assertEqual(request["selected_role_admission_terminal_summary_path"], str(role_summary_path))
        self.assertEqual(request["selected_role_admission_artifact"], str(role_path))
        self.assertEqual(request["selected_continuation_artifact"], str(continuation_path))

    def test_v1_v2_v3_predecessor_preservation_posture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            result, _, _, _, _ = self.result_from_artifacts(Path(tmp))
        self.assert_recorded_clean_result(result)
        boundary = self.boundary(result)
        ...
        for key in (
            ...
            self.assertIs(boundary[key], False, key)
        self.assertIs(boundary["role_admission_treated_as_participation_permission"], False)
        metadata = result["local_relevance_medium_read_only_derivative_participant_participation_motion_boundary_metadata"]
        self.assertIs(metadata["predecessor_v1_preserved_as_evidence_only"], True)
        self.assertIs(metadata["predecessor_v2_preserved_as_evidence_only"], True)
        self.assertIs(metadata["predecessor_v3_preserved_as_evidence_only"], True)
        self.assert_hidden_repo_state_false(result)
