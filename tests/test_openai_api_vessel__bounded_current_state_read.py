"""Bounded tests for the first OpenAI API derivative vessel.

This suite exercises
``src/openai_api_vessel__bounded_current_state_read.py`` as one shell-owned,
bounded derivative read over one already-standing current-state surface. It
does not test broad orchestration, whole-body reading, middleware behavior,
agent behavior, or live network integration as the primary execution path.
"""
...
import openai_api_vessel__bounded_current_state_read as vessel
...
class FakeOpenAI:
    output_text = json.dumps({"answer": "bounded derivative read"})
    ...
    @classmethod
    def reset(cls) -> None:
        cls.output_text = json.dumps({"answer": "bounded derivative read"})
        cls.create_error = None
        cls.instances = []


class OpenAIBoundedCurrentStateReadTests(unittest.TestCase):
        ...
        *,
        ...
        result = {
            ...
            "what_stands_now_request": {
                ...
                "query_basis": "bounded current-state vessel test request",
            },
    ...
    def write_what_stands_now_result(
        ...
        output_path = (
            temp_root / vessel.CURRENT_STATE_WHAT_STANDS_NOW_ROOT / file_name
        )
    ...
    def test_builds_bounded_packet_from_real_what_stands_now_shape(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                vessel.SUPPORTED_USE_CLASS,
                packet["admitted_use_class"],
            )
            self.assertEqual(
            ...
            self.assertEqual(
                str(
                    (
                        vessel.CURRENT_STATE_WHAT_STANDS_NOW_ROOT / "selected.json"
                    ).as_posix()
                ),
            ...
            self.assertEqual(
                2,
                packet["bounded_source_payload"]["preserved_run_count"],
            )
            self.assertEqual(
    ...
    def test_refuses_source_family_widening_in_packet_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            result = self.build_what_stands_now_result()
            artifact_path = self.write_what_stands_now_result(
            ...
            with mock.patch.object(vessel, "_repo_root", return_value=temp_root):
                ...
                with self.assertRaises(vessel.OpenAIDerivativeVesselError) as raised:
                    vessel._validate_vessel_request(packet)

            self.assertEqual("WIDENED_SOURCE_ATTEMPT", raised.exception.block_code)

    def test_resolve_success_selects_latest_answered_source_and_wraps_result(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            ...
            self.assertEqual(
                str(
                    (
                        vessel.CURRENT_STATE_WHAT_STANDS_NOW_ROOT / "z.json"
                    ).as_posix()
                ),
            ...
            metadata = result["openai_api_derivative_vessel_metadata"]
            for key in (
                "vessel_result_id",
                "vessel_result_type",
            ...
            self.assertEqual("gpt-bounded-test", api_runtime["model_name_used"])
    ...
    def test_invalid_source_family_is_refused_explicitly(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            invalid_path = (
                temp_root / vessel.CURRENT_STATE_WHAT_STANDS_NOW_ROOT / "invalid.json"
            )
            ...
            with self.patched_vessel_runtime(
                ...
                result = vessel.resolve_bounded_current_state_read(invalid_path)

            self.assert_refused(result, "INVALID_SOURCE_FAMILY")
            self.assertEqual(
                str(
                    (
                        vessel.CURRENT_STATE_WHAT_STANDS_NOW_ROOT / "invalid.json"
                    ).as_posix()
                ),
                result["selected_source_surface"]["selected_source_surface_path"],
            )
    ...
    def test_model_refusal_and_malformed_outputs_are_shell_wrapped(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            artifact_path = self.write_what_stands_now_result(
                temp_root,
            ...
            scenarios = [
                (
                    json.dumps({"refusal_code": vessel.MODEL_REFUSAL_CODE}),
                ...
                (
                    json.dumps({"answer": "ok", "provenance": "model-owned"}),
                    "MODEL_OUTPUT_OUTSIDE_CONTRACT",
                ),
            ]
            ...
            for output_text, expected_block_code in scenarios:
                with self.subTest(output_text=output_text):
                    FakeOpenAI.reset()
                    FakeOpenAI.output_text = output_text
                    with self.patched_vessel_runtime(
                        temp_root,
    ...
    def test_missing_local_setup_and_api_failure_are_refused_explicitly(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.patched_vessel_runtime(temp_root, env={}):
            ...
            FakeOpenAI.create_error = RuntimeError("api exploded")
            with self.patched_vessel_runtime(
                temp_root,
                env={"OPENAI_API_KEY": "test-api-key"},
            ):
    ...
    def test_summary_write_helpers_and_default_output_path_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            explicit_path = temp_root / "written" / "nested" / "vessel_result.json"
            ...
            self.assertEqual(
                temp_root / vessel.OPENAI_API_DERIVATIVE_VESSEL_ROOT,
                first_default.parent,
            )
            self.assertTrue(
                first_default.name.endswith(
                    "__bounded_current_state_vessel_result.json"
                )
            )
            self.assertIn("_001", second_default.stem)
            self.assertNotEqual(first_default, second_default)

    def test_vessel_resolution_does_not_mutate_source_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            artifact_path = self.write_what_stands_now_result(
                temp_root,
            ...
            output_root = temp_root / vessel.OPENAI_API_DERIVATIVE_VESSEL_ROOT
