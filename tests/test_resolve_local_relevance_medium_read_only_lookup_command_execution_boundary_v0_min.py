"""Tests for the local read-only lookup command execution boundary resolver.

This suite is bounded to one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_LOOKUP_COMMAND_EXECUTION_BOUNDARY object. It
verifies that the resolver reads one clean selected raw/full state packet body
exposure artifact, records one local read-only selected-state lookup-command-
execution-consideration boundary, and keeps lookup command execution, lookup
performed, lookup result, operation permission, runtime permission,
...
class LookupCommandExecutionBoundaryResolverTests(unittest.TestCase):
    ...
    def write_synthetic_artifact(
        self, directory: Path, artifact: Mapping[str, Any] | None = None
    ) -> Path:
        path = directory / "synthetic_raw_full_state_packet_body_exposure.json"
        material = (
    ...
    def resolve_from_synthetic(
        ...
        artifact_path = self.write_synthetic_artifact(directory, artifact)
        request = self.build_request(artifact_path)
        if request_mutator is not None:
            request_mutator(request)
        return resolver.resolve_local_relevance_medium_read_only_lookup_command_execution_boundary_v0_min(
            request
        )
    ...
    def assert_clean_recorded_boundary(
        ...
        self.assertIs(
            boundary["raw_full_state_packet_body_exposure_local_only"], True
        )
        self.assertIs(
            boundary["raw_full_state_packet_body_exposure_read_only"], True
        )
        self.assertIs(
    ...
    def test_successful_recorded_result_from_default_artifact_if_present(self) -> None:
        ...
        self.assertIs(boundary["lookup_command_executed"], False)
    ...
    def test_representative_blocking_behavior(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            for name, artifact_mutator, request_mutator, direct in cases:
                with self.subTest(name=name):
                    if direct == "missing_request":
                        ...
                        if artifact_mutator is not None:
                            artifact_mutator(artifact)
                        artifact_path = tmp_path / f"{name.replace(' ', '_')}.json"
