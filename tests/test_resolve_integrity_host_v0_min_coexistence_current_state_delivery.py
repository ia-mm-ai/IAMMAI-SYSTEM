"""Bounded tests for the v0-min current-state delivery resolver.
...
current-state handoff results, and current-state export results emitted into
temporary roots.

The suite verifies one bounded downstream delivery over an explicit exported
...
EXPECTED_TOP_LEVEL_KEYS = {
    "delivery_metadata",
    "selected_current_state_export",
    "effective_delivery_inputs",
    "checks",
    "outcome",
    "block",
    "delivery_output",
...
class IntegrityHostV0MinCoexistenceCurrentStateDeliveryTests(unittest.TestCase):
    def emit_source_run(
        self,
        temp_root: Path,
    ...
    def valid_proposal(
        ...
        return {
            "transition_proposal_id": "proposal_accepted_for_current_state_delivery",
            ...
            "proposal_basis_ref": "bounded explicit current-state-delivery test basis",
    ...
    def resolve_delivery_default(self, temp_root: Path) -> dict[str, Any]:
        with mock.patch.object(delivery_resolver, "_repo_root", return_value=temp_root):
            return delivery_resolver.resolve_current_state_delivery()

    def resolve_delivery_object(
        self,
        temp_root: Path,
        current_state_export: dict[str, Any],
    ) -> dict[str, Any]:
        with mock.patch.object(delivery_resolver, "_repo_root", return_value=temp_root):
            return delivery_resolver.resolve_current_state_delivery(
    ...
    def resolve_delivery_from_path(
        self,
        temp_root: Path,
        export_path: Path,
    ) -> dict[str, Any]:
        with mock.patch.object(delivery_resolver, "_repo_root", return_value=temp_root):
            return delivery_resolver.resolve_current_state_delivery_from_path(
                export_path
            )
    ...
    def assert_result_shape(self, result: dict[str, Any]) -> None:
        ...
        self.assertIsInstance(result["delivery_metadata"], dict)
        self.assertIsInstance(result["selected_current_state_export"], dict)
        self.assertIsInstance(result["effective_delivery_inputs"], dict)
        self.assertIsInstance(result["checks"], list)
        self.assertIsInstance(result["block"], dict)
        self.assertIsInstance(result["delivery_summary"], dict)
    ...
    def assert_delivered_result(self, result: dict[str, Any]) -> None:
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], delivery_resolver.OUTCOME_DELIVERED)
        self.assertIsNone(result["block"]["block_code"])
        self.assertIsNone(result["block"]["block_reason"])
        self.assertIsInstance(result["delivery_output"], dict)
        self.assertEqual(set(result["effective_delivery_inputs"]), DELIVERY_INPUT_KEYS)
        for value in result["effective_delivery_inputs"].values():
    ...
    def assert_blocked_result(
        ...
        *,
        ...
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], delivery_resolver.OUTCOME_BLOCKED)
        ...
        self.assertIsNone(result["delivery_output"])
        if expect_non_claims_false:
            self.assert_non_claims_false(result)

    def assert_delivery_inputs_match_export(
        self,
        temp_root: Path,
        result: dict[str, Any],
        export: dict[str, Any],
    ) -> None:
        for key in DELIVERY_INPUT_KEYS:
            self.assert_paths_same(
                temp_root,
                result["effective_delivery_inputs"][key],
                export["effective_export_inputs"][key],
            )

    def assert_under_delivery_root(self, temp_root: Path, path: Path) -> None:
        root = (temp_root / delivery_resolver.CURRENT_STATE_DELIVERY_ROOT).resolve()
        try:
            path.resolve().relative_to(root)
        except ValueError as exc:
            self.fail(f"{path} was not written under {root}: {exc}")

    def parse_effective_delivery_files(
        self,
        temp_root: Path,
        result: dict[str, Any],
    ...
    def test_real_delivered_current_state_delivery_default_selection(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_delivery_default(temp_root)

            self.assert_delivered_result(result)
            selected = result["selected_current_state_export"]
            self.assert_paths_same(
                temp_root,
                selected["current_state_export_result_path"],
                stack["export_path"],
            )
            self.assertEqual(
                selected["current_state_export_result_id"],
                stack["export_result"]["export_metadata"]["export_result_id"],
            )
            self.assertEqual(selected["outcome"], export_resolver.OUTCOME_EXPORTED)
            self.assertEqual(
                selected["selected_current_state_handoff_id"],
                stack["handoff_result"]["handoff_metadata"]["handoff_result_id"],
            )
            self.assert_delivery_inputs_match_export(
                temp_root,
                result,
                stack["export_result"],
            )
            metadata = result["delivery_metadata"]
            for key in (
                "delivery_result_id",
    ...
    def test_resolution_from_path_and_object_are_consistent(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            object_result = self.resolve_delivery_object(
                temp_root,
                stack["export_result"],
            )
            path_result = self.resolve_delivery_from_path(
                temp_root,
                stack["export_path"],
            )

            self.assert_delivered_result(object_result)
            self.assert_delivered_result(path_result)
            self.assertIsNone(
                object_result["selected_current_state_export"][
                    "current_state_export_result_path"
                ]
            )
            self.assert_paths_same(
                temp_root,
                path_result["selected_current_state_export"][
                    "current_state_export_result_path"
                ],
                stack["export_path"],
            )
            ...
            self.assertEqual(
                object_result["effective_delivery_inputs"],
                path_result["effective_delivery_inputs"],
            )
            self.assertEqual(
                object_result["delivery_output"],
                path_result["delivery_output"],
            )
            self.assertEqual(
                object_result["delivery_summary"],
                path_result["delivery_summary"],
            )

    def test_effective_inputs_checks_delivery_output_and_summary(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_delivery_object(
                temp_root,
                stack["export_result"],
            )
            ...
            self.assertEqual(set(result["delivery_output"]), DELIVERY_OUTPUT_KEYS)
            output = result["delivery_output"]
            export_output = stack["export_result"]["export_output"]
            self.assert_paths_same(
                temp_root,
                output["current_governing_source_run_path"],
                export_output["current_governing_source_run_path"],
            )
            self.assert_paths_same(
                temp_root,
                output["current_governing_ingress_run_path"],
                export_output["current_governing_ingress_run_path"],
            )
            for output_key, input_key in (
                ...
                self.assert_paths_same(
                    temp_root,
                    output[output_key],
                    result["effective_delivery_inputs"][input_key],
                )
            ...
            self.assertEqual(
                ...
                export_output["readout_basis"],
            )
            self.assertEqual(
                output["handoff_basis"],
                export_output["handoff_basis"],
            )
            self.assertEqual(
                output["export_basis"],
                export_output["export_basis"],
            )
            self.assertEqual(output["delivery_basis"], "exported_current_state_result")

            summary = delivery_resolver.build_current_state_delivery_summary(result)
            for key in (
                "delivery_result_id",
                "outcome",
                "block_code",
            ...
            self.assertEqual(summary["outcome"], delivery_resolver.OUTCOME_DELIVERED)
            self.assertIsNone(summary["block_code"])
            self.assertIsNone(summary["block_reason"])
            self.assertEqual(
                summary["selected_current_state_export_id"],
                stack["export_result"]["export_metadata"]["export_result_id"],
            )
    ...
    def test_delivered_semantics_preserve_prior_family(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            ...
            for effective_key, prior_key in (
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        result["effective_delivery_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
            ...
            parsed = self.parse_effective_delivery_files(temp_root, result)
            ...
            self.assertEqual(
                result["delivery_output"]["delivery_basis"],
                "exported_current_state_result",
            )
            self.assertEqual(
                result["delivery_output"]["export_basis"],
                "handed_off_current_state_result",
            )
            self.assertEqual(
                result["delivery_output"]["handoff_basis"],
                "emitted_current_state_readout",
            )
            self.assertEqual(
                result["delivery_output"]["readout_basis"],
                "completed_current_work_operation",
            )

    def test_no_current_state_export_result_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()

            result = self.resolve_delivery_default(temp_root)

            self.assert_blocked_result(result, "NO_CURRENT_STATE_EXPORT_RESULT")
            selected = result["selected_current_state_export"]
            self.assertIsNone(selected["current_state_export_result_id"])
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            stack = self.build_multi_run_stack(
                temp_root,
                write_export_artifact=False,
            )
            blocked_export = self.copy_payload(stack["export_result"])
            blocked_export["outcome"] = export_resolver.OUTCOME_BLOCKED
            blocked_export["block"] = {
                "block_code": "TEST_BLOCKED_EXPORT",
                "block_reason": "Blocked current-state export fixture.",
            }
            blocked_export["export_output"] = None
            root = temp_root / export_resolver.CURRENT_STATE_EXPORT_ROOT
            self.write_json(root / "blocked_current_state_export.json", blocked_export)

            result = self.resolve_delivery_default(temp_root)

            self.assert_blocked_result(result, "NO_CURRENT_STATE_EXPORT_RESULT")

    def test_current_state_export_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            missing_path = temp_root / "missing_current_state_export.json"

            result = self.resolve_delivery_from_path(temp_root, missing_path)

            self.assert_blocked_result(result, "CURRENT_STATE_EXPORT_UNREADABLE")

        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            malformed_path = temp_root / "malformed_current_state_export.json"
            malformed_path.write_text("[1, 2, 3]\n", encoding="utf-8")

            with self.assertRaises(delivery_resolver.CurrentStateDeliveryError):
                self.resolve_delivery_from_path(temp_root, malformed_path)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = self.resolve_delivery_object(temp_root, blocked_export)

            self.assert_blocked_result(result, "CURRENT_STATE_EXPORT_NOT_EXPORTED")

    def test_effective_delivery_artifact_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(
                temp_root,
            ...
            exported = self.copy_payload(stack["export_result"])
            missing_path = self.resolve_display_path(
                temp_root,
                exported["effective_export_inputs"]["effective_status_packet_path"],
            )
            missing_path.unlink()

            result = self.resolve_delivery_object(temp_root, exported)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            malformed_path = self.resolve_display_path(
                temp_root,
                exported["effective_export_inputs"][
                    "effective_authority_artifact_path"
                ],
            )
            ...
            with self.assertRaises(delivery_resolver.CurrentStateDeliveryError):
                self.resolve_delivery_object(temp_root, exported)
    ...
    def test_canonical_execution_line_mismatch_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            family_path = self.resolve_display_path(
                temp_root,
                exported["effective_export_inputs"]["effective_family_packet_path"],
            )
    ...
    def test_stale_prior_family_fallback_is_not_used(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            delivered = self.resolve_delivery_object(
                temp_root,
                stack["export_result"],
            )
            self.assert_delivered_result(delivered)
            for effective_key, prior_key in (
                ("effective_authority_artifact_path", "prior_authority_artifact_path"),
                ("effective_family_packet_path", "prior_family_packet_path"),
                ("effective_status_packet_path", "prior_status_packet_path"),
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        delivered["effective_delivery_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
                        stack["effective_result"]["prior_current_family"][prior_key],
            ...
            stale = self.copy_payload(stack["export_result"])
            stale["effective_export_inputs"].update(
                {
            ...
            blocked = self.resolve_delivery_object(temp_root, stale)
    ...
    def test_non_claim_shortcuts_block_delivery(self) -> None:
        cases = (
    ...
    def test_multiple_current_state_export_result_conflict_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            conflict = self.copy_payload(stack["export_result"])
            conflict["export_metadata"]["export_result_id"] = (
                "conflicting_current_state_export"
            )
            conflict_path = (
                temp_root
                / export_resolver.CURRENT_STATE_EXPORT_ROOT
                / "conflicting_current_state_export.json"
            )
    ...
    def test_hard_malformed_artifact_failures(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            root_file = temp_root / export_resolver.CURRENT_STATE_EXPORT_ROOT
            root_file.parent.mkdir(parents=True, exist_ok=True)
            root_file.write_text("not a directory\n", encoding="utf-8")

            with self.assertRaises(delivery_resolver.CurrentStateDeliveryError):
                self.resolve_delivery_default(temp_root)

        malformed_export_cases = (
            ("non_mapping_input", "not a mapping"),
            (
                "missing_effective_export_inputs",
                {"outcome": export_resolver.OUTCOME_EXPORTED},
            ),
        )
        for label, payload in malformed_export_cases:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    with self.assertRaises(delivery_resolver.CurrentStateDeliveryError):
                        with mock.patch.object(
                            ...
                            delivery_resolver.resolve_current_state_delivery(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            exported = self.copy_payload(stack["export_result"])
            exported["effective_export_inputs"]["effective_family_packet_path"] = 42
    ...
    def test_write_behavior_and_default_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_path = temp_root / "nested" / "current_state_delivery" / "result.json"

            with mock.patch.object(delivery_resolver, "_repo_root", return_value=temp_root):
                written = delivery_resolver.write_current_state_delivery_result(
                    result,
                    output_path,
                )
            ...
            with mock.patch.object(delivery_resolver, "_repo_root", return_value=temp_root):
                with self.assertRaises(FileExistsError):
                    delivery_resolver.write_current_state_delivery_result(
                        result,
                        output_path,
                    )

            full_delivery_id = result["delivery_metadata"]["delivery_result_id"]
            with mock.patch.object(delivery_resolver, "_repo_root", return_value=temp_root):
                default_first = delivery_resolver.write_current_state_delivery_result(
                    result
                )
                default_second = delivery_resolver.write_current_state_delivery_result(
                    result
                )
            ...
            self.assert_under_delivery_root(temp_root, default_first)
            self.assert_under_delivery_root(temp_root, default_second)
            self.assertEqual(default_first.name, "current_state_delivery_result.json")
            self.assertEqual(default_second.name, "current_state_delivery_result_001.json")
            self.assertLess(len(default_first.name), 255)
            self.assertLess(len(default_second.name), 255)
            self.assertNotIn(full_delivery_id, default_first.name)
            self.assertNotIn(full_delivery_id, default_second.name)

            parsed_default = self.read_json(default_first)
            self.assertEqual(set(parsed_default), EXPECTED_TOP_LEVEL_KEYS)
            self.assertEqual(
                parsed_default["delivery_metadata"]["delivery_result_id"],
                full_delivery_id,
            )
            self.assertEqual(
                parsed_default["delivery_metadata"]["delivery_result_version"],
                delivery_resolver.DELIVERY_RESULT_VERSION,
            )
    ...
    def test_non_mutation_posture(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = self.resolve_delivery_object(
                temp_root,
                stack["export_result"],
            )
            after_first = {
            ...
            second = self.resolve_delivery_from_path(
                temp_root,
                stack["export_path"],
            )
            after_second = {
            ...
            self.assertEqual(
                first["selected_current_state_export"][
                    "current_state_export_result_id"
                ],
                second["selected_current_state_export"][
                    "current_state_export_result_id"
                ],
            )
            self.assertEqual(
                first["effective_delivery_inputs"],
                second["effective_delivery_inputs"],
            )
            self.assertEqual(first["delivery_output"], second["delivery_output"])
            self.assertEqual(first["delivery_summary"], second["delivery_summary"])
