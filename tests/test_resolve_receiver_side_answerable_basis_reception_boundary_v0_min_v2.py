...
import resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2 as resolver
import resolve_receiver_side_answerable_basis_reception_boundary_v0_min as v1_resolver


class ReceiverSideAnswerableBasisReceptionBoundaryV0MinV2Tests(unittest.TestCase):
    ...
    def build_valid_synthetic_request(self, base: Path) -> tuple[dict[str, Any], dict[str, Path]]:
        ...
        request = resolver.build_receiver_side_answerable_basis_reception_boundary_v0_min_v2_request(
    ...
    def failed_check_count(self, result: dict[str, Any]) -> int:
        ...
        if isinstance(value, int):
            return value
        return self.summary(result)["failed_check_count"]

    def passed_check_count(self, result: dict[str, Any]) -> int:
        value = result.get("passed_check_count")
        if isinstance(value, int):
            return value
        return self.summary(result)["passed_check_count"]
    ...
    def assert_marker_classes_true(self, result: dict[str, Any]) -> None:
        ...
        for marker_classes in observed:
            self.assertTrue(all(marker_classes.values()), marker_classes)
    ...
    def test_01_public_api_constants_and_default_request(self) -> None:
        for name in (
            "resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2",
            "resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2_from_path",
            "write_receiver_side_answerable_basis_reception_boundary_v0_min_v2_result",
            "build_receiver_side_answerable_basis_reception_boundary_v0_min_v2_summary",
            "build_receiver_side_answerable_basis_reception_boundary_v0_min_v2_request",
            "build_declared_receiver_side_answerable_basis_reception_boundary_v0_min_v2_request",
        ):
            self.assertTrue(callable(getattr(resolver, name)))
        self.assertEqual(resolver.RESULT_VERSION, "0.2.0")
        self.assertEqual(resolver.RESOLVER_MODULE, "resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2")
        ...
        self.assertTrue(
            str(resolver.OUTPUT_ROOT).endswith(
                "artifacts/integrity_host_v0_min_coexistence_receiver_side_answerable_basis_reception_boundary_v0_min_v2"
            )
        )
        for code in (
            "REQUEST_NOT_MAPPING",
        ...
        request = resolver.build_receiver_side_answerable_basis_reception_boundary_v0_min_v2_request()
        self.assertIs(
            resolver.build_receiver_side_answerable_basis_reception_boundary_v0_min_request,
            resolver.build_receiver_side_answerable_basis_reception_boundary_v0_min_v2_request,
        )
        self.assertIs(
            resolver.build_declared_receiver_side_answerable_basis_reception_boundary_v0_min_request,
            resolver.build_receiver_side_answerable_basis_reception_boundary_v0_min_v2_request,
        )
        self.assertEqual(
        ...
        self.assertTrue(set(v1_resolver.PROHIBITED_REQUEST_FLAGS).issubset(resolver.PROHIBITED_REQUEST_FLAGS))
        for key, value in v1_resolver.PROHIBITED_REQUEST_FLAGS.items():
            self.assertEqual(resolver.PROHIBITED_REQUEST_FLAGS[key], value, key)
        self.assertEqual(
            set(resolver.PROHIBITED_REQUEST_FLAGS) - set(v1_resolver.PROHIBITED_REQUEST_FLAGS),
            {"request_receiver_answerable_receipt_creation"},
        )
        self.assertIs(request["request_receiver_answerable_receipt_creation"], False)
        for key in resolver.REQUIRED_FALSE_NON_CLAIMS:
    ...
    def test_02_synthetic_allowed_result_and_wrapper_separation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2(request)
    ...
    def test_06_do_not_record_and_explicit_block_intents(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            not_recorded = resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2(not_recorded_request)
            ...
            blocked = resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2(blocked_request)
    ...
    def test_07_request_shape_and_exact_value_blocking(self) -> None:
        self.assert_blocked_with_public_code(
            resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2(["not", "a", "mapping"])
        )
        with tempfile.TemporaryDirectory() as temporary:
            request, _ = self.build_valid_synthetic_request(Path(temporary))
            cases: dict[str, Any] = {
            ...
            for name, value in cases.items():
                with self.subTest(name=name):
                    ...
                    if name == "unsupported_intent":
                        ...
                        altered[name] = value
                    result = resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2(altered)
                    self.assert_blocked_with_public_code(result)
                    self.assert_final_false_posture(result)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for flag in expected_flags:
                with self.subTest(flag=flag):
                    ...
                    self.assert_blocked_with_public_code(result)
                    self.assert_final_false_posture(result)
                    if flag == "request_receiver_answerable_receipt_creation":
                        self.assertEqual(
                            self.block_code(result),
                            "PROHIBITED_RECEIVER_ANSWERABLE_RECEIPT_REQUESTED",
                        )
                        self.assertIs(
                            self.boundary(result)["receiver_answerable_receipt_present"], False
                        )
                        self.assertIs(self.boundary(result)["receiver_attestation_created"], False)
                    if flag == "request_receiver_attestation_creation":
                        self.assertEqual(
                            self.block_code(result), "PROHIBITED_RECEIVER_ATTESTATION_REQUESTED"
                        )
    ...
    def test_11_path_write_and_non_mutation_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            v1_resolver_path = SRC_ROOT / "resolve_receiver_side_answerable_basis_reception_boundary_v0_min.py"
            v1_test_path = REPO_ROOT / "tests" / "test_resolve_receiver_side_answerable_basis_reception_boundary_v0_min.py"
            v1_resolver_bytes = v1_resolver_path.read_bytes()
            v1_test_bytes = v1_test_path.read_bytes()
            request_path = base / "request" / self.safe_json_filename("valid request")
            request_path.parent.mkdir(parents=True, exist_ok=True)
            request_path.write_text(json.dumps(request), encoding="utf-8")
            result = resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2_from_path(request_path)
            self.assert_allowed_boundary(result)
            self.assertEqual(request, original_request)
            self.assertEqual({key: path.read_text(encoding="utf-8") for key, path in paths.items()}, original_texts)
            self.assertEqual(v1_resolver_path.read_bytes(), v1_resolver_bytes)
            self.assertEqual(v1_test_path.read_bytes(), v1_test_bytes)
            malformed = base / "request" / self.safe_json_filename("malformed")
            malformed.write_text("{", encoding="utf-8")
            self.assert_blocked_with_public_code(
                resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2_from_path(malformed)
            )
            array_path = base / "request" / self.safe_json_filename("array")
            array_path.write_text("[]", encoding="utf-8")
            self.assert_blocked_with_public_code(
                resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2_from_path(array_path)
            )
            self.assert_blocked_with_public_code(
                resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2_from_path(
                    base / "request" / self.safe_json_filename("missing")
                )
            )
            output = resolver.write_receiver_side_answerable_basis_reception_boundary_v0_min_v2_result(
                result, base / "output" / resolver.OUTPUT_FILENAME
            )
            output_second = resolver.write_receiver_side_answerable_basis_reception_boundary_v0_min_v2_result(
                result, base / "output" / resolver.OUTPUT_FILENAME
            )
            self.assertTrue(output.exists())
            self.assertTrue(output_second.exists())
            self.assertNotEqual(output, output_second)
            self.assertIn("receiver_side_answerable_basis_reception_boundary_v0_min_v2_result", output.name)
            ...
            self.assertNotEqual(
                str(resolver.OUTPUT_ROOT),
                str(REPO_ROOT / "artifacts" / "integrity_host_v0_min_coexistence_receiver_side_answerable_basis_reception_boundary_v0_min"),
            )
    ...
    def test_12_summary_and_direct_smoke_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = resolver.resolve_receiver_side_answerable_basis_reception_boundary_v0_min_v2(request)
        summary = resolver.build_receiver_side_answerable_basis_reception_boundary_v0_min_v2_summary(result)
