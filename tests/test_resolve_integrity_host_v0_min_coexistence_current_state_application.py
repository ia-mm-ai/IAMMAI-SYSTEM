"""Bounded tests for the v0-min current-state application resolver.
...
current-state handoff results, current-state export results, and current-state
delivery results emitted into temporary roots.

The suite verifies one bounded downstream application over an explicit delivered
...
EXPECTED_TOP_LEVEL_KEYS = {
    ...
    "selected_current_state_delivery",
    "effective_application_inputs",
    "checks",
    "outcome",
    "block",
    "application_output",
...
class IntegrityHostV0MinCoexistenceCurrentStateApplicationTests(unittest.TestCase):
    def emit_source_run(
        self,
        temp_root: Path,
    ...
    def build_multi_run_stack(
        ...
        *,
        ...
        write_export_artifact: bool = True,
        write_delivery_artifact: bool = True,
    ) -> dict[str, Any]:
        ...
        export_path, export_result = self.emit_export_result_from_object(
            temp_root,
            handoff_result,
            write_artifact=write_export_artifact,
        )
        ...
        delivery_path, delivery_result = self.emit_delivery_result_from_object(
            temp_root,
            export_result,
            write_artifact=write_delivery_artifact,
        )
    ...
    def valid_proposal(
        ...
        return {
            "transition_proposal_id": "proposal_accepted_for_current_state_application",
            ...
            "proposal_basis_ref": "bounded explicit current-state-application test basis",
    ...
    def resolve_application_default(self, temp_root: Path) -> dict[str, Any]:
        with mock.patch.object(application_resolver, "_repo_root", return_value=temp_root):
            return application_resolver.resolve_current_state_application()

    def resolve_application_object(
        self,
        temp_root: Path,
        current_state_delivery: dict[str, Any],
    ) -> dict[str, Any]:
        with mock.patch.object(application_resolver, "_repo_root", return_value=temp_root):
            return application_resolver.resolve_current_state_application(
    ...
    def resolve_application_from_path(
        self,
        temp_root: Path,
        delivery_path: Path,
    ) -> dict[str, Any]:
        with mock.patch.object(application_resolver, "_repo_root", return_value=temp_root):
            return application_resolver.resolve_current_state_application_from_path(
                delivery_path
            )
    ...
    def assert_result_shape(self, result: dict[str, Any]) -> None:
        ...
        self.assertIsInstance(result["application_metadata"], dict)
        self.assertIsInstance(result["selected_current_state_delivery"], dict)
        self.assertIsInstance(result["effective_application_inputs"], dict)
        self.assertIsInstance(result["checks"], list)
        self.assertIsInstance(result["block"], dict)
        self.assertIsInstance(result["application_summary"], dict)
    ...
    def assert_applied_result(self, result: dict[str, Any]) -> None:
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], application_resolver.OUTCOME_APPLIED)
        self.assertIsNone(result["block"]["block_code"])
        self.assertIsNone(result["block"]["block_reason"])
        self.assertIsInstance(result["application_output"], dict)
        self.assertEqual(set(result["effective_application_inputs"]), APPLICATION_INPUT_KEYS)
        for value in result["effective_application_inputs"].values():
    ...
    def assert_blocked_result(
        ...
        *,
        ...
        self.assert_result_shape(result)
        self.assertEqual(result["outcome"], application_resolver.OUTCOME_BLOCKED)
        ...
        self.assertIsNone(result["application_output"])
        if expect_non_claims_false:
            self.assert_non_claims_false(result)

    def assert_application_inputs_match_delivery(
        self,
        temp_root: Path,
        result: dict[str, Any],
        delivery: dict[str, Any],
    ) -> None:
        for key in APPLICATION_INPUT_KEYS:
            self.assert_paths_same(
                temp_root,
                result["effective_application_inputs"][key],
                delivery["effective_delivery_inputs"][key],
            )

    def assert_under_application_root(self, temp_root: Path, path: Path) -> None:
        root = (
            temp_root / application_resolver.CURRENT_STATE_APPLICATION_ROOT
        ).resolve()
        try:
    ...
    def parse_effective_application_files(
        self,
        temp_root: Path,
        result: dict[str, Any],
    ...
    def test_real_applied_current_state_application_default_selection(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_application_default(temp_root)

            self.assert_applied_result(result)
            selected = result["selected_current_state_delivery"]
            self.assert_paths_same(
                temp_root,
                selected["current_state_delivery_result_path"],
                stack["delivery_path"],
            )
            self.assertEqual(
                selected["current_state_delivery_result_id"],
                stack["delivery_result"]["delivery_metadata"]["delivery_result_id"],
            )
            self.assertEqual(selected["outcome"], delivery_resolver.OUTCOME_DELIVERED)
            self.assertEqual(
                selected["selected_current_state_export_id"],
                stack["export_result"]["export_metadata"]["export_result_id"],
            )
            self.assert_application_inputs_match_delivery(
                temp_root,
                result,
                stack["delivery_result"],
            )
            metadata = result["application_metadata"]
            for key in (
                "application_result_id",
                "application_result_type",
                "application_result_version",
                "generated_at",
                "resolver_module",
            ):
    ...
    def test_resolution_from_path_and_object_are_consistent(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            object_result = self.resolve_application_object(
                temp_root,
                stack["delivery_result"],
            )
            path_result = self.resolve_application_from_path(
                temp_root,
                stack["delivery_path"],
            )

            self.assert_applied_result(object_result)
            self.assert_applied_result(path_result)
            self.assertIsNone(
                object_result["selected_current_state_delivery"][
                    "current_state_delivery_result_path"
                ]
            )
            self.assert_paths_same(
                temp_root,
                path_result["selected_current_state_delivery"][
                    "current_state_delivery_result_path"
                ],
                stack["delivery_path"],
            )
            ...
            self.assertEqual(
                object_result["effective_application_inputs"],
                path_result["effective_application_inputs"],
            )
            self.assertEqual(
                object_result["application_output"],
                path_result["application_output"],
            )
            self.assertEqual(
                object_result["application_summary"],
                path_result["application_summary"],
            )

    def test_effective_inputs_checks_application_output_and_summary(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)

            result = self.resolve_application_object(
                temp_root,
                stack["delivery_result"],
            )
            ...
            self.assertEqual(set(result["application_output"]), APPLICATION_OUTPUT_KEYS)
            output = result["application_output"]
            delivery_output = stack["delivery_result"]["delivery_output"]
            self.assert_paths_same(
                temp_root,
                output["current_governing_source_run_path"],
                delivery_output["current_governing_source_run_path"],
            )
            self.assert_paths_same(
                temp_root,
                output["current_governing_ingress_run_path"],
                delivery_output["current_governing_ingress_run_path"],
            )
            for output_key, input_key in (
                ...
                self.assert_paths_same(
                    temp_root,
                    output[output_key],
                    result["effective_application_inputs"][input_key],
                )
            ...
            self.assertEqual(
                ...
                delivery_output["readout_basis"],
            )
            self.assertEqual(
                output["handoff_basis"],
                delivery_output["handoff_basis"],
            )
            self.assertEqual(
                output["export_basis"],
                delivery_output["export_basis"],
            )
            self.assertEqual(
                output["delivery_basis"],
                delivery_output["delivery_basis"],
            )
            self.assertEqual(output["application_basis"], "delivered_current_state_result")

            summary = application_resolver.build_current_state_application_summary(
                result
            )
            for key in (
                "application_result_id",
            ...
            self.assertEqual(summary["outcome"], application_resolver.OUTCOME_APPLIED)
            self.assertIsNone(summary["block_code"])
            self.assertIsNone(summary["block_reason"])
            self.assertEqual(
                summary["selected_current_state_delivery_id"],
                stack["delivery_result"]["delivery_metadata"]["delivery_result_id"],
            )
    ...
    def test_applied_semantics_preserve_prior_family(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            ...
            for effective_key, prior_key in (
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        result["effective_application_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
            ...
            parsed = self.parse_effective_application_files(temp_root, result)
            ...
            self.assertEqual(
                result["application_output"]["application_basis"],
                "delivered_current_state_result",
            )
            self.assertEqual(
                result["application_output"]["delivery_basis"],
                "exported_current_state_result",
            )
            self.assertEqual(
                result["application_output"]["export_basis"],
                "handed_off_current_state_result",
            )
            self.assertEqual(
                result["application_output"]["handoff_basis"],
                "emitted_current_state_readout",
            )
            self.assertEqual(
                result["application_output"]["readout_basis"],
                "completed_current_work_operation",
            )

    def test_no_current_state_delivery_result_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()

            result = self.resolve_application_default(temp_root)

            self.assert_blocked_result(result, "NO_CURRENT_STATE_DELIVERY_RESULT")
            selected = result["selected_current_state_delivery"]
            self.assertIsNone(selected["current_state_delivery_result_id"])
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            stack = self.build_multi_run_stack(
                temp_root,
                write_delivery_artifact=False,
            )
            blocked_delivery = self.copy_payload(stack["delivery_result"])
            blocked_delivery["outcome"] = delivery_resolver.OUTCOME_BLOCKED
            blocked_delivery["block"] = {
                "block_code": "TEST_BLOCKED_DELIVERY",
                "block_reason": "Blocked current-state delivery fixture.",
            }
            blocked_delivery["delivery_output"] = None
            root = temp_root / delivery_resolver.CURRENT_STATE_DELIVERY_ROOT
            self.write_json(
                root / "blocked_current_state_delivery.json",
                blocked_delivery,
            )
    ...
    def test_current_state_delivery_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            missing_path = temp_root / "missing_current_state_delivery.json"

            result = self.resolve_application_from_path(temp_root, missing_path)

            self.assert_blocked_result(result, "CURRENT_STATE_DELIVERY_UNREADABLE")

        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            malformed_path = temp_root / "malformed_current_state_delivery.json"
            malformed_path.write_text("[1, 2, 3]\n", encoding="utf-8")

            with self.assertRaises(application_resolver.CurrentStateApplicationError):
                self.resolve_application_from_path(temp_root, malformed_path)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = self.resolve_application_object(temp_root, blocked_delivery)

            self.assert_blocked_result(result, "CURRENT_STATE_DELIVERY_NOT_DELIVERED")

    def test_effective_application_artifact_unreadable_or_malformed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(
                temp_root,
            ...
            delivered = self.copy_payload(stack["delivery_result"])
            missing_path = self.resolve_display_path(
                temp_root,
                delivered["effective_delivery_inputs"]["effective_status_packet_path"],
            )
            missing_path.unlink()

            result = self.resolve_application_object(temp_root, delivered)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            malformed_path = self.resolve_display_path(
                temp_root,
                delivered["effective_delivery_inputs"][
                    "effective_authority_artifact_path"
                ],
            )
            ...
            with self.assertRaises(application_resolver.CurrentStateApplicationError):
                self.resolve_application_object(temp_root, delivered)
    ...
    def test_canonical_execution_line_mismatch_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            family_path = self.resolve_display_path(
                temp_root,
                delivered["effective_delivery_inputs"]["effective_family_packet_path"],
            )
    ...
    def test_stale_prior_family_fallback_is_not_used(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            applied = self.resolve_application_object(
                temp_root,
                stack["delivery_result"],
            )
            self.assert_applied_result(applied)
            for effective_key, prior_key in (
                ("effective_authority_artifact_path", "prior_authority_artifact_path"),
                ("effective_family_packet_path", "prior_family_packet_path"),
                ("effective_status_packet_path", "prior_status_packet_path"),
                ...
                self.assertNotEqual(
                    self.resolve_display_path(
                        temp_root,
                        applied["effective_application_inputs"][effective_key],
                    ),
                    self.resolve_display_path(
                        temp_root,
                        stack["effective_result"]["prior_current_family"][prior_key],
            ...
            stale = self.copy_payload(stack["delivery_result"])
            stale["effective_delivery_inputs"].update(
                {
            ...
            blocked = self.resolve_application_object(temp_root, stale)
    ...
    def test_non_claim_shortcuts_block_application(self) -> None:
        cases = (
    ...
    def test_multiple_current_state_delivery_result_conflict_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            stack = self.build_multi_run_stack(temp_root)
            conflict = self.copy_payload(stack["delivery_result"])
            conflict["delivery_metadata"]["delivery_result_id"] = (
                "conflicting_current_state_delivery"
            )
            conflict_path = (
                temp_root
                / delivery_resolver.CURRENT_STATE_DELIVERY_ROOT
                / "conflicting_current_state_delivery.json"
            )
    ...
    def test_hard_malformed_artifact_failures(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir).resolve()
            root_file = temp_root / delivery_resolver.CURRENT_STATE_DELIVERY_ROOT
            root_file.parent.mkdir(parents=True, exist_ok=True)
            root_file.write_text("not a directory\n", encoding="utf-8")

            with self.assertRaises(application_resolver.CurrentStateApplicationError):
                self.resolve_application_default(temp_root)

        malformed_delivery_cases = (
            ("non_mapping_input", "not a mapping"),
            (
                "missing_effective_delivery_inputs",
                {"outcome": delivery_resolver.OUTCOME_DELIVERED},
            ),
        )
        for label, payload in malformed_delivery_cases:
            with self.subTest(label=label):
                with tempfile.TemporaryDirectory() as temp_dir:
                    temp_root = Path(temp_dir).resolve()
                    with self.assertRaises(
                        application_resolver.CurrentStateApplicationError
                    ):
                        with mock.patch.object(
                            ...
                            application_resolver.resolve_current_state_application(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            delivered = self.copy_payload(stack["delivery_result"])
            delivered["effective_delivery_inputs"]["effective_family_packet_path"] = 42
    ...
    def test_write_behavior_and_default_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_path = (
                temp_root / "nested" / "current_state_application" / "result.json"
            )
            ...
            with mock.patch.object(
                ...
                written = application_resolver.write_current_state_application_result(
                    result,
                    output_path,
                )
            ...
            with mock.patch.object(
                ...
                with self.assertRaises(FileExistsError):
                    application_resolver.write_current_state_application_result(
                        result,
                        output_path,
                    )

            full_application_id = result["application_metadata"][
                "application_result_id"
            ]
            with mock.patch.object(
                application_resolver,
            ...
            self.assert_under_application_root(temp_root, default_first)
            self.assert_under_application_root(temp_root, default_second)
            self.assertEqual(default_first.name, "current_state_application_result.json")
            self.assertEqual(
                default_second.name,
                "current_state_application_result_001.json",
            )
            self.assertLess(len(default_first.name), 255)
            self.assertLess(len(default_second.name), 255)
            self.assertNotIn(full_application_id, default_first.name)
            self.assertNotIn(full_application_id, default_second.name)

            parsed_default = self.read_json(default_first)
            self.assertEqual(set(parsed_default), EXPECTED_TOP_LEVEL_KEYS)
            self.assertEqual(
                parsed_default["application_metadata"]["application_result_id"],
                full_application_id,
            )
            self.assertEqual(
                parsed_default["application_metadata"]["application_result_version"],
                application_resolver.APPLICATION_RESULT_VERSION,
            )
    ...
    def test_non_mutation_posture(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = self.resolve_application_object(
                temp_root,
                stack["delivery_result"],
            )
            after_first = {
            ...
            second = self.resolve_application_from_path(
                temp_root,
                stack["delivery_path"],
            )
            after_second = {
            ...
            self.assertEqual(
                first["selected_current_state_delivery"][
                    "current_state_delivery_result_id"
                ],
                second["selected_current_state_delivery"][
                    "current_state_delivery_result_id"
                ],
            )
            self.assertEqual(
                first["effective_application_inputs"],
                second["effective_application_inputs"],
            )
            self.assertEqual(first["application_output"], second["application_output"])
            self.assertEqual(first["application_summary"], second["application_summary"])
