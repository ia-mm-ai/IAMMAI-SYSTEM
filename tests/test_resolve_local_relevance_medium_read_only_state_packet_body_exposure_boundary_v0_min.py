"""Tests for the selected-state packet body exposure boundary resolver.

This suite is bounded to one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_STATE_PACKET_BODY_EXPOSURE_BOUNDARY object. It
verifies that the resolver reads one clean selected-state result object
artifact, records one local read-only selected-state packet-body-exposure
consideration boundary, and keeps state packet body exposure, raw/full packet
...
import resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min as resolver  # noqa: E402
...
REQUEST_ONLY_FALSE_FIELDS = tuple(
    ...
    if key not in BOUNDARY_OBJECT_FALSE_FIELDS
)
...
class LocalRelevanceMediumReadOnlyStatePacketBodyExposureBoundaryTests(
    unittest.TestCase
):
    def make_bundle(self, temp_dir: str) -> dict[str, Any]:
        ...
        request = (
            resolver.build_declared_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_request(
    ...
    def resolve_bundle(
        ...
        return resolver.resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min(
            bundle["request"]
        )
    ...
    def recorded_result(self) -> tuple[dict[str, Any], Path]:
        ...
        _write_json(bundle["artifact_path"], bundle["artifact"])
        result = resolver.resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min(
            bundle["request"]
        )
        return result, bundle["artifact_path"]

    def summary(self, result: Mapping[str, Any]) -> dict[str, Any]:
        return resolver.build_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_summary(
            result
        )
    ...
    def test_public_api_constants_and_default_builder(self) -> None:
        for name in (
            ...
            "resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_from_path",
            "write_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_result",
            "build_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_summary",
            "build_declared_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_request",
        ):
        ...
        self.assertFalse(
            any(_path_has_component_prefix(resolver.OUTPUT_ROOT, root) for root in FORBIDDEN_OUTPUT_ROOTS)
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for key in resolver.REQUIRED_FALSE_NON_CLAIMS:
                with self.subTest(non_claim=key):
                    ...
                    result = (
                        resolver.resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min(
                            request
                        )
                    )
    ...
    def test_representative_blocking_behavior(self) -> None:
        ...
        cases: list[tuple[str, Callable[[dict[str, Any]], None]]] = [
            ...
            (
                ...
                lambda bundle: _pop(bundle["request"], "selected_command"),
            ),
            (
            ...
            (
                ...
                lambda bundle: _pop(bundle["request"], "boundary_type"),
            ),
            (
                "boundary type wrong",
                lambda bundle: _set(bundle["request"], "boundary_type", "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_STATE_PACKET_BODY_EXPOSURE"),
            ),
            (
                "boundary scope missing",
                lambda bundle: _pop(bundle["request"], "boundary_scope"),
            ),
            (
            ...
            (
                ...
                lambda bundle: _set(bundle["request"], "predecessor_failure_hidden", True),
            ),
            (
        ...
        missing_result = resolver.resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min(
            None
        )
    ...
    def test_missing_incomplete_declared_non_claims_behavior(self) -> None:
        variants: list[tuple[str, Callable[[dict[str, Any]], None]]] = [
            (
                "remove declared_non_claims",
                lambda request: _pop(request, "declared_non_claims"),
            ),
            (
                "empty declared_non_claims",
                lambda request: _set(request, "declared_non_claims", {}),
            ),
            (
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = resolver.resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_from_path(
                request_path
            )
            summary = self.summary(result)
            self.assertEqual(result["outcome"], resolver.OUTCOME_RECORDED)
            ...
            malformed = (
                resolver.resolve_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_from_path(
                    malformed_path
                )
            )
            ...
            output_root = root / "local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min"
            with mock.patch.object(resolver, "OUTPUT_ROOT", output_root):
                output_path = (
                    resolver.write_local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min_result(
                        result
                    )
                )
                second_output_path = (
            ...
            self.assertIn(
                "local_relevance_medium_read_only_state_packet_body_exposure_boundary_v0_min",
                str(output_path),
            )
