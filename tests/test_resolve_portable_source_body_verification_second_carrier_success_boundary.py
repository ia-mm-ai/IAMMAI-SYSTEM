"""Executable checks for second-carrier success-boundary posture only.

This suite is downstream of the bounded second-carrier result line. It verifies
that the resolver records one future second-carrier success step only while
preserving that success, verification, external result, cross-carrier evidence,
...
continuation, reusable permission, and follow-on work are not created. It also
verifies that official enum, scope, and block-code strings are not redacted
...
class PortableSourceBodyVerificationSecondCarrierSuccessBoundaryTests(unittest.TestCase):
        ...
        for section_key in POSTURE_KEYS:
            ...
            for key, value in section.items():
                ...
                if isinstance(value, bool):
                    ...
                    self.assertNotIn(str(value), REDACTION_STRINGS)
    ...
    def assert_official_enum_strings_unredacted(self, result: dict[str, Any]) -> None:
        ...
        for value in scope:
            self.assertNotEqual(value, "[bounded-success-boundary-redacted]")
            self.assertNotIn("redacted", value.lower())
        for supported_value in SUPPORTED_SCOPE:
            self.assertNotIn("redacted", supported_value.lower())
    ...
    def test_successful_recorded_result_from_builder(self) -> None:
        ...
        self.assert_official_enum_strings_unredacted(result)
        self.assert_no_forbidden_creation(result)
    ...
    def test_official_enum_strings_are_not_redacted(self) -> None:
        ...
        for check in checks(result):
            for key in ("block_code", "failure_code"):
                code = check.get(key)
                if code is not None:
                    self.assertNotIn("redacted", code.lower())
