"""Executable checks for portable-verification-closure posture only.

This suite is downstream of the portable-verification-closure-boundary line. It
verifies that the resolver records one bounded line-level portable verification
closure posture only. Final completion, source transfer, source receipt,
reception authorization, source, authority, currentness, runtime, deployment,
public release, operation permission, continuation, reusable permission, and
...
import resolve_portable_source_body_verification_portable_verification_closure as resolver


RECORDED = resolver.OUTCOME_RECORDED
BLOCKED = resolver.OUTCOME_BLOCKED
OUTCOME_FAMILY = set(resolver.OUTCOME_FAMILY)
SUPPORTED_SCOPE = tuple(resolver.SUPPORTED_PORTABLE_VERIFICATION_CLOSURE_SCOPE)
...
REDACTION_PLACEHOLDERS = {
    "[bounded-portable-verification-closure-redacted]",
...
TOP_LEVEL_SECTIONS = (
    ...
    "portable_verification_closure_scope",
    "portable_verification_closure_checks",
    "portable_verification_closure_statement",
    "portable_verification_closure_non_meaning",
    ...
    "outcome",
    "block",
    "portable_source_body_verification_portable_verification_closure_summary",
)
...
def build_request(**overrides: Any) -> dict[str, Any]:
    return resolver.build_declared_portable_source_body_verification_portable_verification_closure_request(
        **overrides
    )


def resolved(request: Any) -> dict[str, Any]:
    return resolver.resolve_portable_source_body_verification_portable_verification_closure(
        request
    )


def statement(result: dict[str, Any]) -> dict[str, Any]:
    return result.get("portable_verification_closure_statement", {})
...
def checks(result: dict[str, Any]) -> list[dict[str, Any]]:
    return list(result.get("portable_verification_closure_checks", []))
...
def scope_values(result: dict[str, Any]) -> list[str]:
    scope = result.get("portable_verification_closure_scope", {})
...
class PortableVerificationClosureTest(unittest.TestCase):
    ...
    def assert_generated_booleans_are_bool(self, result: dict[str, Any]) -> None:
        for section in (
            statement(result),
            non_claims(result),
            result.get("portable_verification_closure_non_meaning", {}),
        ):
    ...
    def assert_path_blocks_or_raises(self, path: Path) -> None:
        try:
            result = resolver.resolve_portable_source_body_verification_portable_verification_closure_from_path(
                path
            )
        except resolver.PortableSourceBodyVerificationPortableVerificationClosureError:
            return
        self.assert_blocked_result(result)

    def test_public_api_and_constants(self) -> None:
        for name in (
            "resolve_portable_source_body_verification_portable_verification_closure",
            "resolve_portable_source_body_verification_portable_verification_closure_from_path",
            "write_portable_source_body_verification_portable_verification_closure_result",
            "build_portable_source_body_verification_portable_verification_closure_summary",
            "build_declared_portable_source_body_verification_portable_verification_closure_request",
        ):
        ...
        for name in (
            ...
            "SUPPORTED_PORTABLE_VERIFICATION_CLOSURE_SCOPE",
        ...
        self.assertEqual(
            resolver.SUPPORTED_PORTABLE_VERIFICATION_CLOSURE_SCOPE,
        ...
        for official in OFFICIAL_SCOPE_VALUES:
            self.assertIn(official, resolver.SUPPORTED_PORTABLE_VERIFICATION_CLOSURE_SCOPE)
    ...
    def test_successful_recorded_result_from_no_argument_builder(self) -> None:
        ...
        compact = resolver.build_portable_source_body_verification_portable_verification_closure_summary(
            result
        )
        ...
        self.assertEqual(
            compact["request_id"],
            request["portable_verification_closure_request_id"],
        )
    ...
    def test_official_enum_strings_are_preserved_without_redaction_placeholders(self) -> None:
        ...
        request = build_request(portable_verification_closure_scope=list(SUPPORTED_SCOPE))
    ...
    def test_raw_and_hidden_hostile_content_is_contained(self) -> None:
        ...
        self.assertIs(claims["string_zero_treated_as_portable_verification_closure_standalone"], False)
    ...
    def test_representative_blocking_behavior(self) -> None:
        ...
        cases: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ("explicit block intent", set_field("portable_verification_closure_intent", "BLOCK_PORTABLE_SOURCE_BODY_VERIFICATION_PORTABLE_VERIFICATION_CLOSURE")),
            ("undeclared question", set_field("portable_verification_closure_question", "")),
            ("unsupported intent", set_field("portable_verification_closure_intent", "UNSUPPORTED_INTENT")),
            ("unsupported scope", set_field("portable_verification_closure_scope", ["UNSUPPORTED_PORTABLE_VERIFICATION_CLOSURE_SCOPE_VALUE"])),
            ("missing portable verification closure boundary basis", clear_field("selected_portable_verification_closure_boundary_basis")),
            ("missing portable verification closure boundary terminal summary basis", clear_field("selected_portable_verification_closure_boundary_terminal_summary_basis")),
            ("boundary not recorded", set_field("selected_portable_verification_closure_boundary_result_outcome", "NOT_RECORDED")),
            ("boundary failed checks", set_field("selected_portable_verification_closure_boundary_failed_check_count", 1)),
            ("boundary version wrong", set_field("selected_portable_verification_closure_boundary_result_version", "9.9.9")),
            ("boundary did not declare future closure review", set_field("selected_portable_verification_closure_boundary_declared_future_closure_review_step", False)),
            ("boundary already created closure", set_field("selected_portable_verification_closure_boundary_already_created_portable_verification_closure", True)),
            ("boundary already created final completion", set_field("selected_portable_verification_closure_boundary_already_created_final_completion", True)),
            ("boundary treated boundary as closure", set_field("selected_portable_verification_closure_boundary_treated_boundary_as_portable_verification_closure", True)),
            ("boundary treated boundary as final completion", set_field("selected_portable_verification_closure_boundary_treated_boundary_as_final_completion", True)),
            ("boundary treated zero exit as closure", set_field("selected_portable_verification_closure_boundary_treated_zero_exit_code_as_portable_verification_closure", True)),
            ("boundary treated string zero as closure", set_field("selected_portable_verification_closure_boundary_treated_string_zero_as_portable_verification_closure", True)),
            ("boundary treated string zero as doctrine", set_field("selected_portable_verification_closure_boundary_treated_string_zero_as_doctrine", True)),
            ("boundary treated OK as closure", set_field("selected_portable_verification_closure_boundary_treated_ok_as_portable_verification_closure", True)),
            ("boundary treated Ran 7 as closure", set_field("selected_portable_verification_closure_boundary_treated_ran_7_tests_as_portable_verification_closure", True)),
            ("boundary treated returned capture as closure", set_field("selected_portable_verification_closure_boundary_treated_returned_capture_as_portable_verification_closure", True)),
            ("boundary redacted official strings", set_field("selected_portable_verification_closure_boundary_official_enum_scope_strings_redacted", True)),
            ("predecessor cross-carrier evidence failure missing", set_fields({"predecessor_cross_carrier_evidence_failure_preserved": False, "selected_cross_carrier_evidence_predecessor_failure_preserved": False, "selected_portable_verification_closure_boundary_predecessor_cross_carrier_evidence_failure_preserved": False})),
            ...
            ("returned capture treated as closure", set_non_claim("returned_capture_treated_as_portable_verification_closure_standalone")),
            ("zero exit code treated as closure", set_non_claim("zero_exit_code_treated_as_portable_verification_closure_standalone")),
            ("string zero treated as closure", set_non_claim("string_zero_treated_as_portable_verification_closure_standalone")),
            ("OK output treated as closure", set_non_claim("ok_output_treated_as_portable_verification_closure_standalone")),
            ("Ran 7 tests treated as closure", set_non_claim("ran_7_tests_treated_as_portable_verification_closure_standalone")),
            ...
            ("portable verification closure treated as final completion", set_non_claim("portable_verification_closure_treated_as_final_completion")),
            ("portable verification closure treated as source transfer", set_non_claim("portable_verification_closure_treated_as_source_transfer")),
            ("portable verification closure treated as source receipt", set_non_claim("portable_verification_closure_treated_as_source_receipt")),
            ("portable verification closure treated as reception authorization", set_non_claim("portable_verification_closure_treated_as_reception_authorization")),
            ("portable verification closure treated as source", set_non_claim("portable_verification_closure_treated_as_source")),
            ("portable verification closure treated as authority", set_non_claim("portable_verification_closure_treated_as_authority")),
            ("portable verification closure treated as currentness", set_non_claim("portable_verification_closure_treated_as_currentness")),
            ("portable verification closure treated as runtime", set_non_claim("portable_verification_closure_treated_as_runtime")),
            ("portable verification closure treated as deployment", set_non_claim("portable_verification_closure_treated_as_deployment")),
            ("portable verification closure treated as public release", set_non_claim("portable_verification_closure_treated_as_public_release")),
            ("portable verification closure treated as operation permission", set_non_claim("portable_verification_closure_treated_as_operation_permission")),
            ("portable verification closure treated as continuation", set_non_claim("portable_verification_closure_treated_as_continuation")),
            ("portable verification closure treated as reusable permission", set_non_claim("portable_verification_closure_treated_as_reusable_permission")),
            ("portable verification closure treated as follow-on work", set_non_claim("portable_verification_closure_treated_as_follow_on_work")),
            ...
            ("full prior artifact body emitted outside bounded posture", set_field("full_prior_artifact_body_emitted_outside_bounded_portable_verification_closure", True)),
        ...
        for label, mutate in cases:
            with self.subTest(label=label):
                ...
                self.assert_blocked_result(result)
                claims = non_claims(result)
                self.assertIs(claims["zero_exit_code_treated_as_portable_verification_closure_standalone"], False)
                self.assertIs(claims["string_zero_treated_as_portable_verification_closure_standalone"], False)
                self.assertIs(claims["string_zero_representation_turned_into_doctrine"], False)
                self.assertIs(claims["ok_output_treated_as_portable_verification_closure_standalone"], False)
                self.assertIs(claims["ran_7_tests_treated_as_portable_verification_closure_standalone"], False)
                self.assertIs(claims["returned_capture_treated_as_portable_verification_closure_standalone"], False)
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(resolver, "OUTPUT_ROOT", output_root):
                first_path = resolver.write_portable_source_body_verification_portable_verification_closure_result(
                    result
                )
                second_path = resolver.write_portable_source_body_verification_portable_verification_closure_result(
                    result
                )
    ...
    def test_non_mutation_of_input_sections(self) -> None:
        ...
        request["portable_verification_closure_scope"] = {
