"""Tests for the local relevance medium second bounded relevance receipt resolver.

This suite is bounded to one second receipt object. It verifies that the
resolver reads one clean local relevance medium second bounded relevance
reception artifact, preserves the basis second reception, candidate admission,
request, index-entry, orientation-view, source-receipt, referenced reception,
existing received identifier basis, and second received identifiers, and
records one inspectable receipt object only.

The suite does not create second orientation view, second index entry, local
medium multiplicity result, relation view, comparison view, repeated reception
permission, arbitrary reception, feed, index system, registry, search, ranking,
source transfer, source receipt, authority, currentness, truth, action,
...
import resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min as resolver  # noqa: E402


DEFAULT_SECOND_RECEPTION_ARTIFACT = (
    REPO_ROOT
    / "artifacts/"
...
EXPECTED_TOP_LEVEL_SECTIONS = {
    ...
    "declared_local_relevance_medium_second_bounded_relevance_receipt_question",
    ...
    "local_relevance_medium_second_bounded_relevance_receipt_statement",
    "local_relevance_medium_second_bounded_relevance_receipt_non_meaning",
...
SECOND_RECEIPT_OBJECT_FIELDS = {
    "second_receipt_id",
    "second_receipt_type",
    "second_receipt_version",
...
WRAPPER_KEYS_FORBIDDEN_IN_SECOND_RECEIPT_OBJECT = {
    "outcome",
    "block",
    "local_relevance_medium_second_bounded_relevance_receipt_checks",
    "non_claims",
...
def _synthetic_second_reception_artifact() -> dict[str, Any]:
    return {
        "outcome": "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEPTION_RECORDED",
        "local_relevance_medium_second_bounded_relevance_reception_metadata": {
            ...
            "local_relevance_medium_second_bounded_relevance_reception_type": (
                "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEPTION"
            ),
            "local_relevance_medium_second_bounded_relevance_reception_version": "0.1.0",
        ...
        "second_bounded_relevance_reception_object": {
            ...
            "second_reception_version": "0.1.0",
            ...
            "basis_successor_candidate_admission_result_version": "0.1.0",
            "basis_successor_candidate_admission_failed_check_count": 0,
            "basis_successor_reception_request_artifact": (
                BASIS_SUCCESSOR_RECEPTION_REQUEST_ARTIFACT
            ),
...
def _clean_request(second_reception_artifact_path: Path) -> dict[str, Any]:
    return resolver.build_declared_local_relevance_medium_second_bounded_relevance_receipt_v0_min_request(
        selected_second_bounded_relevance_reception_artifact=second_reception_artifact_path,
    )
...
def _checks(result: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        check
        for check in result.get("local_relevance_medium_second_bounded_relevance_receipt_checks", [])
...
def _set_second_reception_object_field(
    artifact: dict[str, Any],
    field: str,
    value: Any = None,
) -> dict[str, Any]:
    updated = copy.deepcopy(artifact)
    updated["second_bounded_relevance_reception_object"][field] = value
    return updated
...
class LocalRelevanceMediumSecondBoundedRelevanceReceiptTests(unittest.TestCase):
    def write_synthetic_artifact(
        self,
        tmp_path: Path,
        artifact: dict[str, Any] | None = None,
        name: str = "second_bounded_relevance_reception",
    ) -> tuple[dict[str, Any], Path, dict[str, Any]]:
        synthetic = copy.deepcopy(artifact or _synthetic_second_reception_artifact())
    ...
    def assert_output_root_is_second_receipt_only(self) -> None:
    ...
    def assert_path_under_second_receipt_output_root_only(
        self,
        output_path: Path,
        output_root: Path,
    ) -> None:
    ...
    def assert_blocked(self, result: dict[str, Any]) -> None:
        ...
        self.assert_second_receipt_object_is_small(result)
    ...
    def assert_second_receipt_object_is_small(self, result: dict[str, Any]) -> None:
        second_receipt_object = result.get("second_bounded_relevance_receipt_object")
        if second_receipt_object is None:
            return
        self.assertIsInstance(second_receipt_object, dict)
        self.assertLessEqual(set(second_receipt_object), SECOND_RECEIPT_OBJECT_FIELDS)
        for key in WRAPPER_KEYS_FORBIDDEN_IN_SECOND_RECEIPT_OBJECT:
            self.assertNotIn(key, second_receipt_object)
    ...
    def assert_no_created_posture(self, result: dict[str, Any]) -> None:
        ...
        second_receipt_object = result.get("second_bounded_relevance_receipt_object")
        if isinstance(second_receipt_object, dict) and second_receipt_object:
            self.assertIs(second_receipt_object["second_reception_artifact_preserved"], True)
            self.assertIs(second_receipt_object["second_received_identifiers_preserved"], True)
            self.assertIs(second_receipt_object["second_receipt_does_not_expand_reception"], True)
            for key in (
                "second_orientation_view_created",
                "second_index_entry_created",
                "local_medium_multiplicity_result_created",
                ...
                self.assertIs(second_receipt_object[key], False)

    def assert_official_values_preserved(self, result: dict[str, Any]) -> None:
        second_receipt_object = result["second_bounded_relevance_receipt_object"]
        self.assertEqual(
            second_receipt_object["second_receipt_type"],
            "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT",
        )
        self.assertEqual(
            second_receipt_object["second_receipt_scope"],
            "SECOND_INSPECTABLE_RECEIPT_ONLY",
        )
        ...
        self.assertNotIn("[REDACTED_RAW_BODY_CONTENT]", second_receipt_object.values())
    ...
    def test_public_api_and_constants(self) -> None:
        for name in (
            ...
            "resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min_from_path",
            "write_local_relevance_medium_second_bounded_relevance_receipt_v0_min_result",
            "build_local_relevance_medium_second_bounded_relevance_receipt_v0_min_summary",
            "build_declared_local_relevance_medium_second_bounded_relevance_receipt_v0_min_request",
        ):
        ...
        for name in (
            ...
            "RESOLVER_MODULE",
            "OUTPUT_ROOT",
            "SUPPORTED_SECOND_RECEIPT_SCOPE_VALUES",
            "SUPPORTED_SECOND_RECEIPT_TYPE_VALUES",
        ...
        self.assert_output_root_is_second_receipt_only()
        self.assertIn(
            "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT",
            resolver.SUPPORTED_SECOND_RECEIPT_TYPE_VALUES,
        )
        self.assertIn("SECOND_INSPECTABLE_RECEIPT_ONLY", resolver.SUPPORTED_SECOND_RECEIPT_SCOPE_VALUES)

    def test_successful_recorded_result_from_synthetic_second_reception_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            request, artifact_path, _ = self.write_synthetic_artifact(Path(tmp))
            result = resolver.resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min(
                request
            )
            summary = resolver.build_local_relevance_medium_second_bounded_relevance_receipt_v0_min_summary(
                result
            )
        ...
        self.assertEqual(summary["second_receipt_id"], "local_relevance_medium_second_bounded_relevance_receipt_001")
        self.assertTrue(EXPECTED_TOP_LEVEL_SECTIONS.issubset(result))

        receipt_object = result["second_bounded_relevance_receipt_object"]
        self.assertEqual(receipt_object["second_receipt_id"], "local_relevance_medium_second_bounded_relevance_receipt_001")
        self.assertEqual(
            receipt_object["second_receipt_type"],
            "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT",
        )
        self.assertEqual(receipt_object["second_receipt_version"], "0.1.0")
        self.assertEqual(receipt_object["second_receipt_scope"], "SECOND_INSPECTABLE_RECEIPT_ONLY")
        self.assertEqual(
            receipt_object["basis_second_bounded_relevance_reception_artifact"],
            str(artifact_path),
        )
        self.assertEqual(
            receipt_object["basis_second_bounded_relevance_reception_outcome"],
            "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEPTION_RECORDED",
        )
        self.assertEqual(receipt_object["basis_second_bounded_relevance_reception_result_version"], "0.1.0")
        self.assertEqual(receipt_object["basis_second_bounded_relevance_reception_failed_check_count"], 0)
        self.assertEqual(
            receipt_object["basis_successor_candidate_admission_artifact"],
            BASIS_SUCCESSOR_CANDIDATE_ADMISSION_ARTIFACT,
        )
        self.assertEqual(
            receipt_object["basis_successor_reception_request_artifact"],
            BASIS_SUCCESSOR_RECEPTION_REQUEST_ARTIFACT,
        )
        self.assertEqual(receipt_object["basis_index_entry_artifact"], BASIS_INDEX_ENTRY_ARTIFACT)
        self.assertEqual(receipt_object["existing_orientation_view_artifact"], ORIENTATION_VIEW_ARTIFACT)
        self.assertEqual(receipt_object["existing_source_receipt_artifact"], SOURCE_RECEIPT_ARTIFACT)
        self.assertEqual(
            receipt_object["existing_referenced_reception_artifact"],
            REFERENCED_RECEPTION_ARTIFACT,
        )
        self.assertEqual(receipt_object["existing_received_signal_id"], "bounded_relevance_signal_001")
        self.assertEqual(receipt_object["existing_received_relevance_basis_id"], "bounded_relevance_basis_001")
        self.assertEqual(receipt_object["existing_received_relevance_scope_id"], "bounded_relevance_scope_001")
        self.assertEqual(
            receipt_object["existing_received_carrier_context_id"],
            "bounded_relevance_signal_carrier_context_001",
        )
        self.assertEqual(
            receipt_object["existing_received_reception_envelope_id"],
            "bounded_relevance_reception_envelope_001",
        )
        self.assertEqual(
            receipt_object["admitted_successor_candidate_id"],
            "bounded_relevance_signal_candidate_002",
        )
        self.assertEqual(receipt_object["second_received_signal_id"], "bounded_relevance_signal_002")
        self.assertNotEqual(
            receipt_object["second_received_signal_id"],
            receipt_object["existing_received_signal_id"],
        )
        self.assertEqual(receipt_object["second_relevance_basis_id"], "bounded_relevance_basis_002")
        self.assertEqual(receipt_object["second_relevance_scope_id"], "bounded_relevance_scope_002")
        self.assertEqual(
            receipt_object["second_carrier_context_id"],
            "bounded_relevance_signal_carrier_context_002",
        )
        self.assertEqual(
            receipt_object["second_reception_envelope_id"],
            "bounded_relevance_reception_envelope_002",
        )
        self.assert_no_created_posture(result)
        self.assert_second_receipt_object_is_small(result)

        statement = result["local_relevance_medium_second_bounded_relevance_receipt_statement"]
    ...
    def test_successful_recorded_result_from_default_artifact_if_present(self) -> None:
        if not DEFAULT_SECOND_RECEPTION_ARTIFACT.exists():
            self.skipTest("default local relevance medium second bounded relevance reception artifact absent")
        request = resolver.build_declared_local_relevance_medium_second_bounded_relevance_receipt_v0_min_request()
        result = resolver.resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min(request)
        summary = resolver.build_local_relevance_medium_second_bounded_relevance_receipt_v0_min_summary(
            result
        )
        ...
        self.assertIs(receipt_object["second_receipt_does_not_expand_reception"], True)
        self.assert_no_created_posture(result)
    ...
    def test_representative_blocking_behavior(self) -> None:
        def mutate_artifact(field: str, value: Any) -> Any:
            return lambda artifact, request, tmp: (
                _set_second_reception_object_field(artifact, field, value),
                request,
            )
        ...
        cases: list[tuple[str, Any]] = [
            ("explicit block intent", lambda a, r, t: (a, {**r, "local_relevance_medium_second_bounded_relevance_receipt_intent": "BLOCK_LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT"})),
            ("missing request", lambda a, r, t: (a, {})),
            ("unsupported intent", lambda a, r, t: (a, {**r, "local_relevance_medium_second_bounded_relevance_receipt_intent": "UNSUPPORTED"})),
            ("artifact path missing", lambda a, r, t: (a, {**r, "selected_second_bounded_relevance_reception_artifact": ""})),
            ("artifact unreadable", lambda a, r, t: (a, {**r, "selected_second_bounded_relevance_reception_artifact": str(t / "missing.json"), "__skip_artifact_write": True})),
            ...
            ("artifact wrong version", lambda a, r, t: (_set_metadata(a, "local_relevance_medium_second_bounded_relevance_reception_version", "9.9.9"), r)),
            ("object missing", lambda a, r, t: (_without_key(a, "second_bounded_relevance_reception_object"), r)),
            ("second reception type invalid", mutate_artifact("second_reception_type", "WRONG")),
            ("second reception scope invalid", mutate_artifact("second_reception_scope", "WRONG")),
            ("second received signal missing", mutate_artifact("second_received_signal_id", None)),
            ("second received signal equals existing", mutate_artifact("second_received_signal_id", "bounded_relevance_signal_001")),
            ...
            ("second receipt type missing", lambda a, r, t: (a, {**r, "second_receipt_type": None})),
            ("second receipt type invalid", lambda a, r, t: (a, {**r, "second_receipt_type": "WRONG"})),
            ("second receipt scope missing", lambda a, r, t: (a, {**r, "second_receipt_scope": None})),
            ("second receipt scope invalid", lambda a, r, t: (a, {**r, "second_receipt_scope": "WRONG"})),
            ("second reception artifact missing", request_flag("second_reception_artifact_missing")),
            ("second identifiers not preserved", request_flag("second_received_identifiers_not_preserved")),
            ("second receipt expands reception", request_flag("second_receipt_expands_second_reception")),
            ("second orientation view created", request_flag("second_orientation_view_created")),
            ...
            ("artifact existence authority", request_flag("artifact_existence_treated_as_second_receipt_authority")),
            ("latest file posture authority", request_flag("latest_file_posture_treated_as_second_receipt_authority")),
            ("repo local availability authority", request_flag("repo_local_availability_treated_as_second_receipt_authority")),
            ("hidden repo state content", request_flag("hidden_repo_state_used_as_second_receipt_content")),
            ("hidden repo state authority", request_flag("hidden_repo_state_used_as_second_receipt_authority")),
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            base_artifact = _synthetic_second_reception_artifact()
            for name, mutator in cases:
                with self.subTest(name=name):
                    ...
                    if isinstance(artifact, list):
                        artifact_path = _write_json(Path(request["selected_second_bounded_relevance_reception_artifact"]), artifact)
                    elif request.get("selected_second_bounded_relevance_reception_artifact") and not request.get("__skip_artifact_write"):
                        artifact_path = _write_json(
                            Path(request["selected_second_bounded_relevance_reception_artifact"]),
                            artifact,
                        )
                        request["selected_second_bounded_relevance_reception_artifact"] = str(artifact_path)
                    result = resolver.resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min(
                        request
                    )
                    self.assert_blocked(result)

        non_mapping_result = resolver.resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min(
            ["not", "mapping"]
        )
    ...
    def test_official_values_are_preserved(self) -> None:
        ...
        self.assertEqual(
            set(resolver.OUTCOME_FAMILY),
            {
                ...
                "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT_NOT_RECORDED",
                "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT_REQUIRES_ADDITIONAL_BASIS",
                "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT_BLOCKED",
            },
        )
    ...
    def test_raw_hidden_hostile_content_containment_and_input_non_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            request["not_recorded_basis"] = [
                {"raw_second_receipt_body": HOSTILE_SENTINELS[0]},
        ...
        self.assertIn("LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT", json.dumps(result))
        self.assert_non_claims_canonical_false(result)
        self.assert_no_created_posture(result)
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = resolver.resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min_from_path(
                request_path
            )
            summary = resolver.build_local_relevance_medium_second_bounded_relevance_receipt_v0_min_summary(
                result
            ...
            with self.assertRaises(resolver.LocalRelevanceMediumSecondBoundedRelevanceReceiptV0MinError):
                resolver.resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min_from_path(
                    malformed_path
                )

            array_path = _write_json(tmp_path / "array.json", [])
            array_result = resolver.resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min_from_path(
                array_path
            )
            ...
            with mock.patch.object(resolver, "OUTPUT_ROOT", output_root):
                first_path = resolver.write_local_relevance_medium_second_bounded_relevance_receipt_v0_min_result(
                    result
                )
                second_path = resolver.write_local_relevance_medium_second_bounded_relevance_receipt_v0_min_result(
                    result
                )
            ...
            self.assert_path_under_second_receipt_output_root_only(first_path, output_root)
            self.assert_path_under_second_receipt_output_root_only(second_path, output_root)
    ...
    def test_non_mutation(self) -> None:
        ...
        self.assertEqual(request["second_receipt_scope"], "SECOND_INSPECTABLE_RECEIPT_ONLY")
        self.assertEqual(
            request["second_receipt_type"],
            "LOCAL_RELEVANCE_MEDIUM_SECOND_BOUNDED_RELEVANCE_RECEIPT",
        )
        self.assertEqual(
            request["selected_second_bounded_relevance_reception_artifact"],
            before_request["selected_second_bounded_relevance_reception_artifact"],
        )
    ...
    def test_predecessor_failure_preservation(self) -> None:
        ...
        for field in (
            ...
            with self.subTest(field=field):
                ...
                blocked_result = resolver.resolve_local_relevance_medium_second_bounded_relevance_receipt_v0_min(
                    blocked_request
                )
...
def _set_metadata(artifact: dict[str, Any], field: str, value: Any) -> dict[str, Any]:
    updated = copy.deepcopy(artifact)
    updated["local_relevance_medium_second_bounded_relevance_reception_metadata"][field] = value
    return updated
