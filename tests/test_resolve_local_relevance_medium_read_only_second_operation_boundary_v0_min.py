"""Executable tests for the local read-only second-operation boundary resolver.

The target records one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_SECOND_OPERATION_BOUNDARY object only. These
tests keep the object selected-state-only, local, read-only,
selected-second-operation-consideration-only, raw-state-body-excluding,
...
import resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min as resolver  # noqa: E402
...
class TestLocalRelevanceMediumReadOnlySecondOperationBoundaryV0Min(unittest.TestCase):
    ...
    def selected_object(self, artifacts: Mapping[str, dict[str, Any]], name: str) -> dict[str, Any]:
    ...
    def write_artifacts(self, root: Path, artifacts: Mapping[str, Any], prefix: str = "basis") -> dict[str, Path]:
        paths = {}
    ...
    def request_for(self, paths: Mapping[str, Path], **overrides: Any) -> dict[str, Any]:
        ...
        return resolver.build_declared_local_relevance_medium_read_only_second_operation_boundary_v0_min_request(**kwargs)
    ...
    def resolve_clean(self, root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, dict[str, Any]], dict[str, Path]]:
        request, artifacts, paths = self.clean_context(root)
        result = resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min(request)
    ...
    def assert_boundary_false_posture(self, result: Mapping[str, Any]) -> None:
        boundary = self.boundary(result)
        for key in REQ_FALSE:
            self.assertIn(key, boundary)
            self.assertIs(boundary[key], False, key)
        self.assertIs(boundary["second_operation_created"], False)
        self.assertIs(boundary["continuation_created"], False)
        self.assertIs(boundary["runtime_hosting_created"], False)
        self.assertIs(boundary["runtime_loop_created"], False)
        self.assertIs(boundary["daemon_behavior_created"], False)
        ...
        self.assertIs(boundary["older_runtime_permission_treated_as_current"], False)
        self.assertIs(boundary["runtime_authority_imported"], False)
    ...
    def assert_blocked_with_public_code(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_boundary_false_posture(result)
    ...
    def assert_same_or_stable_artifact_path(self, actual: Any, expected: Any) -> None:
        ...
        try:
            ...
            pass
        self.assertTrue(
            str(actual).replace("\\", "/").endswith(expected_path.name)
            or str(actual).replace("\\", "/").endswith(str(expected).replace("\\", "/")),
    ...
    def assert_lineage_and_closure(self, result: Mapping[str, Any]) -> None:
        ...
        self.assertIs(summary["failure_not_repaired_hidden_or_claimed_passed"], True)
    ...
    def add_explanatory_true_values(self, artifacts: dict[str, dict[str, Any]]) -> None:
        ...
        for artifact in artifacts.values():
            artifact["statement"] = copy.deepcopy(explanatory)
            artifact["summary"] = copy.deepcopy(explanatory)
            artifact["non_meaning"] = copy.deepcopy(explanatory)
    ...
    def inject_hostile_content(self, artifacts: dict[str, dict[str, Any]], request: dict[str, Any]) -> None:
    ...
    def test_public_api_constants_and_default_builder(self) -> None:
        ...
        self.assertEqual(resolver.RESOLVER_MODULE, "resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min")
        self.assertTrue(str(resolver.OUTPUT_ROOT).endswith("artifacts/integrity_host_v0_min_coexistence_local_relevance_medium_read_only_second_operation_boundary_v0_min"))
        ...
        self.assertNotIn("future_second_operation_may_be_considered", resolver.REQUIRED_FALSE_NON_CLAIMS)
        for code in (
            "SECOND_OPERATION_CREATED",
            "CONTINUATION_CREATED",
            "RAW_STATE_BODY_EMBEDDED",
            ...
            self.assertIn(code, resolver.BLOCK_CODES)
        request = resolver.build_declared_local_relevance_medium_read_only_second_operation_boundary_v0_min_request()
    ...
    def test_successful_recorded_result_from_synthetic_artifacts(self) -> None:
        ...
        for basis in BASIS:
            ...
            self.assert_same_or_stable_artifact_path(boundary[f"{basis_prefix}_artifact"], paths[name])
        ...
        statement = self.statement(result)
        self.assertIs(statement["local_relevance_medium_read_only_second_operation_boundary_recorded"], True)
    ...
    def test_successful_recorded_result_from_default_live_artifacts_if_present(self) -> None:
        ...
        for basis in BASIS:
            ...
            self.assert_same_or_stable_artifact_path(self.boundary(result)[f"{basis_prefix}_artifact"], request[request_key])
    ...
    def test_exact_selected_object_and_exact_non_claim_true_values_block(self) -> None:
        for index, key in enumerate(EXACT_FORBIDDEN, start=1):
            with self.subTest(selected_object_key=key), tempfile.TemporaryDirectory() as temp_dir:
                def flip_object(artifacts: dict[str, dict[str, Any]], field: str = key) -> None:
                ...
                self.assertIn(FIELD_CODES[key], self.emitted_codes(result))

            with self.subTest(top_level_non_claim_key=key), tempfile.TemporaryDirectory() as temp_dir:
                def flip_non_claim(artifacts: dict[str, dict[str, Any]], field: str = key) -> None:
    ...
    def test_declared_request_blocking_and_non_claim_canonicalization(self) -> None:
        for key in (*EXACT_FORBIDDEN, *LINEAGE_FALSE):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for key in REQ_FALSE:
                with self.subTest(declared_non_claim=key):
                    ...
                    result = resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min(mutated)
                    self.assert_blocked_with_public_code(result)
    ...
    def test_representative_blocking_behavior(self) -> None:
        special = (None, "not a mapping", {})
        for request in special:
            with self.subTest(request=request):
                result = resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min(request)  # type: ignore[arg-type]
                if request is None:
        ...
        request_cases = (
            (
                ...
                lambda request: request.update({"local_relevance_medium_read_only_second_operation_boundary_intent": "BLOCK_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_SECOND_OPERATION_BOUNDARY"}),
                "LOCAL_RELEVANCE_MEDIUM_READ_ONLY_SECOND_OPERATION_BOUNDARY_BLOCK_REQUESTED",
            ),
            (
                "unsupported_intent",
                lambda request: request.update({"local_relevance_medium_read_only_second_operation_boundary_intent": "UNSUPPORTED"}),
            ...
            ("boundary_type_wrong", lambda request: request.update({"boundary_type": "LOCAL_RELEVANCE_MEDIUM_SECOND_OPERATION"}), "BOUNDARY_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_SECOND_OPERATION_BOUNDARY"),
            ("boundary_scope_missing", lambda request: request.pop("boundary_scope"), "BOUNDARY_SCOPE_MISSING"),
            ("boundary_scope_wrong", lambda request: request.update({"boundary_scope": "GENERAL"}), "BOUNDARY_SCOPE_NOT_SELECTED_SECOND_OPERATION_CONSIDERATION_ONLY"),
            ("future_second_operation_not_considered", lambda request: request.update({"future_second_operation_may_be_considered": False}), "FUTURE_SECOND_OPERATION_MAY_NOT_BE_CONSIDERED"),
        )
        for case_name, mutator, code in request_cases:
            with self.subTest(case=case_name), tempfile.TemporaryDirectory() as temp_dir:
                request, _artifacts, _paths = self.clean_context(Path(temp_dir), request_mutator=mutator, prefix=self.safe_json_filename(case_name).removesuffix(".json"))
                result = resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min(request)
                self.assert_blocked_with_public_code(result)
        ...
        true_posture_cases = (
            ...
            ("prior_result_reentry_cycle", "prior_result_reentry_cycle_local_only", "PRIOR_RESULT_REENTRY_CYCLE_LOCAL_ONLY_NOT_TRUE"),
            ("prior_result_reentry_cycle", "prior_result_reentry_cycle_read_only", "PRIOR_RESULT_REENTRY_CYCLE_READ_ONLY_NOT_TRUE"),
            ("prior_result_reentry_cycle", "cycle_basis_reference_only", "CYCLE_BASIS_REFERENCE_ONLY_NOT_TRUE"),
            ...
            ("prior_result_reentry_cycle", "runtime_v0_failure_evidence_preserved", "PREDECESSOR_FAILURE_EVIDENCE_HIDDEN_OR_REPAIRED"),
        )
        for artifact_name, field, code in true_posture_cases:
            with self.subTest(artifact=artifact_name, field=field), tempfile.TemporaryDirectory() as temp_dir:
                def mutate(artifacts: dict[str, dict[str, Any]], name: str = artifact_name, key: str = field) -> None:
                    self.selected_object(artifacts, name)[key] = False

                request, _artifacts, _paths = self.clean_context(Path(temp_dir), artifact_mutator=mutate, prefix=self.safe_json_filename(f"{artifact_name}_{field}").removesuffix(".json"))
                result = resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min(request)
                self.assert_blocked_with_public_code(result)
                self.assertIn(code, self.emitted_codes(result))
        ...
        for basis in BASIS:
            ...
            for suffix, code in cases:
                with self.subTest(artifact=name, case=suffix), tempfile.TemporaryDirectory() as temp_dir:
                    root = Path(temp_dir)
                    artifacts = self.make_artifacts()
                    paths = self.write_artifacts(root, artifacts, prefix=self.safe_json_filename(f"{name}_{suffix}").removesuffix(".json"))
                    request = self.request_for(paths)
    ...
    def test_official_values_and_hostile_content_containment(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for official in (resolver.OUTCOME_RECORDED, BOUNDARY_TYPE, BOUNDARY_SCOPE, SELECTED_COMMAND, resolver.RESOLVER_MODULE):
                self.assertIn(official, serialized)
            self.assertNotIn("[redacted_raw_content]", self.boundary(result)["boundary_type"])
            self.assertNotIn("[redacted_raw_content]", self.boundary(result)["boundary_scope"])

        with tempfile.TemporaryDirectory() as temp_dir:
            request, artifacts, _paths = self.clean_context(Path(temp_dir))
            self.inject_hostile_content(artifacts, request)
            original_artifacts = copy.deepcopy(artifacts)
            hostile_paths = self.write_artifacts(Path(temp_dir) / "hostile", artifacts, prefix="hostile")
            for basis in BASIS:
    ...
    def test_path_write_non_mutation_and_lineage_preservation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            from_path = resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min_from_path(request_path)
            self.assert_recorded_boundary(from_path)

            malformed_path = root / "malformed.json"
            malformed_path.write_text("{", encoding="utf-8")
            self.assert_blocked_with_public_code(resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min_from_path(malformed_path))
            array_path = root / "array.json"
            self.write_json(array_path, [])
            self.assert_blocked_with_public_code(resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min_from_path(array_path))
            self.assert_blocked_with_public_code(resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min_from_path(root / "missing.json"))

            output_root = root / "artifacts" / "integrity_host_v0_min_coexistence_local_relevance_medium_read_only_second_operation_boundary_v0_min"
            with mock.patch.object(resolver, "OUTPUT_ROOT", output_root):
                written = resolver.write_local_relevance_medium_read_only_second_operation_boundary_v0_min_result(result)
                written_again = resolver.write_local_relevance_medium_read_only_second_operation_boundary_v0_min_result(result)
            self.assertTrue(written.exists())
            self.assertTrue(written_again.exists())
            self.assertNotEqual(written, written_again)
            self.assertIn("local_relevance_medium_read_only_second_operation_boundary_v0_min", str(written.parent))
            with written.open("r", encoding="utf-8") as file:
                self.assertEqual(json.load(file)["outcome"], resolver.OUTCOME_RECORDED)
            for forbidden_root in resolver.FORBIDDEN_OUTPUT_ROOTS:
                try:
                    written.resolve().relative_to((REPO_ROOT / forbidden_root).resolve())
                except ValueError:
                    continue
                self.fail(f"{written} wrote under forbidden root {forbidden_root}")

            request_before = copy.deepcopy(request)
            artifacts_before = copy.deepcopy(artifacts)
            paths_before = copy.deepcopy(paths)
            resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min(request)
            ...
            for key in LINEAGE_FALSE:
                with self.subTest(lineage_flip=key):
                    mutated = copy.deepcopy(request)
                    mutated[key] = True
                    blocked = resolver.resolve_local_relevance_medium_read_only_second_operation_boundary_v0_min(mutated)
                    self.assert_blocked_with_public_code(blocked)
                    self.assertIn(FIELD_CODES[key], self.emitted_codes(blocked))
