"""V2 tests for the local relevance medium read-only continuation resolver.

This suite is additive successor evidence for
tests/test_resolve_local_relevance_medium_read_only_continuation_v0_min.py. The
v1 continuation test is preserved as failed-lineage evidence only: it ran 13
tests and failed with 13 IsADirectoryError errors in representative
artifact_json_array subtests because fixture paths were reused as both
directories and files. That is a fixture path-isolation failure, not resolver
evidence.

The v2 suite preserves the v1 coverage posture while fixing only fixture path
isolation. Directory/unreadable cases and JSON-array file cases use distinct
paths. The target resolver still records one
LOCAL_RELEVANCE_MEDIUM_READ_ONLY_CONTINUATION object only: local, read-only,
selected-continuation-only, selected-state-only, basis-reference-only, anchored
to the selected first-operation / second-operation line, and operation sequence
count 2. Continuation-created true is continuation-only, not hosting, loop,
daemon behavior, authority, currentness, truth, or follow-on work.
"""
...
def _true_fields_for_basis() -> dict[str, Any]:
    fields = {
    ...
    fields.update({key: True for key in resolver.POSITIVE_BASIS_FIELDS})
    fields.update({key: True for key in LINEAGE_TRUE_FIELDS})
    return fields


class LocalRelevanceMediumReadOnlyContinuationV0MinV2Tests(unittest.TestCase):
    def safe_json_filename(self, name: Any, index: int | None = None) -> str:
        safe = _safe_fragment(name)
        if index is not None:
            safe = f"{index:03d}_{safe}"
        return f"{safe}.json"

    def case_dir(self, base: Path, case_kind: str, artifact_key: str) -> Path:
        directory = (
            base
            / _safe_fragment(case_kind)
            / _safe_fragment(artifact_key)
            / "unreadable_artifact_dir"
        )
        self.assertFalse(
            directory.is_file(),
            f"fixture path collision: directory case path is already a file: {directory}",
        )
    ...
    def case_file(
        ...
        directory = base / _safe_fragment(case_kind) / _safe_fragment(artifact_key)
        directory.mkdir(parents=True, exist_ok=True)
        path = directory / _safe_fragment(filename)
        self.assertFalse(
            path.is_dir(),
            f"fixture path collision: file case path is already a directory: {path}",
        )
        return path

    def case_workspace(self, base: Path, case_kind: str, artifact_key: str) -> Path:
        directory = (
            base
            / "workspace_case"
            / _safe_fragment(case_kind)
    ...
    def missing_case_path(self, base: Path, artifact_key: str) -> Path:
        return (
            base
            / "missing_path_case"
            / _safe_fragment(artifact_key)
    ...
    def _write_json(self, path: Path, value: Any) -> Path:
        self.assertFalse(
            path.is_dir(),
            f"fixture path collision: attempted to write JSON into directory {path}",
        )
    ...
    def assert_continuation_false_posture(self, result: Mapping[str, Any]) -> None:
    ...
    def assert_blocked_with_public_code(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_continuation_false_posture(result)
    ...
    def assert_clean_basis_sections(self, result: Mapping[str, Any]) -> None:
        ...
        for spec in BASIS_SPECS:
            basis = result[str(spec["basis_section"])]
            self.assertEqual(basis["outcome"], spec["expected_outcome"])
            self.assertEqual(basis["result_version"], resolver.RESULT_VERSION)
            self.assertEqual(basis["failed_check_count"], 0)
            self.assertIs(basis["artifact_preserved"], True)
            prefix = str(spec["basis_prefix"])
            self.assertEqual(continuation[f"{prefix}_outcome"], spec["expected_outcome"])
            self.assertEqual(continuation[f"{prefix}_result_version"], "0.1.0")
    ...
    def assert_failure_lineage_preserved(self, result: Mapping[str, Any]) -> None:
        ...
        for stem in (
            ...
            for suffix in ("repaired", "hidden", "claimed_passed"):
                self.assertIs(continuation.get(f"{stem}_{suffix}"), False)
        for key in (
            "predecessor_failure_repaired",
            "predecessor_failure_hidden",
            "predecessor_failure_claimed_passed",
        ):
            self.assertIs(continuation.get(key), False)
    ...
    def assert_clean_continuation_object(self, result: Mapping[str, Any]) -> None:
        ...
        self.assertEqual(continuation["continuation_version"], "0.1.0")
        self.assertEqual(continuation["selected_command"], SELECTED_COMMAND)
        self.assertIs(continuation["selected_command_is_state"], True)
        self.assertIs(continuation["selected_continuation_boundary_recorded"], True)
        self.assertIs(continuation["future_continuation_may_be_considered"], True)
        self.assertIs(continuation["selected_second_operation_recorded"], True)
        self.assertIs(continuation["second_operation_created"], True)
        self.assertIs(continuation["second_operation_local_only"], True)
        self.assertIs(continuation["second_operation_read_only"], True)
        self.assertIs(continuation["second_operation_basis_reference_only"], True)
        self.assertIs(continuation["second_operation_sequence_index_is_2"], True)
    ...
    def assert_clean_statement(self, result: Mapping[str, Any]) -> None:
    ...
    def assert_result_recorded_clean(self, result: Mapping[str, Any]) -> None:
        ...
        self.assert_all_emitted_codes_public(result)
        self.assert_canonical_false_non_claims(result)
        metadata = result["local_relevance_medium_read_only_continuation_metadata"]
        self.assertEqual(metadata["resolver_module"], resolver.RESOLVER_MODULE)
        self.assertEqual(
            metadata["local_relevance_medium_read_only_continuation_version"],
            "0.1.0",
        )
        ...
        self.assert_clean_statement(result)
    ...
    def _artifact_for(
        ...
        *,
        ...
        true_fields = _true_fields_for_basis()
    ...
    def _write_synthetic_artifacts(
        ...
        *,
        ...
        for index, spec in enumerate(BASIS_SPECS, start=1):
            name = str(spec["name"])
            artifact = self._artifact_for(spec, **dict(mutations.get(name, {})))
    ...
    def resolve_clean(self, root: Path) -> dict[str, Any]:
        request, _, _ = self._valid_request(root)
    ...
    def test_successful_recorded_result_from_synthetic_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            result = self.resolve_clean(Path(td))
    ...
    def test_successful_recorded_result_from_default_live_artifacts_if_present(self) -> None:
        ...
        missing = [
            REPO_ROOT / request[key]
            for key in DEFAULT_SUFFIXES
            if not (REPO_ROOT / request[key]).exists()
        ]
        if missing:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for index, key in enumerate(forbidden_cases, start=1):
                with self.subTest(selected_object_key=key):
                    ...
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "selected_exact_true", f"{index}_{key}"),
                        mutations=mutations,
                ...
                with self.subTest(top_level_non_claim=key):
                    ...
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "top_non_claim_true", f"{index}_{key}"),
                        mutations=mutations,
    ...
    def test_top_level_declared_and_declared_non_claim_blocking(self) -> None:
        top_level_cases = (
            "consumed_request_reopened",
            "authorization_token_reused",
            "older_runtime_lineage_imported_as_authority",
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for index, key in enumerate(top_level_cases, start=1):
                with self.subTest(top_level=key):
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "top_level", f"{index}_{key}"),
            ...
            for index, key in enumerate(REQ_FALSE, start=1):
                with self.subTest(declared_non_claim_flipped=key):
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "declared_non_claim_flip", f"{index}_{key}"),
                ...
                with self.subTest(declared_non_claim_missing=key):
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "declared_non_claim_missing", f"{index}_{key}"),
                ...
                with self.subTest(declared_non_claim_non_bool=key):
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "declared_non_claim_non_bool", f"{index}_{key}"),
    ...
    def test_representative_blocking_with_isolated_fixture_paths(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            malformed = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
                []
            )
            ...
            self.assert_blocked_with_public_code(missing_request)
            ...
            header_cases = (
                ...
                ("selected_command_not_state", "SELECTED_COMMAND_NOT_STATE", {"selected_command": "status"}),
                ("continuation_type_unsupported", "CONTINUATION_TYPE_NOT_LOCAL_RELEVANCE_MEDIUM_READ_ONLY_CONTINUATION", {"continuation_type": "PUBLIC_API"}),
                ("continuation_scope_unsupported", "CONTINUATION_SCOPE_NOT_SELECTED_CONTINUATION_ONLY", {"continuation_scope": "GENERAL"}),
            )
            for name, expected_code, overrides in header_cases:
                with self.subTest(header=name):
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "header", name),
            ...
            missing_field_cases = (
                ("selected_command", "SELECTED_COMMAND_MISSING"),
                ("continuation_type", "CONTINUATION_TYPE_MISSING"),
                ("continuation_scope", "CONTINUATION_SCOPE_MISSING"),
            )
            for field, expected_code in missing_field_cases:
                with self.subTest(missing_field=field):
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "missing_header_field", field),
                        name_suffix=field,
                    )
                    request.pop(field)
                    result = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
            ...
            for spec in BASIS_SPECS:
                ...
                clean_workspace = self.case_workspace(root, "artifact_cases", request_key)

                with self.subTest(artifact_path_missing=request_key):
                    request, _, _ = self._valid_request(
                        clean_workspace / "missing_path_basis",
                        name_suffix=f"missing_{request_key}",
                    )
                    request[request_key] = str(self.missing_case_path(root, request_key))
                    result = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
                        request
                    )
                    self.assert_blocked_with_public_code(result)
                ...
                with self.subTest(artifact_unreadable_directory=request_key):
                    request, _, _ = self._valid_request(
                        clean_workspace / "dir_basis",
                        name_suffix=f"dir_{request_key}",
                    )
                    request[request_key] = str(self.case_dir(root, "dir_case", request_key))
                    result = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
                        request
                    )
                    self.assert_blocked_with_public_code(result)
                ...
                with self.subTest(artifact_json_array=request_key):
                    request, _, _ = self._valid_request(
                        clean_workspace / "json_array_basis",
                        name_suffix=f"array_{request_key}",
                    )
                    array_path = self.case_file(
                        root,
                        "json_array_case",
                        request_key,
                        "array_artifact.json",
                ...
                with self.subTest(artifact_not_recorded=request_key):
                    request, _, _ = self._valid_request(
                        clean_workspace / "not_recorded_basis",
                ...
                with self.subTest(artifact_failed_check=request_key):
                    request, _, _ = self._valid_request(
                        clean_workspace / "failed_check_basis",
                        name_suffix=f"failed_{request_key}",
                    )
                    artifact = self._artifact_for(spec, failed_check_count=1)
                    path = self.case_file(
                        root,
                        "failed_check_case",
                        request_key,
                        "failed_check_artifact.json",
                    )
                    self._write_json(path, artifact)
                    request[request_key] = str(path)
                    result = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
                ...
                with self.subTest(artifact_wrong_version=request_key):
                    request, _, _ = self._valid_request(
                        clean_workspace / "wrong_version_basis",
                        name_suffix=f"version_{request_key}",
                    )
                    artifact = self._artifact_for(spec, result_version="9.9.9")
                    path = self.case_file(
                        root,
                        "wrong_version_case",
                        request_key,
                        "wrong_version_artifact.json",
                    )
                    self._write_json(path, artifact)
                    request[request_key] = str(path)
                    result = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
            ...
            representative_positive = (
                ("continuation_boundary", "selected_continuation_boundary_recorded"),
                ("continuation_boundary", "future_continuation_may_be_considered"),
                ("second_operation", "selected_second_operation_recorded"),
                ...
                ("second_operation_boundary", "selected_second_operation_boundary_recorded"),
            ...
            for basis_name, field in representative_positive:
                with self.subTest(positive_false=f"{basis_name}:{field}"):
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "positive_false", f"{basis_name}_{field}"),
                        mutations={basis_name: {"selected_overrides": {field: False}}},
                        name_suffix=f"{basis_name}_{field}",
                    )
                    result = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
                        request
                    )
            ...
            with self.subTest(second_operation_sequence_index_not_two=True):
                request, _, _ = self._valid_request(
                    self.case_workspace(root, "positive_false", "sequence_index"),
                    mutations={
                        "second_operation": {
                            "selected_overrides": {
                    ...
                    name_suffix="sequence_index_not_2",
                )
                result = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
                    request
                )
    ...
    def test_raw_hidden_and_older_runtime_sentinels_are_sanitized(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            for index, spec in enumerate(BASIS_SPECS):
                ...
                artifact["raw_full_prior_artifact_body"] = HOSTILE_SENTINELS[
                    index % len(HOSTILE_SENTINELS)
                ]
                artifact["hidden_repo_state"] = "HIDDEN_REPO_STATE_MUST_NOT_RETURN"
                artifact["raw_state_body"] = "RAW_STATE_BODY_MUST_NOT_RETURN"
                self._write_json(Path(paths[str(spec["request_key"])]), artifact)
            result = resolver.resolve_local_relevance_medium_read_only_continuation_v0_min(
                request
            )
            ...
            serialized = json.dumps(result, sort_keys=True)
            self.assertIn(CONTINUATION_TYPE, serialized)
    ...
    def test_path_and_write_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            forbidden = set(resolver.FORBIDDEN_OUTPUT_ROOT_PARTS)
            forbidden.discard(
                "integrity_host_v0_min_coexistence_local_relevance_medium_read_only_continuation_v0_min"
            )
            self.assertFalse(forbidden.intersection(first.parts))
    ...
    def test_failure_lineage_and_predecessor_preservation(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            result = self.resolve_clean(root)
            self.assert_result_recorded_clean(result)
            self.assert_failure_lineage_preserved(result)
            summary = self.summary(result)
            for key in (
            ...
            for index, key in enumerate(flip_fields, start=1):
                with self.subTest(failure_lineage_flip=key):
                    request, _, _ = self._valid_request(
                        self.case_workspace(root, "failure_lineage_flip", f"{index}_{key}"),
                        request_overrides={key: True},
                        name_suffix=key,
                    )
                    blocked = (
