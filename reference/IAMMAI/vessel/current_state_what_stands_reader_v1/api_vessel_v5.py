#!/usr/bin/env python3
"""
API-backed vessel v5 for current_state_what_stands_reader_v1.

This additive file preserves the narrowed vessel family while correcting the
remaining candidate-selection failure visible in api_vessel_v4.py. The local
...
class CurrentStateWhatStandsAPIVesselV5(CurrentStateWhatStandsHarness):
    ...
    @classmethod
    def from_environment(cls) -> "CurrentStateWhatStandsAPIVesselV5":
    ...
    def _write_debug_artifact(
        ...
        *,
        ...
        filename = (
            f"api_vessel_v5_debug__{self._timestamp_slug()}__"
        ...
        try:
            response = self._client.responses.create(
                ...
                text={
                    "format": {
                        "type": "json_schema",
                        "name": "current_state_what_stands_reader_v1_draft_v5",
                        "schema": DRAFT_SCHEMA,
                        "strict": True,
                    }
                },
...
def print_usage() -> None:
    usage = (
        "Usage:\n"
        "  python vessel/current_state_what_stands_reader_v1/api_vessel_v5.py answer <path-to-input-packet.json>\n"
        "  python vessel/current_state_what_stands_reader_v1/api_vessel_v5.py eval\n"
    )
...
def main(argv: Sequence[str]) -> int:
    ...
    vessel = CurrentStateWhatStandsAPIVesselV5.from_environment()
    mode = argv[1]
...
if __name__ == "__main__":
    try:
        ...
        _print_stage_error("api_vessel_v5 candidate legitimacy error", exc)
        raise SystemExit(2)
    except DraftExtractionError as exc:
        _print_stage_error("api_vessel_v5 extraction error", exc)
        raise SystemExit(2)
    except DraftFinalizationError as exc:
        _print_stage_error("api_vessel_v5 finalization error", exc)
        raise SystemExit(2)
    except DraftContractError as exc:
        print(f"api_vessel_v5 contract error: {exc}", file=sys.stderr)
        raise SystemExit(2)
    except HarnessError as exc:
