#!/usr/bin/env python3
"""
API-backed vessel v3 for current_state_what_stands_reader_v1.

This additive file preserves the narrowed vessel family while improving
Responses API draft extraction and bounded diagnostic visibility. The local
shell remains authoritative over manifest enforcement, input validation, source
selection, provenance, refusal visibility, final packet assembly, and output
validation. The model remains a bounded derivative drafting surface only.
"""
...
class StageDiagnostic:
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {
            ...
            "debug_artifact_path": self.debug_artifact_path,
        }
...
class DraftContractError(HarnessError):
    """Raised when an extracted draft violates the narrowed vessel contract."""


class CurrentStateWhatStandsAPIVesselV3(CurrentStateWhatStandsHarness):
    ...
    def __init__(self, *, api_key: str, model: str) -> None:
        if OpenAI is None:
            ...
            raise HarnessError(f"The openai package is required for api_vessel_v3.py: {detail}")
    ...
    @classmethod
    def from_environment(cls) -> "CurrentStateWhatStandsAPIVesselV3":
        api_key = os.environ.get("OPENAI_API_KEY")
        if not api_key:
            raise HarnessError("OPENAI_API_KEY is required for api_vessel_v3.py.")
    ...
    def _parse_json_candidate(
        ...
        *,
        ...
        return self._draft_from_json_value(parsed)
    ...
    def _refusal_text_from_candidate(self, value: Any) -> Optional[str]:
        ...
        refusal_attr = self._get_field(value, "refusal")
        return self._string_from_candidate(refusal_attr)
            ...
            for output_item in output_items[:MAX_SUMMARY_OUTPUT_ITEMS]:
                ...
                if isinstance(content_items, list):
                    for content_item in content_items[:MAX_SUMMARY_CONTENT_ITEMS]:
                        content_types.append(str(self._get_field(content_item, "type") or type(content_item).__name__))
    ...
    def _write_debug_artifact(
        ...
        *,
        ...
        filename = (
            f"api_vessel_v3_extraction_debug__{self._timestamp_slug()}__"
        ...
        payload = {
            ...
            "bounded_note": (
                "This debug artifact is implementation-local extraction visibility only. "
                "It is not protocol law, standing, or authority."
            ),
        }
    ...
    def _stage_diagnostic(
        ...
        *,
        ...
        return StageDiagnostic(
            ...
            reason=reason,
            response_summary=response_summary,
            attempted_paths=tuple(attempted_paths[:MAX_ATTEMPTED_PATHS]),
            parse_error=parse_error,
    ...
    def _extraction_error(
        ...
        *,
        ...
        diagnostic = self._stage_diagnostic(
            request_id=request_id,
            stage=stage,
            reason=reason,
            response=response,
        ...
        *,
        ...
        return DraftExtractionError(
            "Structured extraction and fallback extraction both failed.",
            {
                ...
                "stages": [
                    *structured_failure.diagnostics.get("stages", []),
                    *fallback_failure.diagnostics.get("stages", []),
                ],
            },
        )
    ...
    def _extract_draft_from_response(
        self,
        response: Any,
        *,
        request_id: str,
        ...
        if saw_structured_candidate:
            raise self._extraction_error(
                request_id=request_id,
                stage=stage,
                reason="Responses API returned structured content, but no bounded draft payload with required fields was extractable.",
                response=response,
                attempted_paths=attempted_paths,
                parse_errors=parse_errors,
            )
    ...
    def _validate_draft(self, payload: Any) -> Dict[str, Any]:
        if not isinstance(payload, dict):
            raise DraftContractError("Draft response must be a JSON object.")

        extra_keys = sorted(set(payload.keys()) - DRAFT_KEYS)
        if extra_keys:
            raise DraftContractError(f"Draft response contains unsupported fields: {extra_keys}")

        missing_keys = sorted(DRAFT_KEYS - set(payload.keys()))
        if missing_keys:
            raise DraftContractError(f"Draft response is missing required fields: {missing_keys}")
        ...
        if status not in self.config.allowed_statuses:
            raise DraftContractError("Draft response status is not allowed.")

        if not isinstance(answer, str):
            raise DraftContractError("Draft response answer must be a string.")

        if refusal_reason is not None and refusal_reason not in self.config.allowed_refusal_reasons:
            raise DraftContractError("Draft response refusal_reason is not allowed.")

        if status == "answered":
            if not answer.strip():
                raise DraftContractError("Draft answered status must carry a non-empty answer.")
            if refusal_reason is not None:
                raise DraftContractError("Draft answered status must not carry a refusal reason.")
        elif status in {"refused", "out_of_scope"}:
            if answer != "":
                raise DraftContractError("Draft refused or out_of_scope status must carry an empty answer.")
            if refusal_reason is None:
                raise DraftContractError("Draft refused or out_of_scope status must carry a refusal reason.")
        elif status == "insufficient_grounding":
            if not answer.strip():
                raise DraftContractError("Draft insufficient_grounding status must carry a non-empty answer.")
            if refusal_reason is not None:
                raise DraftContractError("Draft insufficient_grounding status must not carry a refusal reason.")
    ...
    def _call_model_structured(
        ...
        *,
        ...
        try:
            response = self._client.responses.create(
                ...
                text={
                    "format": {
                        "type": "json_schema",
                        "name": "current_state_what_stands_reader_v1_draft_v3",
                        "schema": DRAFT_SCHEMA,
                        "strict": True,
                    }
                },
        ...
        payload = self._extract_draft_from_response(
            response,
            request_id=request_id,
            stage="structured",
        )
    ...
    def _evaluate_case(self, case: Mapping[str, Any]) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            reasons.append(f"diagnostics={diagnostics_json}")
            return eval_id, reasons
    ...
    def run_eval(self) -> int:
        ...
        for case in cases:
            if not isinstance(case, Mapping):
                failures.append(("unknown_eval", ["failure_class=schema_or_contract_violation", "Eval case is not a JSON object."]))
                contract_failures += 1
                continue
            ...
            if first_reason == "failure_class=extraction_failure":
                ...
                runtime_failures += 1
            elif first_reason == "failure_class=schema_or_contract_violation":
                contract_failures += 1
            else:
                contract_failures += 1
...
def print_usage() -> None:
    usage = (
        "Usage:\n"
        "  python vessel/current_state_what_stands_reader_v1/api_vessel_v3.py answer <path-to-input-packet.json>\n"
        "  python vessel/current_state_what_stands_reader_v1/api_vessel_v3.py eval\n"
    )
    print(usage, file=sys.stderr)


def _print_extraction_diagnostics(exc: DraftExtractionError) -> None:
    print(f"api_vessel_v3 extraction error: {exc.reason}", file=sys.stderr)
    print(
        json.dumps(exc.diagnostics, indent=2, ensure_ascii=True),
        file=sys.stderr,
    )
...
def main(argv: Sequence[str]) -> int:
    ...
    vessel = CurrentStateWhatStandsAPIVesselV3.from_environment()
    mode = argv[1]
...
if __name__ == "__main__":
    try:
        ...
        _print_extraction_diagnostics(exc)
        raise SystemExit(2)
    except DraftContractError as exc:
        print(f"api_vessel_v3 contract error: {exc}", file=sys.stderr)
        raise SystemExit(2)
    except HarnessError as exc:
        print(f"api_vessel_v3 error: {exc}", file=sys.stderr)
        raise SystemExit(2)
