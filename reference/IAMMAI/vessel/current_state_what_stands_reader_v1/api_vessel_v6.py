#!/usr/bin/env python3
"""
API-backed vessel v6 for current_state_what_stands_reader_v1.

This additive file preserves the narrowed vessel family while correcting the
remaining answer-bearing extraction failure visible in api_vessel_v5.py. The
local shell remains authoritative over source manifest enforcement, request
...
MAX_REJECTED_TEXT_CANDIDATES = 8
TEXT_EXTRACTION_MAX_DEPTH = 5
MAX_TEXT_PREVIEW = 160
MIN_DERIVED_TEXT_WORDS = 5
MIN_DERIVED_TEXT_ALPHA_CHARS = 20
...
OUT_OF_SCOPE_TEXT_MARKERS = (
    ...
    "not in the manifest",
    "would require unapproved sources",
)

REFUSAL_TEXT_MARKERS = (
    "would require interpretive sovereignty",
    "beyond this narrowed vessel",
    "broader doctrinal generation",
    "cannot decide the doctrine",
    "cannot determine the doctrine",
    "cannot issue doctrine",
    "refuse",
    "refusal",
)

INSUFFICIENT_TEXT_MARKERS = (
    "insufficient grounding",
    "insufficiently grounded",
    "not enough grounding",
    "cannot be conclusively answered",
    "not conclusively answered",
    "no single approved surface",
    "do not provide a single approved surface",
    "do not provide a single conclusive",
    "not settled by the approved surfaces",
    "not conclusively ratified",
)
...
class CandidateLegitimacyError(DraftExtractionError):
    """Raised when only illegitimate draft-shaped candidates were encountered."""


class DraftDerivationError(DraftExtractionError):
    """Raised when structured answer-bearing content cannot be lawfully derived."""
...
class CurrentStateWhatStandsAPIVesselV6(CurrentStateWhatStandsHarness):
    ...
    def __init__(self, *, api_key: str, model: str) -> None:
        if OpenAI is None:
            ...
            raise HarnessError(f"The openai package is required for api_vessel_v6.py: {detail}")
    ...
    @classmethod
    def from_environment(cls) -> "CurrentStateWhatStandsAPIVesselV6":
        api_key = os.environ.get("OPENAI_API_KEY")
        if not api_key:
            raise HarnessError("OPENAI_API_KEY is required for api_vessel_v6.py.")
    ...
    def _candidate_summary_from_mapping(self, mapping_candidate: Mapping[str, Any]) -> Dict[str, Any]:
        return {
            "keys": sorted(str(key) for key in mapping_candidate.keys() if str(key) in DRAFT_KEYS),
            "status_type": type(mapping_candidate.get("status")).__name__,
            "answer_type": type(mapping_candidate.get("answer")).__name__,
            "refusal_reason_type": type(mapping_candidate.get("refusal_reason")).__name__,
            "status_preview": self._json_safe_scalar(mapping_candidate.get("status")),
            "answer_preview": (
                mapping_candidate.get("answer")[:120]
                if isinstance(mapping_candidate.get("answer"), str)
                else self._json_safe_scalar(mapping_candidate.get("answer"))
            ),
            "refusal_reason_preview": self._json_safe_scalar(mapping_candidate.get("refusal_reason")),
        }

    def _candidate_legitimacy_issue(
        self,
        mapping_candidate: Mapping[str, Any],
        *,
        source_path: str,
    ) -> Optional[str]:
        ...
        path_segments = set(self._path_segments(source_path))
        if path_segments & SCHEMAISH_PATH_SEGMENTS:
            return "Candidate path sits inside schema or config structure rather than actual response payload."
        ...
        if self._looks_like_schema_mapping(mapping_candidate):
            return "Candidate mapping itself looks like schema or config structure rather than a model-produced payload."

        if self._looks_like_schema_mapping(status) or isinstance(status, list):
            return "Candidate status field is schema/config structure rather than a bounded payload string."

        if self._looks_like_schema_mapping(answer) or isinstance(answer, list):
            return "Candidate answer field is schema/config structure rather than a bounded payload string."

        if self._looks_like_schema_mapping(refusal_reason) or isinstance(refusal_reason, list):
            return "Candidate refusal_reason field is schema/config structure rather than a bounded payload value."

        if not isinstance(status, str):
            return "Candidate status field is not a bounded string."

        if not isinstance(answer, str):
            return "Candidate answer field is not a bounded string."

        if refusal_reason is not None and not isinstance(refusal_reason, str):
            return "Candidate refusal_reason field is neither a bounded string nor null."

        return None

    def _candidate_from_mapping_candidate(
        self,
        mapping_candidate: Mapping[str, Any],
        *,
        source_path: str,
        ...
        legitimacy_issue = self._candidate_legitimacy_issue(
            mapping_candidate,
            source_path=source_path,
        )
        if legitimacy_issue is not None:
            rejected_candidates.append(
                CandidateRejection(
                    source_path=source_path,
                    reason=legitimacy_issue,
                    candidate_summary=self._candidate_summary_from_mapping(mapping_candidate),
                )
            )
            return None
    ...
    def _meta_text_issue(self, lowered: str) -> Optional[str]:
        if any(marker in lowered for marker in META_TEXT_MARKERS):
            return "Structured text candidate looked like prompt or protocol scaffolding rather than vessel answer content."

        if lowered.startswith(("need ", "should ", "must ", "return ", "use ")) and not any(
            marker in lowered for marker in ANSWER_BEARING_MARKERS
        ):
            return "Structured text candidate looked like instruction-following meta text rather than answer content."

        return None

    def _internal_draft_from_text_candidate(
        self,
        text: str,
        *,
        source_path: str,
    ) -> Tuple[Optional[InternalDraft], Optional[str]]:
        stripped = self._strip_code_fence(text).strip()
        if not stripped:
            return None, "Structured text candidate was empty."

        if set(self._path_segments(source_path)) & SCHEMAISH_PATH_SEGMENTS:
            return None, "Structured text candidate path sits inside schema or config structure rather than actual response content."

        if stripped.startswith("{") or stripped.startswith("["):
            return None, "Structured text candidate looked like JSON or config text rather than answer-bearing vessel content."

        lowered = stripped.lower()
        meta_issue = self._meta_text_issue(lowered)
        if meta_issue is not None:
            return None, meta_issue

        if self._word_count(stripped) < MIN_DERIVED_TEXT_WORDS or self._alpha_count(stripped) < MIN_DERIVED_TEXT_ALPHA_CHARS:
            return None, "Structured text candidate was too slight to support a lawful bounded derivation."

        if any(marker in lowered for marker in OUT_OF_SCOPE_TEXT_MARKERS):
            return (
                InternalDraft(
        ...
        if any(marker in lowered for marker in REFUSAL_TEXT_MARKERS):
            return (
                InternalDraft(
                    raw_status="refused",
                    answer="",
        ...
        if any(marker in lowered for marker in INSUFFICIENT_TEXT_MARKERS):
            return (
                InternalDraft(
                    raw_status="draft_insufficient_grounding",
                    answer=stripped,
                    refusal_reason=None,
                    source_path=source_path,
                ),
    ...
    def _internal_draft_from_json_value(
        ...
        *,
        ...
        if mapping_candidate is not None:
            draft = self._candidate_from_mapping_candidate(
                mapping_candidate,
                source_path=source_path,
        ...
        if isinstance(value, list) and len(value) == 1:
            return self._internal_draft_from_json_value(
                value[0],
                source_path=source_path,
                rejected_candidates=rejected_candidates,
            )
    ...
    def _iter_dump_fallback_nodes(self, response: Any) -> Iterable[Tuple[str, Any]]:
        dumped = self._response_dump_json(response)
        if not isinstance(dumped, Mapping):
            return

        for root_key in DUMP_FALLBACK_ROOTS:
            if root_key not in dumped:
                continue
            yield from self._iter_dump_nodes(
                dumped[root_key],
                path=f"dump.{root_key}",
                depth=1,
            )
    ...
    def _iter_candidate_nodes(self, response: Any) -> Iterable[Tuple[str, Any]]:
        ...
        if isinstance(output_items, list):
            for index, output_item in enumerate(output_items):
                ...
                for field_name in ("output_parsed", "parsed", "summary", "text", "arguments", "refusal"):
        ...
        for path, value in self._iter_dump_fallback_nodes(response):
            if path in yielded_paths:
                continue
            yielded_paths.add(path)
            yield path, value

    def _iter_explicit_answer_text_nodes(self, response: Any) -> Iterable[Tuple[str, Any]]:
        yielded_paths: set[str] = set()
    ...
    def _iter_dump_answer_text_nodes(self, response: Any) -> Iterable[Tuple[str, Any]]:
        dumped = self._response_dump_json(response)
        if not isinstance(dumped, Mapping):
            return
        ...
        for index, output_item in enumerate(output_items):
            ...
            if isinstance(output_item, Mapping):
                for field_name in ("output_parsed", "parsed", "summary", "text", "arguments", "output_text"):
                    if field_name in output_item:
                        for item in emit(f"{base}.{field_name}", output_item.get(field_name)):
                            yield item
                ...
                if isinstance(content_items, list):
                    for content_index, content_item in enumerate(content_items):
                        ...
                        if not isinstance(content_item, Mapping):
                            continue
                        for field_name in ("output_parsed", "parsed", "summary", "text", "arguments", "output_text"):
                            if field_name in content_item:
                                for item in emit(f"{content_base}.{field_name}", content_item.get(field_name)):
                                    yield item
            else:
                for item in emit(base, output_item):
                    yield item

    def _iter_structured_answer_text_nodes(self, response: Any) -> Iterable[Tuple[str, Any]]:
        yielded_paths: set[str] = set()

        for iterator in (
            self._iter_explicit_answer_text_nodes(response),
            self._iter_dump_answer_text_nodes(response),
        ):
                ...
                output_summaries.append(
                    {
                        ...
                        "has_summary": self._get_field(output_item, "summary") is not None,
                        "summary_count": (
                            len(self._get_field(output_item, "summary"))
                            if isinstance(self._get_field(output_item, "summary"), list)
                            else 0
                        ),
    ...
    def _write_debug_artifact(
        ...
        *,
        ...
        filename = (
            f"api_vessel_v6_debug__{self._timestamp_slug()}__"
    ...
    def _candidate_legitimacy_error(
        self,
        *,
        request_id: str,
        stage: str,
        ...
        response: Any,
        attempted_paths: Sequence[str],
        rejected_candidates: Sequence[CandidateRejection],
        parse_errors: Sequence[json.JSONDecodeError],
    ) -> CandidateLegitimacyError:
        parse_error = str(parse_errors[-1]) if parse_errors else None
        diagnostics = {
            "request_id": request_id,
            "reason": reason,
            ...
            "attempted_paths": list(attempted_paths)[:MAX_ATTEMPTED_PATHS],
            "rejected_candidates": [
                rejection.as_dict() for rejection in rejected_candidates[:MAX_REJECTED_CANDIDATES]
            ],
            "parse_error": parse_error,
        }
        diagnostics["debug_artifact_path"] = self._write_debug_artifact(
            request_id=request_id,
            stage=f"candidate_legitimacy_{stage}",
            payload=diagnostics,
        )
        return CandidateLegitimacyError(reason, diagnostics)

    def _derivation_error(
        self,
        *,
        request_id: str,
        stage: str,
        ...
        response: Any,
        attempted_paths: Sequence[str],
        rejected_text_candidates: Sequence[TextCandidateRejection],
    ) -> DraftDerivationError:
        diagnostics = {
            "request_id": request_id,
            "reason": reason,
            "stage": stage,
            "response_summary": self._response_summary(response),
            "attempted_paths": list(attempted_paths)[:MAX_ATTEMPTED_PATHS],
            "rejected_text_candidates": [
                rejection.as_dict()
                for rejection in rejected_text_candidates[:MAX_REJECTED_TEXT_CANDIDATES]
            ],
        }
        diagnostics["debug_artifact_path"] = self._write_debug_artifact(
            request_id=request_id,
            stage=f"derivation_{stage}",
            payload=diagnostics,
        )
        return DraftDerivationError(reason, diagnostics)
    ...
    def _combined_extraction_error(
        ...
        *,
        ...
        if isinstance(structured_failure, DraftDerivationError) or isinstance(
            fallback_failure, DraftDerivationError
        ):
            return DraftDerivationError(
                "Structured extraction and fallback extraction both failed after answer-bearing derivation could not produce a lawful bounded packet.",
                diagnostics,
            )
        if isinstance(structured_failure, CandidateLegitimacyError) or isinstance(
            fallback_failure, CandidateLegitimacyError
        ):
            return CandidateLegitimacyError(
                "Structured extraction and fallback extraction both failed after rejecting illegitimate draft-shaped candidates.",
                diagnostics,
            )
    ...
    def _derive_internal_draft_from_structured_content(
        self,
        response: Any,
        *,
        request_id: str,
        ...
        rejected_text_candidates: List[TextCandidateRejection] = []

        for path, value in self._iter_structured_answer_text_nodes(response):
            ...
            internal_draft, rejection_reason = self._internal_draft_from_text_candidate(
                text_candidate,
                source_path=path,
            )
            ...
            rejected_text_candidates.append(
                TextCandidateRejection(
                    source_path=path,
                    reason=rejection_reason or "Structured text candidate could not be lawfully derived.",
                    text_preview=self._text_preview(text_candidate),
                )
            )

        if rejected_text_candidates:
            raise self._derivation_error(
                request_id=request_id,
                stage=stage,
                reason="Structured response content was present, but no lawful bounded internal candidate could be derived from the answer-bearing text that was available.",
                response=response,
                attempted_paths=attempted_paths,
                rejected_text_candidates=rejected_text_candidates,
            )
        ...
        *,
        ...
        for path, value in self._iter_candidate_nodes(response):
            ...
            if mapping_candidate is not None:
                ...
                internal_draft = self._candidate_from_mapping_candidate(
                    mapping_candidate,
                    source_path=path,
                    rejected_candidates=rejected_candidates,
                )
        ...
        derived_internal_draft = self._derive_internal_draft_from_structured_content(
            response,
            request_id=request_id,
            stage=stage,
        )
        if derived_internal_draft is not None:
            return derived_internal_draft

        if rejected_candidates:
            raise self._candidate_legitimacy_error(
                request_id=request_id,
                stage=stage,
                reason="Encountered draft-shaped candidates, but they were schema/config structures or otherwise illegitimate payloads.",
                response=response,
                attempted_paths=attempted_paths,
                rejected_candidates=rejected_candidates,
                parse_errors=parse_errors,
        ...
        try:
            response = self._client.responses.create(
                ...
                text={
                    "format": {
                        "type": "json_schema",
                        "name": "current_state_what_stands_reader_v1_draft_v6",
                        "schema": DRAFT_SCHEMA,
                        "strict": True,
                    }
                },
    ...
    def _evaluate_case(self, case: Mapping[str, Any]) -> Tuple[str, List[str]]:
        ...
        except DraftDerivationError as exc:
            return eval_id, [
                "failure_class=derivation_failure",
                f"reason={exc.reason}",
                f"diagnostics={json.dumps(exc.diagnostics, sort_keys=True)}",
            ]
        except CandidateLegitimacyError as exc:
            return eval_id, [
                "failure_class=candidate_legitimacy_failure",
                f"reason={exc.reason}",
                f"diagnostics={json.dumps(exc.diagnostics, sort_keys=True)}",
            ]
        except DraftExtractionError as exc:
    ...
    def run_eval(self) -> int:
        ...
        candidate_legitimacy_failures = 0
        contract_failures = 0
        runtime_failures = 0
        ...
        for case in cases:
            ...
            if first_reason == "failure_class=derivation_failure":
                derivation_failures += 1
            elif first_reason == "failure_class=extraction_failure":
                extraction_failures += 1
            elif first_reason == "failure_class=candidate_legitimacy_failure":
                candidate_legitimacy_failures += 1
        ...
        print(
            ...
            f"candidate_legitimacy_failure={candidate_legitimacy_failures} "
            f"derivation_failure={derivation_failures} "
...
def print_usage() -> None:
    usage = (
        "Usage:\n"
        "  python vessel/current_state_what_stands_reader_v1/api_vessel_v6.py answer <path-to-input-packet.json>\n"
        "  python vessel/current_state_what_stands_reader_v1/api_vessel_v6.py eval\n"
    )
...
def main(argv: Sequence[str]) -> int:
    ...
    vessel = CurrentStateWhatStandsAPIVesselV6.from_environment()
    mode = argv[1]
...
if __name__ == "__main__":
    try:
        ...
        _print_stage_error("api_vessel_v6 derivation error", exc)
        raise SystemExit(2)
    except CandidateLegitimacyError as exc:
        _print_stage_error("api_vessel_v6 candidate legitimacy error", exc)
        raise SystemExit(2)
    except DraftExtractionError as exc:
        _print_stage_error("api_vessel_v6 extraction error", exc)
        raise SystemExit(2)
    except DraftFinalizationError as exc:
        _print_stage_error("api_vessel_v6 finalization error", exc)
        raise SystemExit(2)
    except DraftContractError as exc:
        print(f"api_vessel_v6 contract error: {exc}", file=sys.stderr)
        raise SystemExit(2)
    except HarnessError as exc:
        print(f"api_vessel_v6 error: {exc}", file=sys.stderr)
        raise SystemExit(2)
