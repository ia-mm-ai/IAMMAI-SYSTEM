#!/usr/bin/env python3
"""
API-backed vessel v6 for current_state_standing_reader_v1.

This additive file keeps the v5 shell architecture intact while tightening the
answer-versus-pointer boundary. The model owns less, the shell owns more, and
pointer paths in next_read_paths no longer authorize answer-space mentions.
...
- Do not mention repo surfaces in answer unless they are present in the selected source paths.
- Do not mention pointer surfaces, next-read surfaces, or unread paths in answer text.
...
PROVENANCE_DOWNGRADE_NOTE = (
    "A generated response field exceeded bounded provenance and was downgraded conservatively."
)

INSUFFICIENT_GROUNDING_NOTE = (
    "The selected approved sources were insufficient to support a tighter answer without exceeding bounded provenance."
)
...
class CurrentStateStandingReaderAPIVesselV6(CurrentStateStandingReaderHarnessV3):
    ...
    @classmethod
    def from_environment(cls) -> "CurrentStateStandingReaderAPIVesselV6":
    ...
    def _local_boundedness_note(
        ...
        *,
        ...
        if status in {"answered", "insufficient_grounding"}:
            return self._format_selected_surface_note(consulted_sources)
        if status == "refused":
            return "This vessel remained inside its read-only current-state / standing role and returned a visible refusal."
        if status == "out_of_scope":
            return "This vessel remained inside its approved contract and returned a visible out-of-scope result."
        raise HarnessError(f"Unhandled status for local boundedness note: {status}")

    def _local_ambiguity_note(self, *, status: str) -> Optional[str]:
        if status == "insufficient_grounding":
            return INSUFFICIENT_GROUNDING_NOTE
        return None
        ...
        *,
        ...
        response = self._client.responses.create(
            ...
            text={
                "format": {
                    "type": "json_schema",
                    "name": "current_state_standing_reader_v1_draft_v6",
                    "schema": DRAFT_SCHEMA,
                    "strict": True,
                }
            },
    ...
    def _provenance_self_check(self, plan: AnswerPlan) -> PacketTrace:
        ...
        undeclared_answer_paths = sorted(
            path for path in answer_path_mentions if path and path not in set(plan.consulted_sources)
        )
        if undeclared_answer_paths:
            return PacketTrace(
                consulted_sources=plan.consulted_sources,
                next_read_paths=plan.next_read_paths,
                provenance_ok=False,
                public_note=PROVENANCE_DOWNGRADE_NOTE,
                debug_note=(
                    "Answer mentioned path-like surfaces outside consulted sources: "
                    + ", ".join(undeclared_answer_paths)
                ),
            )
        ...
        for field_name, text in (
            ...
            if note_path_mentions:
                return PacketTrace(
                    ...
                    public_note=PROVENANCE_DOWNGRADE_NOTE,
                    debug_note=(
                        f"{field_name} mentioned repo file paths even though shell notes must remain path-safe: "
                        + ", ".join(sorted(note_path_mentions))
                    ),
                )
    ...
    def _render_answer_plan(self, request: Mapping[str, Any]) -> AnswerPlan:
        ...
        return AnswerPlan(
            ...
            else self._next_read_paths(question_class, selected_sources),
...
def print_usage() -> None:
    usage = (
        "Usage:\n"
        "  python vessel/current_state_standing_reader_v1/api_vessel_v6.py answer <path-to-input-packet.json>\n"
        "  python vessel/current_state_standing_reader_v1/api_vessel_v6.py eval\n"
    )
...
def main(argv: Sequence[str]) -> int:
    ...
    vessel = CurrentStateStandingReaderAPIVesselV6.from_environment()
