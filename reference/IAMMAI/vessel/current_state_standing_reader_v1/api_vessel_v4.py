#!/usr/bin/env python3
"""
API-backed vessel v4 for current_state_standing_reader_v1.

This additive file keeps the v3 shell architecture intact while making
structured-response extraction more defensive and using JSON-object mode as the
bounded recovery path when structured output is unusable. The model still
drafts bounded answer content only. Local code still owns manifest enforcement,
input validation, source selection, provenance, refusal handling, final packet
assembly, and final output validation.
"""
...
class CurrentStateStandingReaderAPIVesselV4(CurrentStateStandingReaderHarnessV3):
    ...
    @classmethod
    def from_environment(cls) -> "CurrentStateStandingReaderAPIVesselV4":
    ...
    def _extract_response_text(self, response: Any) -> str:
        top_level_text = self._string_from_candidate(self._get_field(response, "output_text"))
        if top_level_text:
        ...
        if isinstance(output_items, list):
            for item in output_items:
                ...
                for content in content_items:
                    text_candidate = self._string_from_candidate(self._get_field(content, "text"))
                    if text_candidate and text_candidate not in text_candidates:
                        text_candidates.append(text_candidate)

        combined = "".join(text_candidates).strip()
        if combined:
            return combined
    ...
    def _extract_structured_draft_payload(self, response: Any) -> Any:
        ...
        for field_name in ("output_parsed", "parsed"):
            direct_candidate = self._get_field(response, field_name)
            if direct_candidate is not None:
                direct_candidates.append(direct_candidate)

        top_level_text = self._string_from_candidate(self._get_field(response, "output_text"))
        if top_level_text:
            text_candidates.append(top_level_text)

        output_items = self._get_field(response, "output")
        if isinstance(output_items, list):
            for item in output_items:
                direct_candidate = self._get_field(item, "parsed")
                if direct_candidate is not None:
                    direct_candidates.append(direct_candidate)
                ...
                for content in content_items:
                    direct_candidate = self._get_field(content, "parsed")
                    if direct_candidate is not None:
                        direct_candidates.append(direct_candidate)
        ...
        for candidate in direct_candidates:
            ...
            text_candidate = self._string_from_candidate(candidate)
            if text_candidate:
                text_candidates.append(text_candidate)
        ...
        if text_candidates and last_json_error is not None:
            raise StructuredDraftUnusableError(f"Model returned invalid JSON draft: {last_json_error}")
        ...
        *,
        ...
        response = self._client.responses.create(
            ...
            text={
                "format": {
                    "type": "json_schema",
                    "name": "current_state_standing_reader_v1_draft_v4",
                    "schema": DRAFT_SCHEMA,
                    "strict": True,
                }
            },
...
def print_usage() -> None:
    usage = (
        "Usage:\n"
        "  python vessel/current_state_standing_reader_v1/api_vessel_v4.py answer <path-to-input-packet.json>\n"
        "  python vessel/current_state_standing_reader_v1/api_vessel_v4.py eval\n"
    )
...
def main(argv: Sequence[str]) -> int:
    ...
    vessel = CurrentStateStandingReaderAPIVesselV4.from_environment()
