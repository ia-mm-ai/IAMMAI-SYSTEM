#!/usr/bin/env python3
"""
API-backed vessel v3 for current_state_standing_reader_v1.

This additive file keeps the v2 shell architecture intact while removing the
Responses API parameter that the current model rejects. The model still drafts
bounded answer content only. Local code still owns manifest enforcement, input
validation, source selection, provenance, refusal handling, final packet
...
class CurrentStateStandingReaderAPIVesselV3(CurrentStateStandingReaderHarnessV3):
    ...
    @classmethod
    def from_environment(cls) -> "CurrentStateStandingReaderAPIVesselV3":
        ...
        *,
        ...
        response = self._client.responses.create(
            ...
            text={
                "format": {
                    "type": "json_schema",
                    "name": "current_state_standing_reader_v1_draft_v3",
                    "schema": DRAFT_SCHEMA,
                    "strict": True,
                }
            },
...
def print_usage() -> None:
    usage = (
        "Usage:\n"
        "  python vessel/current_state_standing_reader_v1/api_vessel_v3.py answer <path-to-input-packet.json>\n"
        "  python vessel/current_state_standing_reader_v1/api_vessel_v3.py eval\n"
    )
...
def main(argv: Sequence[str]) -> int:
    ...
    vessel = CurrentStateStandingReaderAPIVesselV3.from_environment()
