#!/usr/bin/env python3
"""
Additive successor to the first bounded checker for first-pass continuity records.

This script preserves the first checker’s structural and file-ref validation,
and closes two anti-collapse gaps exposed by the invalid fixtures:

- lineage records fail when from_ref == to_ref
- currentness records fail when current_ref appears in non_current_refs
...
class CheckResult:
    ...
    anti_collapse_errors: list[str] = field(default_factory=list)
    ...
    def passed(self) -> bool:
        return (
            ...
            and not self.ref_errors
            and not self.anti_collapse_errors
        )
...
def usage() -> str:
    return (
        "Usage:\n"
        "  python3 continuity/check_continuity_record_v2.py /path/to/continuity_record.json"
    )
...
def check_lineage_anti_collapse(
    from_ref: str | None,
    to_ref: str | None,
    result: CheckResult,
) -> None:
    if from_ref is None or to_ref is None:
        return
    if from_ref == to_ref:
        result.anti_collapse_errors.append(
            "Lineage record collapses additive relation because from_ref and to_ref are identical."
        )


def check_currentness_anti_collapse(
    current_ref: str | None,
    non_current_refs: list[str] | None,
    result: CheckResult,
) -> None:
    if current_ref is None or non_current_refs is None:
        return
    if current_ref in non_current_refs:
        result.anti_collapse_errors.append(
            "Currentness record collapses anti-confusion posture because current_ref also appears in non_current_refs."
        )
...
def validate_lineage_record(data: dict[str, Any], result: CheckResult) -> None:
    ...
    check_lineage_anti_collapse(from_ref, to_ref, result)
...
def validate_currentness_record(data: dict[str, Any], result: CheckResult) -> None:
    ...
    check_currentness_anti_collapse(current_ref, non_current_refs, result)
...
def print_result(result: CheckResult) -> None:
    ...
    print(
        ...
        f"anti_collapse_issues={len(result.anti_collapse_errors)}"
    )
    ...
    for issue in result.ref_errors:
        print(f"ref: {issue}")
    for issue in result.anti_collapse_errors:
        print(f"anti_collapse: {issue}")
