#!/usr/bin/env python3
"""
First bounded executable cross-carrier seam proof.

This runner proves one bounded carrier-to-carrier release and ingress flow for
the witness_artifact family only. It keeps source as source, treats receiver
arrival as derivative-only, and makes refusal visible when ingress is not
lawful.
"""
...
EXPERIMENT_ID = "cross_carrier_seam_proof_001"
...
GROUNDING_SURFACES: Tuple[Tuple[str, str], ...] = (
    ...
    (
        ...
        "Bounded seam-case law for contaminated channel, refusal, and seam outcomes.",
    ),
    (
...
class ConformanceRecord:
    ...
    package_attempted_unlawful_standing_transfer: Optional[bool] = None
    ...
    def as_record(self) -> Dict[str, Any]:
        ...
        if self.package_attempted_unlawful_standing_transfer is not None:
            record["package_attempted_unlawful_standing_transfer"] = (
                self.package_attempted_unlawful_standing_transfer
            )
        return record
...
class PackageEvaluation:
    lawful: bool
    package_attempted_unlawful_standing_transfer: bool
    reasons: Tuple[str, ...]
...
def load_optional_module(path: Path, module_name: str) -> Tuple[Optional[ModuleType], Optional[str]]:
    if not path.is_file():
...
def build_package_conformance(
    ...
    *,
    ...
    issues: List[ConformanceIssue] = []
    package_attempted_unlawful_standing_transfer = False
    ...
    if seam.get("standing_transfer_on_receipt") is True:
        package_attempted_unlawful_standing_transfer = True
        issues.append(
            ConformanceIssue(
                "Package attempts unlawful standing transfer on receipt."
            )
        )
    if seam.get("derivative_only_arrival_rule") is not True:
        ...
        issues.append(
            ConformanceIssue(
                "Package does not preserve derivative-only arrival on lawful ingress."
            )
        )
    ...
    return (
        ConformanceRecord(
            ...
            conformance_kind="ingress_legality",
            status=status,
            checked_at=utc_now(),
            artifact_family=ARTIFACT_FAMILY,
...
def create_run_layout(base_root: Path, execution_id: str, directory_names: Sequence[str]) -> Dict[str, Path]:
...
def create_canonical_witness_body(matter_id: str) -> Dict[str, Any]:
    return {
        ...
        "surface_ref": "lab:cross_carrier_seam_proof_001:source_release",
...
def build_release_record(
    *,
    ...
    return {
        ...
        "matter_id": matter_id,
        "release_statement": (
            "One source-side carrier releases one witness artifact package for explicit seam ingress only."
        ),
        "seam_statement": (
            "Source remains source. Receiving-side arrival is derivative-only only if ingress is lawful. "
            "No silent authority inheritance occurs."
        ),
...
def build_transfer_package(
    *,
    ...
    standing_transfer_on_receipt = release_mode == "release_invalid"
    derivative_only_arrival_rule = release_mode != "release_invalid"

    if release_mode == "release":
        package_note = (
            "Lawful source-side release package for derivative-only ingress testing."
        )
        seam_statement = (
            "Source remains source. Receiver must perform explicit ingress. "
            "Arrival stays derivative-only and standing does not transfer on receipt."
        )
    else:
        package_note = (
            "Intentionally invalid release package for refusal testing. It attempts unlawful standing transfer on receipt."
        )
        seam_statement = (
            "This package is intentionally invalid for ingress because it claims standing on receipt "
            "instead of derivative-only arrival."
        )
    ...
    return {
        ...
        "release_context": {
            ...
            "release_mode": release_mode,
            "release_statement": (
                "Source-side carrier releases one witness artifact package for explicit receiving-side ingress."
            ),
            "seam_statement": seam_statement,
            "expected_receiving_outcome": (
                "lawful_derivative_ingress"
                if release_mode == "release"
                else "refusal_non_passage"
            ),
        ...
        "seam": {
            ...
            "derivative_only_arrival_rule": derivative_only_arrival_rule,
            "shared_authority": False,
            "standing_transfer_on_receipt": standing_transfer_on_receipt,
...
def evaluate_ingress(
    ...
    *,
    ...
    if not reasons:
        reasons.extend(
            [
                "Explicit ingress was performed on the receiving-side carrier.",
                "Source remains source and arrival is derivative-only.",
                "No shared authority or standing upgrade occurs on receipt.",
            ]
        )
        ...
        reasons.append(
            "Receiving-side ingress therefore refuses or non-passes visibly instead of silently upgrading arrival."
        )
        evaluation = IngressEvaluation(
            ...
            non_passage=True,
            package_attempted_unlawful_standing_transfer=(
                package_evaluation.package_attempted_unlawful_standing_transfer
            ),
            source_legibility_status=legibility,
            reasons=tuple(reasons),
        )
...
def build_source_summary(
    *,
    ...
    return {
        ...
        "package_ingress_lawful_if_received": package_conformance.status == "pass",
        "package_attempted_unlawful_standing_transfer": (
            package_conformance.package_attempted_unlawful_standing_transfer
        ),
        "expected_ingress_outcome": expected_ingress_outcome,
        "package_path": repo_relative(package_path),
        "identity_generation_mode": identity_generation_mode(),
        "note": (
            "Bounded source-side release summary only. "
            "It does not transfer source authority or create shared canonical state."
        ),
    }
...
def build_receipt_record(
    *,
    ...
    return {
        ...
        "package_attempted_unlawful_standing_transfer": (
            ingress_evaluation.package_attempted_unlawful_standing_transfer
        ),
        "note": (
            "Explicit package receipt only. Transport or file arrival is not itself lawful ingress."
        ),
    }
...
def build_ingress_record(
    *,
    ...
    return {
        ...
        "explicit_ingress": True,
        "source_legibility_status": ingress_evaluation.source_legibility_status,
        "reasoning": list(ingress_evaluation.reasons),
        "note": (
            "Bounded derivative-only ingress record only. "
            "It does not upgrade arriving witness body into receiving-side canonical standing."
        ),
    }
...
def build_refusal_record(
    *,
    ...
    return {
        ...
        "note": (
            "Visible refusal and non-passage record only. "
            "The refusal is a lawful seam outcome, not a silent parsing failure."
        ),
    }
...
def build_receiving_summary(
    *,
    ...
    return {
        ...
        "reasoning": list(ingress_evaluation.reasons),
        "note": (
            "Bounded receiving-side ingress summary only. "
            "Transport success is not treated as automatic lawful ingress."
        ),
    }
...
def build_manifest(
    *,
    ...
    return {
        ...
        "identity_generation_mode": identity_generation_mode(),
        "note": (
            "Implementation-local manifest only. "
            "It preserves lineage for this bounded lab proof."
        ),
    }
...
def print_release_summary(summary: Mapping[str, Any]) -> None:
    ...
    print(f"Package path: {summary['package_path']}")
    print(f"Expected ingress outcome: {summary['expected_ingress_outcome']}")
...
def run_release_mode(release_mode: str) -> int:
    ...
    package_conformance_path = layout["conformance"] / "package_preflight_conformance.json"
    ...
    manifest_payload = build_manifest(
        ...
        run_dir=run_dir,
        mode=release_mode,
        written_files=tuple(written_files) + (layout["manifest"] / "manifest.json",),
    )
...
def run_ingress_mode(package_argument: str) -> int:
    ...
    package_conformance_path = layout["conformance"] / "package_ingress_conformance.json"
    write_json(package_conformance_path, package_conformance.as_record())
    written_files.append(package_conformance_path)
...
def usage() -> str:
    return (
        "Usage:\n"
        "  python lab/run_cross_carrier_seam_proof_001.py release\n"
        "  python lab/run_cross_carrier_seam_proof_001.py release_invalid\n"
        "  python lab/run_cross_carrier_seam_proof_001.py ingress <path-to-package>\n"
    )
...
def main(argv: Sequence[str]) -> int:
    ...
    if mode == "release" and len(argv) == 2:
        return run_release_mode("release")
    if mode == "release_invalid" and len(argv) == 2:
        return run_release_mode("release_invalid")
