#!/usr/bin/env python3
"""
Build a bounded human-readable companion report for the cross-carrier proof family.

This script loads the cross-carrier family snapshot builder, writes a fresh
machine-readable family snapshot under lab/snapshots/, and renders one
additive Markdown report from that family-level data. It does not rerun the
proofs, mutate old artifacts, or flatten distinct cross-carrier proof turns
into one generic transfer story.
"""
...
FAMILY_SNAPSHOT_BUILDER_PATH = LAB_ROOT / "build_cross_carrier_family_snapshot.py"
...
def choose_report_path() -> Path:
    base_name = f"{FAMILY_ID}_report__{timestamp_slug()}"
...
def load_snapshot_builder() -> ModuleType:
    if not FAMILY_SNAPSHOT_BUILDER_PATH.is_file():
        raise RuntimeError(
            "Cross-carrier family snapshot builder is missing: "
            f"{FAMILY_SNAPSHOT_BUILDER_PATH}"
        )

    spec = importlib.util.spec_from_file_location(
        "iammai_cross_carrier_family_snapshot_builder",
        FAMILY_SNAPSHOT_BUILDER_PATH,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(
            f"Could not create an import specification for {FAMILY_SNAPSHOT_BUILDER_PATH}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to import the cross-carrier family snapshot builder: {exc}"
        ) from exc
    ...
    if missing:
        raise RuntimeError(
            "Cross-carrier family snapshot builder is missing required callables: "
            + ", ".join(missing)
        )
...
def build_fresh_snapshot() -> Tuple[Path, Dict[str, Any]]:
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Family snapshot builder returned a non-object payload.")
...
def add_proof_section(lines: List[str], proof: Mapping[str, Any]) -> None:
    ...
    lines.append(
        "- Script path: "
        f"{md_code(proof.get('script_path'))}"
    )
    lines.append(
    ...
    lines.append(
        "- Proof-specific snapshot surface: "
        f"{md_code(latest_snapshot.get('path'))}"
    )
    lines.append(
        "- Proof-specific report surface: "
        f"{md_code(latest_report.get('path'))}"
    )
    lines.append(
        "- Threshold note: "
        f"{threshold_note_for_proof(proof_id)}"
    )
    lines.append("")
...
def render_report(
    ...
    *,
    ...
    lines.append(
        ...
        "cross-carrier family snapshot. It is an implementation-local readability "
...
def print_summary(
    *,
    ...
    family_summary = as_mapping(payload.get("family_summary"))
    print("Cross-carrier family report written.")
