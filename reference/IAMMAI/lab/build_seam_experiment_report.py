#!/usr/bin/env python3
"""
Build a bounded human-readable companion report for the seam experiment family.

This script loads the existing seam experiment snapshot builder, writes a fresh
machine-readable snapshot under lab/snapshots/, and renders one additive
Markdown report from that ranked family data. It does not rerun experiments,
mutate old artifacts, or flatten the seam lab into one generic doctrine object.
"""
...
SNAPSHOT_BUILDER_PATH = LAB_ROOT / "build_seam_experiment_snapshot.py"
...
def load_snapshot_builder() -> ModuleType:
    if not SNAPSHOT_BUILDER_PATH.is_file():
        raise RuntimeError(
            f"Seam experiment snapshot builder is missing: {SNAPSHOT_BUILDER_PATH}"
        )

    spec = importlib.util.spec_from_file_location(
        "iammai_seam_experiment_snapshot_builder",
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to import the seam experiment snapshot builder: {exc}"
        ) from exc
    ...
    if missing:
        raise RuntimeError(
            "Seam experiment snapshot builder is missing required callables: "
            + ", ".join(missing)
        )
...
def collect_partial_notes(payload: Mapping[str, Any]) -> List[str]:
    notes: List[str] = []
    experiments = as_mapping(payload.get("experiments"))
    ...
    family_notes = as_mapping(payload.get("family_level_notes"))
...
def add_experiment_section(lines: List[str], record: Mapping[str, Any]) -> None:
    ...
    lines.append(f"- Latest grounding path: {md_code(record.get('latest_grounding_path'))}")
    lines.append(f"- Latest run selection method: {md_code(record.get('latest_run_selection_method'))}")
    lines.append(f"- Total cases in latest readable summary: {md_code(summary_reading.get('total_cases'))}")
    lines.append(
    ...
    if isinstance(mismatched_cases, list) and mismatched_cases:
        lines.append(f"- Mismatched case ids: {', '.join(str(item) for item in mismatched_cases)}")

    partial_notes = as_list(record.get("partial_read_notes"))
    if partial_notes:
...
def render_report(
    ...
    *,
    ...
    family_summary = as_mapping(payload.get("experiment_family_summary"))
    experiments = as_mapping(payload.get("experiments"))
    family_notes = as_mapping(payload.get("family_level_notes"))
...
def print_summary(
    *,
    ...
    print(
        ...
        f"{family_summary.get('total_experiments_discovered')}"
    )
    print(f"Total runs discovered: {family_summary.get('total_runs_discovered')}")
