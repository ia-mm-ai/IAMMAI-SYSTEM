#!/usr/bin/env python3
"""
Refresh the current readability layer across all standing readable families in
fully snapshot-consistent order where report-from-snapshot builders are
available.
...
STAGES: Sequence[Stage] = (
    ...
    Stage(
        name="Cross-carrier family report from snapshot",
        script_path=REPO_ROOT / "lab" / "build_cross_carrier_family_report_from_snapshot.py",
        family="cross-carrier",
    ),
    Stage(
...
def refresh_all() -> int:
    ...
    print("")
    print("Current readability refresh v2 completed.")
