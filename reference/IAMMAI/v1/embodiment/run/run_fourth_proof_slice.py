#!/usr/bin/env python3
"""
Bounded v1 runner for the fourth ordinary proof slice.

This script forms canonical bodies for the five ordinary v1 artifact families,
validates them through the shared conformance surface, carries them in
explicitly separable execution-bearing envelopes, generates run identities
through the shared identifier surface, and preserves the result through the
shared implementation-local preservation layout under v1/registry/runs/.
"""
...
CONDITION_SET_REF = "condition-set-v1-fourth-proof-slice-threshold-validation"
ACTOR_REF = "actor-v1-local-operator"
LEGITIMACY_BASIS_REF = "legitimacy-basis-v1-fourth-proof-slice"
...
def build_run_summary(
    ...
    return {
        "summary_type": "v1_fourth_proof_slice_summary",
...
def main() -> int:
    try:
        schema_catalog = load_schema_catalog(ORDINARY_SCHEMA_NAMES)
    ...
    try:
        ...
        threshold_state_ref = new_context_id("state_preparation_threshold")
        preservation_id = new_preservation_id()
        manifest_id = new_manifest_id()
        negative_validation_id = new_artifact_id("validation_artifact")
        artifact_ids = {
            spec.name: new_artifact_id(spec.name) for spec in FAMILY_SPECS
        }
    except IdentifierGenerationError as exc:
        print(str(exc), file=sys.stderr)
        return 1
    ...
    try:
        ...
        for spec in FAMILY_SPECS:
            ...
            body_result = validate_ordinary_body(
                schema_catalog,
                spec.name,
                body,
                body_ref=body_output.path,
            ...
            envelope_result = check_envelope_bearing_conformance(
                spec.name,
                envelope,
                body,
                execution_identity=execution_id,
                canonical_body_ref=body_output.path,
                envelope_ref=envelope_output.path,
                expected_status="pass",
            ).as_record()
        ...
        negative_body = build_negative_validation_fixture(
            validation_id=negative_validation_id,
            matter_id=matter_id,
            recorded_at=utc_now(),
        )
        ...
        negative_classification = {
            "fixture_class": "conformance_failure",
            "scope": "fourth_ordinary_proof_slice",
        ...
        negative_result = validate_ordinary_body(
            schema_catalog,
            "validation_artifact",
            negative_body,
        ...
        manifest_payload = build_manifest_payload(
            layout,
            written_outputs,
            manifest_type="v1_fourth_proof_slice_preservation_manifest",
            extra={
                "matter_identity": matter_id,
    ...
    print("Bounded v1 fourth ordinary proof slice completed.")
